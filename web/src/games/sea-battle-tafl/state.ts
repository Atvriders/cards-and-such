...
export interface SeaBattleTaflSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface SeaBattleTaflState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type SeaBattleTaflAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: SeaBattleTaflSettings): SeaBattleTaflState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: SeaBattleTaflState, action: SeaBattleTaflAction): SeaBattleTaflState {
...
export function isTerminal(state: SeaBattleTaflState): { score: number } | null {
