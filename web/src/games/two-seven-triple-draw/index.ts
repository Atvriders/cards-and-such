...
const TwoSevenTripleDrawGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TwoSevenTripleDrawGame as unknown as React.ComponentType<unknown> })));
...
export const twoSevenTripleDrawPlugin: GamePlugin<TwoSevenTripleDrawState, TwoSevenTripleDrawAction, typeof settings> = {
  ...
  hint: (state: TwoSevenTripleDrawState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-two-seven-triple-draw-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-two-seven-triple-draw-next"]', pulses: 3 };
    return null;
  },
};
