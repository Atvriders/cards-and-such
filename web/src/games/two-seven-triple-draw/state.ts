...
export interface TwoSevenTripleDrawSettings { dummy: boolean; }
export interface TwoSevenTripleDrawState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TwoSevenTripleDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TwoSevenTripleDrawSettings): TwoSevenTripleDrawState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TwoSevenTripleDrawState, action: TwoSevenTripleDrawAction): TwoSevenTripleDrawState {
...
export function isTerminal(state: TwoSevenTripleDrawState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
