    ...
    <div className="arc-wrap">
      ...
      {state.phase === "aiming" ? (
        <>
          ...
          <button className="arc-btn" onClick={() => dispatch({ type:"throw" } as LasagnaLayerAction)}>Go!</button>
        </>
      ) : (
        <>
          <div className="arc-result">+{state.lastPts} pts (off by {state.lastDiff})</div>
          <button className="arc-btn" onClick={() => dispatch({ type:"next" } as LasagnaLayerAction)}>Next</button>
        </>
      )}
    </div>
  );
