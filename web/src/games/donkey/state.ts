...
export interface DonkeySettings { dummy: boolean; }
export interface DonkeyState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type DonkeyAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: DonkeySettings): DonkeyState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: DonkeyState, action: DonkeyAction): DonkeyState {
...
export function isTerminal(state: DonkeyState): { score: number } | null {
