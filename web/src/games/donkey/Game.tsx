...
export function DonkeyGame({ state, dispatch, onGameOver }: GameProps<DonkeyState, DonkeySettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="donkey-wrap dk-shed"><div className="donkey-done bounce-in"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="donkey-final">{state.score} pts</div></div></div>;
  return (
    <div className="donkey-wrap dk-shed fade-in">
      <div className="donkey-info">Round {state.round} / {TOTAL_ROUNDS} — W{state.wins} L{state.losses}</div>
      <div className="donkey-score pulse">{state.score} pts</div>
      <div className="donkey-info">You: {state.you} cards · CPU: {state.cpu} cards</div>
      {state.phase === "ready" && <button data-testid="hint-target-donkey-primary" className="donkey-btn" onClick={() => dispatch({ type: "play" } as DonkeyAction)}>Play Round</button>}
      {state.phase === "scored" && <>
