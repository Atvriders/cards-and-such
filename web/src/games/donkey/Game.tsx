...
export function DonkeyGame({ state, dispatch, onGameOver }: GameProps<DonkeyState, DonkeySettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="donkey-wrap"><div className="donkey-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="donkey-final">{state.score} pts</div></div></div>;
  return (
    <div className="donkey-wrap">
