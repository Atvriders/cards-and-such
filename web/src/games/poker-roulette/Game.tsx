...
import "./Game.css";
export function PokerRouletteGame({ state, dispatch, onGameOver }: GameProps<PokerRouletteState, PokerRouletteSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-poker-roulette-deal" onClick={() => dispatch({ type: "deal" } as PokerRouletteAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-poker-roulette-next" onClick={() => dispatch({ type: "next" } as PokerRouletteAction)}>Next</button>
      </>}
    </div>
  );
}
