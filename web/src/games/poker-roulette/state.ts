...
export interface PokerRouletteSettings { dummy: boolean; }
export interface PokerRouletteState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type PokerRouletteAction = { type: "deal" } | { type: "next" };
...
function rateFiveCards(hand5: number[]): { rank: string; pts: number } {
  ...
  // roulette wild rank: hash of all card sum mod 13
  const wildRank = hand5.reduce((a,b)=>a+b,0) % 13;
  const wilds = ranks.filter(r=>r===wildRank).length; const nonWild = ranks.filter(r=>r!==wildRank);
  const counts: Record<number,number> = {}; for(const r of nonWild) counts[r]=(counts[r]||0)+1;
  const cv = Object.values(counts).sort((a,b)=>b-a);
  const top = (cv[0]||0) + wilds;
  const flush = suits.every(s=>s===suits[0]);
  const sortedR = [...new Set(ranks)].sort((a,b)=>a-b);
  let straight = sortedR.length>=5-wilds; // simplistic
  if (top >= 5) return { rank: "Five of a Kind", pts: 250 };
  if (straight && flush) return { rank: "Straight Flush", pts: 200 };
  if (top === 4) return { rank: "Four of a Kind", pts: 150 };
  if ((cv[0]===3 && cv[1]===2) || (top===3 && cv[1]===2 && wilds===0)) return { rank: "Full House", pts: 100 };
  if (flush) return { rank: "Flush", pts: 80 };
  if (straight) return { rank: "Straight", pts: 70 };
  if (top === 3) return { rank: "Three of a Kind", pts: 50 };
  if (cv[0]===2 && cv[1]===2) return { rank: "Two Pair", pts: 30 };
  if (top === 2) return { rank: "Pair", pts: 10 };
...
export function initialState(seed: number, _s: PokerRouletteSettings): PokerRouletteState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: PokerRouletteState, action: PokerRouletteAction): PokerRouletteState {
...
export function isTerminal(state: PokerRouletteState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
