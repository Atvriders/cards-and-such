...
export interface BombPotSettings { dummy: boolean; }
export interface BombPotState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type BombPotAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BombPotSettings): BombPotState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: BombPotState, action: BombPotAction): BombPotState {
...
export function isTerminal(state: BombPotState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
