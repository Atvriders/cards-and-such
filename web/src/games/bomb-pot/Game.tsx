...
import "./Game.css";
export function BombPotGame({ state, dispatch, onGameOver }: GameProps<BombPotState, BombPotSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-bomb-pot-deal" onClick={() => dispatch({ type: "deal" } as BombPotAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-bomb-pot-next" onClick={() => dispatch({ type: "next" } as BombPotAction)}>Next</button>
      </>}
    </div>
  );
}
