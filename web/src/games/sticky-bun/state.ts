...
export interface StickyBunSettings { dummy: boolean; }

export interface StickyBunState {
  rngSeed: number;
  targets: number[];   // sweet spot 0-100 for each round
  roundIndex: number;
  score: number;
  phase: "aiming" | "result" | "done";
  power: number;
  lastDiff: number;
  lastPts: number;
}

export type StickyBunAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, settings: StickyBunSettings): StickyBunState {
...
export function reducer(state: StickyBunState, action: StickyBunAction): StickyBunState {
...
export function isTerminal(state: StickyBunState): { score: number } | null {
