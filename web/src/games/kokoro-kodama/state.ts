...
export interface KokoroKodamaSettings { rounds: "10"; }

export interface KokoroKodamaState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type KokoroKodamaAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: KokoroKodamaSettings): KokoroKodamaState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: KokoroKodamaState, action: KokoroKodamaAction): KokoroKodamaState {
...
export function isTerminal(state: KokoroKodamaState): { score: number } | null {
