...
export function KokoroKodama({ state, dispatch, onGameOver }: GameProps<KokoroKodamaState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      <div className="g-controls">
        {state.phase === "ready" && <button className="g-btn" onClick={() => dispatch({ type: "play" } as KokoroKodamaAction)}>Play Hand</button>}
        {state.phase === "played" && <button className="g-btn" onClick={() => dispatch({ type: "next" } as KokoroKodamaAction)}>Next</button>}
      </div>
    </div>
  );
}
