...
// Spike Dodger: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface SpikeDodgerSettings { dummy: boolean; }

export interface SpikeDodgerTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface SpikeDodgerState {
  rngSeed: number;
  targets: SpikeDodgerTarget[];
  nextId: number;
...
export type SpikeDodgerAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: SpikeDodgerSettings): SpikeDodgerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: SpikeDodgerState, action: SpikeDodgerAction): SpikeDodgerState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: SpikeDodgerTarget[] = [];
...
export function isTerminal(state: SpikeDodgerState): { score: number } | null {
