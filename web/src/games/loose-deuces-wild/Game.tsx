...
import "./Game.css";
export function LooseDeucesWildGame({ state, dispatch, onGameOver }: GameProps<LooseDeucesWildState, LooseDeucesWildSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-loose-deuces-wild-deal" onClick={() => dispatch({ type: "deal" } as LooseDeucesWildAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-loose-deuces-wild-next" onClick={() => dispatch({ type: "next" } as LooseDeucesWildAction)}>Next</button>
      </>}
    </div>
  );
}
