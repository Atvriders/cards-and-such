...
export interface LooseDeucesWildSettings { dummy: boolean; }
export interface LooseDeucesWildState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type LooseDeucesWildAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: LooseDeucesWildSettings): LooseDeucesWildState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: LooseDeucesWildState, action: LooseDeucesWildAction): LooseDeucesWildState {
...
export function isTerminal(state: LooseDeucesWildState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
