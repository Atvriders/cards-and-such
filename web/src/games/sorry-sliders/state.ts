// Auto-generated race-game state for sorry-sliders.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const TRACK_LEN = 44;
...
export const SPECIALS: Record<number, Special> = {5: {type:"slide", to:12},14: {type:"slide", to:22},23: {type:"slide", to:32},33: {type:"slide", to:42}};

export type Side = "P" | "C";
export interface SorrySlidersSettings { dummy: boolean }

export interface SorrySlidersState {
  rngSeed: number;
...
export type SorrySlidersAction =
  | { type: "roll" }
...
export function initialState(seed: number, _s: SorrySlidersSettings): SorrySlidersState {
  return {
    rngSeed: seed,
...
function checkWin(s: SorrySlidersState): SorrySlidersState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: SorrySlidersState): SorrySlidersState {
  let s = state;
...
export function legalMovesForP(state: SorrySlidersState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: SorrySlidersState, action: SorrySlidersAction): SorrySlidersState {
...
export function isTerminal(state: SorrySlidersState): { score: number } | null {
