...
export interface MiniBaccaratMiniSettings { dummy: boolean; }
export interface MiniBaccaratMiniState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type MiniBaccaratMiniAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MiniBaccaratMiniSettings): MiniBaccaratMiniState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: MiniBaccaratMiniState, action: MiniBaccaratMiniAction): MiniBaccaratMiniState {
...
export function isTerminal(state: MiniBaccaratMiniState): { score: number } | null {
