...
export const DEAL_COUNT = 6;
export interface CardAuroraSettings { dummy: boolean; }
export interface CardAuroraState {
  rngSeed: number;
  round: number;
  cards: number[];
  picked: number | null;
...
export type CardAuroraAction = { type: "pick"; index: number } | { type: "next" } | { type: "deal" };
...
export function initialState(seed: number, _settings: CardAuroraSettings): CardAuroraState {
...
export function scoreFor(c: number, _cards: number[], target: number): number {
  const suit = Math.floor(c / 13);
    ...
    return (matches ? 18 : 8) + (r >= 9 ? 5 : 0);
}
export function reducer(state: CardAuroraState, action: CardAuroraAction): CardAuroraState {
...
export function isTerminal(state: CardAuroraState): { score: number } | null {
