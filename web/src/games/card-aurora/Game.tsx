...
export function CardAuroraGame({ state, dispatch, onGameOver }: GameProps<CardAuroraState, CardAuroraSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      <div className="cm-hint">Card Aurora</div>
      ...
      <div className="cm-row">
        {state.cards.map((c, i) => (
          <button key={i}
            ...
            onClick={() => dispatch({ type:"pick", index:i } as CardAuroraAction)}>{cardName(c)}</button>
        ))}
      </div>
      {state.phase === "scored" && (
        <>
          <div className="cm-result">Picked: {cardName(state.picked!)} — {state.lastPts > 0 ? `+${state.lastPts}` : "0"}</div>
          <button data-testid="hint-target-card-aurora-primary" className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardAuroraAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
