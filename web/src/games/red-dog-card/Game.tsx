...
export function RedDogCardGame({ state, dispatch, onGameOver }: GameProps<RedDogCardState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      <div className="g-controls">
        {state.phase === "ready" && <button data-testid="hint-target-red-dog-card-deal" className="g-btn" onClick={() => dispatch({ type: "deal" } as RedDogCardAction)}>Deal Hand</button>}
        {state.phase === "dealt" && <button data-testid="hint-target-red-dog-card-next" className="g-btn" onClick={() => dispatch({ type: "next" } as RedDogCardAction)}>Next Round</button>}
      </div>
    </div>
  );
}
