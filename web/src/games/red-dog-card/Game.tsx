  ...
  return (
    <div className="g-wrap">
      ...
      <div className="g-controls">
        {state.phase === "ready" && <button className="g-btn" onClick={() => dispatch({ type: "deal" } as RedDogCardAction)}>Deal Hand</button>}
        {state.phase === "dealt" && <button className="g-btn" onClick={() => dispatch({ type: "next" } as RedDogCardAction)}>Next Round</button>}
      </div>
    </div>
  );
}
