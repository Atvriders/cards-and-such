...
export interface RedDogCardSettings { rounds: "10"; }

export interface RedDogCardState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type RedDogCardAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: RedDogCardSettings): RedDogCardState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: RedDogCardState, action: RedDogCardAction): RedDogCardState {
...
export function isTerminal(state: RedDogCardState): { score: number } | null {
