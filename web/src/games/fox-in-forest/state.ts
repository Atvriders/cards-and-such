// The Fox in the Forest — Two-player trumping game — set-trump duel.
...
export type FoxInForestPhase = "playing" | "done";

export interface FoxInForestState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: FoxInForestPhase;
  playerLeads: boolean;
  message: string;
}

export type FoxInForestAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `fox-in-forest-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: FoxInForestState, trick: readonly { seat: number; card: Card }[]): FoxInForestState {
...
export function reducer(state: FoxInForestState, action: FoxInForestAction): FoxInForestState {
  ...
  let s: FoxInForestState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: FoxInForestState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: FoxInForestState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): FoxInForestState {
...
export function isTerminal(state: FoxInForestState): { score: number } | null {
