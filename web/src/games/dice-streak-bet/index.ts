...
const DiceStreakBetGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceStreakBetGame as unknown as React.ComponentType<unknown> })));
...
Start with 100 coins. Choose 8 or 12 rounds in Settings. Your final coin total is your score.`,
  ...
  hint: (state: DiceStreakBetState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "betting") return { selector: '[data-testid="hint-target-dicestreak-bet"]', pulses: 3 };
    if (state.phase === "revealed") return { selector: '[data-testid="hint-target-dicestreak-next"]', pulses: 3 };
    return null;
  },
};
