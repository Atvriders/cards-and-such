...
export interface avenueRoadsSettings { dummy: boolean; }
export interface avenueRoadsState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type avenueRoadsAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: avenueRoadsSettings): avenueRoadsState {
  return {
    rngSeed: seed,
...
export function reducer(state: avenueRoadsState, action: avenueRoadsAction): avenueRoadsState {
...
export function isTerminal(state: avenueRoadsState): { score: number } | null {
