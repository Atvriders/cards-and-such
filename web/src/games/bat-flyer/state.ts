...
// Bat Flyer: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface BatFlyerSettings { dummy: boolean; }

export interface BatFlyerTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface BatFlyerState {
  rngSeed: number;
  targets: BatFlyerTarget[];
  nextId: number;
...
export type BatFlyerAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: BatFlyerSettings): BatFlyerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: BatFlyerState, action: BatFlyerAction): BatFlyerState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: BatFlyerTarget[] = [];
...
export function isTerminal(state: BatFlyerState): { score: number } | null {
