// Auto-generated race-game state for parchis-spanish.
...
export const SPECIALS: Record<number, Special> = {12: {type:"safe"},17: {type:"safe"},29: {type:"safe"},34: {type:"safe"},46: {type:"safe"},51: {type:"safe"},63: {type:"safe"}};

export type Side = "P" | "C";
export interface ParchisSettings { dummy: boolean }
...
export function initialState(seed: number, _s: ParchisSettings): ParchisState {
  return {
    rngSeed: seed,
...
function checkWin(s: ParchisState): ParchisState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: ParchisState): ParchisState {
  let s = state;
...
export function legalMovesForP(state: ParchisState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: ParchisState, action: ParchisAction): ParchisState {
...
export function isTerminal(state: ParchisState): { score: number } | null {
