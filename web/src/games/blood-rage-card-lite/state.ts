...
export const SUIT_NAMES = ["Bear","Wolf","Raven","Serpent"] as const;
...
export type BloodRageCardLiteState = DraftState;
export type BloodRageCardLiteAction = DraftAction;
...
export function initialState(seed: number, _settings: BloodRageCardLiteSettings): BloodRageCardLiteState {
...
export function score(state: BloodRageCardLiteState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: BloodRageCardLiteState): { score: number } | null {
