...
export const SUIT_NAMES: readonly string[] = ["Bear","Wolf","Raven","Serpent"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface BloodRageCardLiteSettings { dummy: boolean; }
export interface BloodRageCardLiteState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BloodRageCardLiteAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BloodRageCardLiteSettings): BloodRageCardLiteState {
...
export function reducer(state: BloodRageCardLiteState, action: BloodRageCardLiteAction): BloodRageCardLiteState {
...
export function score(s: BloodRageCardLiteState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BloodRageCardLiteState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
