...
export interface RoyalCasinoSettings { dummy: boolean; }
export interface RoyalCasinoState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type RoyalCasinoAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: RoyalCasinoSettings): RoyalCasinoState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: RoyalCasinoState, action: RoyalCasinoAction): RoyalCasinoState {
...
export function isTerminal(state: RoyalCasinoState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
