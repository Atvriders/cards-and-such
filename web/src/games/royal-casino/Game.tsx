...
export function RoyalCasinoGame({ state, dispatch, onGameOver }: GameProps<RoyalCasinoState, RoyalCasinoSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as RoyalCasinoAction)}>Capture!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as RoyalCasinoAction)}>Next</button>
      </>}
    </div>
  );
}
