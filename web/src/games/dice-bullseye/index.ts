...
const DiceBullseyeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceBullseyeGame as unknown as React.ComponentType<unknown> })));
...
export const diceBullseyePlugin: GamePlugin<DiceBullseyeState, DiceBullseyeAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceBullseyeState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "aim") return { selector: '[data-testid="hint-target-dice-bullseye-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-dice-bullseye-next"]', pulses: 3 };
    return null;
  },
  component:DiceBullseyeGame,
};
