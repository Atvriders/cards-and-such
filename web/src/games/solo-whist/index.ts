...
const SoloWhistGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SoloWhistGame as unknown as React.ComponentType<unknown> })));
...
export const soloWhistPlugin: GamePlugin<SoloWhistState, SoloWhistAction, typeof settings> = {
  ...
  description: "Solo Whist — every player for themselves.",
  howToPlay: "Solo Whist — every player for themselves. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as SoloWhistSettings),
  reducer,
  isTerminal,
  hint: (state: SoloWhistState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-solowh-${pick.id}"]`, pulses: 3 };
    }
