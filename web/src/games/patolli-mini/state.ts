// Auto-generated race-game state for patolli-mini.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const TRACK_LEN = 16;
export const PAWNS_PER_SIDE = 2;
...
export const SPECIALS: Record<number, Special> = {4: {type:"safe"},8: {type:"safe"},12: {type:"safe"}};

export type Side = "P" | "C";
export interface PatolliMiniSettings { dummy: boolean }

export interface PatolliMiniState {
  rngSeed: number;
...
export function initialState(seed: number, _s: PatolliMiniSettings): PatolliMiniState {
  return {
    rngSeed: seed,
...
function checkWin(s: PatolliMiniState): PatolliMiniState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: PatolliMiniState): PatolliMiniState {
  let s = state;
...
export function legalMovesForP(state: PatolliMiniState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: PatolliMiniState, action: PatolliMiniAction): PatolliMiniState {
...
export function isTerminal(state: PatolliMiniState): { score: number } | null {
