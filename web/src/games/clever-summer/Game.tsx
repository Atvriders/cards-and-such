  ...
  if (state.phase === "done") {
    return (
      <div className="rw-wrap">
        <h3 className="rw-title">Clever: Summer</h3>
      ...
      <div className="rw-grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 44px)` }}>
        {state.cells.map((filled, i) => (
          <button
            ...
            onClick={() => dispatch({ type: "mark", index: i } as CleverSummerAction)}
          >{filled ? state.cellValues[i] : ""}</button>
        ))}
      </div>
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as CleverSummerAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as CleverSummerAction)}>Skip</button>
      )}
    </div>
  );
}
