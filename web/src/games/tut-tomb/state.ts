...
export interface TutTombSettings { dummy: boolean; }

export interface TutTombState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export function reducer(state: TutTombState, action: TutTombAction): TutTombState {
...
export function isTerminal(state: TutTombState): { score: number } | null {
