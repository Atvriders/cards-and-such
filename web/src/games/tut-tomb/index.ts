...
const TutTombGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TutTombGame as unknown as React.ComponentType<unknown> })));
...
export const tutTombPlugin: GamePlugin<TutTombState, TutTombAction, typeof settings> = {
  ...
  hint: (state: TutTombState): HintTarget | null => {
    ...
    if (king) return { selector: `[data-testid="hint-target-tut-tomb-pyramid-${king.row}-${king.col}"]`, pulses: 3 };
    for (const a of avail) {
      for (const b of avail) {
        if (a === b) continue;
        if (a.rank + b.rank === 13) {
          return { selector: `[data-testid="hint-target-tut-tomb-pyramid-${a.row}-${a.col}"]`, pulses: 3 };
        }
      }
    }
    ...
    if (wasteTop) {
      ...
      if (m) return { selector: `[data-testid="hint-target-tut-tomb-pyramid-${m.row}-${m.col}"]`, pulses: 3 };
    }
    if (state.stock.length > 0) return { selector: '[data-testid="hint-target-tut-tomb-draw"]', pulses: 3 };
    if (state.redealsRemaining > 0) return { selector: '[data-testid="hint-target-tut-tomb-redeal"]', pulses: 3 };
    return null;
  },
  reducer,
  isTerminal,
