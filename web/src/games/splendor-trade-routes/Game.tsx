...
export function SplendorTradeRoutesGame({ state, dispatch, onGameOver }: GameProps<SplendorTradeRoutesState, SplendorTradeRoutesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Splendor: Trade Routes</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SplendorTradeRoutesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SplendorTradeRoutesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
