...
export type SplendorTradeRoutesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SplendorTradeRoutesSettings): SplendorTradeRoutesState {
...
export function score(s: SplendorTradeRoutesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SplendorTradeRoutesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
