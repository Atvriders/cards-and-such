...
export interface DaldosSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface DaldosState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type DaldosAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: DaldosSettings): DaldosState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: DaldosState, action: DaldosAction): DaldosState {
...
export function isTerminal(state: DaldosState): { score: number } | null {
