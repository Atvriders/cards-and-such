...
import "./Game.css";
export function BurnCardPokerGame({ state, dispatch, onGameOver }: GameProps<BurnCardPokerState, BurnCardPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-burn-card-poker-deal" onClick={() => dispatch({ type: "deal" } as BurnCardPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-burn-card-poker-next" onClick={() => dispatch({ type: "next" } as BurnCardPokerAction)}>Next</button>
      </>}
    </div>
  );
}
