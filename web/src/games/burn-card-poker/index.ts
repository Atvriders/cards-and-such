...
const BurnCardPokerGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BurnCardPokerGame as unknown as React.ComponentType<unknown> })));
...
export const burnCardPokerPlugin: GamePlugin<BurnCardPokerState, BurnCardPokerAction, typeof settings> = {
  ...
  reducer,isTerminal,  hint: (state: BurnCardPokerState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-burn-card-poker-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-burn-card-poker-next"]', pulses: 3 };
    return null;
  },
  component:BurnCardPokerGame,
};
