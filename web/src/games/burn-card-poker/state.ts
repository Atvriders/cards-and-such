...
export interface BurnCardPokerSettings { dummy: boolean; }
export interface BurnCardPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type BurnCardPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BurnCardPokerSettings): BurnCardPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: BurnCardPokerState, action: BurnCardPokerAction): BurnCardPokerState {
...
export function isTerminal(state: BurnCardPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
