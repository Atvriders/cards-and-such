...
export interface ValeriaDiceBuildSettings { rounds: "10"; }

export interface ValeriaDiceBuildState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type ValeriaDiceBuildAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: ValeriaDiceBuildSettings): ValeriaDiceBuildState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: ValeriaDiceBuildState, action: ValeriaDiceBuildAction): ValeriaDiceBuildState {
...
export function isTerminal(state: ValeriaDiceBuildState): { score: number } | null {
