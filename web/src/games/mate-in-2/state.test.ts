...
describe("mate-in-2 initialState", () => {
  ...
  it("board has pieces", () => {
    const s = initialState();
    expect(s.board.filter(p => p !== null).length).toBeGreaterThan(2);
  });
});

describe("mate-in-2 reducer", () => {
  it("wrong first move sets status to wrong", () => {
    const s = initialState();
    // pick a move that is definitely not the key move — move from a corner
    const after = reducer(s, { type: "move", from: { row: 7, col: 0 }, to: { row: 6, col: 0 } });
    expect(after.status).not.toBe("solved");
  });

  it("retry resets to initial puzzle position", () => {
