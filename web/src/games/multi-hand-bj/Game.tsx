  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <div className="dm-row">
        <button className="dm-btn" onClick={() => dispatch({ type: "hit" } as MultiHandBjAction)}>Hit</button>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "stand" } as MultiHandBjAction)}>Stand</button>
      </div>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MultiHandBjAction)}>Next</button>
      </>}
    </div>
  );
}
