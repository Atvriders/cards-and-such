...
import "./Game.css";
export function CardSavannaGame({ state, dispatch, onGameOver }: GameProps<CardSavannaState, CardSavannaSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      <div className="cm-info">Savanna — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardSavannaAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
