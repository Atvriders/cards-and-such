...
export function SevenWondersPyramidExtraGame({ state, dispatch, onGameOver }: GameProps<SevenWondersPyramidExtraState, SevenWondersPyramidExtraSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">7 Wonders: Pyramid Extra</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SevenWondersPyramidExtraAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SevenWondersPyramidExtraAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
