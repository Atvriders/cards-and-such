...
export const SUIT_NAMES: readonly string[] = ["Age 1","Age 2","Age 3","Guild"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface SevenWondersPyramidExtraSettings { dummy: boolean; }
export interface SevenWondersPyramidExtraState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersPyramidExtraAction = { type: "pick"; idx: number } | { type: "next" };
...
function dealOffer(rng: () => number): DraftCard[] {
  const out: DraftCard[] = [];
  for (let i = 0; i < 3; i++) {
    out.push({ id: Math.floor(rng() * 1e9), rank: 1 + Math.floor(rng() * 9), suit: Math.floor(rng() * SUIT_NAMES.length) });
  }
  return out;
}
...
export function initialState(seed: number, _s: SevenWondersPyramidExtraSettings): SevenWondersPyramidExtraState {
...
export function reducer(state: SevenWondersPyramidExtraState, action: SevenWondersPyramidExtraAction): SevenWondersPyramidExtraState {
...
export function score(s: SevenWondersPyramidExtraState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersPyramidExtraState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
