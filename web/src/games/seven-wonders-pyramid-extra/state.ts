...
export const SUIT_NAMES = ["Sand","Sun","Slave","Pharaoh"] as const;
...
export type SevenWondersPyramidExtraState = DraftState;
export type SevenWondersPyramidExtraAction = DraftAction;
...
export function initialState(seed: number, _settings: SevenWondersPyramidExtraSettings): SevenWondersPyramidExtraState {
...
export function score(state: SevenWondersPyramidExtraState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersPyramidExtraState): { score: number } | null {
