...
export const RATING_BANDS = [0,30,70,110] as const;

export interface CastlesInSpainSettings { dummy: boolean; }

export interface CastlesInSpainState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type CastlesInSpainAction =
  | { type: "keep" }
...
function variantBonus(hand: number[]): number {
  ...
  const royals = ranks.filter((r) => r >= 11).length; return royals * 3;
}
...
export function initialState(seed: number, _s: CastlesInSpainSettings): CastlesInSpainState {
...
export function reducer(state: CastlesInSpainState, action: CastlesInSpainAction): CastlesInSpainState {
...
export function isTerminal(state: CastlesInSpainState): { score: number } | null {
