...
export interface ScrewGrabSettings { dummy: boolean; }
...
export interface ScrewGrabState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type ScrewGrabAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: ScrewGrabSettings): ScrewGrabState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: ScrewGrabState, action: ScrewGrabAction): ScrewGrabState {
...
export function isTerminal(state: ScrewGrabState): { score: number } | null {
