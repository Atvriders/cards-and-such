...
const ScrewGrabGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ScrewGrabGame as unknown as React.ComponentType<unknown> })));
...
export const screwGrabPlugin: GamePlugin<ScrewGrabState, ScrewGrabAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: ScrewGrabState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-screw-grab-target"]', pulses: 3 };
  },
  component:ScrewGrabGame,
};
