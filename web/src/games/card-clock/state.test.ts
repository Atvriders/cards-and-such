...
describe("CardClock", () => {
  ...
  it("isHit returns boolean for any card 0..51 at hour 1", () => { for (let c = 0; c < 52; c++) expect(typeof isHit(c, 1)).toBe("boolean"); });
  it("Ace matches hour 1", () => { expect(isHit(12, 1)).toBe(true); });
  it("currentHour increments on next", () => {
    let s = initialState(1, S);
    expect(s.currentHour).toBe(1);
    s = reducer(s, { type: "draw" });
    if (s.phase === "shown") s = reducer(s, { type: "next" });
    expect(s.currentHour).toBe(2);
  });
