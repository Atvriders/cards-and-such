...
export function ZodiacSoliGame({ state, dispatch, onGameOver }: GameProps<ZodiacSoliState, ZodiacSoliSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as ZodiacSoliAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
