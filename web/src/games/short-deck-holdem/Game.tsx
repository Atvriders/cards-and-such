...
export function ShortDeckHoldemGame({ state, dispatch, onGameOver }: GameProps<ShortDeckHoldemState, ShortDeckHoldemSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as ShortDeckHoldemAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ShortDeckHoldemAction)}>Next</button>
      </>}
    </div>
  );
}
