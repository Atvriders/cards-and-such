...
export interface ShortDeckHoldemSettings { dummy: boolean; }
export interface ShortDeckHoldemState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ShortDeckHoldemAction = { type: "deal" } | { type: "next" };

// Short-deck (36 cards): ranks 6..A only (9 ranks across 4 suits)
export function cardName(c: number): string { const r=["6","7","8","9","10","J","Q","K","A"]; const s=["♠","♥","♦","♣"]; return r[c%9]!+s[Math.floor(c/9)]!; }
export function isRed(c: number): boolean { const s=Math.floor(c/9); return s===1||s===2; }
function dealHand(rng:()=>number, n:number): number[] { const u=new Set<number>(); const out:number[]=[]; while(out.length<n){const c=Math.floor(rng()*36); if(!u.has(c)){u.add(c); out.push(c);}} return out; }

function rateFiveCards(hand5: number[]): { rank: string; pts: number } {
  const ranks = hand5.map(c=>c%9).sort((a,b)=>a-b);
  const suits = hand5.map(c=>Math.floor(c/9));
  ...
  // short-deck wheel: 0,1,2,3,8 (6-7-8-9-A as low-A wheel)
  if(!straight && ranks[0]===0 && ranks[1]===1 && ranks[2]===2 && ranks[3]===3 && ranks[4]===8) straight=true;
...
export function initialState(seed: number, _s: ShortDeckHoldemSettings): ShortDeckHoldemState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: ShortDeckHoldemState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
