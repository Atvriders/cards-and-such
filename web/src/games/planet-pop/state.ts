...
export interface PlanetPopSettings { dummy: boolean; }
export interface PlanetPopTarget { id: number; lane: number; ticksLeft: number; }
export interface PlanetPopState {
  rngSeed: number;
  targets: PlanetPopTarget[];
  nextId: number;
...
export type PlanetPopAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: PlanetPopSettings): PlanetPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: PlanetPopState, action: PlanetPopAction): PlanetPopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: PlanetPopTarget[] = [];
...
export function isTerminal(state: PlanetPopState): { score: number } | null {
