...
export interface SnookerSixRedSettings { dummy: boolean; }

export interface SnookerSixRedState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type SnookerSixRedAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: SnookerSixRedSettings): SnookerSixRedState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: SnookerSixRedState, action: SnookerSixRedAction): SnookerSixRedState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<SnookerSixRedState> = {};
    const pot = dice[0]! >= 4;
    if (!pot) {
      pts = 0;
      extra.breakRun = 0;
...
export function isTerminal(state: SnookerSixRedState): { score: number } | null {
