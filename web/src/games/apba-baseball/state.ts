...
export interface ApbaBaseballSettings { dummy: boolean; }

export interface ApbaBaseballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type ApbaBaseballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: ApbaBaseballSettings): ApbaBaseballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: ApbaBaseballState, action: ApbaBaseballAction): ApbaBaseballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<ApbaBaseballState> = {};
...
export function isTerminal(state: ApbaBaseballState): { score: number } | null {
