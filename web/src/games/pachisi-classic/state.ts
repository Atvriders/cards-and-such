// Auto-generated race-game state for pachisi-classic.
...
export const SPECIALS: Record<number, Special> = {7: {type:"safe"},14: {type:"safe"},21: {type:"safe"},28: {type:"safe"},35: {type:"safe"},42: {type:"safe"},49: {type:"safe"}};

export type Side = "P" | "C";
export interface PachisiSettings { dummy: boolean }
...
export function initialState(seed: number, _s: PachisiSettings): PachisiState {
  return {
    rngSeed: seed,
...
function checkWin(s: PachisiState): PachisiState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: PachisiState): PachisiState {
  let s = state;
...
export function legalMovesForP(state: PachisiState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: PachisiState, action: PachisiAction): PachisiState {
...
export function isTerminal(state: PachisiState): { score: number } | null {
