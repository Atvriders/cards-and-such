...
export const pachisiClassicPlugin: GamePlugin<PachisiState, PachisiAction, typeof settings> = {
  id: "pachisi-classic",
  ...
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as PachisiSettings),
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "done" || p === "gameover" || p === "ended" || p === "finished" || (s as any).gameOver || (s as any).won || (s as any).complete || (s as any).isComplete) return null; return { selector: ".pachisi-btn", pulses: 3 }; },
  component: PachisiGame,
};
