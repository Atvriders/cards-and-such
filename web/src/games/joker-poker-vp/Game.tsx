...
import "./Game.css";
export function JokerPokerVpGame({ state, dispatch, onGameOver }: GameProps<JokerPokerVpState, JokerPokerVpSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-joker-poker-vp-primary" onClick={() => dispatch({ type: "deal" } as JokerPokerVpAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as JokerPokerVpAction)}>Next</button>
      </>}
    </div>
  );
}
