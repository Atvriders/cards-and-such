...
export interface JokerPokerVpSettings { dummy: boolean; }
export interface JokerPokerVpState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type JokerPokerVpAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: JokerPokerVpSettings): JokerPokerVpState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: JokerPokerVpState, action: JokerPokerVpAction): JokerPokerVpState {
...
export function isTerminal(state: JokerPokerVpState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
