...
export interface CardToyStoreSettings { dummy: boolean; }
export interface CardToyStoreState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardToyStoreAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardToyStoreSettings): CardToyStoreState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardToyStoreState, action: CardToyStoreAction): CardToyStoreState {
...
export function isTerminal(state: CardToyStoreState): { score: number } | null {
