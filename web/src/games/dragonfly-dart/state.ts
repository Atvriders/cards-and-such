...
export interface DragonflyDartSettings { dummy: boolean; }
...
export interface DragonflyDartState {
  rngSeed: number;
  bugs: Bug[];
  nextId: number;
...
export type DragonflyDartAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: DragonflyDartSettings): DragonflyDartState {
  return { rngSeed: seed, bugs: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: DragonflyDartState, action: DragonflyDartAction): DragonflyDartState {
...
export function isTerminal(state: DragonflyDartState): { score: number } | null {
