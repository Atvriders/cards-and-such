...
export interface ZhengShangyouShedSettings { dummy: boolean; }
export interface ZhengShangyouShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type ZhengShangyouShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: ZhengShangyouShedSettings): ZhengShangyouShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: ZhengShangyouShedState, action: ZhengShangyouShedAction): ZhengShangyouShedState {
...
export function isTerminal(state: ZhengShangyouShedState): { score: number } | null {
