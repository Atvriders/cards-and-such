...
export interface HymnCountSettings { dummy: boolean; }
export interface HymnCountState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type HymnCountAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: HymnCountSettings): HymnCountState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: HymnCountState, action: HymnCountAction): HymnCountState {
...
export function isTerminal(state: HymnCountState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
