...
export function GreekCuisineQuiz({ state, dispatch, onGameOver }: GameProps<GreekCuisineQuizState, GreekCuisineQuizSettings>): JSX.Element {
      ...
      {entry && <>
        ...
        <div className="cq-choices">
          {entry.choices.map((c, i) => {
            ...
            return <button key={i} className={cls} disabled={state.selected !== null} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", index:i } as GreekCuisineQuizAction)}>{c}</button>;
          })}
        </div>
        {state.selected !== null && <button className="cq-next" onClick={() => dispatch({ type:"next" } as GreekCuisineQuizAction)}>{state.current + 1 < state.entries.length ? "Next" : "Finish"}</button>}
      </>}
    </div>
  );
}
