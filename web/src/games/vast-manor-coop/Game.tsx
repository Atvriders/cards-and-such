...
import "./Game.css";
export function vastManorCoopGame({ state, dispatch, onGameOver }: GameProps<vastManorCoopState, vastManorCoopSettings>): JSX.Element {
  ...
  return (
    <div className="coop-wrap fade-in">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as vastManorCoopAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as vastManorCoopAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
