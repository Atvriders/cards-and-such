...
export interface vastManorCoopSettings { dummy: boolean; }
export interface vastManorCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type vastManorCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: vastManorCoopSettings): vastManorCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: vastManorCoopState, action: vastManorCoopAction): vastManorCoopState {
...
export function isTerminal(state: vastManorCoopState): { score: number } | null {
