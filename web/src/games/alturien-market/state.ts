...
export interface AlturienMarketSettings { dummy: boolean; }
export interface AlturienMarketState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type AlturienMarketAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: AlturienMarketSettings): AlturienMarketState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Demand spike on copper","Futures volatility","Wheat harvest panic","Oil glut suspected","Currency devaluation","Gold rally"];

export function reducer(state: AlturienMarketState, action: AlturienMarketAction): AlturienMarketState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Goods (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Trader (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Traded 1 Goods (+$" + market + ")"; delta = market;
      } else { evt = "No Goods to trade"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
...
export function score(s: AlturienMarketState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: AlturienMarketState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
