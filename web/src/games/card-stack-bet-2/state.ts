...
export interface CardStackBet2Settings { rounds: "8" | "12"; }
export interface CardStackBet2State { deck: number[]; pos: number; topCard: number; coins: number; round: number; maxRounds: number; phase: "betting" | "revealed" | "gameover"; lastResult: "win" | "lose" | "tie" | null; revealedCard: number | null; bid: number; }
export type CardStackBet2Action = { type: "bet"; amount: number; dir: "higher" | "lower" } | { type: "next" };
...
export function initialState(seed: number, settings: CardStackBet2Settings): CardStackBet2State {
  const rng=mulberry32(seed); const deck=makeDeck(rng);
  return {deck,pos:1,topCard:deck[0]!,coins:120,bid:0,round:1,maxRounds:parseInt(settings.rounds,10),phase:"betting",lastResult:null,revealedCard:null};
}
export function reducer(state: CardStackBet2State, action: CardStackBet2Action): CardStackBet2State {
...
export function isTerminal(state: CardStackBet2State): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
