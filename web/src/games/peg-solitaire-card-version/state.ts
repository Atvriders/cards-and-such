...
export const RATING_BANDS = [0,28,65,105] as const;

export interface PegSolitaireCardVersionSettings { dummy: boolean; }

export interface PegSolitaireCardVersionState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type PegSolitaireCardVersionAction =
  | { type: "keep" }
...
function variantBonus(hand: number[]): number {
  ...
  const evens = ranks.filter((r) => r % 2 === 0).length; return evens >= 3 ? evens * 2 : 0;
}
...
export function initialState(seed: number, _s: PegSolitaireCardVersionSettings): PegSolitaireCardVersionState {
...
export function reducer(state: PegSolitaireCardVersionState, action: PegSolitaireCardVersionAction): PegSolitaireCardVersionState {
...
export function isTerminal(state: PegSolitaireCardVersionState): { score: number } | null {
