...
export interface FarmageddonCropsSettings { dummy: boolean; }
export interface FarmageddonCropsState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type FarmageddonCropsAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: FarmageddonCropsSettings): FarmageddonCropsState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Drought reduces yields","Locust swarm threatens","Bumper harvest","Pest control success","Rainstorm boost","Frost warning"];

export function reducer(state: FarmageddonCropsState, action: FarmageddonCropsAction): FarmageddonCropsState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Crops (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Farmhand (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Traded 1 Crops (+$" + market + ")"; delta = market;
      } else { evt = "No Crops to trade"; }
    } else { return state; }
    const div = assets * 6;
    cash += div;
    const pay = workers * 10;
    cash += pay;
...
export function score(s: FarmageddonCropsState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: FarmageddonCropsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
