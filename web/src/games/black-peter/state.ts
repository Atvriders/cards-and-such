...
export interface BlackPeterSettings { dummy: boolean; }
export interface BlackPeterState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type BlackPeterAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: BlackPeterSettings): BlackPeterState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: BlackPeterState, action: BlackPeterAction): BlackPeterState {
...
export function isTerminal(state: BlackPeterState): { score: number } | null {
