...
export interface MelonMashSettings { dummy: boolean; }
...
export interface MelonMashState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type MelonMashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: MelonMashSettings): MelonMashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: MelonMashState, action: MelonMashAction): MelonMashState {
...
export function isTerminal(state: MelonMashState): { score: number } | null {
