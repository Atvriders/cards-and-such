...
export interface BlackBashSettings { dummy: boolean; }

export interface BlackBashState {
  rngSeed: number;
  draw: number;
  card: number | null;
  hits: number;
...
export type BlackBashAction = { type: "draw" } | { type: "next" };
...
export function isHit(c: number): boolean { return isBlack(c); }

export function initialState(seed: number, _settings: BlackBashSettings): BlackBashState {
  return { rngSeed: seed, draw: 1, card: null, hits: 0, score: 0, phase: "drawing", lastWasHit: false };
}

export function reducer(state: BlackBashState, action: BlackBashAction): BlackBashState {
...
export function isTerminal(state: BlackBashState): { score: number } | null {
