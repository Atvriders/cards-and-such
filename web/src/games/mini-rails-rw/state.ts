...
export interface MiniRailsRwSettings { dummy: boolean; }
export interface MiniRailsRwState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type MiniRailsRwAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: MiniRailsRwSettings): MiniRailsRwState {
  return {
    rngSeed: seed,
...
export function reducer(state: MiniRailsRwState, action: MiniRailsRwAction): MiniRailsRwState {
...
export function isTerminal(state: MiniRailsRwState): { score: number } | null {
