...
import "./Game.css";
export function GrabDiceGame({ state, dispatch, onGameOver }: GameProps<GrabDiceState, GrabDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-grab-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as GrabDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-grab-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as GrabDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
