...
export function MiniCanfieldGame({ state, dispatch, onGameOver }: GameProps<MiniCanfieldState, MiniCanfieldSettings>): JSX.Element {
    ...
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.layout.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "remove", index: i } as MiniCanfieldAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
