...
export interface WingspanAsiaRwSettings { dummy: boolean; }
export interface WingspanAsiaRwState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type WingspanAsiaRwAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: WingspanAsiaRwSettings): WingspanAsiaRwState {
  return {
    rngSeed: seed,
...
export function reducer(state: WingspanAsiaRwState, action: WingspanAsiaRwAction): WingspanAsiaRwState {
...
export function isTerminal(state: WingspanAsiaRwState): { score: number } | null {
