...
export interface CometCatchSettings { dummy: boolean; }
export interface CometCatchTarget { id: number; lane: number; ticksLeft: number; }
export interface CometCatchState {
  rngSeed: number;
  targets: CometCatchTarget[];
  nextId: number;
...
export type CometCatchAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: CometCatchSettings): CometCatchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: CometCatchState, action: CometCatchAction): CometCatchState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CometCatchTarget[] = [];
...
export function isTerminal(state: CometCatchState): { score: number } | null {
