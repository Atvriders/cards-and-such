...
import "./Game.css";
export function TumblinFlickGame({ state, dispatch, onGameOver }: GameProps<TumblinFlickState, TumblinFlickSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-tumblin-flick-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as TumblinFlickAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-tumblin-flick-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as TumblinFlickAction)}>Next</button>
        </>
      )}
    </div>
  );
