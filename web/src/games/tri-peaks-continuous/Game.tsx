...
export function TriPeaksContinuousGame({ state, dispatch, onGameOver }: GameProps<TriPeaksContinuousState, TriPeaksContinuousSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as TriPeaksContinuousAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as TriPeaksContinuousAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as TriPeaksContinuousAction)}>Discard Hand</button>
      </div>
