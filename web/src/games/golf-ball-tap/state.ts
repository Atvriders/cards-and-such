...
export interface GolfBallTapSettings { dummy: boolean; }
export interface Target {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface GolfBallTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type GolfBallTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: GolfBallTapSettings): GolfBallTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: GolfBallTapState, action: GolfBallTapAction): GolfBallTapState {
...
export function isTerminal(state: GolfBallTapState): { score: number } | null {
