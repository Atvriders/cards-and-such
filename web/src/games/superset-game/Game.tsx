...
export function SupersetGameGame({ state, dispatch, onGameOver }: GameProps<SupersetGameState, SupersetGameSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="supsetg-wrap"><div className="supsetg-done bounce-in"><h2>Done!</h2><div>Correct: {state.correctCount} / {state.rounds.length}</div><div className="supsetg-final">{state.score} pts</div></div></div>;
  }
  const r = state.rounds[state.currentIndex]!;
  return (
    <div className="supsetg-wrap fade-in">
      <div className="supsetg-info">Round {state.currentIndex + 1} / {state.rounds.length}</div>
      <div className="supsetg-score pulse">{state.score} pts</div>
