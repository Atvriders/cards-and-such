...
export interface rollingHeightsSettings { dummy: boolean; }
export interface rollingHeightsState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type rollingHeightsAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: rollingHeightsSettings): rollingHeightsState {
  return {
    rngSeed: seed,
...
export function reducer(state: rollingHeightsState, action: rollingHeightsAction): rollingHeightsState {
...
export function isTerminal(state: rollingHeightsState): { score: number } | null {
