...
export interface OnirimDreamSettings { dummy: boolean; }
export interface OnirimDreamState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type OnirimDreamAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: OnirimDreamSettings): OnirimDreamState {
...
export function reducer(state: OnirimDreamState, action: OnirimDreamAction): OnirimDreamState {
...
export function score(s: OnirimDreamState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: OnirimDreamState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
