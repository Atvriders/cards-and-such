...
import "./Game.css";
export function DudakTavernGame({ state, dispatch, onGameOver }: GameProps<DudakTavernState, DudakTavernSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-dudak-tavern-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as DudakTavernAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-dudak-tavern-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DudakTavernAction)}>Next</button>
        </>
      )}
    </div>
  );
