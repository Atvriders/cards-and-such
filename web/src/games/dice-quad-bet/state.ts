...
export interface DiceQuadBetSettings { rounds: "8" | "12"; }
export interface DiceQuadBetState {
...
export type DiceQuadBetAction = { type: "roll"; amount: number } | { type: "next" };
...
function checkWin(dice: number[]): { win: boolean; label: string } {
  ...
  const win=sum>=15;
  return {win,label:win?`Sum ${sum} — High score!`:`Sum ${sum} — Need 15+`};
}

export function initialState(seed: number, settings: DiceQuadBetSettings): DiceQuadBetState {
...
export function reducer(state: DiceQuadBetState, action: DiceQuadBetAction): DiceQuadBetState {
...
export function isTerminal(state: DiceQuadBetState): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
