...
export interface BlackjackSwitchCasSettings { dummy: boolean }
export interface BlackjackSwitchCasState {
  rngSeed: number;
  round: number;
  you: number[];
  dealer: number[];
...
export type BlackjackSwitchCasAction = { type: "hit" } | { type: "stand" } | { type: "double" } | { type: "surrender" } | { type: "next" };
...
export function initialState(seed: number, _s: BlackjackSwitchCasSettings): BlackjackSwitchCasState {
...
export function reducer(state: BlackjackSwitchCasState, action: BlackjackSwitchCasAction): BlackjackSwitchCasState {
  ...
  if (action.type === "double" && state.phase === "play" && state.you.length === 2) {
    ...
    const r = settleHand(you, dealer, { bjPays: 1.0 });
    const pts = r.pts * 2;
    const isLast = state.round >= TOTAL_ROUNDS;
  ...
  if (action.type === "surrender" && state.phase === "play" && state.you.length === 2 && false) {
    const pts = 5;
    const isLast = state.round >= TOTAL_ROUNDS;
  ...
  if (action.type === "stand" && state.phase === "play") {
    ...
    const r = settleHand(state.you, dealer, { bjPays: 1.0 });
    const isLast = state.round >= TOTAL_ROUNDS;
...
export function isTerminal(state: BlackjackSwitchCasState): { score: number } | null {
