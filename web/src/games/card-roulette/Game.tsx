...
export function CardRouletteGame({ state, dispatch, onGameOver }: GameProps<CardRouletteState, CardRouletteSettings>): JSX.Element {
    ...
    <div className="rg-wrap">
      ...
      {state.phase === "predict" && (
        <div className="rg-row">
          {CHOICES.map(c => (
            <button key={c} className="rg-btn" onClick={() => dispatch({ type:"go", choice:c } as CardRouletteAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && (
