...
describe("IdiomQuiz initialState", () => {
  ...
  it("is deterministic under same seed", () => {
    ...
    expect(s1.entries.map(e => e.idiom)).toEqual(s2.entries.map(e => e.idiom));
  });

  it("entries contain known idioms", () => {
    const s = initialState(42, defaultSettings);
    for (const entry of s.entries) {
      expect(entry.idiom.length).toBeGreaterThan(3);
      expect(entry.answer.length).toBeGreaterThan(3);
    }
  });
});

describe("IdiomQuiz reducer", () => {
