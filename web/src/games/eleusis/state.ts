...
export interface EleusisSettings { dummy: boolean; }
export interface EleusisState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type EleusisAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: EleusisSettings): EleusisState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: EleusisState, action: EleusisAction): EleusisState {
...
export function isTerminal(state: EleusisState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
