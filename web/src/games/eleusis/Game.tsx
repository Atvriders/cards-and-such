...
import "./Game.css";
export function EleusisGame({ state, dispatch, onGameOver }: GameProps<EleusisState, EleusisSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-eleusis-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as EleusisAction)}>Test!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as EleusisAction)}>Next</button>
      </>}
    </div>
  );
}
