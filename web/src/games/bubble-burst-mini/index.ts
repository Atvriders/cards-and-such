...
const BubbleBurstMiniGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BubbleBurstMiniGame as unknown as React.ComponentType<unknown> })));
...
export const bubbleBurstMiniPlugin: GamePlugin<BubbleBurstMiniState, BubbleBurstMiniAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: BubbleBurstMiniState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.items || state.items.length === 0) return null;
    return { selector: '[data-testid="hint-target-bubble-burst-mini-target"]', pulses: 3 };
  },
  component:BubbleBurstMiniGame,
};
