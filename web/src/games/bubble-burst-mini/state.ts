...
export interface BubbleBurstMiniSettings { dummy: boolean; }
export interface BubbleBurstMiniItem {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface BubbleBurstMiniState {
  rngSeed: number;
  items: BubbleBurstMiniItem[];
  nextId: number;
...
export type BubbleBurstMiniAction =
  | { type: "tick" }
  | { type: "pop"; id: number };
export function initialState(seed: number, _settings: BubbleBurstMiniSettings): BubbleBurstMiniState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: BubbleBurstMiniState, action: BubbleBurstMiniAction): BubbleBurstMiniState {
  ...
  if (action.type === "tick") {
    ...
    const newItems: BubbleBurstMiniItem[] = [];
...
export function isTerminal(state: BubbleBurstMiniState): { score: number } | null {
