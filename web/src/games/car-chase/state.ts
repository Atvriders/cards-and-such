...
export interface CarChaseSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface CarChaseState { rngSeed: number; targets: Target[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type CarChaseAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: CarChaseSettings): CarChaseState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: CarChaseState, action: CarChaseAction): CarChaseState {
...
export function isTerminal(state: CarChaseState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
