...
export interface TensOrBetterSettings { dummy: boolean; }
export interface TensOrBetterState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TensOrBetterAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TensOrBetterSettings): TensOrBetterState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TensOrBetterState, action: TensOrBetterAction): TensOrBetterState {
...
export function isTerminal(state: TensOrBetterState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
