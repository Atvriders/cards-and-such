...
const UltiHungarianGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.UltiHungarianGame as unknown as React.ComponentType<unknown> })));
...
export const ultiHungarianPlugin: GamePlugin<UltiHungarianState, UltiHungarianAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: UltiHungarianState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-ultic-${pick.id}"]`, pulses: 3 };
    }
