...
import { KingdominoGame } from "./Game.js";
...
export const kingdominoPlugin: GamePlugin<KingdominoState, KingdominoAction, typeof settings> = {
  ...
  description: "Domino-landscape building; connect matching terrain for crowns.",
  howToPlay: `Kingdomino is a single-player tile-placement puzzle. Domino-landscape building; connect matching terrain for crowns. Tiles draw from a randomized queue, and your job is to place each one onto a small grid in a way that maximises end-of-game scoring.

How it works: Place 12 tiles on a 4x4 grid. Each tile-cluster of matching terrain scores cluster_size * adjacent_count. Bigger same-type clusters compound rapidly.
...
After 12 placements, the game ends and your final score is computed from the board layout. The scoring rule rewards adjacency between matching tile types, so clustering same-coloured tiles is the dominant strategy.
