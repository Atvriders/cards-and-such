...
export function DiceSpinRollGame({ state, dispatch, onGameOver }: GameProps<DiceSpinRollState, DiceSpinRollSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "waiting" && <button className="dm-btn" onClick={() => dispatch({ type:"spin" } as DiceSpinRollAction)}>Spin!</button>}
      {state.phase === "result" && state.dice && <>
        ...
        <button className="dm-btn" onClick={() => dispatch({ type:"next" } as DiceSpinRollAction)}>{state.round >= state.maxRounds ? "Finish" : "Next"}</button>
      </>}
    </div>
  );
}
