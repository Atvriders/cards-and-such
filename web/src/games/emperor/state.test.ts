...
import type { EmperorState } from "./state.js";
...
describe("Emperor initialState", () => {
  ...
  it("10 tableau columns of 4 cards each, 1 face-up", () => {
    const s = initialState(42, settings);
    for (let i = 1; i <= 10; i++) {
...
describe("Emperor ruleset", () => {
  it("allows alternating-color descending tableau move", () => {
    const piles: Pile[] = [
      { id: "t1", kind: "tableau", cards: [{ id: "c1", suit: "♠", rank: 8 }], faceUpCount: 1 },
      { id: "t2", kind: "tableau", cards: [{ id: "c2", suit: "♥", rank: 7 }], faceUpCount: 1 },
    ];
    expect(canMove(piles, { fromPile: "t2", toPile: "t1", count: 1 }, emperorRuleset)).toBe(true);
  });
  ...
  it("rejects same-color tableau move", () => {
    ...
    expect(canMove(piles, { fromPile: "t2", toPile: "t1", count: 1 }, emperorRuleset)).toBe(false);
  });

  it("rejects picking up face-down card", () => {
    const piles: Pile[] = [
      { id: "t1", kind: "tableau", cards: [{ id: "c1", suit: "♠", rank: 8 }], faceUpCount: 0 },
      { id: "t2", kind: "tableau", cards: [{ id: "c2", suit: "♥", rank: 7 }], faceUpCount: 1 },
    ];
    expect(canMove(piles, { fromPile: "t1", toPile: "t2", count: 1 }, emperorRuleset)).toBe(false);
  });
});

describe("Emperor reducer", () => {
  ...
  it("legal move increments score when targeting foundation", () => {
    const piles: Pile[] = [
      ...
      ...Array.from({ length: 9 }, (_, i) => ({ id: `t${i + 2}`, kind: "tableau" as const, cards: [], faceUpCount: 0 })),
    ];
    const s: EmperorState = { piles, score: 0, movesMade: 0, won: false, settings };
...
describe("Emperor isTerminal", () => {
  ...
  it("returns score when 104 cards on foundations", () => {
    ...
    const wonState: EmperorState = { piles: wonPiles, score: 1040, movesMade: 400, won: true, settings };
