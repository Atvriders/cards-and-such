...
export const SCORE_TIE = 6;
export interface PeaceCardsSettings { dummy: boolean; }
export interface PeaceCardsState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type PeaceCardsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: PeaceCardsSettings): PeaceCardsState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: PeaceCardsState, action: PeaceCardsAction): PeaceCardsState {
...
export function isTerminal(state: PeaceCardsState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
