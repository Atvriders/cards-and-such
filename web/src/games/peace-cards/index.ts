...
const PeaceCardsGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PeaceCardsGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: PeaceCardsState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-peace-cards-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-peace-cards-secondary"]', pulses: 3 };
  return null;
};
...
export const peaceCardsPlugin: GamePlugin<PeaceCardsState, PeaceCardsAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: PeaceCardsGame,
};
