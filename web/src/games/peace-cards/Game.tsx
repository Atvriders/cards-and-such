...
export function PeaceCardsGame({ state, dispatch, onGameOver }: GameProps<PeaceCardsState, PeaceCardsSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as PeaceCardsAction)}>Reveal!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PeaceCardsAction)}>Next</button>
      </>}
    </div>
  );
}
