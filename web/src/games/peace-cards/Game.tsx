...
import "./Game.css";
export function PeaceCardsGame({ state, dispatch, onGameOver }: GameProps<PeaceCardsState, PeaceCardsSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-peace-cards-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as PeaceCardsAction)}>Reveal!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-peace-cards-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PeaceCardsAction)}>Next</button>
      </>}
    </div>
  );
}
