...
export interface ZammaAfricanSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface ZammaAfricanState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type ZammaAfricanAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: ZammaAfricanSettings): ZammaAfricanState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: ZammaAfricanState, action: ZammaAfricanAction): ZammaAfricanState {
...
export function isTerminal(state: ZammaAfricanState): { score: number } | null {
