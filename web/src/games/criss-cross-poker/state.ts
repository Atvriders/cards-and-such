...
export interface CrissCrossPokerSettings { dummy: boolean; }
export interface CrissCrossPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type CrissCrossPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CrissCrossPokerSettings): CrissCrossPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: CrissCrossPokerState, action: CrissCrossPokerAction): CrissCrossPokerState {
...
export function isTerminal(state: CrissCrossPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
