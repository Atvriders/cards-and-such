...
import "./Game.css";
export function ThreeTwoOneDownGame({ state, dispatch, onGameOver }: GameProps<ThreeTwoOneDownState, ThreeTwoOneDownSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-three-two-one-down-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as ThreeTwoOneDownAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-three-two-one-down-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ThreeTwoOneDownAction)}>Next</button>
        </>
      )}
    </div>
  );
