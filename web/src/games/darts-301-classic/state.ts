...
export interface Darts301ClassicSettings { dummy: boolean; }

export interface Darts301ClassicState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type Darts301ClassicAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: Darts301ClassicSettings): Darts301ClassicState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: Darts301ClassicState, action: Darts301ClassicAction): Darts301ClassicState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<Darts301ClassicState> = {};
...
export function isTerminal(state: Darts301ClassicState): { score: number } | null {
