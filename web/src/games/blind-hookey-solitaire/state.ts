...
export interface BlindHookeySolitaireSettings { dummy: boolean; }

export interface BlindHookeySolitaireState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type BlindHookeySolitaireAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { const r = cardRank(c); if (r % 2 === 0) s += 4; else s += 1; }
  if (s === 0) s = 1; return s;
}

export function initialState(seed: number, _s: BlindHookeySolitaireSettings): BlindHookeySolitaireState {
...
export function reducer(state: BlindHookeySolitaireState, action: BlindHookeySolitaireAction): BlindHookeySolitaireState {
...
export function isTerminal(state: BlindHookeySolitaireState): { score: number } | null {
