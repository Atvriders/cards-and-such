...
export function BlindHookeySolitaireGame({ state, dispatch, onGameOver }: GameProps<BlindHookeySolitaireState, BlindHookeySolitaireSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as BlindHookeySolitaireAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-blind-hookey-solitaire-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as BlindHookeySolitaireAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as BlindHookeySolitaireAction)}>Discard Hand</button>
      </div>
