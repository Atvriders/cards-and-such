...
import "./Game.css";
export function OklahomaGinRGame({ state, dispatch, onGameOver }: GameProps<OklahomaGinRState, OklahomaGinRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as OklahomaGinRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as OklahomaGinRAction)}>Next</button>
      </>}
    </div>
  );
}
