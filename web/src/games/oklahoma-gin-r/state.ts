...
export interface OklahomaGinRSettings { dummy: boolean; }
export interface OklahomaGinRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type OklahomaGinRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: OklahomaGinRSettings): OklahomaGinRState {
