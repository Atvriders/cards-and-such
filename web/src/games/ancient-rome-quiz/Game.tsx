...
export function AncientRomeQuizGame({ state, dispatch, onGameOver }: GameProps<AncientRomeQuizState, AncientRomeQuizSettings>): JSX.Element {
    ...
    <div className="hq-wrap">
      ...
      {!state.done && entry ? (
        <>
            ...
            {entry.choices.map((choice, i) => {
              ...
              return (
                <button key={i} className={cls} disabled={state.selected !== null}
                  data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type: "select", index: i } as AncientRomeQuizAction)}>
                  {choice}
                </button>
              );
            })}
          ...
          {state.selected !== null && (
            <button className="hq-next" onClick={() => dispatch({ type: "next" } as AncientRomeQuizAction)}>
              {state.current + 1 < state.entries.length ? "Next" : "Finish"}
            </button>
          )}
        </>
      ) : (
        <div className="hq-done">
          <h2>Quiz Complete!</h2>
          <div className="hq-final">Score: {state.score} / {state.entries.length * 10}</div>
        </div>
      )}
    </div>
  );
