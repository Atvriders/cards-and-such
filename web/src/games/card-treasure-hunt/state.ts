...
export interface CardTreasureHuntSettings { dummy: boolean; }
export interface CardTreasureHuntState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardTreasureHuntAction = { type: "draw" } | { type: "next" };
...
export function cardWins(c: number): boolean { const r = c % 13; return r === 12; }
export function initialState(seed: number, _settings: CardTreasureHuntSettings): CardTreasureHuntState {
  return { rngSeed: seed, round: 1, card: null, score: 0, phase: "draw", lastWin: false, lastPts: 0 };
}
export function reducer(state: CardTreasureHuntState, action: CardTreasureHuntAction): CardTreasureHuntState {
...
export function isTerminal(state: CardTreasureHuntState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
