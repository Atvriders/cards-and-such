...
import "./Game.css";
export function CardTreasureHuntGame({ state, dispatch, onGameOver }: GameProps<CardTreasureHuntState, CardTreasureHuntSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "result" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardTreasureHuntAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
