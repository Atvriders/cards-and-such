...
export const DEAL_COUNT = 5;
export interface CardTornadoSettings { dummy: boolean; }
export interface CardTornadoState {
  rngSeed: number;
  round: number;
  cards: number[];
  picked: number | null;
...
export type CardTornadoAction = { type: "pick"; index: number } | { type: "next" } | { type: "deal" };
...
export function initialState(seed: number, _settings: CardTornadoSettings): CardTornadoState {
...
export function scoreFor(c: number, _cards: number[], target: number): number {
  const r = c % 13;
    ...
    const matches = suit === target;
    return rankVal * 2 + (matches ? 10 : 0);
}
export function reducer(state: CardTornadoState, action: CardTornadoAction): CardTornadoState {
...
export function isTerminal(state: CardTornadoState): { score: number } | null {
