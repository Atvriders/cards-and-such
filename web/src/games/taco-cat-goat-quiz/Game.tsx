...
export function TacoCatGoatQuiz({ state, dispatch, onGameOver }: GameProps<TacoCatGoatQuizState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap fade-in">
      ...
      {state.phase === "answered" && (
        <button className="g-btn" onClick={() => dispatch({ type: "next" } as TacoCatGoatQuizAction)}>Next</button>
      )}
    </div>
  );
}
