...
export interface SkittlesWestCountrySettings { dummy: boolean; }

export interface SkittlesWestCountryState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type SkittlesWestCountryAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: SkittlesWestCountrySettings): SkittlesWestCountryState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: SkittlesWestCountryState, action: SkittlesWestCountryAction): SkittlesWestCountryState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<SkittlesWestCountryState> = {};
...
export function isTerminal(state: SkittlesWestCountryState): { score: number } | null {
