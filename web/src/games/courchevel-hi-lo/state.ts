...
export interface CourchevelHiLoSettings { dummy: boolean; }
export interface CourchevelHiLoState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type CourchevelHiLoAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CourchevelHiLoSettings): CourchevelHiLoState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: CourchevelHiLoState, action: CourchevelHiLoAction): CourchevelHiLoState {
...
export function isTerminal(state: CourchevelHiLoState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
