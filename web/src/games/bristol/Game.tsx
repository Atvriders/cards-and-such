...
export function BristolGame({ state, dispatch, onGameOver }: GameProps<BristolState, BristolSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as BristolAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-bristol-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as BristolAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as BristolAction)}>Discard Hand</button>
      </div>
