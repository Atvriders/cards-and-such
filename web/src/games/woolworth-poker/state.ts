...
export interface WoolworthPokerSettings { rounds: "10"; }

export interface WoolworthPokerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type WoolworthPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: WoolworthPokerSettings): WoolworthPokerState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: WoolworthPokerState, action: WoolworthPokerAction): WoolworthPokerState {
...
export function isTerminal(state: WoolworthPokerState): { score: number } | null {
