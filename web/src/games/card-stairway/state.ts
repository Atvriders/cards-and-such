...
export interface CardStairwaySettings { dummy: boolean; }

export interface CardStairwayState {
  rngSeed: number;
  round: number;
  hand: number[];
  score: number;
...
export type CardStairwayAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardStairwaySettings): CardStairwayState {
  return { rngSeed: seed, round: 1, hand: [], score: 0, lastPts: 0, phase: "dealing" };
}

export function reducer(state: CardStairwayState, action: CardStairwayAction): CardStairwayState {
...
export function isTerminal(state: CardStairwayState): { score: number } | null {
