...
export function CardStairwayGame({ state, dispatch, onGameOver }: GameProps<CardStairwayState, CardStairwaySettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardStairwayAction)}>Next</button>
        </>
      )}
    </div>
  );
