...
const DiceStratBasketballGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceStratBasketballGame as unknown as React.ComponentType<unknown> })));
...
export const diceStratBasketballPlugin: GamePlugin<DiceStratBasketballState, DiceStratBasketballAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceStratBasketballState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-strat-basketball-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-strat-basketball-next"]', pulses: 3 };
    return null;
  },
