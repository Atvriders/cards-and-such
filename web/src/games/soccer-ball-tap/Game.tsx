...
import "./Game.css";
export function SoccerBallTapGame({ state, dispatch, onGameOver }: GameProps<SoccerBallTapState, SoccerBallTapSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as SoccerBallTapAction), 750);
  ...
  return (
    <div className="ftap-wrap">
      <div className="ftap-header">
        <span className="ftap-info">Soccer — Hits: {state.popped}</span>
        ...
        {state.targets.map(p => {
          ...
          return (
            <button key={p.id}
              ...
              onClick={() => dispatch({ type:"pop", id:p.id } as SoccerBallTapAction)}
