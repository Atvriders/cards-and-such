...
export interface SoccerBallTapSettings { dummy: boolean; }
export interface Target {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface SoccerBallTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type SoccerBallTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: SoccerBallTapSettings): SoccerBallTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: SoccerBallTapState, action: SoccerBallTapAction): SoccerBallTapState {
...
export function isTerminal(state: SoccerBallTapState): { score: number } | null {
