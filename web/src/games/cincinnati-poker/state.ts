...
export interface CincinnatiPokerSettings { dummy: boolean; }
export interface CincinnatiPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type CincinnatiPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CincinnatiPokerSettings): CincinnatiPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: CincinnatiPokerState, action: CincinnatiPokerAction): CincinnatiPokerState {
...
export function isTerminal(state: CincinnatiPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
