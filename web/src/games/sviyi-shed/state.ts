...
export interface SviyiShedSettings { dummy: boolean; }
export interface SviyiShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SviyiShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: SviyiShedSettings): SviyiShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SviyiShedState, action: SviyiShedAction): SviyiShedState {
...
export function isTerminal(state: SviyiShedState): { score: number } | null {
