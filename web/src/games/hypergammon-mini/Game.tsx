    ...
    <div className="race-wrap">
      ...
      <div className="race-dice-row">
        ...
        {state.phase === "rolling" && isPTurn && (
          <button data-testid="hint-target-hypergammon-mini-primary" className="race-btn" onClick={() => dispatch({ type: "roll" } as RaceAction)}>Roll</button>
        )}
          ...
          <div className="race-pickrow">
            {state.pPositions.map((p, idx) => p < TRACK ? (
              <div key={idx} className="race-pick">
                <span>#{idx+1} @ {p}</span>
                {dieAvailable(0) && <button title="Move checker with die 1" onClick={() => dispatch({ type:"move", checkerIdx: idx, die: 0 } as RaceAction)}>+{state.dice[0]}</button>}
                {dieAvailable(1) && <button title="Move checker with die 2" onClick={() => dispatch({ type:"move", checkerIdx: idx, die: 1 } as RaceAction)}>+{state.dice[1]}</button>}
                {dieAvailable(0) && dieAvailable(1) && <button title="Move checker with both dice" onClick={() => dispatch({ type:"move", checkerIdx: idx, die: 2 } as RaceAction)}>+{state.dice[0]+state.dice[1]}</button>}
              </div>
