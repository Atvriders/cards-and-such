...
const DoubleDoubleBonusCasGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DoubleDoubleBonusCasGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: DoubleDoubleBonusCasState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-double-double-bonus-cas-primary"]', pulses: 3 };
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-double-double-bonus-cas-secondary"]', pulses: 3 };
  return null;
};
export const doubleDoubleBonusCasPlugin: GamePlugin<DoubleDoubleBonusCasState, DoubleDoubleBonusCasAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: DoubleDoubleBonusCasGame,
};
