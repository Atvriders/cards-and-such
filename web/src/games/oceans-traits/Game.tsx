...
export function OceansTraitsGame({ state, dispatch, onGameOver }: GameProps<OceansTraitsState, OceansTraitsSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Oceans: Traits</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as OceansTraitsAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as OceansTraitsAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
