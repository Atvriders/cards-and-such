...
export const SUIT_NAMES = ["Predator","Filter","Symbiote","Migrant","Defender"] as const;
...
export type OceansTraitsState = DraftState;
export type OceansTraitsAction = DraftAction;
...
export function initialState(seed: number, _settings: OceansTraitsSettings): OceansTraitsState {
...
export function score(state: OceansTraitsState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: OceansTraitsState): { score: number } | null {
