...
export const SUIT_NAMES: readonly string[] = ["Predator","Forager","Symbiote","Migrator"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface OceansTraitsSettings { dummy: boolean; }
export interface OceansTraitsState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type OceansTraitsAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: OceansTraitsSettings): OceansTraitsState {
...
export function reducer(state: OceansTraitsState, action: OceansTraitsAction): OceansTraitsState {
...
export function score(s: OceansTraitsState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: OceansTraitsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
