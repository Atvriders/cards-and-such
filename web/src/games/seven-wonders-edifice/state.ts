...
export interface SevenWondersEdificeState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersEdificeAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersEdificeSettings): SevenWondersEdificeState {
...
export function score(s: SevenWondersEdificeState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersEdificeState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
