...
export const SUIT_NAMES = ["Pillar","Stone","Marble","Brick"] as const;
...
export type SevenWondersEdificeState = DraftState;
export type SevenWondersEdificeAction = DraftAction;
export interface SevenWondersEdificeSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SevenWondersEdificeSettings): SevenWondersEdificeState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SevenWondersEdificeState, action: SevenWondersEdificeAction): SevenWondersEdificeState {
  return draftReducer(state, action);
}

export function score(state: SevenWondersEdificeState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersEdificeState): { score: number } | null {
