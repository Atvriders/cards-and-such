...
export function CardBridgeCrossGame({ state, dispatch, onGameOver }: GameProps<CardBridgeCrossState, CardBridgeCrossSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          <div className="cm-result">Plank: +{state.lastPts}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardBridgeCrossAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
