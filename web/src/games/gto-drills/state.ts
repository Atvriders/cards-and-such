...
export interface GtoDrillsSettings { dummy: boolean; }
export interface GtoDrillsState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type GtoDrillsAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: GtoDrillsSettings): GtoDrillsState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: GtoDrillsState, action: GtoDrillsAction): GtoDrillsState {
...
export function isTerminal(state: GtoDrillsState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
