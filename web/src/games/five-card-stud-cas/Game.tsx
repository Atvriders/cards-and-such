...
import "./Game.css";
export function FiveCardStudCasGame({ state, dispatch, onGameOver }: GameProps<FiveCardStudCasState, FiveCardStudCasSettings>): JSX.Element {
  const t = isTerminal(state); useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") return <div className="dm-wrap thm5StudCas"><div className="dm-done bounce-in"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap thm5StudCas">
      ...
      {state.phase === "see" && <div className="dm-row">
        <button data-testid="hint-target-five-card-stud-cas-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as FiveCardStudCasAction)}>Play</button>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "fold" } as FiveCardStudCasAction)}>Fold</button>
      </div>}
      {state.phase === "scored" && <>
