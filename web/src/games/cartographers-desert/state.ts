...
export interface CartographersDesertSettings { dummy: boolean; }
export interface CartographersDesertState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type CartographersDesertAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: CartographersDesertSettings): CartographersDesertState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: CartographersDesertState, action: CartographersDesertAction): CartographersDesertState {
...
export function isTerminal(state: CartographersDesertState): { score: number } | null {
