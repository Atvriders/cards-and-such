...
export function CartographersDesertGame({ state, dispatch, onGameOver }: GameProps<CartographersDesertState, CartographersDesertSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return (
      <div className="rw-wrap">
        <h3 className="rw-title">Cartographers: Desert</h3>
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as CartographersDesertAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as CartographersDesertAction)}>Skip</button>
      )}
    </div>
  );
}
