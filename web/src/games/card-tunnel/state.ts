...
export interface CardTunnelSettings { dummy: boolean; }

export interface CardTunnelState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardTunnelAction = { type: "draw" } | { type: "next" };
...
export function scoreCard(c: number): number {
  ...
  const red = isRed(c);
  if (r >= 9) return 20; if (r >= 6) return 10; return 0;
}

export function initialState(seed: number, _settings: CardTunnelSettings): CardTunnelState {
  return { rngSeed: seed, round: 1, card: null, score: 0, lastPts: 0, phase: "draw" };
}

export function reducer(state: CardTunnelState, action: CardTunnelAction): CardTunnelState {
...
export function isTerminal(state: CardTunnelState): { score: number } | null {
