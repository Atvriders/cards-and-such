...
export interface RedDogCasSettings { dummy: boolean; }
export interface RedDogCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type RedDogCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: RedDogCasSettings): RedDogCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: RedDogCasState, action: RedDogCasAction): RedDogCasState {
...
export function isTerminal(state: RedDogCasState): { score: number } | null {
