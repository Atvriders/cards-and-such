...
const hint = (state: RedDogCasState): HintTarget | null => {
  ...
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-red-dog-cas-primary"]', pulses: 3 };
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-red-dog-cas-secondary"]', pulses: 3 };
  return null;
};
export const redDogCasPlugin: GamePlugin<RedDogCasState, RedDogCasAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: RedDogCasGame,
};
