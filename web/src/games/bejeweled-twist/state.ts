...
// Bejeweled Twist: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface BejeweledTwistSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface BejeweledTwistState {
  rngSeed: number;
  grid: Grid;
...
export type BejeweledTwistAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: BejeweledTwistSettings): BejeweledTwistState {
...
export function reducer(state: BejeweledTwistState, action: BejeweledTwistAction): BejeweledTwistState {
...
export function isTerminal(state: BejeweledTwistState): { score: number } | null {
