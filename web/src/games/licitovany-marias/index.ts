...
const LicitovanyMariasGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.LicitovanyMariasGame as unknown as React.ComponentType<unknown> })));
...
export const licitovanyMariasPlugin: GamePlugin<LicitovanyMariasState, LicitovanyMariasAction, typeof settings> = {
  ...
  description: "Licitovaný Mariáš — bidding Mariáš.",
  howToPlay: "Licitovaný Mariáš — bidding Mariáš. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as LicitovanyMariasSettings),
  reducer,
  isTerminal,
  hint: (state: LicitovanyMariasState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-lic-m-${pick.id}"]`, pulses: 3 };
    }
