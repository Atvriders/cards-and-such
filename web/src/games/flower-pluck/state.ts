...
export interface FlowerPluckSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface FlowerPluckState { rngSeed: number; targets: Target[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type FlowerPluckAction = { type:"tick" } | { type:"pop"; id: number };
export function initialState(seed:number, _s:FlowerPluckSettings):FlowerPluckState {
  return { rngSeed:seed, targets:[], nextId:1, ticksRemaining:TIMER_TICKS, score:0, popped:0, missed:0, phase:"playing" };
}
export function reducer(state:FlowerPluckState, action:FlowerPluckAction):FlowerPluckState {
...
export function isTerminal(state:FlowerPluckState):{score:number}|null { return state.phase==="done"?{score:state.score}:null; }
