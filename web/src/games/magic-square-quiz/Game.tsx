...
export function MagicSquareQuizGame({ state, dispatch, onGameOver }: GameProps<MagicSquareQuizState, MagicSquareQuizSettings>): JSX.Element {
  ...
  return (
    <div className="nlp-wrap">
      ...
      <div className="nlp-actions">
        {!isResult && <button className="nlp-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as MagicSquareQuizAction)}>Submit</button>}
        {isResult && <button className="nlp-btn next" onClick={() => dispatch({ type: "next" } as MagicSquareQuizAction)}>{state.idx + 1 >= state.puzzles.length ? "Finish" : "Next"}</button>}
      </div>
    </div>
  );
}
