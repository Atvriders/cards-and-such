...
export interface CartographersHeroesSettings { dummy: boolean; }
export interface CartographersHeroesState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type CartographersHeroesAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: CartographersHeroesSettings): CartographersHeroesState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: CartographersHeroesState, action: CartographersHeroesAction): CartographersHeroesState {
...
export function isTerminal(state: CartographersHeroesState): { score: number } | null {
