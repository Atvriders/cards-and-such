...
export interface DoubleKlondikePatSettings { dummy: boolean; }

export interface DoubleKlondikePatState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type DoubleKlondikePatAction =
  | { type: "keep" }
...
function scoreHand(hand: number[]): { score: number; reason: string } {
  ...
  const ranks = [...ranksAll].sort((a, b) => a - b);
  let asc = 1, best = 1;
...
export function initialState(seed: number, _s: DoubleKlondikePatSettings): DoubleKlondikePatState {
...
export function reducer(state: DoubleKlondikePatState, action: DoubleKlondikePatAction): DoubleKlondikePatState {
...
export function isTerminal(state: DoubleKlondikePatState): { score: number } | null {
