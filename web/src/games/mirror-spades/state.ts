// Mirror Spades — Spades where your bid equals your spade count and you must hit it.
...
export type MirrorSpadesPhase = "playing" | "done";

export interface MirrorSpadesState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: MirrorSpadesPhase;
  playerLeads: boolean;
  message: string;
}

export type MirrorSpadesAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `mirror-spades-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: MirrorSpadesState, trick: readonly { seat: number; card: Card }[]): MirrorSpadesState {
...
export function reducer(state: MirrorSpadesState, action: MirrorSpadesAction): MirrorSpadesState {
  ...
  let s: MirrorSpadesState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: MirrorSpadesState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: MirrorSpadesState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): MirrorSpadesState {
...
export function isTerminal(state: MirrorSpadesState): { score: number } | null {
