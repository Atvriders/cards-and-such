...
export function ForbiddenJungleCoopGame({ state, dispatch, onGameOver }: GameProps<ForbiddenJungleCoopState, ForbiddenJungleCoopSettings>): JSX.Element {
  ...
  const theme = "Vine-Choked Temple";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as ForbiddenJungleCoopAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as ForbiddenJungleCoopAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
