...
export interface SpoonsShedSettings { dummy: boolean; }
export interface SpoonsShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SpoonsShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: SpoonsShedSettings): SpoonsShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SpoonsShedState, action: SpoonsShedAction): SpoonsShedState {
...
export function isTerminal(state: SpoonsShedState): { score: number } | null {
