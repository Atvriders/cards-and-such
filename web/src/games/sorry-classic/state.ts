// Auto-generated race-game state for sorry-classic.
...
export const SPECIALS: Record<number, Special> = {6: {type:"slide", to:11},18: {type:"slide", to:23},30: {type:"slide", to:35}};

export type Side = "P" | "C";
export interface SorryClSettings { dummy: boolean }

export interface SorryClState {
  rngSeed: number;
...
export function initialState(seed: number, _s: SorryClSettings): SorryClState {
  return {
    rngSeed: seed,
...
function checkWin(s: SorryClState): SorryClState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: SorryClState): SorryClState {
  let s = state;
...
export function legalMovesForP(state: SorryClState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: SorryClState, action: SorryClAction): SorryClState {
...
export function isTerminal(state: SorryClState): { score: number } | null {
