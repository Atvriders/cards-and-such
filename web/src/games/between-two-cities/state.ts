...
export const SUIT_NAMES = ["Tavern","Office","Park","Factory","Shop"] as const;
...
export type BetweenTwoCitiesState = DraftState;
export type BetweenTwoCitiesAction = DraftAction;
export interface BetweenTwoCitiesSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: BetweenTwoCitiesSettings): BetweenTwoCitiesState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: BetweenTwoCitiesState, action: BetweenTwoCitiesAction): BetweenTwoCitiesState {
  return draftReducer(state, action);
}

export function score(state: BetweenTwoCitiesState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: BetweenTwoCitiesState): { score: number } | null {
