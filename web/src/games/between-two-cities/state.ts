...
export interface BetweenTwoCitiesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BetweenTwoCitiesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BetweenTwoCitiesSettings): BetweenTwoCitiesState {
...
export function score(s: BetweenTwoCitiesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BetweenTwoCitiesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
