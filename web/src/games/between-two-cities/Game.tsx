...
export function BetweenTwoCitiesGame({ state, dispatch, onGameOver }: GameProps<BetweenTwoCitiesState, BetweenTwoCitiesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Between Two Cities</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BetweenTwoCitiesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BetweenTwoCitiesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
