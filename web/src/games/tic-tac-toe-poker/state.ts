...
export interface TicTacToePokerSettings { dummy: boolean; }
export interface TicTacToePokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TicTacToePokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TicTacToePokerSettings): TicTacToePokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TicTacToePokerState, action: TicTacToePokerAction): TicTacToePokerState {
...
export function isTerminal(state: TicTacToePokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
