...
export function MrsMopGame({ state, dispatch, onGameOver }: GameProps<MrsMopState, MrsMopSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap fade-in">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button title="Play card" key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as MrsMopAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as MrsMopAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as MrsMopAction)}>Discard Hand</button>
      </div>
