...
export interface SuperiorCanfieldSettings { dummy: boolean; }

export interface SuperiorCanfieldState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type SuperiorCanfieldAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const f = h.filter(c=>cardRank(c)>=11).length; s += f*10; if (s===0) s=1; return s;
}
...
export function reducer(state: SuperiorCanfieldState, action: SuperiorCanfieldAction): SuperiorCanfieldState {
...
export function isTerminal(state: SuperiorCanfieldState): { score: number } | null {
