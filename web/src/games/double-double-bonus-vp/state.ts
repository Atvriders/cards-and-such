...
export interface DoubleDoubleBonusVpSettings { rounds: "10"; }

export interface DoubleDoubleBonusVpState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type DoubleDoubleBonusVpAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DoubleDoubleBonusVpSettings): DoubleDoubleBonusVpState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: DoubleDoubleBonusVpState, action: DoubleDoubleBonusVpAction): DoubleDoubleBonusVpState {
...
export function isTerminal(state: DoubleDoubleBonusVpState): { score: number } | null {
