...
export interface CardBid4Settings { rounds: "8" | "12"; }
export interface CardBid4State { deck: number[]; pos: number; topCard: number; coins: number; round: number; maxRounds: number; phase: "betting" | "revealed" | "gameover"; lastResult: "win" | "lose" | "tie" | null; revealedCard: number | null; bid: number; }
export type CardBid4Action = { type: "bet"; amount: number; dir: "higher" | "lower" } | { type: "next" };
...
export function initialState(seed: number, settings: CardBid4Settings): CardBid4State {
  const rng=mulberry32(seed); const deck=makeDeck(rng);
  return {deck,pos:1,topCard:deck[0]!,coins:200,bid:0,round:1,maxRounds:parseInt(settings.rounds,10),phase:"betting",lastResult:null,revealedCard:null};
}
export function reducer(state: CardBid4State, action: CardBid4Action): CardBid4State {
  ...
  if(action.type==="bet"&&state.phase==="betting"){
    ...
    const newCoins=Math.max(0,state.coins+(result==="win"?amount*2:result==="tie"?0:-amount));
...
export function isTerminal(state: CardBid4State): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
