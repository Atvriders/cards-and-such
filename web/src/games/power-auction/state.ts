...
export interface PowerAuctionSettings { dummy: boolean; }
export interface PowerAuctionState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type PowerAuctionAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: PowerAuctionSettings): PowerAuctionState {
...
export function reducer(state: PowerAuctionState, action: PowerAuctionAction): PowerAuctionState {
...
export function score(s: PowerAuctionState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: PowerAuctionState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
