...
export const DECK: { name: string; value: number }[] = [
  { name: "Rifleman", value: 3 },
  { name: "Sergeant", value: 4 },
  { name: "Scout", value: 2 },
  { name: "Officer", value: 5 },
  { name: "Strike", value: 6 },
];

export interface UndauntedNormandySettings { dummy: boolean; }
export interface UndauntedNormandyState {
  rngSeed: number;
  round: number;
  hand: number[];
  lastPts: number;
...
export type UndauntedNormandyAction = { type: "draw" } | { type: "next" };
export function initialState(seed: number, _s: UndauntedNormandySettings): UndauntedNormandyState {
