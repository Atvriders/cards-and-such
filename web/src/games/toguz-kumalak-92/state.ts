...
export interface ToguzKumalak92Settings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface ToguzKumalak92State {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type ToguzKumalak92Action = { type: "place"; idx: number };
export function initialState(seed: number, _settings: ToguzKumalak92Settings): ToguzKumalak92State {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: ToguzKumalak92State, action: ToguzKumalak92Action): ToguzKumalak92State {
...
export function isTerminal(state: ToguzKumalak92State): { score: number } | null {
