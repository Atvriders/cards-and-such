...
import { VastMysteriousCoop_CFG, initialState, reducer, isTerminal } from "./state.js";
import { coopHintSelector } from "../_shared/coop-engine.js";
const VastMysteriousCoopGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.VastMysteriousCoopGame as unknown as React.ComponentType<unknown> })));
const settings = {
...
export const vastMysteriousCoopPlugin: GamePlugin<VastMysteriousCoopState, VastMysteriousCoopAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: VastMysteriousCoopState): HintTarget | null => {
    const sel = coopHintSelector(state, VastMysteriousCoop_CFG);
    return sel ? { selector: sel, pulses: 3 } : null;
  },
  component: VastMysteriousCoopGame,
};

export default vastMysteriousCoopPlugin;
