...
export interface SvoyiKoziriShedSettings { dummy: boolean; }
export interface SvoyiKoziriShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SvoyiKoziriShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: SvoyiKoziriShedSettings): SvoyiKoziriShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SvoyiKoziriShedState, action: SvoyiKoziriShedAction): SvoyiKoziriShedState {
...
export function isTerminal(state: SvoyiKoziriShedState): { score: number } | null {
