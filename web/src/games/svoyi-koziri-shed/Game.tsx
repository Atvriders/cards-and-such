...
export function SvoyiKoziriShedGame({ state, dispatch, onGameOver }: GameProps<SvoyiKoziriShedState, SvoyiKoziriShedSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as SvoyiKoziriShedAction)}>Play Round</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SvoyiKoziriShedAction)}>Next</button>
      </>}
    </div>
  );
}
