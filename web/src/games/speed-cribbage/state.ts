...
export const SCORE_TIE = 2;
export interface SpeedCribbageSettings { dummy: boolean; }
export interface SpeedCribbageState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type SpeedCribbageAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SpeedCribbageSettings): SpeedCribbageState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: SpeedCribbageState, action: SpeedCribbageAction): SpeedCribbageState {
...
export function isTerminal(state: SpeedCribbageState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
