...
export function SpeedCribbageGame({ state, dispatch, onGameOver }: GameProps<SpeedCribbageState, SpeedCribbageSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as SpeedCribbageAction)}>Snap!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SpeedCribbageAction)}>Next</button>
      </>}
    </div>
  );
}
