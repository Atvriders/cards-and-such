...
// Clickomania Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface ClickomaniaMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface ClickomaniaMiniState {
  rngSeed: number;
  grid: Grid;
...
export type ClickomaniaMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: ClickomaniaMiniSettings): ClickomaniaMiniState {
...
export function reducer(state: ClickomaniaMiniState, action: ClickomaniaMiniAction): ClickomaniaMiniState {
...
export function isTerminal(state: ClickomaniaMiniState): { score: number } | null {
