...
const DuplicateBridgeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DuplicateBridgeGame as unknown as React.ComponentType<unknown> })));
...
export const duplicateBridgePlugin: GamePlugin<DuplicateBridgeState, DuplicateBridgeAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DuplicateBridgeState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-dupb-${pick.id}"]`, pulses: 3 };
    }
