...
export interface SilverAndGoldSettings { dummy: boolean; }
export interface SilverAndGoldState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type SilverAndGoldAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: SilverAndGoldSettings): SilverAndGoldState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: SilverAndGoldState, action: SilverAndGoldAction): SilverAndGoldState {
...
export function isTerminal(state: SilverAndGoldState): { score: number } | null {
