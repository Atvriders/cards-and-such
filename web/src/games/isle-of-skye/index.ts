...
const IsleOfSkyeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.IsleOfSkyeGame as unknown as React.ComponentType<unknown> })));
...
export const isleOfSkyePlugin: GamePlugin<IsleOfSkyeState, IsleOfSkyeAction, typeof settings> = {
  ...
  howToPlay: "Isle of Skye is a tile-placement game on a 5x5 grid. A randomized queue of 16 tiles is generated. Each turn the next tile from the queue is shown; click any empty cell to place it. Tile types are: Castle, Mountain, Sheep, Sea, Field. Each orthogonal pair of same-type tiles scores +2. Same-type connected clusters of 3+ score a +4 bonus, clusters of 5+ score an additional +8. Strategy: place tiles next to existing same-type neighbors to grow clusters efficiently. Don't waste placements in isolated corners. The grid has 25 cells but you only place 16 tiles, so plan compact clusters.",
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as IsleOfSkyeSettings),
  reducer,
  isTerminal,
  hint: (state: IsleOfSkyeState): HintTarget | null => {
    const sel = tileHintSelector(state, "ios-grid");
