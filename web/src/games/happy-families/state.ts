...
export interface HappyFamiliesSettings { dummy: boolean; }
export interface HappyFamiliesState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type HappyFamiliesAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: HappyFamiliesSettings): HappyFamiliesState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: HappyFamiliesState, action: HappyFamiliesAction): HappyFamiliesState {
...
export function isTerminal(state: HappyFamiliesState): { score: number } | null {
