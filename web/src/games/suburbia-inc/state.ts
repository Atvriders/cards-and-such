...
export interface SuburbiaIncSettings { dummy: boolean; }
export interface SuburbiaIncState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type SuburbiaIncAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: SuburbiaIncSettings): SuburbiaIncState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Population grows","Reputation up","Lake built","Highway opens","Skyscraper announced","Restaurant boom","Industry strike","Park dedicated"];

export function reducer(state: SuburbiaIncState, action: SuburbiaIncAction): SuburbiaIncState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Tile (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Bonus (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Sold 1 Tile (+$" + market + ")"; delta = market;
      } else { evt = "No Tile to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
...
export function score(s: SuburbiaIncState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: SuburbiaIncState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
