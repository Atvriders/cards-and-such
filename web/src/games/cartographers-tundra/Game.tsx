...
export function CartographersTundraGame({ state, dispatch, onGameOver }: GameProps<CartographersTundraState, CartographersTundraSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return (
      <div className="rw-wrap">
        <h3 className="rw-title">Cartographers: Tundra</h3>
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as CartographersTundraAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as CartographersTundraAction)}>Skip</button>
      )}
    </div>
  );
}
