...
export interface CartographersTundraSettings { dummy: boolean; }
export interface CartographersTundraState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type CartographersTundraAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: CartographersTundraSettings): CartographersTundraState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: CartographersTundraState, action: CartographersTundraAction): CartographersTundraState {
...
export function isTerminal(state: CartographersTundraState): { score: number } | null {
