...
export interface TrekAmericasSettings { dummy: boolean; }
export interface TrekAmericasState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type TrekAmericasAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: TrekAmericasSettings): TrekAmericasState {
  return {
    rngSeed: seed,
...
export function reducer(state: TrekAmericasState, action: TrekAmericasAction): TrekAmericasState {
...
export function isTerminal(state: TrekAmericasState): { score: number } | null {
