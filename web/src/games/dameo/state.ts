...
export interface DameoSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface DameoState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type DameoAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: DameoSettings): DameoState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: DameoState, action: DameoAction): DameoState {
...
export function isTerminal(state: DameoState): { score: number } | null {
