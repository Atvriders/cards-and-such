...
export interface CenturyEasternWondersSettings { dummy: boolean; }
export interface CenturyEasternWondersState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type CenturyEasternWondersAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: CenturyEasternWondersSettings): CenturyEasternWondersState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Monsoon","Calm seas","Harbor open","Naval skirmish","Trade winds","Pirates","Royal patron","Port closes"];

export function reducer(state: CenturyEasternWondersState, action: CenturyEasternWondersAction): CenturyEasternWondersState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Ship (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Outpost (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Sold 1 Ship (+$" + market + ")"; delta = market;
      } else { evt = "No Ship to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
    ...
    if (pay > 0) evt += " | Outpost earned +$" + pay;
...
export function score(s: CenturyEasternWondersState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: CenturyEasternWondersState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
