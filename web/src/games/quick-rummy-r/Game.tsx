...
export function QuickRummyRGame({ state, dispatch, onGameOver }: GameProps<GState, object>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="qkrr-wrap"><div className="qkrr-done bounce-in"><h2>Done!</h2><div className="qkrr-final">{state.score} pts</div></div></div>;
  return (
    <div className="qkrr-wrap fade-in">
      <div className="qkrr-info">Round {state.round} / {TOTAL_ROUNDS}</div>
      <div className="qkrr-score pulse">{state.score} pts</div>
