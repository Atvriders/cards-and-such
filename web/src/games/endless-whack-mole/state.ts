...
// Endless Whack-a-Mole: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface EndlessWhackMoleSettings { dummy: boolean; }

export interface EndlessWhackMoleTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface EndlessWhackMoleState {
  rngSeed: number;
  targets: EndlessWhackMoleTarget[];
  nextId: number;
...
export type EndlessWhackMoleAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: EndlessWhackMoleSettings): EndlessWhackMoleState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: EndlessWhackMoleState, action: EndlessWhackMoleAction): EndlessWhackMoleState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: EndlessWhackMoleTarget[] = [];
...
export function isTerminal(state: EndlessWhackMoleState): { score: number } | null {
