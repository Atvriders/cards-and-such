...
const DiceBakeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceBakeGame as unknown as React.ComponentType<unknown> })));
...
export const diceBakePlugin: GamePlugin<DiceBakeState, DiceBakeAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceBakeState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-bake-roll"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-dice-bake-next"]', pulses: 3 };
    return null;
  },
  component:DiceBakeGame,
};
