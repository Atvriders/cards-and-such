...
import "./Game.css";
export function UltimateHoldemGame({ state, dispatch, onGameOver }: GameProps<UltimateHoldemState, UltimateHoldemSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-ultimate-holdem-deal" onClick={() => dispatch({ type: "deal" } as UltimateHoldemAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-ultimate-holdem-next" onClick={() => dispatch({ type: "next" } as UltimateHoldemAction)}>Next</button>
      </>}
    </div>
  );
}
