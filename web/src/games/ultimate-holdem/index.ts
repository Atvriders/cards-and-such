...
const UltimateHoldemGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.UltimateHoldemGame as unknown as React.ComponentType<unknown> })));
...
export const ultimateHoldemPlugin: GamePlugin<UltimateHoldemState, UltimateHoldemAction, typeof settings> = {
  ...
  reducer, isTerminal,   hint: (state: UltimateHoldemState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-ultimate-holdem-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-ultimate-holdem-next"]', pulses: 3 };
    return null;
  },
  component:UltimateHoldemGame,
};
