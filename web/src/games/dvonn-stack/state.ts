...
export interface DvonnStackSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface DvonnStackState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type DvonnStackAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: DvonnStackSettings): DvonnStackState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: DvonnStackState, action: DvonnStackAction): DvonnStackState {
...
export function isTerminal(state: DvonnStackState): { score: number } | null {
