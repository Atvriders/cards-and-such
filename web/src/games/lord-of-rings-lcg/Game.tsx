...
export function LordOfRingsLcgGame({ state, dispatch, onGameOver }: GameProps<LordOfRingsLcgState, LordOfRingsLcgSettings>): JSX.Element {
  ...
  const theme = "Middle-earth Quest";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as LordOfRingsLcgAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as LordOfRingsLcgAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
