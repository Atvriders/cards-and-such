...
const TressetteGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TressetteGame as unknown as React.ComponentType<unknown> })));
...
export const tressettePlugin: GamePlugin<TressetteState, TressetteAction, typeof settings> = {
  ...
  description: "Tressette — Italian point-trick game.",
  howToPlay: "Tressette — Italian point-trick game. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as TressetteSettings),
  reducer,
  isTerminal,
  hint: (state: TressetteState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-trec-${pick.id}"]`, pulses: 3 };
    }
