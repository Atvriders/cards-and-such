...
export interface CherryBurstSettings { dummy: boolean; }
...
export interface CherryBurstState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type CherryBurstAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CherryBurstSettings): CherryBurstState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CherryBurstState, action: CherryBurstAction): CherryBurstState {
...
export function isTerminal(state: CherryBurstState): { score: number } | null {
