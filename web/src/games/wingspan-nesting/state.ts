...
export const SUIT_NAMES: readonly string[] = ["Bowl","Cup","Cavity","Platform"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface WingspanNestingSettings { dummy: boolean; }
export interface WingspanNestingState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WingspanNestingAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WingspanNestingSettings): WingspanNestingState {
...
export function reducer(state: WingspanNestingState, action: WingspanNestingAction): WingspanNestingState {
...
export function score(s: WingspanNestingState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WingspanNestingState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
