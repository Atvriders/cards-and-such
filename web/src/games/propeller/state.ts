...
export interface PropellerSettings { dummy: boolean; }
...
function scoreHand(h: number[]): number {
  let s = 0; const r=h.map(cardRank).sort((a,b)=>a-b); let alt=1, best=1; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; if (Math.abs(a-b)===1){alt++; if(alt>best)best=alt;} else alt=1; } s += best*5; const suits = new Set(h.map(cardSuit)); s += (4-suits.size)*3; if (s===0) s=1; return s;
}
...
export function reducer(state: PropellerState, action: PropellerAction): PropellerState {
...
export function isTerminal(state: PropellerState): { score: number } | null {
