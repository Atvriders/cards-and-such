...
export interface ThaiDraughtsSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface ThaiDraughtsState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type ThaiDraughtsAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: ThaiDraughtsSettings): ThaiDraughtsState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: ThaiDraughtsState, action: ThaiDraughtsAction): ThaiDraughtsState {
...
export function isTerminal(state: ThaiDraughtsState): { score: number } | null {
