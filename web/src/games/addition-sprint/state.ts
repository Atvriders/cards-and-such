...
export interface AdditionSprintSettings {
...
export interface AdditionSprintState {
...
export type AdditionSprintAction =
...
function genQuestions(seed: number, settings: AdditionSprintSettings): AdditionQuestion[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const b = 1 + Math.floor(rng1(s++) * max);
    questions.push({ a, b, answer: a + b });
  }
  return questions;
}

export function initialState(seed: number, settings: AdditionSprintSettings): AdditionSprintState {
  return {
    settings,
...
export function reducer(state: AdditionSprintState, action: AdditionSprintAction): AdditionSprintState {
...
export function isTerminal(state: AdditionSprintState): { score: number } | null {
