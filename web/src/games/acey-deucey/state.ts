// Auto-generated backgammon-family state for acey-deucey.
...
export interface AceySettings { dummy: boolean }

export interface AceyState {
  rngSeed: number;
...
export function initialState(seed: number, _s: AceySettings): AceyState {
...
export function legalMoves(state: AceyState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: AceyState, side: Side, from: number, pips: number): AceyState | null {
  const next: AceyState = {
    ...state,
...
function checkWin(s: AceyState): AceyState {
...
function cpuTurn(state: AceyState): AceyState {
  let s = state;
...
export function reducer(state: AceyState, action: AceyAction): AceyState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: AceyState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: AceyState): { score: number } | null {
