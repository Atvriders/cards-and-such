...
export interface BowlingPinTapSettings { dummy: boolean; }
export interface Target {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface BowlingPinTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type BowlingPinTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: BowlingPinTapSettings): BowlingPinTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: BowlingPinTapState, action: BowlingPinTapAction): BowlingPinTapState {
...
export function isTerminal(state: BowlingPinTapState): { score: number } | null {
