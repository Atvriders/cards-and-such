...
export interface DiceFantasyFootballDraftSettings { dummy: boolean; }

export interface DiceFantasyFootballDraftState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceFantasyFootballDraftAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceFantasyFootballDraftSettings): DiceFantasyFootballDraftState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceFantasyFootballDraftState, action: DiceFantasyFootballDraftAction): DiceFantasyFootballDraftState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceFantasyFootballDraftState> = {};
...
export function isTerminal(state: DiceFantasyFootballDraftState): { score: number } | null {
