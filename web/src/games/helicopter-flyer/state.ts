...
// Helicopter Flyer: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface HelicopterFlyerSettings { dummy: boolean; }

export interface HelicopterFlyerTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface HelicopterFlyerState {
  rngSeed: number;
  targets: HelicopterFlyerTarget[];
  nextId: number;
...
export type HelicopterFlyerAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: HelicopterFlyerSettings): HelicopterFlyerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: HelicopterFlyerState, action: HelicopterFlyerAction): HelicopterFlyerState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: HelicopterFlyerTarget[] = [];
...
export function isTerminal(state: HelicopterFlyerState): { score: number } | null {
