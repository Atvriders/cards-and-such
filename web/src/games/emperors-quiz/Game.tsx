      ...
      {!state.done && entry ? (<>
        ...
        <div className="hq-choices">
          {entry.choices.map((choice, i) => {
            ...
            return <button key={i} className={cls} disabled={state.selected !== null} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", index:i } as EmperorsQuizAction)}>{choice}</button>;
          })}
        </div>
