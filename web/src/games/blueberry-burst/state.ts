...
export interface BlueberryBurstSettings { dummy: boolean; }
...
export interface BlueberryBurstState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type BlueberryBurstAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BlueberryBurstSettings): BlueberryBurstState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BlueberryBurstState, action: BlueberryBurstAction): BlueberryBurstState {
...
export function isTerminal(state: BlueberryBurstState): { score: number } | null {
