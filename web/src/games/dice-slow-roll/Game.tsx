...
export function DiceSlowRollGame({ state, dispatch, onGameOver }: GameProps<DiceSlowRollState, DiceSlowRollSettings>): JSX.Element {
    ...
    <div className="dm-wrap">
      ...
      {state.phase === "betting" ? (
        <div className="dm-bid-row">
          ...
          <button data-testid="hint-target-dice-slow-roll-roll" className="dm-btn" onClick={() => dispatch({ type:"roll", amount:bid } as DiceSlowRollAction)}>Roll</button>
        </div>
      ) : (
        <>
          ...
          <button className="dm-btn" onClick={() => dispatch({ type:"next" } as DiceSlowRollAction)}>Next</button>
        </>
      )}
    </div>
  );
