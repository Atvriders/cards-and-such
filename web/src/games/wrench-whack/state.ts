...
export interface WrenchWhackSettings { dummy: boolean; }
...
export interface WrenchWhackState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type WrenchWhackAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: WrenchWhackSettings): WrenchWhackState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: WrenchWhackState, action: WrenchWhackAction): WrenchWhackState {
  if (state.phase === "done") return state;
  if (action.type === "pop") {
    const target = state.targets.find(t => t.id === action.id);
    if (!target) return state;
    return { ...state, targets: state.targets.filter(t => t.id !== action.id), score: state.score + 10, popped: state.popped + 1 };
  }
  if (action.type === "tick") {
...
export function isTerminal(state: WrenchWhackState): { score: number } | null {
