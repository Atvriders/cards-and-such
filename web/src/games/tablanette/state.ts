...
export interface TablanetteSettings { dummy: boolean; }
export interface TablanetteState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type TablanetteAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: TablanetteSettings): TablanetteState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: TablanetteState, action: TablanetteAction): TablanetteState {
...
export function isTerminal(state: TablanetteState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
