...
import "./Game.css";
export function TablanetteGame({ state, dispatch, onGameOver }: GameProps<TablanetteState, TablanetteSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-tablanette-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as TablanetteAction)}>Sweep!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-tablanette-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as TablanetteAction)}>Next</button>
      </>}
    </div>
  );
}
