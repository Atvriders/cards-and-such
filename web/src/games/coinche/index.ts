...
const CoincheGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CoincheGame as unknown as React.ComponentType<unknown> })));
...
export const coinchePlugin: GamePlugin<CoincheState, CoincheAction, typeof settings> = {
  ...
  description: "Coinche — French belote variant.",
  howToPlay: "Coinche — French belote variant. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as CoincheSettings),
  reducer,
  isTerminal,
  hint: (state: CoincheState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-coincc-${pick.id}"]`, pulses: 3 };
    }
