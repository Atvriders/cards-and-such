// Coinche — Bidding Belote variant — simplified to a head-to-head trick duel.
...
export type CoinchePhase = "playing" | "done";

export interface CoincheState {
  rngSeed: number;
...
export type CoincheAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `coinche-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: CoincheState, trick: readonly { seat: number; card: Card }[]): CoincheState {
...
export function reducer(state: CoincheState, action: CoincheAction): CoincheState {
  ...
  let s: CoincheState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: CoincheState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: CoincheState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): CoincheState {
...
export function isTerminal(state: CoincheState): { score: number } | null {
