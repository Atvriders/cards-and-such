...
export interface WelcomeToSpringSettings { dummy: boolean; }
export interface WelcomeToSpringState {
  rngSeed: number;
...
export type WelcomeToSpringAction =
  | { type: "roll" }
  | { type: "place"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: WelcomeToSpringSettings): WelcomeToSpringState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: WelcomeToSpringState, action: WelcomeToSpringAction): WelcomeToSpringState {
...
export function isTerminal(state: WelcomeToSpringState): { score: number } | null {
