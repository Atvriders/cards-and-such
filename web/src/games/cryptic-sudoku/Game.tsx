...
export function CrypticSudokuGame({ state, dispatch, onGameOver }: GameProps<CrypticSudokuState, CrypticSudokuSettings>): JSX.Element {
    ...
    <div className="nlp-wrap">
      ...
      <div className="nlp-choices">
        {p.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type: "select", choice: i } as CrypticSudokuStateAction)}><span className="nlp-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
      <div className="nlp-actions">
        {!isResult && <button className="nlp-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as CrypticSudokuStateAction)}>Submit</button>}
        {isResult && <button className="nlp-btn next" onClick={() => dispatch({ type: "next" } as CrypticSudokuStateAction)}>{state.idx + 1 >= state.puzzles.length ? "Finish" : "Next"}</button>}
      </div>
    </div>
  );
}
