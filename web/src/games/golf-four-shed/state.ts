...
export interface GolfFourShedSettings { dummy: boolean; }
export interface GolfFourShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type GolfFourShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: GolfFourShedSettings): GolfFourShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: GolfFourShedState, action: GolfFourShedAction): GolfFourShedState {
...
export function isTerminal(state: GolfFourShedState): { score: number } | null {
