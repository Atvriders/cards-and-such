...
import "./Game.css";
export function EscobaMiniGame({ state, dispatch, onGameOver }: GameProps<EscobaMiniState, EscobaMiniSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-escoba-mini-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as EscobaMiniAction)}>Escoba!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as EscobaMiniAction)}>Next</button>
      </>}
    </div>
  );
}
