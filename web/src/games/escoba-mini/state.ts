...
export const SCORE_WIN = 15;
export const SCORE_TIE = 5;
export interface EscobaMiniSettings { dummy: boolean; }
export interface EscobaMiniState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type EscobaMiniAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: EscobaMiniSettings): EscobaMiniState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: EscobaMiniState, action: EscobaMiniAction): EscobaMiniState {
...
export function isTerminal(state: EscobaMiniState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
