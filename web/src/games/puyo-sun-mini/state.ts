...
// Puyo Sun Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface PuyoSunMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface PuyoSunMiniState {
  rngSeed: number;
  grid: Grid;
...
export function initialState(seed: number, _settings: PuyoSunMiniSettings): PuyoSunMiniState {
...
export function reducer(state: PuyoSunMiniState, action: PuyoSunMiniAction): PuyoSunMiniState {
...
export function isTerminal(state: PuyoSunMiniState): { score: number } | null {
