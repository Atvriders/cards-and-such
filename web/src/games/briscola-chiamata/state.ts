// Briscola Chiamata — 5-player called Briscola — reduced to a 1v1 trick duel.
...
export type BriscolaChiamataPhase = "playing" | "done";

export interface BriscolaChiamataState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: BriscolaChiamataPhase;
  playerLeads: boolean;
  message: string;
}

export type BriscolaChiamataAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `briscola-chiamata-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: BriscolaChiamataState, trick: readonly { seat: number; card: Card }[]): BriscolaChiamataState {
...
export function reducer(state: BriscolaChiamataState, action: BriscolaChiamataAction): BriscolaChiamataState {
  ...
  let s: BriscolaChiamataState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: BriscolaChiamataState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: BriscolaChiamataState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): BriscolaChiamataState {
...
export function isTerminal(state: BriscolaChiamataState): { score: number } | null {
