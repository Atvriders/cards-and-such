...
describe("briscola-chiamata initialState", () => {
  it("deals correct number of cards to each player", () => {
    const s = initialState(42);
    expect(s.playerHand.length).toBe(8);
    expect(s.botHand.length).toBe(8);
  });
...
describe("briscola-chiamata legalPlays", () => {
  it("all cards legal when leading", () => {
    const s = initialState(42);
    expect(legalPlays(s.playerHand, s.currentTrick).length).toBeGreaterThanOrEqual(8);
  });
});

describe("briscola-chiamata reducer", () => {
...
describe("briscola-chiamata full game", () => {
  it("completes a full game", () => {
    ...
    expect(s.playerTricks + s.botTricks).toBeGreaterThanOrEqual(4);
  });
});
