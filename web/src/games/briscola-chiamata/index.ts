...
const BriscolaChiamataGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BriscolaChiamataGame as unknown as React.ComponentType<unknown> })));
...
export const briscolaChiamataPlugin: GamePlugin<BriscolaChiamataState, BriscolaChiamataAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: BriscolaChiamataState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-briscch-${pick.id}"]`, pulses: 3 };
    }
