...
export function BibliosDiceDraftGame({ state, dispatch, onGameOver }: GameProps<BibliosDiceDraftState, BibliosDiceDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Biblios Dice</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BibliosDiceDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BibliosDiceDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
