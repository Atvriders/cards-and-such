...
export interface BibliosDiceDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BibliosDiceDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BibliosDiceDraftSettings): BibliosDiceDraftState {
...
export function score(s: BibliosDiceDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BibliosDiceDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
