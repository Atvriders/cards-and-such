...
export function TrialTrolleyQuiz({ state, dispatch, onGameOver }: GameProps<TrialTrolleyQuizState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      {state.phase === "answered" && (
        <button className="g-btn" onClick={() => dispatch({ type: "next" } as TrialTrolleyQuizAction)}>Next</button>
      )}
    </div>
  );
}
