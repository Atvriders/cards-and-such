...
export interface MatrimonyPatienceSettings { dummy: boolean; }

export interface MatrimonyPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type MatrimonyPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const ranks=h.map(cardRank); const suits=h.map(cardSuit); for (let i=0;i<h.length;i++){ for (let j=i+1;j<h.length;j++){ const a=ranks[i] as number; const b=ranks[j] as number; const sa=suits[i] as number; const sb=suits[j] as number; if (a===b && (sa%2)===(sb%2)) s += 10; } } if (s===0) s=1; return s;
}
...
export function reducer(state: MatrimonyPatienceState, action: MatrimonyPatienceAction): MatrimonyPatienceState {
...
export function isTerminal(state: MatrimonyPatienceState): { score: number } | null {
