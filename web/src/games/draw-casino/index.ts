...
const DrawCasinoGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DrawCasinoGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: DrawCasinoState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-draw-casino-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-draw-casino-secondary"]', pulses: 3 };
  return null;
};
...
export const drawCasinoPlugin: GamePlugin<DrawCasinoState, DrawCasinoAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: DrawCasinoGame,
};
