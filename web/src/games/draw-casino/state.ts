...
export interface DrawCasinoSettings { dummy: boolean; }
export interface DrawCasinoState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type DrawCasinoAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: DrawCasinoSettings): DrawCasinoState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: DrawCasinoState, action: DrawCasinoAction): DrawCasinoState {
...
export function isTerminal(state: DrawCasinoState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
