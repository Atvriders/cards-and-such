...
export function DrawCasinoGame({ state, dispatch, onGameOver }: GameProps<DrawCasinoState, DrawCasinoSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as DrawCasinoAction)}>Draw!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DrawCasinoAction)}>Next</button>
      </>}
    </div>
  );
}
