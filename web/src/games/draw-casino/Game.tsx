...
import "./Game.css";
export function DrawCasinoGame({ state, dispatch, onGameOver }: GameProps<DrawCasinoState, DrawCasinoSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-draw-casino-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as DrawCasinoAction)}>Draw!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-draw-casino-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DrawCasinoAction)}>Next</button>
      </>}
    </div>
  );
}
