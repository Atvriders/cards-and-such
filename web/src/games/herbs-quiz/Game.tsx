...
export function HerbsQuiz({ state, dispatch, onGameOver }: GameProps<HerbsQuizState, HerbsQuizSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as HerbsQuizAction), 1000);
      ...
      <div className="trivia-actions">
        {!isResult && (
          <button className="trivia-btn submit" disabled={state.selected === null}
            onClick={() => dispatch({ type: "submit" } as HerbsQuizAction)}>Submit</button>
        )}
        {isResult && (
          <button className="trivia-btn next"
            onClick={() => dispatch({ type: "next" } as HerbsQuizAction)}>
