...
export interface AgnesSorelSettings { dummy: boolean; }

export interface AgnesSorelState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export function reducer(state: AgnesSorelState, action: AgnesSorelAction): AgnesSorelState {
...
export function isTerminal(state: AgnesSorelState): { score: number } | null {
