...
export interface HeadsUpCashSettings { dummy: boolean; }
export interface HeadsUpCashState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HeadsUpCashAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HeadsUpCashSettings): HeadsUpCashState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HeadsUpCashState, action: HeadsUpCashAction): HeadsUpCashState {
...
export function isTerminal(state: HeadsUpCashState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
