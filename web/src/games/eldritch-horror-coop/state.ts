...
export interface eldritchHorrorCoopSettings { dummy: boolean; }
export interface eldritchHorrorCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type eldritchHorrorCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: eldritchHorrorCoopSettings): eldritchHorrorCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: eldritchHorrorCoopState, action: eldritchHorrorCoopAction): eldritchHorrorCoopState {
...
export function isTerminal(state: eldritchHorrorCoopState): { score: number } | null {
