...
export interface RamenRushSettings { dummy: boolean; }
...
export interface RamenRushState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type RamenRushAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: RamenRushSettings): RamenRushState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: RamenRushState, action: RamenRushAction): RamenRushState {
...
export function isTerminal(state: RamenRushState): { score: number } | null {
