...
// SameGame Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface SamegameMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface SamegameMiniState {
  rngSeed: number;
  grid: Grid;
...
export type SamegameMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: SamegameMiniSettings): SamegameMiniState {
...
export function reducer(state: SamegameMiniState, action: SamegameMiniAction): SamegameMiniState {
...
export function isTerminal(state: SamegameMiniState): { score: number } | null {
