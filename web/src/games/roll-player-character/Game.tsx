...
import "./Game.css";
export function RollPlayerCharacterGame({ state, dispatch, onGameOver }: GameProps<RollPlayerCharacterState, RollPlayerCharacterSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as RollPlayerCharacterAction)}>Roll</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">+{state.lastPts} pts{state.bonus > 0 ? ` (bonus +${state.bonus})` : ""}</div>
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as RollPlayerCharacterAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
