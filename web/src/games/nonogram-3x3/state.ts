...
export type Nonogram3x3Action = { type: "toggle"; index: number } | { type: "check" } | { type: "reset" };
function rowsOf(cells: boolean[]): boolean[][] { return [cells.slice(0,3), cells.slice(3,6), cells.slice(6,9)]; }
function colsOf(cells: boolean[]): boolean[][] { return [[cells[0]!,cells[3]!,cells[6]!],[cells[1]!,cells[4]!,cells[7]!],[cells[2]!,cells[5]!,cells[8]!]]; }
function clueLine(line: boolean[]): number[] {
  const out: number[] = []; let run = 0;
  for (const v of line) { if (v) run++; else if (run > 0) { out.push(run); run = 0; } }
  if (run > 0) out.push(run);
  return out.length ? out : [0];
}
function randomSolution(seed: number): boolean[] {
  const rng = mulberry32(seed);
  // make 4-6 lit cells out of 9
  const target = 4 + Math.floor(rng() * 3);
  const cells = new Array(9).fill(false);
  let lit = 0;
  while (lit < target) {
    const i = Math.floor(rng() * 9);
    if (!cells[i]) { cells[i] = true; lit++; }
  }
  return cells;
}
export function initialState(seed: number, _s: Nonogram3x3Settings): Nonogram3x3State {
  ...
  return { rngSeed: seed, solution, cells: new Array(9).fill(false), rowClues, colClues, moves: 0, phase: "playing" };
}
export function reducer(state: Nonogram3x3State, action: Nonogram3x3Action): Nonogram3x3State {
  ...
  if (action.type === "reset") {
    ...
    return { rngSeed: newSeed, solution: sol, cells: new Array(9).fill(false), rowClues: rowsOf(sol).map(clueLine), colClues: colsOf(sol).map(clueLine), moves: 0, phase: "playing" };
  }
  if (action.type === "toggle") {
    const cells = [...state.cells]; cells[action.index] = !cells[action.index];
