...
import "./Game.css";
export function FourHandCribbageGame({ state, dispatch, onGameOver }: GameProps<FourHandCribbageState, FourHandCribbageSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-four-hand-cribbage-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as FourHandCribbageAction)}>Cut!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-four-hand-cribbage-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as FourHandCribbageAction)}>Next</button>
      </>}
    </div>
  );
}
