...
export interface FourHandCribbageSettings { dummy: boolean; }
export interface FourHandCribbageState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type FourHandCribbageAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: FourHandCribbageSettings): FourHandCribbageState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: FourHandCribbageState, action: FourHandCribbageAction): FourHandCribbageState {
...
export function isTerminal(state: FourHandCribbageState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
