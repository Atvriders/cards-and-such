...
describe("GemstonesQuiz initialState", () => {
  ...
  it("is deterministic", () => {
    expect(initialState(5, def).questions[0]!.question).toBe(initialState(5, def).questions[0]!.question);
  });
...
describe("GemstonesQuiz reducer", () => {
  it("select stores choice", () => {
    expect(reducer(initialState(1, def), { type: "select", choice: 2 }).selected).toBe(2);
  });
...
describe("GemstonesQuiz isTerminal", () => {
