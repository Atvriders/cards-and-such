...
// Puyo Fever Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface PuyoFeverMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface PuyoFeverMiniState {
  rngSeed: number;
  grid: Grid;
...
export type PuyoFeverMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: PuyoFeverMiniSettings): PuyoFeverMiniState {
...
export function reducer(state: PuyoFeverMiniState, action: PuyoFeverMiniAction): PuyoFeverMiniState {
...
export function isTerminal(state: PuyoFeverMiniState): { score: number } | null {
