...
export interface Pool10ballSettings { dummy: boolean; }

export interface Pool10ballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type Pool10ballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: Pool10ballSettings): Pool10ballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: Pool10ballState, action: Pool10ballAction): Pool10ballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<Pool10ballState> = {};
    const sink = dice[0]! >= 4;
    const foul = dice[1]! === 1;
    if (foul) {
      ...
      pts = newLeft === 0 ? 50 : 5;
      logEntry = newLeft === 0 ? `Sank the ${(10)}-ball! WIN` : `Pocketed (${newLeft} left)`;
    } else {
      pts = 0;
      logEntry = `Missed`;
    }
...
export function isTerminal(state: Pool10ballState): { score: number } | null {
