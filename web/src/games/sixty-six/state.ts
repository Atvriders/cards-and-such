// Sixty-Six — German 24-card two-player trick game — set-trump duel here.
...
export type SixtySixPhase = "playing" | "done";

export interface SixtySixState {
  rngSeed: number;
...
export type SixtySixAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `sixty-six-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: SixtySixState, trick: readonly { seat: number; card: Card }[]): SixtySixState {
...
export function reducer(state: SixtySixState, action: SixtySixAction): SixtySixState {
  ...
  let s: SixtySixState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: SixtySixState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: SixtySixState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): SixtySixState {
...
export function isTerminal(state: SixtySixState): { score: number } | null {
