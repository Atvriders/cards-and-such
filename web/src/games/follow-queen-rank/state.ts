...
export interface FollowQueenRankSettings { rounds: "10"; }

export interface FollowQueenRankState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type FollowQueenRankAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FollowQueenRankSettings): FollowQueenRankState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: FollowQueenRankState, action: FollowQueenRankAction): FollowQueenRankState {
...
export function isTerminal(state: FollowQueenRankState): { score: number } | null {
