...
import { FollowQueenRank } from "./Game.js";

const settings = {
  rounds: { kind: "enum" as const, label: "Rounds", options: ["10"] as const, default: "10" as const },
} as const;

const hint = (state: FollowQueenRankState): HintTarget | null => ((state.phase === "ready" || state.phase === "dealt") ? { selector: ".g-btn", pulses: 3 } : null);
...
export const followQueenRankPlugin: GamePlugin<FollowQueenRankState, FollowQueenRankAction, typeof settings> = {
  ...
  reducer, isTerminal, hint, component: FollowQueenRank,
};
