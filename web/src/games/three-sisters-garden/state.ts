...
export interface ThreeSistersGardenSettings { dummy: boolean; }
export interface ThreeSistersGardenState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type ThreeSistersGardenAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: ThreeSistersGardenSettings): ThreeSistersGardenState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: ThreeSistersGardenState, action: ThreeSistersGardenAction): ThreeSistersGardenState {
...
export function isTerminal(state: ThreeSistersGardenState): { score: number } | null {
