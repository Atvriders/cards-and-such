...
export function ThreeSistersGardenGame({ state, dispatch, onGameOver }: GameProps<ThreeSistersGardenState, ThreeSistersGardenSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as ThreeSistersGardenAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as ThreeSistersGardenAction)}>Skip</button>
      )}
    </div>
  );
}
