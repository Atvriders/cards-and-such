...
export interface TennisBallTapSettings { dummy: boolean; }
export interface Target {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface TennisBallTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type TennisBallTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: TennisBallTapSettings): TennisBallTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: TennisBallTapState, action: TennisBallTapAction): TennisBallTapState {
...
export function isTerminal(state: TennisBallTapState): { score: number } | null {
