...
export const cassetteBernauerPlugin: GamePlugin<CassetteBernauerState, CassetteBernauerAction, typeof settings> = {
  id:"cassette-bernauer", title:"Cassette (Agnes Bernauer)", category:"solitaire",
  players:{ min:1, max:1, multiplayer:false },
  description:"Klondike-Agnes hybrid with a reserve row and tighter foundations.",
  howToPlay:"Cassette (Agnes Bernauer) is a ten-round solitaire micro-variant inspired by Cassette (Agnes Bernauer): reserve row; suit-restricted build. Each round you receive a fresh five-card hand drawn from a single seeded 52-card deck. You then choose one of three actions: Keep & Score locks the visible hand and awards points based on face cards, pairs, ascending runs, and same-suit flushes; Discard Hand abandons the hand for a flat one-point consolation and rolls into the next round; and Swap consumes the next card from the deck to replace any single card in the hand without ending the round.\n\nScores compound across all ten rounds. A typical run lands somewhere between 40 and 120 total points; sharp swap usage and well-timed Keeps can push past that. The game ends automatically when ten rounds are reached or the deck is exhausted, and your final score is rated Pass, Fair, Good, or Excellent depending on the total earned.\n\nThe deal is fully seeded, so the same starting seed always produces an identical card sequence for fair comparison and replay.",
  settings,
  initialState:(seed:number,s:S)=>initialState(seed,s as CassetteBernauerSettings),
  reducer,isTerminal,component:CassetteBernauerGame,
};
