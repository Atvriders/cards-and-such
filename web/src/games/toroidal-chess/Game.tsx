...
export function ToroidalChessGame({ state, dispatch, onGameOver }: GameProps<ToroidalChessState, ToroidalChessSettings>): JSX.Element {
    ...
    <div className="torch-wrap">
      ...
      <div className="torch-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button data-testid={`hint-target-toroidal-chess-answer-${i}`} key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as ToroidalChessAction)}><span className="torch-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
