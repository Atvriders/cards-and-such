...
export interface CrazyPineappleSettings { dummy: boolean; }
export interface CrazyPineappleState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type CrazyPineappleAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CrazyPineappleSettings): CrazyPineappleState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: CrazyPineappleState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
