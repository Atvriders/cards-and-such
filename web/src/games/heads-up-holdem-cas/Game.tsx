...
import "./Game.css";
export function HeadsUpHoldemCasGame({ state, dispatch, onGameOver }: GameProps<HeadsUpHoldemCasState, HeadsUpHoldemCasSettings>): JSX.Element {
  const t = isTerminal(state); useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") return <div className="dm-wrap thmHUHld"><div className="dm-done"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap thmHUHld">
      ...
      {state.phase === "scored" && <>
        ...
        <button data-testid="hint-target-heads-up-holdem-cas-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as HeadsUpHoldemCasAction)}>Next</button>
      </>}
    </div>
  );
}
