...
export interface HeadsUpHoldemCasSettings { dummy: boolean; }
export interface HeadsUpHoldemCasState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HeadsUpHoldemCasAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HeadsUpHoldemCasSettings): HeadsUpHoldemCasState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HeadsUpHoldemCasState, action: HeadsUpHoldemCasAction): HeadsUpHoldemCasState {
...
export function isTerminal(state: HeadsUpHoldemCasState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
