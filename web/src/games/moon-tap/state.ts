...
export interface MoonTapSettings { dummy: boolean; }
...
export type MoonTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: MoonTapSettings): MoonTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: MoonTapState, action: MoonTapAction): MoonTapState {
...
export function isTerminal(state: MoonTapState): { score: number } | null {
