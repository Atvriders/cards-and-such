// Auto-generated race-game state for ur-game-mini.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const TRACK_LEN = 14;
...
export const SPECIALS: Record<number, Special> = {4: {type:"safe"},8: {type:"safe"},14: {type:"safe"}};

export type Side = "P" | "C";
export interface UrMiniSettings { dummy: boolean }

export interface UrMiniState {
  rngSeed: number;
...
export function initialState(seed: number, _s: UrMiniSettings): UrMiniState {
  return {
    rngSeed: seed,
...
function checkWin(s: UrMiniState): UrMiniState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: UrMiniState): UrMiniState {
  let s = state;
...
export function legalMovesForP(state: UrMiniState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: UrMiniState, action: UrMiniAction): UrMiniState {
...
export function isTerminal(state: UrMiniState): { score: number } | null {
