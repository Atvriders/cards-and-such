...
const BlackHoleGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BlackHoleGame as unknown as React.ComponentType<unknown> })));
...
export const blackHoleSolPlugin: GamePlugin<BlackHoleState, BlackHoleAction, typeof settings> = {
  ...
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as unknown as BlackHoleSettings),
  hint: (state: BlackHoleState): HintTarget | null => {
      ...
      for (let ci = 0; ci < state.columns.length; ci++) {
        ...
        for (let ri = col.length - 1; ri >= 0; ri--) {
          if (!removed[ri]) {
            ...
            if (adj) {
              return { selector: `[data-testid="hint-target-black-hole-${ci}-${ri}"]`, pulses: 3 };
            }
            break;
          }
        }
    ...
    if (state.stock.length > 0) {
      return { selector: '[data-testid="hint-target-black-hole-draw"]', pulses: 3 };
    }
    return null;
  },
  reducer,
