// Watten — Alpine 4-player trick game with trump — 1v1 duel here.
...
export type WattenPhase = "playing" | "done";

export interface WattenState {
  rngSeed: number;
...
export type WattenAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `watten-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: WattenState, trick: readonly { seat: number; card: Card }[]): WattenState {
...
export function reducer(state: WattenState, action: WattenAction): WattenState {
  ...
  let s: WattenState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: WattenState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: WattenState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): WattenState {
...
export function isTerminal(state: WattenState): { score: number } | null {
