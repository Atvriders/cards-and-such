...
const PassTheTrashCardGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PassTheTrashCardGame as unknown as React.ComponentType<unknown> })));
const settings = {
...
The seed determines the entire shuffle sequence, so you can replay an identical run by entering the same seed. After ten rounds, your final score is locked in. Single-player only — no CPU opponent. A bite-sized poker variant perfect for short play sessions.`,
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-pass-the-trash-card-deal"]', pulses: 3 };
    if (state.phase === "dealt") return { selector: '[data-testid="hint-target-pass-the-trash-card-next"]', pulses: 3 };
    return null;
