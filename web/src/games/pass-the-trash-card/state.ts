...
export interface PassTheTrashCardSettings { rounds: "10"; }

export interface PassTheTrashCardState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type PassTheTrashCardAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: PassTheTrashCardSettings): PassTheTrashCardState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: PassTheTrashCardState, action: PassTheTrashCardAction): PassTheTrashCardState {
...
export function isTerminal(state: PassTheTrashCardState): { score: number } | null {
