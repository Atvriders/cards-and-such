...
import { initialState, reducer, isTerminal, BullsAndCows_CFG } from "./state.js";
...
describe("bulls-and-cows", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(BullsAndCows_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < BullsAndCows_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < BullsAndCows_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
