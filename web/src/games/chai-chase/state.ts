...
export interface ChaiChaseSettings { dummy: boolean; }
export interface ChaiChaseItem { id: number; lane: number; ticksLeft: number; }
export interface ChaiChaseState {
  rngSeed: number;
  items: ChaiChaseItem[];
  nextId: number;
...
export type ChaiChaseAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: ChaiChaseSettings): ChaiChaseState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: ChaiChaseState, action: ChaiChaseAction): ChaiChaseState {
...
export function isTerminal(state: ChaiChaseState): { score: number } | null {
