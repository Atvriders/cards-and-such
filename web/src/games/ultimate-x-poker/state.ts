...
export interface UltimateXPokerSettings { rounds: "10"; }

export interface UltimateXPokerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type UltimateXPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: UltimateXPokerSettings): UltimateXPokerState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: UltimateXPokerState, action: UltimateXPokerAction): UltimateXPokerState {
...
export function isTerminal(state: UltimateXPokerState): { score: number } | null {
