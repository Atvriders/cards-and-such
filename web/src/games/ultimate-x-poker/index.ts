...
const UltimateXPoker = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.UltimateXPoker as unknown as React.ComponentType<unknown> })));
const settings = {
...
export const ultimateXPokerPlugin: GamePlugin<UltimateXPokerState, UltimateXPokerAction, typeof settings> = {
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-ultimate-x-poker-deal"]', pulses: 3 };
    if (state.phase === "dealt") return { selector: '[data-testid="hint-target-ultimate-x-poker-next"]', pulses: 3 };
    return null;
  }, component: UltimateXPoker,
};
