// Auto-generated race-game state for nyout-yut-race.
...
export interface NyoutSettings { dummy: boolean }

export interface NyoutState {
  rngSeed: number;
...
export function initialState(seed: number, _s: NyoutSettings): NyoutState {
  return {
    rngSeed: seed,
...
function checkWin(s: NyoutState): NyoutState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: NyoutState): NyoutState {
  let s = state;
...
export function legalMovesForP(state: NyoutState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: NyoutState, action: NyoutAction): NyoutState {
...
export function isTerminal(state: NyoutState): { score: number } | null {
