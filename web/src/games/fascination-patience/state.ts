...
export interface FascinationPatienceSettings { dummy: boolean; }

export interface FascinationPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type FascinationPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { const su = cardSuit(c); if (su === 1 || su === 2) s += 4; else s += 1; }
  if (s === 0) s = 1; return s;
}

export function initialState(seed: number, _s: FascinationPatienceSettings): FascinationPatienceState {
...
export function reducer(state: FascinationPatienceState, action: FascinationPatienceAction): FascinationPatienceState {
...
export function isTerminal(state: FascinationPatienceState): { score: number } | null {
