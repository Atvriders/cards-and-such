...
export function NinukiRenjuGame({ state, dispatch, onGameOver }: GameProps<NinukiRenjuState, NinukiRenjuSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="nrenju-wrap"><div className="nrenju-done"><h2>Done!</h2><p>Correct: {state.correctCount} / {state.questions.length}</p><p style={{ fontSize:"1.8rem",fontWeight:900,color:"#27ae60" }}>{state.score} pts</p></div></div>;
  const q = state.questions[state.currentIndex]!;
  const isResult = state.phase === "result";
  const urgent = state.timeLeft <= 5 && !state.submitted;
  return (
    <div className="nrenju-wrap">
      <div className="nrenju-header">
        ...
        <span className="nrenju-score">{state.score} pts</span>
      </div>
      ...
      <div className="nrenju-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as NinukiRenjuAction)}><span className="nrenju-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
