...
export interface MinibridgeSettings { dummy: boolean; }
export interface MinibridgeState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type MinibridgeAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: MinibridgeSettings): MinibridgeState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: MinibridgeState, action: MinibridgeAction): MinibridgeState {
...
export function isTerminal(state: MinibridgeState): { score: number } | null {
