...
export interface CoffeeTradersMiniSettings { dummy: boolean; }
export interface CoffeeTradersMiniState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type CoffeeTradersMiniAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: CoffeeTradersMiniSettings): CoffeeTradersMiniState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Brazilian harvest strong","Frost in Colombia","Single-origin demand","Fair-trade premium","Export tariff hike","Cold brew craze"];

export function reducer(state: CoffeeTradersMiniState, action: CoffeeTradersMiniAction): CoffeeTradersMiniState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Beans (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Roaster (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Traded 1 Beans (+$" + market + ")"; delta = market;
      } else { evt = "No Beans to trade"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
    const pay = workers * 14;
    cash += pay;
...
export function score(s: CoffeeTradersMiniState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: CoffeeTradersMiniState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
