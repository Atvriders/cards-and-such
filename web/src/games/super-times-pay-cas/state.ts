...
export interface SuperTimesPayCasSettings { dummy: boolean; }
export interface SuperTimesPayCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type SuperTimesPayCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SuperTimesPayCasSettings): SuperTimesPayCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: SuperTimesPayCasState, action: SuperTimesPayCasAction): SuperTimesPayCasState {
...
export function isTerminal(state: SuperTimesPayCasState): { score: number } | null {
