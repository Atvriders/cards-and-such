// Forty-Five — Irish partnership trick game — simplified to a 1v1 duel.
...
export type FortyFivePhase = "playing" | "done";
...
export type FortyFiveAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `forty-five-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: FortyFiveState, trick: readonly { seat: number; card: Card }[]): FortyFiveState {
...
export function reducer(state: FortyFiveState, action: FortyFiveAction): FortyFiveState {
  ...
  let s: FortyFiveState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: FortyFiveState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: FortyFiveState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): FortyFiveState {
...
export function isTerminal(state: FortyFiveState): { score: number } | null {
