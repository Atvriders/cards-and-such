...
export interface AcesUpFiringSquadSettings { dummy: boolean; }

export interface AcesUpFiringSquadState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type AcesUpFiringSquadAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { if (cardRank(c) === 1) s += 10; else s += 1; }
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: AcesUpFiringSquadState, action: AcesUpFiringSquadAction): AcesUpFiringSquadState {
...
export function isTerminal(state: AcesUpFiringSquadState): { score: number } | null {
