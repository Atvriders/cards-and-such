...
describe("VideoGameQuiz",()=>{
  it("creates 10 questions",()=>{expect(s0().questions.length).toBe(10);});
  it("starts playing",()=>{expect(s0().phase).toBe("playing");});
  it("is deterministic",()=>{expect(initialState(5,{questions:"10"}).questions.map(q=>q.question)).toEqual(initialState(5,{questions:"10"}).questions.map(q=>q.question));});
  it("select stores choice",()=>{expect(reducer(s0(),{type:"select",choice:3}).selected).toBe(3);});
