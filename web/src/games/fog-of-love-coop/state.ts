...
export interface fogOfLoveCoopSettings { dummy: boolean; }
export interface fogOfLoveCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type fogOfLoveCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: fogOfLoveCoopSettings): fogOfLoveCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: fogOfLoveCoopState, action: fogOfLoveCoopAction): fogOfLoveCoopState {
...
export function isTerminal(state: fogOfLoveCoopState): { score: number } | null {
