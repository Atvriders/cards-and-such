...
export function RailroadInkChallengeGame({ state, dispatch, onGameOver }: GameProps<RailroadInkChallengeState, RailroadInkChallengeSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RailroadInkChallengeAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RailroadInkChallengeAction)}>Skip</button>
      )}
    </div>
  );
}
