...
export const TOTAL_ROLLS = 9;
export const DIE_MAX = 6;

export interface RailroadInkChallengeSettings { dummy: boolean; }
export interface RailroadInkChallengeState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type RailroadInkChallengeAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: RailroadInkChallengeSettings): RailroadInkChallengeState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: RailroadInkChallengeState, action: RailroadInkChallengeAction): RailroadInkChallengeState {
...
export function isTerminal(state: RailroadInkChallengeState): { score: number } | null {
