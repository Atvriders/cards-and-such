...
export interface BigBenSettings { dummy: boolean; }

export interface BigBenState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const fives = h.filter(c=>cardRank(c)===5).length; s += fives*15; const inSeq = h.filter(c=>cardRank(c)>=4 && cardRank(c)<=6).length; s += inSeq*4; if (s===0) s=1; return s;
}
...
export function reducer(state: BigBenState, action: BigBenAction): BigBenState {
...
export function isTerminal(state: BigBenState): { score: number } | null {
