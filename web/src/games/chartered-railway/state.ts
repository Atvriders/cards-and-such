...
export interface CharteredRailwaySettings { dummy: boolean; }
export interface CharteredRailwayState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type CharteredRailwayAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: CharteredRailwaySettings): CharteredRailwayState {
...
export function reducer(state: CharteredRailwayState, action: CharteredRailwayAction): CharteredRailwayState {
...
export function score(s: CharteredRailwayState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: CharteredRailwayState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
