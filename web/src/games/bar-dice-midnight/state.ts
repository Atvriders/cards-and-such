...
export interface MidnightBarDiceSettings { dummy: boolean; }
export interface MidnightBarDiceState {
  rngSeed: number;
  turn: number;
  score: number;
  lastPts: number;
...
export type MidnightBarDiceAction = { type: "throw" } | { type: "next" };

export function initialState(seed: number, _s: MidnightBarDiceSettings): MidnightBarDiceState {
  return { rngSeed: seed, turn: 1, score: 0, lastPts: 0, lastRoll: 0, phase: "ready" };
}

export function reducer(state: MidnightBarDiceState, action: MidnightBarDiceAction): MidnightBarDiceState {
...
export function isTerminal(s: MidnightBarDiceState): { score: number } | null { return s.phase === "done" ? { score: s.score } : null; }
