...
export interface DrPepperPokerSettings { dummy: boolean; }
export interface DrPepperPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DrPepperPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DrPepperPokerSettings): DrPepperPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DrPepperPokerState, action: DrPepperPokerAction): DrPepperPokerState {
...
export function isTerminal(state: DrPepperPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
