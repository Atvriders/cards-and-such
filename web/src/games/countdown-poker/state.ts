...
export interface CountdownPokerSettings { rounds: "10"; }

export interface CountdownPokerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type CountdownPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CountdownPokerSettings): CountdownPokerState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: CountdownPokerState, action: CountdownPokerAction): CountdownPokerState {
...
export function isTerminal(state: CountdownPokerState): { score: number } | null {
