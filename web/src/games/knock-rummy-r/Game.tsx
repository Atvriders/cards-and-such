...
export function KnockRummyRGame({ state, dispatch, onGameOver }: GameProps<KnockRummyRState, KnockRummyRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as KnockRummyRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as KnockRummyRAction)}>Next</button>
      </>}
    </div>
  );
}
