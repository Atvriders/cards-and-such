...
const LobaRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.LobaRGame as unknown as React.ComponentType<unknown> })));
...
export const lobaRPlugin: GamePlugin<LobaRState, LobaRAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: LobaRState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "play") return { selector: '[data-testid="hint-target-loba-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-loba-r-next"]', pulses: 3 };
    return null;
  },
  component: LobaRGame,
};
