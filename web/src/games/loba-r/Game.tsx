...
export function LobaRGame({ state, dispatch, onGameOver }: GameProps<LobaRState, LobaRSettings>): JSX.Element {
  ...
  return (
    <div className="lobr-wrap">
      ...
      {state.phase === "play" && <button data-testid="hint-target-loba-r-play" className="lobr-btn" onClick={() => dispatch({ type: "score" } as LobaRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="lobr-result">{state.result} — +{state.pts}</div>
        <button data-testid="hint-target-loba-r-next" className="lobr-btn alt" onClick={() => dispatch({ type: "next" } as LobaRAction)}>Next</button>
      </>}
    </div>
  );
}
