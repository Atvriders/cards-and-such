...
export interface LobaRSettings { dummy: boolean; }
export interface LobaRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type LobaRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: LobaRSettings): LobaRState {
...
export function reducer(state: LobaRState, action: LobaRAction): LobaRState {
...
export function isTerminal(state: LobaRState): { score: number } | null {
