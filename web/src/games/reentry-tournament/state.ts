...
export interface ReentryTournamentSettings { dummy: boolean; }
export interface ReentryTournamentState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ReentryTournamentAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ReentryTournamentSettings): ReentryTournamentState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ReentryTournamentState, action: ReentryTournamentAction): ReentryTournamentState {
...
export function isTerminal(state: ReentryTournamentState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
