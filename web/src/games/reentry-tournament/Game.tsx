...
import "./Game.css";
export function ReentryTournamentGame({ state, dispatch, onGameOver }: GameProps<ReentryTournamentState, ReentryTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-reentry-tournament-deal" onClick={() => dispatch({ type: "deal" } as ReentryTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-reentry-tournament-next" onClick={() => dispatch({ type: "next" } as ReentryTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
