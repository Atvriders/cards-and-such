...
export interface CardEclipseSettings { dummy: boolean; }
export interface CardEclipseState {
  rngSeed: number;
  round: number;
  cards: number[];
  picked: number | null;
...
export type CardEclipseAction = { type: "pick"; index: number } | { type: "next" } | { type: "deal" };
...
export function initialState(seed: number, _settings: CardEclipseSettings): CardEclipseState {
...
export function reducer(state: CardEclipseState, action: CardEclipseAction): CardEclipseState {
...
export function isTerminal(state: CardEclipseState): { score: number } | null {
