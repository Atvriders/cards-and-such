...
export function CardEclipseGame({ state, dispatch, onGameOver }: GameProps<CardEclipseState, CardEclipseSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      <div className="cm-hint">Card Eclipse</div>
      ...
      <div className="cm-row">
        {state.cards.map((c, i) => (
          <button key={i}
            ...
            onClick={() => dispatch({ type:"pick", index:i } as CardEclipseAction)}>{cardName(c)}</button>
        ))}
      </div>
      {state.phase === "scored" && (
        <>
          <div className="cm-result">Picked: {cardName(state.picked!)} — {state.lastPts > 0 ? `+${state.lastPts}` : "0"}</div>
          <button data-testid="hint-target-card-eclipse-primary" className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardEclipseAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
