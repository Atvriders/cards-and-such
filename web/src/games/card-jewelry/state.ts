...
export interface CardJewelrySettings { dummy: boolean; }
export interface CardJewelryState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardJewelryAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardJewelrySettings): CardJewelryState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardJewelryState, action: CardJewelryAction): CardJewelryState {
...
export function isTerminal(state: CardJewelryState): { score: number } | null {
