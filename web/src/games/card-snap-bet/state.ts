...
export interface CardSnapBetSettings { rounds: "8" | "12"; }
export interface CardSnapBetState { deck: number[]; pos: number; topCard: number; coins: number; round: number; maxRounds: number; phase: "betting" | "revealed" | "gameover"; lastResult: "win" | "lose" | "tie" | null; revealedCard: number | null; bid: number; }
export type CardSnapBetAction = { type: "bet"; amount: number; dir: "higher" | "lower" } | { type: "next" };
...
export function initialState(seed: number, settings: CardSnapBetSettings): CardSnapBetState {
  const rng=mulberry32(seed); const deck=makeDeck(rng);
  return {deck,pos:1,topCard:deck[0]!,coins:90,bid:0,round:1,maxRounds:parseInt(settings.rounds,10),phase:"betting",lastResult:null,revealedCard:null};
}
export function reducer(state: CardSnapBetState, action: CardSnapBetAction): CardSnapBetState {
...
export function isTerminal(state: CardSnapBetState): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
