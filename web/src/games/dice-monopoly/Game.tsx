...
export function DiceMonopolyGame({ state, dispatch, onGameOver }: GameProps<DiceMonopolyState, DiceMonopolySettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap"><div className="dm-done"><h2>Done!</h2><div className="dm-final">{Math.max(0, state.score)} pts</div></div></div>;
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "rolling" && <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as DiceMonopolyAction)}>Roll</button>}
      {state.phase === "scored" && <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceMonopolyAction)}>{state.turn >= TOTAL_TURNS ? "Finish" : "Next"}</button>}
    </div>
  );
}
