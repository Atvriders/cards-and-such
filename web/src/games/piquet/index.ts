...
Select cards to discard, then click the Discard button. Then play tricks by clicking cards.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-piquet-action"]', pulses: 3 }; },
  component: Piquet,
};
