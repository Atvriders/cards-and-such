// Auto-generated backgammon-family state for fevga.
...
export interface FevgaSettings { dummy: boolean }

export interface FevgaState {
  rngSeed: number;
...
export function initialState(seed: number, _s: FevgaSettings): FevgaState {
...
export function legalMoves(state: FevgaState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: FevgaState, side: Side, from: number, pips: number): FevgaState | null {
  const next: FevgaState = {
    ...state,
...
function checkWin(s: FevgaState): FevgaState {
...
function cpuTurn(state: FevgaState): FevgaState {
  let s = state;
...
export function reducer(state: FevgaState, action: FevgaAction): FevgaState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: FevgaState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: FevgaState): { score: number } | null {
