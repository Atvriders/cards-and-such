// Spot Hearts — Hearts variant where each heart's pip value is its penalty score.
...
export type SpotHeartsPhase = "playing" | "done";

export interface SpotHeartsState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: SpotHeartsPhase;
  playerLeads: boolean;
  message: string;
}

export type SpotHeartsAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `spot-hearts-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: SpotHeartsState, trick: readonly { seat: number; card: Card }[]): SpotHeartsState {
...
export function reducer(state: SpotHeartsState, action: SpotHeartsAction): SpotHeartsState {
  ...
  let s: SpotHeartsState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: SpotHeartsState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: SpotHeartsState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): SpotHeartsState {
...
export function isTerminal(state: SpotHeartsState): { score: number } | null {
