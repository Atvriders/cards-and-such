...
export interface SecondChanceCardsSettings { rounds: "10"; }

export interface SecondChanceCardsState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type SecondChanceCardsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SecondChanceCardsSettings): SecondChanceCardsState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: SecondChanceCardsState, action: SecondChanceCardsAction): SecondChanceCardsState {
...
export function isTerminal(state: SecondChanceCardsState): { score: number } | null {
