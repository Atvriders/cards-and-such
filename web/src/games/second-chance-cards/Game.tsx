...
export function SecondChanceCards({ state, dispatch, onGameOver }: GameProps<SecondChanceCardsState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      <div className="g-controls">
        {state.phase === "ready" && <button className="g-btn" onClick={() => dispatch({ type: "play" } as SecondChanceCardsAction)}>Play Hand</button>}
        {state.phase === "played" && <button className="g-btn" onClick={() => dispatch({ type: "next" } as SecondChanceCardsAction)}>Next</button>}
      </div>
    </div>
  );
}
