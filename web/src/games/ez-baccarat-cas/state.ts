...
export interface EzBaccaratCasSettings { dummy: boolean; }
export interface EzBaccaratCasState { rngSeed: number; round: number; bet: "player" | "banker" | "tie" | null; player: number[]; banker: number[]; phase: "bet" | "scored" | "done"; score: number; pts: number; result: string; }
export type EzBaccaratCasAction = { type: "bet"; side: "player" | "banker" | "tie" } | { type: "next" };
...
export function initialState(seed: number, _s: EzBaccaratCasSettings): EzBaccaratCasState {
  return { rngSeed: seed, round: 1, bet: null, player: [], banker: [], phase: "bet", score: 0, pts: 0, result: "" };
}
export function reducer(state: EzBaccaratCasState, action: EzBaccaratCasAction): EzBaccaratCasState {
...
export function isTerminal(state: EzBaccaratCasState): { score: number } | null {
