...
export interface EzBaccaratCasState {
  rngSeed: number;
  round: number;
...
export type EzBaccaratCasAction = { type: "bet"; on: "player" | "banker" | "tie" } | { type: "next" };
...
export function reducer(state: EzBaccaratCasState, action: EzBaccaratCasAction): EzBaccaratCasState {
  ...
  if (action.type === "bet" && state.phase === "bet") {
    ...
    if (action.on === r.outcome) {
      ...
      else if (action.on === "banker") { pts = 20; result = `Banker wins ${r.bTotal}-${r.pTotal} (+${pts})`; }
