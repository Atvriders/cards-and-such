...
import "./Game.css";
export function BountyTournamentGame({ state, dispatch, onGameOver }: GameProps<BountyTournamentState, BountyTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-bounty-tournament-deal" onClick={() => dispatch({ type: "deal" } as BountyTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-bounty-tournament-next" onClick={() => dispatch({ type: "next" } as BountyTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
