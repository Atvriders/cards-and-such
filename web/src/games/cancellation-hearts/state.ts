// Cancellation Hearts — Hearts variant scaled down: cancel pairs in a head-to-head duel.
...
export type CancellationHeartsPhase = "playing" | "done";

export interface CancellationHeartsState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: CancellationHeartsPhase;
  playerLeads: boolean;
  message: string;
}

export type CancellationHeartsAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `cancellation-hearts-${s}${r}` });
  }
  return cards;
}

const TRUMP: Suit | null = null;
...
function applyTrick(state: CancellationHeartsState, trick: readonly { seat: number; card: Card }[]): CancellationHeartsState {
...
export function reducer(state: CancellationHeartsState, action: CancellationHeartsAction): CancellationHeartsState {
  ...
  let s: CancellationHeartsState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: CancellationHeartsState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: CancellationHeartsState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): CancellationHeartsState {
  ...
  return {
    ...
    phase: "playing",
    playerLeads: true,
    message: "No trump. Your lead.",
  };
}

export function isTerminal(state: CancellationHeartsState): { score: number } | null {
