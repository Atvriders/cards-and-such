...
export interface truffleShuffleSettings { dummy: boolean; }
export interface truffleShuffleState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type truffleShuffleAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: truffleShuffleSettings): truffleShuffleState {
  return {
    rngSeed: seed,
...
export function reducer(state: truffleShuffleState, action: truffleShuffleAction): truffleShuffleState {
...
export function isTerminal(state: truffleShuffleState): { score: number } | null {
