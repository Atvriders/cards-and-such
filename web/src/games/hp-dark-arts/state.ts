...
export interface hpDarkArtsSettings { dummy: boolean; }
export interface hpDarkArtsState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type hpDarkArtsAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: hpDarkArtsSettings): hpDarkArtsState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: hpDarkArtsState, action: hpDarkArtsAction): hpDarkArtsState {
...
export function isTerminal(state: hpDarkArtsState): { score: number } | null {
