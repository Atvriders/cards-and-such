...
const DoubleDummyWhistGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DoubleDummyWhistGame as unknown as React.ComponentType<unknown> })));
...
export const doubleDummyWhistPlugin: GamePlugin<DoubleDummyWhistState, DoubleDummyWhistAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DoubleDummyWhistState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-ddwh-${pick.id}"]`, pulses: 3 };
    }
