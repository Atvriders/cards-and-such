...
export function PandemicBaseGame({ state, dispatch, onGameOver }: GameProps<PandemicBaseState, PandemicBaseSettings>): JSX.Element {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as PandemicBaseAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as PandemicBaseAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
