...
export function SpanishPatienceGame({ state, dispatch, onGameOver }: GameProps<SpanishPatienceState, SpanishPatienceSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as SpanishPatienceAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
