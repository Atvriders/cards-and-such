...
export interface ConfettiCatchSettings { dummy: boolean; }
...
export interface ConfettiCatchState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type ConfettiCatchAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _s: ConfettiCatchSettings): ConfettiCatchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: ConfettiCatchState, action: ConfettiCatchAction): ConfettiCatchState {
...
export function isTerminal(state: ConfettiCatchState): { score: number } | null {
