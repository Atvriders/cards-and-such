...
const S = { dummy: false };
describe("MonochromeRun", () => {
  it("starts in drawing", () => { const s = initialState(1, S); expect(s.phase).toBe("drawing"); expect(s.score).toBe(0); });
  it("first draw sets streak 1", () => { const s = reducer(initialState(1, S), { type:"draw" }); expect(s.currentStreak).toBe(1); });
  it("score is non-negative after draws", () => {
    let s = initialState(1, S);
    for (let i = 0; i < 6; i++) s = reducer(s, { type:"draw" });
