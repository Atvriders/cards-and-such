...
export function CapitalismShedGame({ state, dispatch, onGameOver }: GameProps<CapitalismShedState, CapitalismShedSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap capsh-shed"><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap capsh-shed">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as CapitalismShedAction)}>Play Round</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as CapitalismShedAction)}>Next</button>
      </>}
    </div>
  );
}
