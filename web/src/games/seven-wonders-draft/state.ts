...
export const SUIT_NAMES = ["War","Trade","Science","Civic"] as const;
...
export type SevenWondersDraftState = DraftState;
export type SevenWondersDraftAction = DraftAction;
export interface SevenWondersDraftSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SevenWondersDraftSettings): SevenWondersDraftState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SevenWondersDraftState, action: SevenWondersDraftAction): SevenWondersDraftState {
  return draftReducer(state, action);
}

export function score(state: SevenWondersDraftState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersDraftState): { score: number } | null {
