...
export interface SevenWondersDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersDraftSettings): SevenWondersDraftState {
...
export function score(s: SevenWondersDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
