...
import "./Game.css";
export function CardRiverGame({ state, dispatch, onGameOver }: GameProps<CardRiverState, CardRiverSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <div className="cm-help">Sum near 35 wins</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardRiverAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
