...
export interface CardRiverSettings { dummy: boolean; }
...
export type CardRiverAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardRiverSettings): CardRiverState {
  return { rngSeed: seed, round: 1, hand: [], sum: 0, score: 0, phase: "dealing", lastPts: 0 };
}
export function reducer(state: CardRiverState, action: CardRiverAction): CardRiverState {
  ...
  if (action.type === "deal") {
    ...
    const pts = Math.max(0, 50 - Math.abs(sum - 35));
...
export function isTerminal(state: CardRiverState): { score: number } | null {
