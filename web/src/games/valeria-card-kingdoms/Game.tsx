...
import "./Game.css";
export function ValeriaCardKingdomsGame({ state, dispatch, onGameOver }: GameProps<ValeriaCardKingdomsState, ValeriaCardKingdomsSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as ValeriaCardKingdomsAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
