...
export function CardCoffeeGame({ state, dispatch, onGameOver }: GameProps<CardCoffeeState, CardCoffeeSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "deal" && (
        <button data-testid="hint-target-card-coffee-primary" className="cm-btn" onClick={() => dispatch({ type:"deal" } as CardCoffeeAction)}>Deal pair</button>
      )}
      {state.phase === "decide" && state.pair && (
        <>
          ...
          <div className="cm-row">
            <button className="cm-btn" onClick={() => dispatch({ type:"take" } as CardCoffeeAction)}>Take</button>
            <button className="cm-btn alt" onClick={() => dispatch({ type:"skip" } as CardCoffeeAction)}>Skip</button>
          </div>
        </>
      )}
