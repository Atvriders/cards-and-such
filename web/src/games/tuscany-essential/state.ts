...
export interface TuscanyEssentialSettings { dummy: boolean; }
export interface TuscanyEssentialState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type TuscanyEssentialAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: TuscanyEssentialSettings): TuscanyEssentialState {
...
export function reducer(state: TuscanyEssentialState, action: TuscanyEssentialAction): TuscanyEssentialState {
...
export function score(s: TuscanyEssentialState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: TuscanyEssentialState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
