...
export interface CardMirageSettings { dummy: boolean; }
export interface CardMirageState {
  rngSeed: number;
  round: number;
  cards: number[];
  picked: number | null;
...
export type CardMirageAction = { type: "pick"; index: number } | { type: "next" } | { type: "deal" };
...
export function initialState(seed: number, _settings: CardMirageSettings): CardMirageState {
...
export function reducer(state: CardMirageState, action: CardMirageAction): CardMirageState {
...
export function isTerminal(state: CardMirageState): { score: number } | null {
