...
export function WindokuMiniGame({ state, dispatch, onGameOver }: GameProps<WindokuMiniState, WindokuMiniSettings>): JSX.Element {
    ...
    <div className="ded-wrap">
      ...
      <div className="ded-options">
        {p.options.map((opt, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={state.resolved} onClick={() => dispatch({ type: "select", index: i } as WindokuMiniAction)}>
              {opt}
            </button>
          );
        })}
