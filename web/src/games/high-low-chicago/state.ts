...
export interface HighLowChicagoSettings { rounds: "10"; }

export interface HighLowChicagoState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type HighLowChicagoAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HighLowChicagoSettings): HighLowChicagoState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: HighLowChicagoState, action: HighLowChicagoAction): HighLowChicagoState {
...
export function isTerminal(state: HighLowChicagoState): { score: number } | null {
