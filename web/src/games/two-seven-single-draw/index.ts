...
const TwoSevenSingleDrawGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TwoSevenSingleDrawGame as unknown as React.ComponentType<unknown> })));
...
export const twoSevenSingleDrawPlugin: GamePlugin<TwoSevenSingleDrawState, TwoSevenSingleDrawAction, typeof settings> = {
  ...
  hint: (state: TwoSevenSingleDrawState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-two-seven-single-draw-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-two-seven-single-draw-next"]', pulses: 3 };
    return null;
  },
};
