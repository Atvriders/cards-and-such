...
export interface TwoSevenSingleDrawSettings { dummy: boolean; }
export interface TwoSevenSingleDrawState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TwoSevenSingleDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TwoSevenSingleDrawSettings): TwoSevenSingleDrawState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TwoSevenSingleDrawState, action: TwoSevenSingleDrawAction): TwoSevenSingleDrawState {
...
export function isTerminal(state: TwoSevenSingleDrawState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
