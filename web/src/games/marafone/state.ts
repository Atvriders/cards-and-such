// Marafone — Romagnan trick-taking game on 40 cards — set-trump duel here.
...
export type MarafonePhase = "playing" | "done";

export interface MarafoneState {
  rngSeed: number;
...
export type MarafoneAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `marafone-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: MarafoneState, trick: readonly { seat: number; card: Card }[]): MarafoneState {
...
export function reducer(state: MarafoneState, action: MarafoneAction): MarafoneState {
  ...
  let s: MarafoneState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: MarafoneState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: MarafoneState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): MarafoneState {
...
export function isTerminal(state: MarafoneState): { score: number } | null {
