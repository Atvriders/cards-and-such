...
export interface InnovationAgesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type InnovationAgesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: InnovationAgesSettings): InnovationAgesState {
...
export function score(s: InnovationAgesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: InnovationAgesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
