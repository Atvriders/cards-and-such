...
export function InnovationAgesGame({ state, dispatch, onGameOver }: GameProps<InnovationAgesState, InnovationAgesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Innovation Ages</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as InnovationAgesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as InnovationAgesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
