...
export interface ChipsChompSettings { dummy: boolean; }
...
export interface ChipsChompState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type ChipsChompAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: ChipsChompSettings): ChipsChompState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: ChipsChompState, action: ChipsChompAction): ChipsChompState {
...
export function isTerminal(state: ChipsChompState): { score: number } | null {
