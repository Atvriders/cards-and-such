...
export interface RainbowSoliSettings { dummy: boolean; }

export interface RainbowSoliState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(hand: number[]): { score: number; reason: string } {
  ...
  // Court-card variant
  if (faces >= 1) { score += faces * 9; reasons.push(`${faces} face`); }
  if (pairs >= 1) { score += pairs * 7; reasons.push(`${pairs} pair`); }
  if (best >= 3) { score += best * 3; reasons.push(`run${best}`); }
  if (flush) { score += 10; reasons.push("flush"); }
  if (score === 0) score = 1;
  return { score, reason: reasons.join(",") || "weak" };
}

export function initialState(seed: number, _s: RainbowSoliSettings): RainbowSoliState {
...
export function reducer(state: RainbowSoliState, action: RainbowSoliAction): RainbowSoliState {
...
export function isTerminal(state: RainbowSoliState): { score: number } | null {
