...
import { initialState, reducer, isTerminal, MastermindNoRepeats_CFG } from "./state.js";

const S = { dummy: false };

describe("mastermind-no-repeats", () => {
  it("starts in guess phase with empty history", () => {
    const s = initialState(1, S);
    expect(s.phase).toBe("guess");
    expect(s.guesses.length).toBe(0);
    expect(s.answer.length).toBe(MastermindNoRepeats_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < MastermindNoRepeats_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < MastermindNoRepeats_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
