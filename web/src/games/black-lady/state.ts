// Black Lady — Hearts variant where only the Queen of Spades carries a penalty.
...
export type BlackLadyPhase = "playing" | "done";

export interface BlackLadyState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: BlackLadyPhase;
  playerLeads: boolean;
  message: string;
}

export type BlackLadyAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `black-lady-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: BlackLadyState, trick: readonly { seat: number; card: Card }[]): BlackLadyState {
...
export function reducer(state: BlackLadyState, action: BlackLadyAction): BlackLadyState {
  ...
  let s: BlackLadyState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: BlackLadyState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: BlackLadyState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): BlackLadyState {
...
export function isTerminal(state: BlackLadyState): { score: number } | null {
