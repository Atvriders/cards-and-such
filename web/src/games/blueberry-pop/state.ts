...
export interface BlueberryPopSettings { dummy: boolean; }
export interface BlueberryPopState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type BlueberryPopAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: BlueberryPopSettings): BlueberryPopState {
...
export function reducer(state: BlueberryPopState, action: BlueberryPopAction): BlueberryPopState {
...
export function isTerminal(state: BlueberryPopState): { score: number } | null {
