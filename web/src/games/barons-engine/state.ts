...
export interface BaronsEngineSettings { dummy: boolean; }
export interface BaronsEngineState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type BaronsEngineAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: BaronsEngineSettings): BaronsEngineState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Iron ore boom","Coal seam discovered","Gold rush","Mine tunnel collapse","Rail line extended","Refinery upgrade"];

export function reducer(state: BaronsEngineState, action: BaronsEngineAction): BaronsEngineState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Mines (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Foreman (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Traded 1 Mines (+$" + market + ")"; delta = market;
      } else { evt = "No Mines to trade"; }
    } else { return state; }
    const div = assets * 7;
    cash += div;
    const pay = workers * 11;
    cash += pay;
...
export function score(s: BaronsEngineState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: BaronsEngineState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
