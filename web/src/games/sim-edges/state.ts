...
function applyMove(b: Cell[], idx: number, mark: "P" | "C"): { board: Cell[]; captures: number } {
  ...
  const trios: Array<[number, number, number]> = [
    [0, 1, 2], [SIZE, SIZE+1, SIZE+2], [0, SIZE, 2*SIZE], [1, SIZE+1, 2*SIZE+1], [0, SIZE+1, 2*SIZE+2], [2, SIZE+1, 2*SIZE]
  ];
  for (const [a, b2, c2] of trios) {
    if (a < nb.length && b2 < nb.length && c2 < nb.length && nb[a] === mark && nb[b2] === mark && nb[c2] === mark) {
      captures = -1; break;
    }
  }
