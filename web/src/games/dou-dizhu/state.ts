...
export interface DouDizhuSettings { dummy: boolean; }
export interface DouDizhuState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type DouDizhuAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: DouDizhuSettings): DouDizhuState {
...
export function isTerminal(state: DouDizhuState): { score: number } | null {
