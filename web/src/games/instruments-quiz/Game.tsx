...
export function InstrumentsQuiz({ state, dispatch, onGameOver }: GameProps<InstrumentsQuizState, InstrumentsQuizSettings>): JSX.Element {
  ...
  return <div className="trivia-wrap">
    ...
    <div className="trivia-actions">
      ...
      {isResult&&<button className="trivia-btn next" onClick={()=>dispatch({type:"next"} as InstrumentsQuizAction)}>{state.currentIndex+1>=state.questions.length?"Finish":"Next"}</button>}
    </div>
  </div>;
}
