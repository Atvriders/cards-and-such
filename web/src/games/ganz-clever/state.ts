...
export interface GanzCleverSettings { rounds: "10"; }

export interface GanzCleverState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type GanzCleverAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: GanzCleverSettings): GanzCleverState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: GanzCleverState, action: GanzCleverAction): GanzCleverState {
...
export function isTerminal(state: GanzCleverState): { score: number } | null {
