...
export const TYPE_NAMES = ["City","Forest","Sea","Field","Factory"] as const;
...
export type HonshuState = TileState;
export type HonshuAction = TileAction;
export interface HonshuSettings { dummy: boolean }

export function typeName(t: number): string { return TYPE_NAMES[t] ?? "?"; }

export function initialState(seed: number, _s: HonshuSettings): HonshuState {
  void mulberry32;
  return initialTileState(seed, CONFIG);
}

export function reducer(state: HonshuState, action: HonshuAction): HonshuState {
  return tileReducer(state, action);
}

export function isTerminal(state: HonshuState): { score: number } | null {
