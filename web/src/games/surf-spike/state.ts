...
export interface SurfSpikeSettings { dummy: boolean; }
export interface SurfSpikeTarget { id: number; lane: number; ticksLeft: number; }
export interface SurfSpikeState {
  rngSeed: number;
  targets: SurfSpikeTarget[];
  nextId: number;
...
export type SurfSpikeAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: SurfSpikeSettings): SurfSpikeState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: SurfSpikeState, action: SurfSpikeAction): SurfSpikeState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: SurfSpikeTarget[] = [];
...
export function isTerminal(state: SurfSpikeState): { score: number } | null {
