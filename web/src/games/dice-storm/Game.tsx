    ...
    <div className="rg-wrap">
      ...
      {state.phase === "predict" && (
        <div className="rg-row">
          {CHOICES.map(c => (
            <button key={c} className="rg-btn" onClick={() => dispatch({ type:"go", choice:c } as DiceStormAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && (
        <>
          ...
          <button className="rg-btn alt" onClick={() => dispatch({ type:"next" } as DiceStormAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
