...
import "./Game.css";
export function HoseMixGame({ state, dispatch, onGameOver }: GameProps<HoseMixState, HoseMixSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-hose-mix-deal" onClick={() => dispatch({ type: "deal" } as HoseMixAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-hose-mix-next" onClick={() => dispatch({ type: "next" } as HoseMixAction)}>Next</button>
      </>}
    </div>
  );
}
