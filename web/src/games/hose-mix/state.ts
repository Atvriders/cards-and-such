...
export interface HoseMixSettings { dummy: boolean; }
export interface HoseMixState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HoseMixAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HoseMixSettings): HoseMixState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HoseMixState, action: HoseMixAction): HoseMixState {
...
export function isTerminal(state: HoseMixState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
