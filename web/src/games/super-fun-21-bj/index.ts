...
export const superFun21BjPlugin: GamePlugin<SuperFun21BjState, SuperFun21BjAction, typeof settings> = {
  ...
  hint: (state) => {
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-super-fun-21-bj-next"]', pulses: 3 };
    if (state.phase !== "play") return null;
    const total = state.total;
    if (total < 12) return { selector: '[data-testid="hint-target-super-fun-21-bj-hit"]', pulses: 3 };
    if (total >= 17) return { selector: '[data-testid="hint-target-super-fun-21-bj-stand"]', pulses: 3 };
    return { selector: '[data-testid="hint-target-super-fun-21-bj-hit"]', pulses: 3 };
  },
