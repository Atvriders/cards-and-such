...
import "./Game.css";
export function GrabDiceGameGame({ state, dispatch, onGameOver }: GameProps<GrabDiceGameState, GrabDiceGameSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-grab-dice-game-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as GrabDiceGameAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-grab-dice-game-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as GrabDiceGameAction)}>Next</button>
        </>
      )}
    </div>
  );
