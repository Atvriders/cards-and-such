...
export interface GearGrabSettings { dummy: boolean; }
export interface Target { id:number; lane:number; ticksLeft:number; }
export interface GearGrabState { rngSeed:number; targets:Target[]; nextId:number; ticksRemaining:number; score:number; popped:number; missed:number; phase:"playing"|"done"; }
export type GearGrabAction = { type:"tick" } | { type:"pop"; id:number };
export function initialState(seed:number,_s:GearGrabSettings):GearGrabState { return { rngSeed:seed, targets:[], nextId:1, ticksRemaining:TIMER_TICKS, score:0, popped:0, missed:0, phase:"playing" }; }
export function reducer(state:GearGrabState, action:GearGrabAction):GearGrabState {
...
export function isTerminal(state:GearGrabState){ return state.phase==="done"?{score:state.score}:null; }
