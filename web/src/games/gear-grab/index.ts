...
const GearGrabGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.GearGrabGame as unknown as React.ComponentType<unknown> })));
...
export const gearGrabPlugin: GamePlugin<GearGrabState, GearGrabAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: GearGrabState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-gear-grab-target"]', pulses: 3 };
  },
  component:GearGrabGame,
};
