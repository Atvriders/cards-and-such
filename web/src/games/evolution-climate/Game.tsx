...
export function EvolutionClimateGame({ state, dispatch, onGameOver }: GameProps<EvolutionClimateState, EvolutionClimateSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Evolution: Climate</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as EvolutionClimateAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as EvolutionClimateAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
