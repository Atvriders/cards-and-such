...
export const SUIT_NAMES = ["Ice","Heat","Drought","Flood"] as const;
...
export type EvolutionClimateState = DraftState;
export type EvolutionClimateAction = DraftAction;
...
export function initialState(seed: number, _settings: EvolutionClimateSettings): EvolutionClimateState {
...
export function score(state: EvolutionClimateState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: EvolutionClimateState): { score: number } | null {
