...
export const SUIT_NAMES: readonly string[] = ["Cold","Temperate","Warm","Tropical"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface EvolutionClimateSettings { dummy: boolean; }
export interface EvolutionClimateState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type EvolutionClimateAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: EvolutionClimateSettings): EvolutionClimateState {
...
export function reducer(state: EvolutionClimateState, action: EvolutionClimateAction): EvolutionClimateState {
...
export function score(s: EvolutionClimateState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: EvolutionClimateState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
