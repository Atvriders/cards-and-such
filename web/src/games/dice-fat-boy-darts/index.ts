...
const DiceFatBoyDartsGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceFatBoyDartsGame as unknown as React.ComponentType<unknown> })));
...
export const diceFatBoyDartsPlugin: GamePlugin<DiceFatBoyDartsState, DiceFatBoyDartsAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceFatBoyDartsState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-fat-boy-darts-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-fat-boy-darts-next"]', pulses: 3 };
    return null;
  },
