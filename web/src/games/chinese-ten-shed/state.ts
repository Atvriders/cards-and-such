...
export interface ChineseTenShedSettings { dummy: boolean; }
export interface ChineseTenShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type ChineseTenShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ChineseTenShedSettings): ChineseTenShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: ChineseTenShedState, action: ChineseTenShedAction): ChineseTenShedState {
...
export function isTerminal(state: ChineseTenShedState): { score: number } | null {
