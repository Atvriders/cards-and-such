...
export interface NapoleonsShoulderSettings { dummy: boolean; }

export interface NapoleonsShoulderState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type NapoleonsShoulderAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const f = h.filter(c=>cardRank(c)>=11).length; s += f*7; const aces = h.filter(c=>cardRank(c)===1).length; s += aces*5; if (s===0) s=1; return s;
}
...
export function reducer(state: NapoleonsShoulderState, action: NapoleonsShoulderAction): NapoleonsShoulderState {
...
export function isTerminal(state: NapoleonsShoulderState): { score: number } | null {
