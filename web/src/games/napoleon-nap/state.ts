...
export interface NapoleonNapSettings { dummy: boolean; }
export interface NapoleonNapState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type NapoleonNapAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: NapoleonNapSettings): NapoleonNapState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: NapoleonNapState, action: NapoleonNapAction): NapoleonNapState {
...
export function isTerminal(state: NapoleonNapState): { score: number } | null {
