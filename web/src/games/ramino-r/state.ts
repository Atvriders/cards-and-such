...
export interface RaminoRSettings { dummy: boolean; }
export interface RaminoRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type RaminoRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: RaminoRSettings): RaminoRState {
...
export function reducer(state: RaminoRState, action: RaminoRAction): RaminoRState {
...
export function isTerminal(state: RaminoRState): { score: number } | null {
