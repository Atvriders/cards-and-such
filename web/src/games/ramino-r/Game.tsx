...
export function RaminoRGame({ state, dispatch, onGameOver }: GameProps<RaminoRState, RaminoRSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="rmnr-wrap"><div className="rmnr-done bounce-in"><h2>Done!</h2><div className="rmnr-final">{state.score} pts</div></div></div>;
  return (
    <div className="rmnr-wrap fade-in">
      <div className="rmnr-info">Round {state.round} / {TOTAL_ROUNDS}</div>
      <div className="rmnr-score pulse">{state.score} pts</div>
