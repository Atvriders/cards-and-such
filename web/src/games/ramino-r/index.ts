...
const RaminoRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RaminoRGame as unknown as React.ComponentType<unknown> })));
...
export const raminoRPlugin: GamePlugin<RaminoRState, RaminoRAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: RaminoRState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "play") return { selector: '[data-testid="hint-target-ramino-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-ramino-r-next"]', pulses: 3 };
    return null;
  },
  component: RaminoRGame,
};
