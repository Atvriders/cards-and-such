...
import "./Game.css";
export function TinyEpicGalaxiesMiniGame({ state, dispatch, onGameOver }: GameProps<TinyEpicGalaxiesMiniState, TinyEpicGalaxiesMiniSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as TinyEpicGalaxiesMiniAction)}>Roll</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">+{state.lastPts} pts{state.bonus > 0 ? ` (bonus +${state.bonus})` : ""}</div>
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as TinyEpicGalaxiesMiniAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
