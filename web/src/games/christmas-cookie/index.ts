...
const ChristmasCookieGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ChristmasCookieGame as unknown as React.ComponentType<unknown> })));
const settings = {
  pairs: {
    kind: "enum" as const,
    label: "Pairs",
...
Settings let you choose 6, 8, or 10 pairs. Fewer pairs are great for a quick game; 10 pairs gives a full memory challenge. The board is randomly shuffled each game, so no two rounds are the same.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver) return null; return { selector: '[data-testid="hint-target-christmas-cookie-action"]', pulses: 3 }; },
