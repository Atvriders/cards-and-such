...
export interface MochiMashSettings { dummy: boolean; }
...
export interface MochiMashState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type MochiMashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: MochiMashSettings): MochiMashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: MochiMashState, action: MochiMashAction): MochiMashState {
...
export function isTerminal(state: MochiMashState): { score: number } | null {
