...
export interface NochMalSoGutSettings { dummy: boolean; }
export interface NochMalSoGutState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type NochMalSoGutAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: NochMalSoGutSettings): NochMalSoGutState {
  return {
    rngSeed: seed,
...
export function reducer(state: NochMalSoGutState, action: NochMalSoGutAction): NochMalSoGutState {
...
export function isTerminal(state: NochMalSoGutState): { score: number } | null {
