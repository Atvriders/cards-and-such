// Cutthroat Spades — Spades simplified to a one-on-one duel — spades are trump.
...
export type CutthroatSpadesPhase = "playing" | "done";

export interface CutthroatSpadesState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: CutthroatSpadesPhase;
  playerLeads: boolean;
  message: string;
}

export type CutthroatSpadesAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `cutthroat-spades-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: CutthroatSpadesState, trick: readonly { seat: number; card: Card }[]): CutthroatSpadesState {
...
export function reducer(state: CutthroatSpadesState, action: CutthroatSpadesAction): CutthroatSpadesState {
  ...
  let s: CutthroatSpadesState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: CutthroatSpadesState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: CutthroatSpadesState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): CutthroatSpadesState {
...
export function isTerminal(state: CutthroatSpadesState): { score: number } | null {
