...
export interface CardElectronicsSettings { dummy: boolean; }
export interface CardElectronicsState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardElectronicsAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardElectronicsSettings): CardElectronicsState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardElectronicsState, action: CardElectronicsAction): CardElectronicsState {
  ...
  if (action.type === "draw") {
    ...
    if (state.history.length > 0 && (state.history[state.history.length - 1]! % 13) === (card % 13)) pts += 10;
...
export function isTerminal(state: CardElectronicsState): { score: number } | null {
