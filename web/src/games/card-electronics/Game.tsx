...
import "./Game.css";
export function CardElectronicsGame({ state, dispatch, onGameOver }: GameProps<CardElectronicsState, CardElectronicsSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "drawing" && (
        <button data-testid="hint-target-card-electronics-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as CardElectronicsAction)}>Draw Card</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardElectronicsAction)}>{state.drawCount >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
