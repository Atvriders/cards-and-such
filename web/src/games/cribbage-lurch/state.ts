...
export interface CribbageLurchSettings { dummy: boolean; }

export interface CribbageLurchState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type CribbageLurchAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: CribbageLurchSettings): CribbageLurchState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: CribbageLurchState, action: CribbageLurchAction): CribbageLurchState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<CribbageLurchState> = {};
...
export function isTerminal(state: CribbageLurchState): { score: number } | null {
