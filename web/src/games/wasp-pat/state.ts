...
export interface WaspPatSettings { dummy: boolean; }

export interface WaspPatState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export function reducer(state: WaspPatState, action: WaspPatAction): WaspPatState {
...
export function isTerminal(state: WaspPatState): { score: number } | null {
