...
export function TigrisEuphratesMini({ state, dispatch, onGameOver }: GameProps<TigrisEuphratesMiniState, { mode: "easy" }>): JSX.Element {
  const terminal = isTerminal(state);
  useEffect(() => { if (terminal) onGameOver(terminal.score); }, [terminal, onGameOver]);

  if (state.phase === "gameover" && state.result) return (
    <div className="bz-tem-wrap g-wrap">
      <h2>Game Over</h2>
      ...
      <div className="g-grid">
        {state.board.map((c, i) => (
          <div
            ...
            onClick={() => c === "" && dispatch({ type: "play", index: i } as TigrisEuphratesMiniAction)}
          >
            {c}
          </div>
        ))}
