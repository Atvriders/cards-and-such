...
export interface RunItThriceSettings { dummy: boolean; }
export interface RunItThriceState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type RunItThriceAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: RunItThriceSettings): RunItThriceState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: RunItThriceState, action: RunItThriceAction): RunItThriceState {
...
export function isTerminal(state: RunItThriceState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
