...
export function GothicChessGame({ state, dispatch, onGameOver }: GameProps<GothicChessState, GothicChessSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="gothch-wrap"><div className="gothch-done bounce-in"><h2>Done!</h2><p>Correct: {state.correctCount} / {state.questions.length}</p><p style={{ fontSize:"1.8rem",fontWeight:900,color:"#27ae60" }}>{state.score} pts</p></div></div>;
  const q = state.questions[state.currentIndex]!;
  const isResult = state.phase === "result";
  const urgent = state.timeLeft <= 5 && !state.submitted;
  return (
    <div className="gothch-wrap fade-in">
      <div className="gothch-header">
        ...
        <span className="gothch-score pulse">{state.score} pts</span>
      </div>
      ...
      <div className="gothch-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button data-testid={`hint-target-gothic-chess-answer-${i}`} key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as GothicChessAction)}><span className="gothch-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
