...
export interface KlondikeThreesNoRedealSettings {
  _dummy?: undefined;
}

export interface KlondikeThreesNoRedealState {
  piles: Pile[];
  score: number;
  movesMade: number;
  won: boolean;
  settings: KlondikeThreesNoRedealSettings;
}

export type KlondikeThreesNoRedealAction =
  | { type: "draw" }
...
export const k3nrRuleset: Ruleset = {
...
export function initialState(seed: number, settings: KlondikeThreesNoRedealSettings): KlondikeThreesNoRedealState {
...
export function reducer(state: KlondikeThreesNoRedealState, action: KlondikeThreesNoRedealAction): KlondikeThreesNoRedealState {
  ...
  switch (action.type) {
    ...
    case "move": {
      ...
      if (!canMove(state.piles, { fromPile, toPile, count }, k3nrRuleset)) return state;
      ...
      while (moved) {
        ...
        for (const sid of ["waste", ...TABLEAU_IDS]) {
          for (const fid of FOUNDATION_IDS) {
            ...
            if (canMove(piles, move, k3nrRuleset)) {
...
export function isTerminal(state: KlondikeThreesNoRedealState): { score: number } | null {
