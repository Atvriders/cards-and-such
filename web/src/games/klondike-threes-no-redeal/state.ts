...
export interface KlondikeThreesNoRedealSettings { dummy: boolean; }
...
function scoreHand(h: number[]): number {
  let s = 0; const ranks = h.map(cardRank); const counts: Record<number, number> = {};
  for (const r of ranks) counts[r] = (counts[r] ?? 0) + 1;
  for (const k in counts) { const v = counts[k]!; if (v >= 2) s += v * 5; }
  s += ranks.reduce((a, b) => a + b, 0);
  if (s === 0) s = 1; return s;
}

export function initialState(seed: number, _s: KlondikeThreesNoRedealSettings): KlondikeThreesNoRedealState {
