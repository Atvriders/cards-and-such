...
export function KlondikeThreesNoRedealGame({ state, dispatch, onGameOver }: GameProps<KlondikeThreesNoRedealState, KlondikeThreesNoRedealSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as KlondikeThreesNoRedealAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as KlondikeThreesNoRedealAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as KlondikeThreesNoRedealAction)}>Discard Hand</button>
      </div>
