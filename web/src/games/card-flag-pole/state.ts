...
export interface CardFlagPoleSettings { dummy: boolean; }
export interface CardFlagPoleState { rngSeed: number; deck: number[]; drawn: number[]; score: number; phase: "drawing" | "done"; }
export type CardFlagPoleAction = { type: "draw" } | { type: "reset" };
...
export function initialState(seed: number, _settings: CardFlagPoleSettings): CardFlagPoleState {
  const rng=mulberry32(seed); const deck=shuffle(rng);
  return { rngSeed: seed, deck, drawn: [], score: 0, phase: "drawing" };
}
export function reducer(state: CardFlagPoleState, action: CardFlagPoleAction): CardFlagPoleState {
...
export function isTerminal(state: CardFlagPoleState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
