...
export function FakeArtistQuiz({ state, dispatch, onGameOver }: GameProps<FakeArtistQuizState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      {state.phase === "answered" && (
        <button className="g-btn" onClick={() => dispatch({ type: "next" } as FakeArtistQuizAction)}>Next</button>
      )}
    </div>
  );
}
