...
export interface AbsSettings { dummy: boolean; }
...
export type AbsAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: AbsSettings): AbsState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: AbsState, action: AbsAction): AbsState {
...
export function isTerminal(state: AbsState): { score: number } | null {
