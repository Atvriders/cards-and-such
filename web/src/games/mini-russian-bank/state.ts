...
export const INITIAL_FLIP = 16;
export const MAX_CLICKS = 26;

export interface MiniRussianBankSettings { dummy: boolean; }

export interface MiniRussianBankState {
  rngSeed: number;
...
export type MiniRussianBankAction = { type: "remove"; index: number } | { type: "noop" };
...
export function initialState(seed: number, _settings: MiniRussianBankSettings): MiniRussianBankState {
...
export function reducer(state: MiniRussianBankState, action: MiniRussianBankAction): MiniRussianBankState {
...
export function isTerminal(state: MiniRussianBankState): { score: number } | null {
