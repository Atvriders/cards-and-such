...
export function MiniRussianBankGame({ state, dispatch, onGameOver }: GameProps<MiniRussianBankState, MiniRussianBankSettings>): JSX.Element {
    ...
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.layout.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "remove", index: i } as MiniRussianBankAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
