...
// Bejeweled Stars Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface BejeweledStarsMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface BejeweledStarsMiniState {
  rngSeed: number;
  grid: Grid;
...
export type BejeweledStarsMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: BejeweledStarsMiniSettings): BejeweledStarsMiniState {
...
export function reducer(state: BejeweledStarsMiniState, action: BejeweledStarsMiniAction): BejeweledStarsMiniState {
...
export function isTerminal(state: BejeweledStarsMiniState): { score: number } | null {
