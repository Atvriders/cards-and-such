...
export interface BananaSplitSettings { dummy: boolean; }
export interface BananaSplitState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type BananaSplitAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: BananaSplitSettings): BananaSplitState {
...
export function reducer(state: BananaSplitState, action: BananaSplitAction): BananaSplitState {
...
export function isTerminal(state: BananaSplitState): { score: number } | null {
