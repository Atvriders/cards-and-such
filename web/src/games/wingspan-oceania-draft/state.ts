...
export interface WingspanOceaniaDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WingspanOceaniaDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WingspanOceaniaDraftSettings): WingspanOceaniaDraftState {
...
export function score(s: WingspanOceaniaDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WingspanOceaniaDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
