...
export function WingspanOceaniaDraftGame({ state, dispatch, onGameOver }: GameProps<WingspanOceaniaDraftState, WingspanOceaniaDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Wingspan: Oceania</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as WingspanOceaniaDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as WingspanOceaniaDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
