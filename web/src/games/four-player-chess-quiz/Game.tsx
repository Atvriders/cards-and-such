    ...
    <div className="fpcq-wrap">
      ...
      <div className="fpcq-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", choice:i } as FourPlayerChessQuizAction)}><span className="fpcq-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
