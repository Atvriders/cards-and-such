...
export interface DiceApbaBasketballSettings { dummy: boolean; }

export interface DiceApbaBasketballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceApbaBasketballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceApbaBasketballSettings): DiceApbaBasketballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceApbaBasketballState, action: DiceApbaBasketballAction): DiceApbaBasketballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceApbaBasketballState> = {};
...
export function isTerminal(state: DiceApbaBasketballState): { score: number } | null {
