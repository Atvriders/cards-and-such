...
export interface DiceHighRollSettings { rounds: "10" | "20"; }
export interface DiceHighRollState {
...
export type DiceHighRollAction = { type: "bet"; amount: number } | { type: "next" };

export function initialState(seed: number, settings: DiceHighRollSettings): DiceHighRollState {
...
export function reducer(state: DiceHighRollState, action: DiceHighRollAction): DiceHighRollState {
  ...
  if (action.type === "bet") {
    ...
    const sum = d1 + d2 + d3;
    const win = sum >= 12; // high is >= 12
    const amount = Math.max(1, Math.min(action.amount, state.coins));
    const newCoins = state.coins + (win ? amount * 2 : -amount); // higher payout for harder win
...
export function isTerminal(state: DiceHighRollState): { score: number } | null {
