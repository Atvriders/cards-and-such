...
export const ASSET_COST = 50;
export const HIRE_COST = 80;

export interface AcquireHotelsSettings { dummy: boolean; }
export interface AcquireHotelsState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type AcquireHotelsAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: AcquireHotelsSettings): AcquireHotelsState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Tower hotel acquired","Beach resort merger","Boutique chain bought","Casino expansion","Skyline tower IPO","Marriott rumor"];

export function reducer(state: AcquireHotelsState, action: AcquireHotelsAction): AcquireHotelsState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Hotels (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (assets >= 1) {
        ...
        cash += market; assets -= 1; evt = "Traded 1 Hotels (+$" + market + ")"; delta = market;
      } else { evt = "No Hotels to trade"; }
    } else { return state; }
    const div = assets * 12;
    cash += div;
    const pay = workers * 18;
    cash += pay;
...
export function score(s: AcquireHotelsState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: AcquireHotelsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
