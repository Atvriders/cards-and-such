...
const ConcentrationCardsGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ConcentrationCardsGame as unknown as React.ComponentType<unknown> })));
  ...
  hint: (state: any) => {
      ...
      for (let i = 0; i < (state.values?.length ?? 0); i++) {
        if (!revealed[i] && !flipped.includes(i)) {
          return { selector: `[data-testid="hint-target-concentration-cards-card-${i}"]`, pulses: 3 };
        }
      }
      return null;
    }, component: ConcentrationCardsGame,
};
