...
export interface BaseballTapSettings { dummy: boolean; }
export interface Target {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface BaseballTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type BaseballTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: BaseballTapSettings): BaseballTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: BaseballTapState, action: BaseballTapAction): BaseballTapState {
...
export function isTerminal(state: BaseballTapState): { score: number } | null {
