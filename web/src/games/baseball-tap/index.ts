...
const BaseballTapGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BaseballTapGame as unknown as React.ComponentType<unknown> })));
...
export const baseballTapPlugin: GamePlugin<BaseballTapState, BaseballTapAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: BaseballTapState) => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: ".ftap-target", pulses: 3 };
  },
  component:BaseballTapGame,
};
