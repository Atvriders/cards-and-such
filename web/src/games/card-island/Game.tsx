...
import "./Game.css";
export function CardIslandGame({ state, dispatch, onGameOver }: GameProps<CardIslandState, CardIslandSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <div className="cm-help">Face cards are treasure</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardIslandAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
