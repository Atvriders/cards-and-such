...
export interface DiceStratHockeySettings { dummy: boolean; }

export interface DiceStratHockeyState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  myPoints: number;
...
export type DiceStratHockeyStateAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: DiceStratHockeySettings): DiceStratHockeyState {
  return { rngSeed: seed, round: 1, dice: null, myPoints: 0, oppPoints: 0, lastDelta: 0, phase: "rolling" };
}

export function reducer(state: DiceStratHockeyState, action: DiceStratHockeyStateAction): DiceStratHockeyState {
...
export function isTerminal(state: DiceStratHockeyState): { score: number } | null {
