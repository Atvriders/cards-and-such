...
export interface BrassLancashireSettings { dummy: boolean; }
export interface BrassLancashireState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type BrassLancashireAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: BrassLancashireSettings): BrassLancashireState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Cotton mill busy","Coal seam strike","Canal silted","Rail line opens","Demand spike","Bank panic","Era ends","Investor signs"];

export function reducer(state: BrassLancashireState, action: BrassLancashireAction): BrassLancashireState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Mill (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Engineer (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Sold 1 Mill (+$" + market + ")"; delta = market;
      } else { evt = "No Mill to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
    ...
    if (pay > 0) evt += " | Engineer earned +$" + pay;
...
export function score(s: BrassLancashireState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: BrassLancashireState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
