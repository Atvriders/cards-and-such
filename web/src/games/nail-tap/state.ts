...
export interface NailTapSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface NailTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type NailTapAction = { type: "tick" } | { type: "hit"; id: number };
export function initialState(seed: number, _settings: NailTapSettings): NailTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}
export function reducer(state: NailTapState, action: NailTapAction): NailTapState {
...
export function isTerminal(state: NailTapState): { score: number } | null {
