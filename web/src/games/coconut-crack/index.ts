...
const CoconutCrackGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CoconutCrackGame as unknown as React.ComponentType<unknown> })));
...
export const coconutCrackPlugin: GamePlugin<CoconutCrackState, CoconutCrackAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: CoconutCrackState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-coconut-crack-target"]', pulses: 3 };
  },
  component:CoconutCrackGame,
};
