...
export interface CoconutCrackSettings { dummy: boolean; }
export interface CoconutCrackTarget { id: number; lane: number; ticksLeft: number; }
export interface CoconutCrackState {
  rngSeed: number;
  targets: CoconutCrackTarget[];
  nextId: number;
...
export type CoconutCrackAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: CoconutCrackSettings): CoconutCrackState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: CoconutCrackState, action: CoconutCrackAction): CoconutCrackState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CoconutCrackTarget[] = [];
...
export function isTerminal(state: CoconutCrackState): { score: number } | null {
