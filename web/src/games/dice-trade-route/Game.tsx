...
export function DiceTradeRouteGame({ state, dispatch, onGameOver }: GameProps<DiceTradeRouteState, DiceTradeRouteSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "roll" && (
        <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as DiceTradeRouteAction)}>Roll</button>
      )}
      {state.phase === "result" && (
        <>
          <div className="dm-result">{state.lastWin ? `Hit! +${state.lastPts}` : "Miss — 0"}</div>
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceTradeRouteAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
