...
export interface BrassCanalsSettings { dummy: boolean; }
export interface BrassCanalsState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type BrassCanalsAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: BrassCanalsSettings): BrassCanalsState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Cotton mill expansion","Coal pit dug","Canal lock built","Rail link approved","Steam engine installed","Iron foundry boom"];

export function reducer(state: BrassCanalsState, action: BrassCanalsAction): BrassCanalsState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Mills (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (assets >= 1) {
        ...
        cash += market; assets -= 1; evt = "Traded 1 Mills (+$" + market + ")"; delta = market;
      } else { evt = "No Mills to trade"; }
    } else { return state; }
    const div = assets * 12;
    cash += div;
...
export function score(s: BrassCanalsState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: BrassCanalsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
