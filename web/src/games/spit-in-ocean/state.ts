...
export interface SpitInOceanSettings { rounds: "10"; }

export interface SpitInOceanState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type SpitInOceanAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SpitInOceanSettings): SpitInOceanState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: SpitInOceanState, action: SpitInOceanAction): SpitInOceanState {
...
export function isTerminal(state: SpitInOceanState): { score: number } | null {
