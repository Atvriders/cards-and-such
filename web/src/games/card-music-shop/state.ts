...
export interface CardMusicShopSettings { dummy: boolean; }
export interface CardMusicShopState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardMusicShopAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardMusicShopSettings): CardMusicShopState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardMusicShopState, action: CardMusicShopAction): CardMusicShopState {
...
export function isTerminal(state: CardMusicShopState): { score: number } | null {
