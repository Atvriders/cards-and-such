...
import "./Game.css";
export function CardMusicShopGame({ state, dispatch, onGameOver }: GameProps<CardMusicShopState, CardMusicShopSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "drawing" && (
        <button data-testid="hint-target-card-music-shop-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as CardMusicShopAction)}>Draw Card</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardMusicShopAction)}>{state.drawCount >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
