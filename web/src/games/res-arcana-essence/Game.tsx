...
import "./Game.css";
export function ResArcanaEssenceGame({ state, dispatch, onGameOver }: GameProps<ResArcanaEssenceState, ResArcanaEssenceSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as ResArcanaEssenceAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
