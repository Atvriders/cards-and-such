...
export interface CandleCapSettings { dummy: boolean; }
export interface CandleCapItem {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface CandleCapState {
  rngSeed: number;
  items: CandleCapItem[];
  nextId: number;
...
export type CandleCapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };
export function initialState(seed: number, _settings: CandleCapSettings): CandleCapState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: CandleCapState, action: CandleCapAction): CandleCapState {
...
export function isTerminal(state: CandleCapState): { score: number } | null {
