...
export interface TurboDealersChoiceSettings { rounds: "10"; }

export interface TurboDealersChoiceState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type TurboDealersChoiceAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TurboDealersChoiceSettings): TurboDealersChoiceState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: TurboDealersChoiceState, action: TurboDealersChoiceAction): TurboDealersChoiceState {
...
export function isTerminal(state: TurboDealersChoiceState): { score: number } | null {
