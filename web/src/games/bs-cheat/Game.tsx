...
export function BsCheatGame({ state, dispatch, onGameOver }: GameProps<BsCheatState, BsCheatSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="dm-wrap bsc2-shed"><div className="dm-done bounce-in"><h2>Done!</h2><div>W: {state.wins} L: {state.losses} T: {state.ties}</div><div className="dm-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="dm-wrap bsc2-shed fade-in">
