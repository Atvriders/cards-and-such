...
export interface BsCheatSettings { dummy: boolean; }
export interface BsCheatState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type BsCheatAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: BsCheatSettings): BsCheatState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: BsCheatState, action: BsCheatAction): BsCheatState {
...
export function isTerminal(state: BsCheatState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
