...
const hint = (state: CasState): HintTarget | null => {
  ...
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-badeucey-cas-primary"]', pulses: 3 };
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-badeucey-cas-secondary"]', pulses: 3 };
  return null;
};
