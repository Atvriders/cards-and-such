...
export interface BigHarpSettings { dummy: boolean; }

export interface BigHarpState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank).sort((a,b)=>a-b); let asc=1, best=1; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; if (a===b+1){asc++; if(asc>best)best=asc;} else if (a!==b) asc=1; } s += best*best*2; if (s===0) s=1; return s;
}
...
export function reducer(state: BigHarpState, action: BigHarpAction): BigHarpState {
...
export function isTerminal(state: BigHarpState): { score: number } | null {
