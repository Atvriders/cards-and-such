  ...
  return (
    <div className="difabadr-wrap">
      ...
      <div className="difabadr-actions">
        {state.phase === "rolling" && (
          <button className="difabadr-btn primary" onClick={() => dispatch({ type: "roll" } as DiceFantasyBasketballDraftAction)}>Roll</button>
        )}
        {state.phase === "rolled" && (
          <button className="difabadr-btn alt" onClick={() => dispatch({ type: "next" } as DiceFantasyBasketballDraftAction)}>Next</button>
        )}
      </div>
    </div>
  );
