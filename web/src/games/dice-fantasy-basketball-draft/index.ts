...
const DiceFantasyBasketballDraftGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceFantasyBasketballDraftGame as unknown as React.ComponentType<unknown> })));
...
export const diceFantasyBasketballDraftPlugin: GamePlugin<DiceFantasyBasketballDraftState, DiceFantasyBasketballDraftAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceFantasyBasketballDraftState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-fantasy-basketball-draft-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-fantasy-basketball-draft-next"]', pulses: 3 };
    return null;
  },
