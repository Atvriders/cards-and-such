...
export interface DiceFantasyBasketballDraftSettings { dummy: boolean; }

export interface DiceFantasyBasketballDraftState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceFantasyBasketballDraftAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceFantasyBasketballDraftSettings): DiceFantasyBasketballDraftState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceFantasyBasketballDraftState, action: DiceFantasyBasketballDraftAction): DiceFantasyBasketballDraftState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceFantasyBasketballDraftState> = {};
...
export function isTerminal(state: DiceFantasyBasketballDraftState): { score: number } | null {
