...
export interface GoldfishGrabSettings { dummy: boolean; }
export interface GoldfishGrabTarget { id: number; lane: number; ticksLeft: number; }
export interface GoldfishGrabState {
  rngSeed: number;
  targets: GoldfishGrabTarget[];
  nextId: number;
...
export type GoldfishGrabAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: GoldfishGrabSettings): GoldfishGrabState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: GoldfishGrabState, action: GoldfishGrabAction): GoldfishGrabState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: GoldfishGrabTarget[] = [];
...
export function isTerminal(state: GoldfishGrabState): { score: number } | null {
