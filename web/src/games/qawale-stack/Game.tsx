...
import "./Game.css";
export function QawaleStackGame({ state, dispatch, onGameOver }: GameProps<QawaleStackState, QawaleStackSettings>): JSX.Element {
    ...
    <div className="ab-wrap fade-in">
      ...
      <div className="ab-board" style={{ gridTemplateColumns: `repeat(${SIZE}, 1fr)` }}>
        {Array.from({ length: SIZE * SIZE }).map((_, i) => {
          ...
          return <button key={i} className={`ab-cell ${cls}`} disabled={v !== null} onClick={() => dispatch({ type:"place", idx:i } as QawaleStackAction)}>{v ?? ""}</button>;
        })}
      </div>
    </div>
  );
