    ...
    return (
      <div className={`${P}-wrap`}>
        <div className={`${P}-final`}>
          <h2 className={`${P}-final-title`}>Monikers Serious Quiz — Complete</h2>
    ...
    <div className={`${P}-wrap`}>
        ...
        {q.choices.map((c, i) => {
          ...
          return (
            <button
              ...
              onClick={() => dispatch({ type: "answer", choice: i, elapsedMs: Date.now() - start } as MonikersSeriousQuizAction)}
            >{c}</button>
          );
        })}
      </div>
      {state.phase === "feedback" && (
        <>
          ...
          <button className={`${P}-next`} type="button" onClick={() => dispatch({ type: "next" } as MonikersSeriousQuizAction)}>
