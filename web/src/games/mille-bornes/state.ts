...
export const SCORE_WIN = 9;
export const SCORE_TIE = 3;
export interface MilleBornesSettings { dummy: boolean; }
export interface MilleBornesState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type MilleBornesAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MilleBornesSettings): MilleBornesState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: MilleBornesState, action: MilleBornesAction): MilleBornesState {
...
export function isTerminal(state: MilleBornesState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
