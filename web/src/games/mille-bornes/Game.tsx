...
import "./Game.css";
export function MilleBornesGame({ state, dispatch, onGameOver }: GameProps<MilleBornesState, MilleBornesSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-mille-bornes-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as MilleBornesAction)}>Drive!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MilleBornesAction)}>Next</button>
      </>}
    </div>
  );
}
