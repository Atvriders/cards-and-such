...
export interface YoteWestSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface YoteWestState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type YoteWestAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: YoteWestSettings): YoteWestState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: YoteWestState, action: YoteWestAction): YoteWestState {
...
export function isTerminal(state: YoteWestState): { score: number } | null {
