...
import "./Game.css";
export function SchockenPubGame({ state, dispatch, onGameOver }: GameProps<SchockenPubState, SchockenPubSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-schocken-pub-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as SchockenPubAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-schocken-pub-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SchockenPubAction)}>Next</button>
        </>
      )}
    </div>
  );
