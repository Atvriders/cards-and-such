...
export interface SevenWondersArchitectsDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersArchitectsDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersArchitectsDraftSettings): SevenWondersArchitectsDraftState {
...
export function score(s: SevenWondersArchitectsDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersArchitectsDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
