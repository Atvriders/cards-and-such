...
export interface MarketBlufferSettings { dummy: boolean; }
export interface MarketBlufferState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type MarketBlufferAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: MarketBlufferSettings): MarketBlufferState {
...
export function reducer(state: MarketBlufferState, action: MarketBlufferAction): MarketBlufferState {
...
export function score(s: MarketBlufferState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: MarketBlufferState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
