...
export function AmongStarsRevivalGame({ state, dispatch, onGameOver }: GameProps<AmongStarsRevivalState, AmongStarsRevivalSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Among the Stars: Revival</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as AmongStarsRevivalAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as AmongStarsRevivalAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
