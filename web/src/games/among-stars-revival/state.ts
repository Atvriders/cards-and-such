...
export const SUIT_NAMES = ["Hangar","Lab","Reactor","Habitat"] as const;
...
export type AmongStarsRevivalState = DraftState;
export type AmongStarsRevivalAction = DraftAction;
...
export function initialState(seed: number, _settings: AmongStarsRevivalSettings): AmongStarsRevivalState {
...
export function score(state: AmongStarsRevivalState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: AmongStarsRevivalState): { score: number } | null {
