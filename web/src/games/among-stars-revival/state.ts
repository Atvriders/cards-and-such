...
export const SUIT_NAMES: readonly string[] = ["Habitat","Defense","Hangar","Trade"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface AmongStarsRevivalSettings { dummy: boolean; }
export interface AmongStarsRevivalState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type AmongStarsRevivalAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: AmongStarsRevivalSettings): AmongStarsRevivalState {
...
export function reducer(state: AmongStarsRevivalState, action: AmongStarsRevivalAction): AmongStarsRevivalState {
...
export function score(s: AmongStarsRevivalState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: AmongStarsRevivalState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
