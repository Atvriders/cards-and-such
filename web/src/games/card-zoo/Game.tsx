...
export function CardZooGame({ state, dispatch, onGameOver }: GameProps<CardZooState, CardZooSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="czo-wrap"><div className="czo-done"><h2>Done!</h2><div className="czo-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="czo-wrap">
      <div className="czo-info">Card {state.drawn.length} / {TOTAL_CARDS}</div>
      <div className="czo-score">{state.score} pts</div>
