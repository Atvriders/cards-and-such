...
export interface CardZooSettings { dummy: boolean; }
export interface CardZooState { rngSeed: number; deck: number[]; drawn: number[]; score: number; phase: "drawing" | "done"; }
export type CardZooAction = { type: "draw" } | { type: "reset" };
...
export function initialState(seed: number, _settings: CardZooSettings): CardZooState {
  const rng=mulberry32(seed); const deck=shuffle(rng);
  return { rngSeed: seed, deck, drawn: [], score: 0, phase: "drawing" };
}
export function reducer(state: CardZooState, action: CardZooAction): CardZooState {
...
export function isTerminal(state: CardZooState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
