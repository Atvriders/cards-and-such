...
const FrozenRiver = /* @__PURE__ */ lazy(() => import("./FrozenRiver.js").then((mod) => ({ default: mod.FrozenRiver as unknown as React.ComponentType<unknown> })));
...
Tips: scan each row for holes before stepping forward. Cracked tiles are worth crossing if they save several steps by avoiding a detour around a rock or hole cluster. Always have an escape route in mind before committing to a direction.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-frozen-river-action"]', pulses: 3 }; },
