...
const RummyTilesRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RummyTilesRGame as unknown as React.ComponentType<unknown> })));
...
export const rummyTilesRPlugin: GamePlugin<GState, GAction, typeof settings> = {
  ...
  hint: (state: GState): HintTarget | null => {
    ...
    if (state.phase === "play") return { selector: '[data-testid="hint-target-rummy-tiles-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-rummy-tiles-r-next"]', pulses: 3 };
    return null;
  },
  component: RummyTilesRGame,
};
