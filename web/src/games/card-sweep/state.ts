...
export interface CardSweepSettings { dummy: boolean; }
...
export type CardSweepAction = { type: "draw" } | { type: "next" };
...
export function cardWins(c: number): boolean { const s = Math.floor(c / 13); return s === 2 || s === 3; }
export function initialState(seed: number, _settings: CardSweepSettings): CardSweepState {
  return { rngSeed: seed, round: 1, card: null, score: 0, phase: "draw", lastWin: false, lastPts: 0 };
}
export function reducer(state: CardSweepState, action: CardSweepAction): CardSweepState {
...
export function isTerminal(state: CardSweepState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
