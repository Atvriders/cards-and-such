...
import "./Game.css";
export function CardSweepGame({ state, dispatch, onGameOver }: GameProps<CardSweepState, CardSweepSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "draw" && (
        <button data-testid="hint-target-card-sweep-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as CardSweepAction)}>Draw</button>
      )}
      {state.phase === "result" && (
        <>
          <div className="cm-result">{state.lastWin ? `Hit! +${state.lastPts}` : "Miss — 0"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardSweepAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
