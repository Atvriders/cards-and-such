...
export interface DiceNinepinBowlSettings { dummy: boolean; }

export interface DiceNinepinBowlState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceNinepinBowlAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceNinepinBowlSettings): DiceNinepinBowlState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceNinepinBowlState, action: DiceNinepinBowlAction): DiceNinepinBowlState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceNinepinBowlState> = {};
...
export function isTerminal(state: DiceNinepinBowlState): { score: number } | null {
