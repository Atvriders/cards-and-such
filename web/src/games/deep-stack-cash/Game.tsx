...
export function DeepStackCashGame({ state, dispatch, onGameOver }: GameProps<DeepStackCashState, DeepStackCashSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as DeepStackCashAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DeepStackCashAction)}>Next</button>
      </>}
    </div>
  );
}
