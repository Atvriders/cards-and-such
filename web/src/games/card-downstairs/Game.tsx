...
export function CardDownstairsGame({ state, dispatch, onGameOver }: GameProps<CardDownstairsState, CardDownstairsSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardDownstairsAction)}>Next</button>
        </>
      )}
    </div>
  );
