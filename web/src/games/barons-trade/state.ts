...
export interface BaronsTradeSettings { dummy: boolean; }
export interface BaronsTradeState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type BaronsTradeAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: BaronsTradeSettings): BaronsTradeState {
...
export function reducer(state: BaronsTradeState, action: BaronsTradeAction): BaronsTradeState {
...
export function score(s: BaronsTradeState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: BaronsTradeState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
