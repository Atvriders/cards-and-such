...
export interface ChicagoLowPokerSettings { dummy: boolean; }
export interface ChicagoLowPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ChicagoLowPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ChicagoLowPokerSettings): ChicagoLowPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ChicagoLowPokerState, action: ChicagoLowPokerAction): ChicagoLowPokerState {
...
export function isTerminal(state: ChicagoLowPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
