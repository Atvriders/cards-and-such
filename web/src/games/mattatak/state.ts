...
export interface MattatakSettings { dummy: boolean; }
export interface MattatakState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type MattatakAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MattatakSettings): MattatakState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: MattatakState, action: MattatakAction): MattatakState {
...
export function isTerminal(state: MattatakState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
