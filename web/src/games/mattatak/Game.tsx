...
import "./Game.css";
export function MattatakGame({ state, dispatch, onGameOver }: GameProps<MattatakState, MattatakSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-mattatak-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as MattatakAction)}>Mat!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MattatakAction)}>Next</button>
      </>}
    </div>
  );
}
