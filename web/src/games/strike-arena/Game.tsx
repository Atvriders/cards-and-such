...
import "./Game.css";
export function StrikeArenaGame({ state, dispatch, onGameOver }: GameProps<StrikeArenaState, StrikeArenaSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-strike-arena-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as StrikeArenaAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-strike-arena-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as StrikeArenaAction)}>Next</button>
        </>
      )}
    </div>
  );
