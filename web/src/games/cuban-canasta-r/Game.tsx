...
export function CubanCanastaRGame({ state, dispatch, onGameOver }: GameProps<CubanCanastaRState, CubanCanastaRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as CubanCanastaRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as CubanCanastaRAction)}>Next</button>
      </>}
    </div>
  );
}
