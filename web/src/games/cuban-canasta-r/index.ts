...
const CubanCanastaRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CubanCanastaRGame as unknown as React.ComponentType<unknown> })));
...
export const cubanCanastaRPlugin: GamePlugin<CubanCanastaRState, CubanCanastaRAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: CubanCanastaRState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "play") return { selector: '[data-testid="hint-target-cuban-canasta-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-cuban-canasta-r-next"]', pulses: 3 };
    return null;
  },
  component: CubanCanastaRGame,
};
