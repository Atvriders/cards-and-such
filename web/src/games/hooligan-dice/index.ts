...
const HooliganDiceGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.HooliganDiceGame as unknown as React.ComponentType<unknown> })));
...
export const hooliganDicePlugin: GamePlugin<HooliganDiceState, HooliganDiceAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: HooliganDiceState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "roll") return { selector: '[data-testid="hint-target-hooligan-dice-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-hooligan-dice-next"]', pulses: 3 };
    return null;
  },
  component:HooliganDiceGame,
};
