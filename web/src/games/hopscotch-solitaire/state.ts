...
export interface HopscotchSolitaireSettings { dummy: boolean; }

export interface HopscotchSolitaireState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type HopscotchSolitaireAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank).sort((a,b)=>a-b); let asc=1, best=1; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; if (a===b+1 || (b===13 && a===1)){asc++; if(asc>best)best=asc;} else if (a!==b) asc=1; } s += best*6; if (s===0) s=1; return s;
}
...
export function reducer(state: HopscotchSolitaireState, action: HopscotchSolitaireAction): HopscotchSolitaireState {
...
export function isTerminal(state: HopscotchSolitaireState): { score: number } | null {
