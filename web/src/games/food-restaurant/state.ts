...
export interface FoodRestaurantSettings { dummy: boolean; }
export interface FoodRestaurantState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type FoodRestaurantAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: FoodRestaurantSettings): FoodRestaurantState {
  return { rngSeed: seed, turn: 1, cash: 220, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Market shift","Demand spike","Supply glut","Trade pact","Toll levy","Tax break","Boom day","Slow day"];

export function reducer(state: FoodRestaurantState, action: FoodRestaurantAction): FoodRestaurantState {
  ...
  if (state.phase === "choosing") {
    ...
    const div = assets * Math.max(1, Math.floor(40 * 0.18));
    cash += div;
    const pay = workers * Math.max(1, Math.floor(60 * 0.18));
    cash += pay;
...
export function score(s: FoodRestaurantState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: FoodRestaurantState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
