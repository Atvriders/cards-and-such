...
export function FaceToFaceGame({ state, dispatch, onGameOver }: GameProps<FaceToFaceState, FaceToFaceSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="gfaceto-wrap"><div className="gfaceto-done bounce-in"><h2>Done!</h2><div>Correct: {state.correctCount} / {state.rounds.length}</div><div className="gfaceto-final">{state.score} pts</div></div></div>;
  }
  const r = state.rounds[state.currentIndex]!;
  return (
    <div className="gfaceto-wrap fade-in">
      <div className="gfaceto-info">Round {state.currentIndex + 1} / {state.rounds.length}</div>
      <div className="gfaceto-score pulse">{state.score} pts</div>
