// Auto-generated race-game state for ludo-mini-race.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const TRACK_LEN = 32;
...
export const SPECIALS: Record<number, Special> = {8: {type:"safe"},16: {type:"safe"},24: {type:"safe"}};

export type Side = "P" | "C";
export interface LudoMiniSettings { dummy: boolean }

export interface LudoMiniState {
  rngSeed: number;
...
export function initialState(seed: number, _s: LudoMiniSettings): LudoMiniState {
  return {
    rngSeed: seed,
...
function checkWin(s: LudoMiniState): LudoMiniState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: LudoMiniState): LudoMiniState {
  let s = state;
...
export function legalMovesForP(state: LudoMiniState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: LudoMiniState, action: LudoMiniAction): LudoMiniState {
...
export function isTerminal(state: LudoMiniState): { score: number } | null {
