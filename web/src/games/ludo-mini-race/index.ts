...
import type { LudoMiniState, LudoMiniAction, LudoMiniSettings } from "./state.js";
import { initialState, reducer, isTerminal } from "./state.js";
const LudoMiniGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.LudoMiniGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;

export const ludoMiniRacePlugin: GamePlugin<LudoMiniState, LudoMiniAction, typeof settings> = {
  id: "ludo-mini-race",
  title: "Ludo (Mini)",
  category: "board",
  ...
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as LudoMiniSettings),
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "done" || p === "gameover" || p === "ended" || p === "finished" || (s as any).gameOver || (s as any).won || (s as any).complete || (s as any).isComplete) return null; return { selector: ".ludomini-btn", pulses: 3 }; },
  component: LudoMiniGame,
};
