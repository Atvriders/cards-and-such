...
export const DECK: { name: string; value: number }[] = [
  { name: "Asteroid", value: 2 },
  { name: "Planet", value: 4 },
  { name: "Star", value: 5 },
  { name: "Nebula", value: 3 },
  { name: "Wormhole", value: 6 },
];

export interface OneDeckGalaxySettings { dummy: boolean; }
export interface OneDeckGalaxyState {
  rngSeed: number;
  round: number;
  hand: number[];
  lastPts: number;
...
export type OneDeckGalaxyAction = { type: "draw" } | { type: "next" };
export function initialState(seed: number, _s: OneDeckGalaxySettings): OneDeckGalaxyState {
