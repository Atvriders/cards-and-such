...
export interface HyperTurboTournamentSettings { dummy: boolean; }
export interface HyperTurboTournamentState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HyperTurboTournamentAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HyperTurboTournamentSettings): HyperTurboTournamentState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HyperTurboTournamentState, action: HyperTurboTournamentAction): HyperTurboTournamentState {
...
export function isTerminal(state: HyperTurboTournamentState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
