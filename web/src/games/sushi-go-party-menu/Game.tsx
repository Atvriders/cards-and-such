...
export function SushiGoPartyMenuGame({ state, dispatch, onGameOver }: GameProps<SushiGoPartyMenuState, SushiGoPartyMenuSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Sushi Go Party: Menu</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SushiGoPartyMenuAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SushiGoPartyMenuAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
