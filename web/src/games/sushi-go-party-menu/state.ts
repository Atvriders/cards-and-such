...
export const SUIT_NAMES = ["Maki","Nigiri","Tempura","Sashimi","Roll"] as const;
...
export type SushiGoPartyMenuState = DraftState;
export type SushiGoPartyMenuAction = DraftAction;
export interface SushiGoPartyMenuSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SushiGoPartyMenuSettings): SushiGoPartyMenuState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SushiGoPartyMenuState, action: SushiGoPartyMenuAction): SushiGoPartyMenuState {
  return draftReducer(state, action);
}

export function score(state: SushiGoPartyMenuState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SushiGoPartyMenuState): { score: number } | null {
