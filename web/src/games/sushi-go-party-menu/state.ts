...
export interface SushiGoPartyMenuState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SushiGoPartyMenuAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SushiGoPartyMenuSettings): SushiGoPartyMenuState {
...
export function score(s: SushiGoPartyMenuState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SushiGoPartyMenuState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
