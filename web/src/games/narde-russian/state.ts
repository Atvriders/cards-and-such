// Auto-generated backgammon-family state for narde-russian.
...
export interface NardeSettings { dummy: boolean }

export interface NardeState {
  rngSeed: number;
...
export function initialState(seed: number, _s: NardeSettings): NardeState {
...
export function legalMoves(state: NardeState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: NardeState, side: Side, from: number, pips: number): NardeState | null {
  const next: NardeState = {
    ...state,
...
function checkWin(s: NardeState): NardeState {
...
function cpuTurn(state: NardeState): NardeState {
  let s = state;
...
export function reducer(state: NardeState, action: NardeAction): NardeState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: NardeState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: NardeState): { score: number } | null {
