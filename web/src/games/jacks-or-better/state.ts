...
export interface JacksOrBetterSettings { dummy: boolean; }
export interface JacksOrBetterState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type JacksOrBetterAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: JacksOrBetterSettings): JacksOrBetterState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: JacksOrBetterState, action: JacksOrBetterAction): JacksOrBetterState {
...
export function isTerminal(state: JacksOrBetterState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
