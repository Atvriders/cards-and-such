...
export interface SevenWondersCitiesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersCitiesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersCitiesSettings): SevenWondersCitiesState {
...
export function score(s: SevenWondersCitiesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersCitiesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
