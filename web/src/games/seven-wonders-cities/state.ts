...
export const SUIT_NAMES = ["Diplomat","Treason","Black Market","Treasury","Spy"] as const;
...
export type SevenWondersCitiesState = DraftState;
export type SevenWondersCitiesAction = DraftAction;
export interface SevenWondersCitiesSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SevenWondersCitiesSettings): SevenWondersCitiesState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SevenWondersCitiesState, action: SevenWondersCitiesAction): SevenWondersCitiesState {
  return draftReducer(state, action);
}

export function score(state: SevenWondersCitiesState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersCitiesState): { score: number } | null {
