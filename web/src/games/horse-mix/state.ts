...
export interface HorseMixSettings { dummy: boolean; }
export interface HorseMixState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HorseMixAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HorseMixSettings): HorseMixState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HorseMixState, action: HorseMixAction): HorseMixState {
...
export function isTerminal(state: HorseMixState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
