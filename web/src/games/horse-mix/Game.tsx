...
export function HorseMixGame({ state, dispatch, onGameOver }: GameProps<HorseMixState, HorseMixSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as HorseMixAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as HorseMixAction)}>Next</button>
      </>}
    </div>
  );
}
