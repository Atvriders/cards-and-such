...
export interface OrchardSoloSettings { rounds: "10"; }

export interface OrchardSoloState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type OrchardSoloAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: OrchardSoloSettings): OrchardSoloState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: OrchardSoloState, action: OrchardSoloAction): OrchardSoloState {
...
export function isTerminal(state: OrchardSoloState): { score: number } | null {
