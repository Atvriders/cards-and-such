...
export interface CrackerCrunchSettings { dummy: boolean; }
...
export interface CrackerCrunchState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type CrackerCrunchAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CrackerCrunchSettings): CrackerCrunchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CrackerCrunchState, action: CrackerCrunchAction): CrackerCrunchState {
...
export function isTerminal(state: CrackerCrunchState): { score: number } | null {
