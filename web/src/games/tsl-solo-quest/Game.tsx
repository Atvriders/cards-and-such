...
export function TslSoloQuestGame({ state, dispatch, onGameOver }: GameProps<TslSoloQuestState, TslSoloQuestSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="tsl-wrap"><div className="tsl-done"><h2>Saga End</h2><p style={{ fontSize:"1.6rem", fontWeight:900, color:"#8e44ad" }}>{state.score} pts</p></div></div>;
  ...
  return (
    <div className="tsl-wrap">
      ...
      <div className="tsl-choices">
        {p.choices.map((c, i) => (
          <button key={i} className={`tsl-choice${state.selected === i ? " selected" : ""}`} disabled={isResult} onClick={() => dispatch({ type:"choose", choice:i } as TslSoloQuestAction)}>
