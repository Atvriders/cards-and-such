...
export function FairyTaleDeluxeGame({ state, dispatch, onGameOver }: GameProps<FairyTaleDeluxeState, FairyTaleDeluxeSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Fairy Tale: Deluxe</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as FairyTaleDeluxeAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as FairyTaleDeluxeAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
