...
export const SUIT_NAMES: readonly string[] = ["Princess","Witch","Knight","Dragon"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface FairyTaleDeluxeSettings { dummy: boolean; }
export interface FairyTaleDeluxeState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type FairyTaleDeluxeAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: FairyTaleDeluxeSettings): FairyTaleDeluxeState {
...
export function reducer(state: FairyTaleDeluxeState, action: FairyTaleDeluxeAction): FairyTaleDeluxeState {
...
export function score(s: FairyTaleDeluxeState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: FairyTaleDeluxeState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
