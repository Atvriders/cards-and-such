...
export const SUIT_NAMES = ["Hero","Beast","Wizard","Royalty","Quest"] as const;
...
export type FairyTaleDeluxeState = DraftState;
export type FairyTaleDeluxeAction = DraftAction;
...
export function initialState(seed: number, _settings: FairyTaleDeluxeSettings): FairyTaleDeluxeState {
...
export function score(state: FairyTaleDeluxeState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: FairyTaleDeluxeState): { score: number } | null {
