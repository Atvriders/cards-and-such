...
export interface PitShedSettings { dummy: boolean; }
export interface PitShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type PitShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: PitShedSettings): PitShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: PitShedState, action: PitShedAction): PitShedState {
...
export function isTerminal(state: PitShedState): { score: number } | null {
