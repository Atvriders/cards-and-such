...
export function PitShedGame({ state, dispatch, onGameOver }: GameProps<PitShedState, PitShedSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap pit-shed-css">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-pit-shed-play" className="dm-btn" onClick={() => dispatch({ type: "play" } as PitShedAction)}>Play Round</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button data-testid="hint-target-pit-shed-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PitShedAction)}>Next</button>
      </>}
    </div>
  );
}
