import { lazy } from "react";
import type * as React from "react";
import type { GamePlugin, HintTarget, SettingsOf } from "../../platform/game-plugin/types.js";
import type { LingoDeductionState, LingoDeductionAction, LingoDeductionSettings } from "./state.js";
import { LingoDeduction_CFG, initialState, reducer, isTerminal } from "./state.js";
import { deductionHintSelector } from "../_shared/deduction-engine.js";
const LingoDeductionGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.LingoDeductionGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;

export const lingoDeductionPlugin: GamePlugin<LingoDeductionState, LingoDeductionAction, typeof settings> = {
  id: "lingo-deduction",
  title: "Lingo",
  category: "board",
  players: { min: 1, max: 1, multiplayer: false },
  description: "Five-letter word deduction.",
  howToPlay: "Lingo adapted as a logic-deduction puzzle: cycle each slot to set a guess, submit, and read the feedback (filled circles = exact, hollow = correct symbol but wrong slot). Crack the code in the allotted guesses to score.",
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as LingoDeductionSettings),
  reducer,
  isTerminal,
  hint: (state: LingoDeductionState): HintTarget | null => {
    const sel = deductionHintSelector(state, LingoDeduction_CFG);
    return sel ? { selector: sel, pulses: 3 } : null;
  },
  component: LingoDeductionGame,
};

export default lingoDeductionPlugin;
