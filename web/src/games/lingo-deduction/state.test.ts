...
import { initialState, reducer, isTerminal, LingoDeduction_CFG } from "./state.js";
...
describe("lingo-deduction", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(LingoDeduction_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < LingoDeduction_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < LingoDeduction_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
