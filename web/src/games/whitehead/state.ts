...
export interface WhiteheadSettings {
  _dummy?: undefined;
}

export interface WhiteheadState {
  piles: Pile[];
  score: number;
  movesMade: number;
  won: boolean;
...
/**
 * Whitehead tableau building: same color (not alternating), descending.
 * Groups moved must be same-suit consecutive sequence.
 */
export const whiteheadRuleset: Ruleset = {
  ...
  canPickUp: (pile, count) => {
    ...
    // In Whitehead all cards are face-up; groups must be same-suit consecutive
...
export function initialState(seed: number, settings: WhiteheadSettings): WhiteheadState {
  ...
  // Classic Klondike triangle deal, but ALL cards are face-up
  for (let i = 0; i < 7; i++) {
    const count = i + 1;
  ...
  // Stock: remaining 24 cards, single deal (no redeal)
...
export function reducer(state: WhiteheadState, action: WhiteheadAction): WhiteheadState {
  ...
  switch (action.type) {
    case "draw": {
      ...
      // Draw top card of stock to waste
    ...
    case "move": {
      ...
      if (!canMove(state.piles, move, whiteheadRuleset)) return state;
    ...
    case "auto-move-to-foundation": {
      ...
      while (moved) {
        moved = false;
        const sourceIds = [...TABLEAU_IDS, "waste"];
        for (const sourceId of sourceIds) {
          ...
          for (const foundId of FOUNDATION_IDS) {
            ...
            if (canMove(piles, move, whiteheadRuleset)) {
...
export function isTerminal(state: WhiteheadState): { score: number } | null {
