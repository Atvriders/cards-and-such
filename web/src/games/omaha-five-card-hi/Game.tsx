...
export function OmahaFiveCardHiGame({ state, dispatch, onGameOver }: GameProps<OmahaFiveCardHiState, OmahaFiveCardHiSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap thmOma5Hi">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as OmahaFiveCardHiAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as OmahaFiveCardHiAction)}>Next</button>
      </>}
    </div>
  );
}
