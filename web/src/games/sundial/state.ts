...
export interface SundialSettings { dummy: boolean; }
...
function scoreHand(h: number[]): number {
  let s = 0; const suits = new Set(h.map(cardSuit)); s += (4-suits.size)*4; const ranks=new Set(h.map(cardRank)); s += (5-ranks.size)*5; if (s===0) s=1; return s;
}
...
export function reducer(state: SundialState, action: SundialAction): SundialState {
...
export function isTerminal(state: SundialState): { score: number } | null {
