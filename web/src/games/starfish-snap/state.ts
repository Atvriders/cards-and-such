...
export interface StarfishSnapSettings { dummy: boolean; }
export interface Critter { id: number; lane: number; ticksLeft: number; }
export interface StarfishSnapState { rngSeed: number; critters: Critter[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type StarfishSnapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: StarfishSnapSettings): StarfishSnapState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: StarfishSnapState, action: StarfishSnapAction): StarfishSnapState {
...
export function isTerminal(s: StarfishSnapState): { score: number } | null { return s.phase==="done" ? { score: s.score } : null; }
