...
import { initialState, reducer, isTerminal, CoupBluff_CFG } from "./state.js";
...
describe("coup-bluff", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(CoupBluff_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < CoupBluff_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < CoupBluff_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
