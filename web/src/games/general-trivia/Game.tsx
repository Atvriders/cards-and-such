...
import type { TriviaState, TriviaAction, TriviaSettings } from "./state.js";
import { isTerminal } from "./state.js";
import "./Game.css";

const LABELS = ["A", "B", "C", "D"];

export function GeneralTrivia({ state, dispatch, onGameOver }: GameProps<TriviaState, TriviaSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => {
      dispatch({ type: "tick" } as TriviaAction);
    }, 1000);
    return () => {
      if (tickRef.current) clearInterval(tickRef.current);
    };
  ...
  return (
    <div className="trivia-wrap">
      <div className="trivia-header">
        ...
        <span className="trivia-category-badge">{q.category}</span>
        ...
        {q.choices.map((choice, i) => {
          ...
          return (
            <button
              ...
              className={cls}
              disabled={isResult}
              onClick={() => dispatch({ type: "select", choice: i } as TriviaAction)}
            >
      ...
      <div className="trivia-actions">
        {!isResult && (
          <button
            className="trivia-btn submit"
            disabled={state.selected === null}
            onClick={() => dispatch({ type: "submit" } as TriviaAction)}
          >
            Submit
          </button>
        )}
        {isResult && (
          <button
            className="trivia-btn next"
            onClick={() => dispatch({ type: "next" } as TriviaAction)}
          >
