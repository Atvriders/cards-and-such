...
const defaultSettings = { questions: "10" as const, category: "mixed" as const };

describe("GeneralTrivia initialState", () => {
  it("creates correct number of questions", () => {
    const s = initialState(1, { questions: "10", category: "mixed" });
    expect(s.questions.length).toBe(10);
  });

  it("filters by category when not mixed", () => {
    const s = initialState(1, { questions: "50", category: "history" });
    expect(s.questions.every(q => q.category === "history")).toBe(true);
  });

  it("starts at index 0 with full timer", () => {
...
describe("GeneralTrivia reducer - select and submit", () => {
  ...
  it("cannot select after submit", () => {
    const s = initialState(1, defaultSettings);
    const s2 = reducer(s, { type: "select", choice: 0 });
    const s3 = reducer(s2, { type: "submit" });
    const s4 = reducer(s3, { type: "select", choice: 1 });
    expect(s4.selected).toBe(0);
  });
});

describe("GeneralTrivia reducer - tick and timer", () => {
  ...
  it("tick does nothing when already submitted", () => {
    const s = initialState(1, defaultSettings);
    const s2 = reducer(s, { type: "select", choice: 0 });
    const s3 = reducer(s2, { type: "submit" });
    const s4 = reducer(s3, { type: "tick" });
    expect(s4.timeLeft).toBe(s3.timeLeft);
  });
});

describe("GeneralTrivia reducer - next", () => {
  ...
  it("next on last question sets phase to done", () => {
    let s = initialState(1, { questions: "10", category: "mixed" });
    for (let i = 0; i < 10; i++) {
...
describe("GeneralTrivia isTerminal", () => {
