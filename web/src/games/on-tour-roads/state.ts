...
export interface OnTourRoadsSettings { dummy: boolean; }
export interface OnTourRoadsState {
  rngSeed: number;
...
export type OnTourRoadsAction =
  | { type: "roll" }
  | { type: "place"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: OnTourRoadsSettings): OnTourRoadsState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: OnTourRoadsState, action: OnTourRoadsAction): OnTourRoadsState {
...
export function isTerminal(state: OnTourRoadsState): { score: number } | null {
