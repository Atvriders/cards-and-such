...
export interface ThreeThirteenRSettings { dummy: boolean; }
export interface ThreeThirteenRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type ThreeThirteenRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: ThreeThirteenRSettings): ThreeThirteenRState {
  const rng = mulberry32(seed); const hand = deal(rng, 7);
  const next = Math.floor(rng() * 2 ** 31);
  return { rngSeed: next, round: 1, hand, phase: "play", melds: [], score: 0, pts: 0, deadwood: 0, result: "" };
}
export function reducer(state: ThreeThirteenRState, action: ThreeThirteenRAction): ThreeThirteenRState {
