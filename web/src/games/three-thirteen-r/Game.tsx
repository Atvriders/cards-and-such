...
export function ThreeThirteenRGame({ state, dispatch, onGameOver }: GameProps<ThreeThirteenRState, ThreeThirteenRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as ThreeThirteenRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ThreeThirteenRAction)}>Next</button>
      </>}
    </div>
  );
}
