...
import "./Game.css";
export function HeartsLetterDiceGame({ state, dispatch, onGameOver }: GameProps<HeartsLetterDiceState, HeartsLetterDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-hearts-letter-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as HeartsLetterDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-hearts-letter-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as HeartsLetterDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
