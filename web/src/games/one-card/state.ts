...
export interface OneCardSettings { dummy: boolean; }
export interface OneCardState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type OneCardAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: OneCardSettings): OneCardState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: OneCardState, action: OneCardAction): OneCardState {
...
export function isTerminal(state: OneCardState): { score: number } | null {
