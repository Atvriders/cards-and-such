...
export interface CoinDribbleSettings { dummy: boolean; }
export interface CoinDribbleState {
  rngSeed: number;
  turn: number;
  score: number;
  lastPts: number;
...
export type CoinDribbleAction = { type: "throw" } | { type: "next" };

export function initialState(seed: number, _s: CoinDribbleSettings): CoinDribbleState {
  return { rngSeed: seed, turn: 1, score: 0, lastPts: 0, lastRoll: 0, phase: "ready" };
}

export function reducer(state: CoinDribbleState, action: CoinDribbleAction): CoinDribbleState {
...
export function isTerminal(s: CoinDribbleState): { score: number } | null { return s.phase === "done" ? { score: s.score } : null; }
