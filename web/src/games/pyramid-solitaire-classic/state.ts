...
export interface PyramidSolitaireClassicSettings { dummy: boolean; }

export interface PyramidSolitaireClassicState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type PyramidSolitaireClassicAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (let i = 0; i < h.length; i++) for (let j = i+1; j < h.length; j++) if (cardRank(h[i]!) + cardRank(h[j]!) === 10) s += 6;
  s += h.length;
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: PyramidSolitaireClassicState, action: PyramidSolitaireClassicAction): PyramidSolitaireClassicState {
...
export function isTerminal(state: PyramidSolitaireClassicState): { score: number } | null {
