...
const PyramidSolitaireClassicGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PyramidSolitaireClassicGame as unknown as React.ComponentType<unknown> })));
...
export const pyramidSolitaireClassicPlugin: GamePlugin<PyramidSolitaireClassicState, PyramidSolitaireClassicAction, typeof settings> = {
  ...
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as unknown as PyramidSolitaireClassicSettings),
  hint: (state: PyramidSolitaireClassicState): HintTarget | null => {
    ...
    if (king) return { selector: `[data-testid="hint-target-pyramid-solitaire-classic-pyramid-${king.row}-${king.col}"]`, pulses: 3 };
    for (const a of avail) {
      for (const b of avail) {
        if (a === b) continue;
        if (a.rank + b.rank === 13) {
          return { selector: `[data-testid="hint-target-pyramid-solitaire-classic-pyramid-${a.row}-${a.col}"]`, pulses: 3 };
        }
      }
    }
    ...
    if (wasteTop) {
      ...
      if (m) return { selector: `[data-testid="hint-target-pyramid-solitaire-classic-pyramid-${m.row}-${m.col}"]`, pulses: 3 };
    }
    if (state.stock.length > 0) return { selector: '[data-testid="hint-target-pyramid-solitaire-classic-draw"]', pulses: 3 };
    if (state.redealsRemaining > 0) return { selector: '[data-testid="hint-target-pyramid-solitaire-classic-redeal"]', pulses: 3 };
    return null;
  },
  reducer,
  isTerminal,
