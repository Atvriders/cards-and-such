...
export function PyramidSolitaireClassicGame({ state, dispatch, onGameOver }: GameProps<PyramidSolitaireClassicState, PyramidSolitaireClassicSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as PyramidSolitaireClassicAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as PyramidSolitaireClassicAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as PyramidSolitaireClassicAction)}>Discard Hand</button>
      </div>
