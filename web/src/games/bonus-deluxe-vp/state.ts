...
export interface BonusDeluxeVpSettings { rounds: "10"; }

export interface BonusDeluxeVpState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type BonusDeluxeVpAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BonusDeluxeVpSettings): BonusDeluxeVpState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: BonusDeluxeVpState, action: BonusDeluxeVpAction): BonusDeluxeVpState {
...
export function isTerminal(state: BonusDeluxeVpState): { score: number } | null {
