...
export interface RunItTwiceSettings { dummy: boolean; }
export interface RunItTwiceState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type RunItTwiceAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: RunItTwiceSettings): RunItTwiceState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: RunItTwiceState, action: RunItTwiceAction): RunItTwiceState {
...
export function isTerminal(state: RunItTwiceState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
