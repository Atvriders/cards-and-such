...
import "./Game.css";
export function RunItTwiceGame({ state, dispatch, onGameOver }: GameProps<RunItTwiceState, RunItTwiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-run-it-twice-deal" onClick={() => dispatch({ type: "deal" } as RunItTwiceAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-run-it-twice-next" onClick={() => dispatch({ type: "next" } as RunItTwiceAction)}>Next</button>
      </>}
    </div>
  );
}
