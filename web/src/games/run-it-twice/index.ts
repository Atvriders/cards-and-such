...
const RunItTwiceGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RunItTwiceGame as unknown as React.ComponentType<unknown> })));
...
export const runItTwicePlugin: GamePlugin<RunItTwiceState, RunItTwiceAction, typeof settings> = {
  ...
  reducer, isTerminal,   hint: (state: RunItTwiceState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-run-it-twice-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-run-it-twice-next"]', pulses: 3 };
    return null;
  },
  component:RunItTwiceGame,
};
