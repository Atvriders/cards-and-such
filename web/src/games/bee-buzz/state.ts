...
export interface BeeBuzzSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface BeeBuzzState { rngSeed: number; targets: Target[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type BeeBuzzAction = { type:"tick" } | { type:"pop"; id: number };
export function initialState(seed:number, _s:BeeBuzzSettings):BeeBuzzState {
  return { rngSeed:seed, targets:[], nextId:1, ticksRemaining:TIMER_TICKS, score:0, popped:0, missed:0, phase:"playing" };
}
export function reducer(state:BeeBuzzState, action:BeeBuzzAction):BeeBuzzState {
...
export function isTerminal(state:BeeBuzzState):{score:number}|null { return state.phase==="done"?{score:state.score}:null; }
