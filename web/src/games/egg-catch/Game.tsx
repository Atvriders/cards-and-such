...
export function EggCatchGame({ state, dispatch, onGameOver }: GameProps<EggCatchState, EggCatchSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as EggCatchAction), 750);
    ...
    <div className="fc-wrap">
      ...
      <div className="fc-board">
        {state.targets.map(p => {
          ...
          return (
            <button key={p.id}
              className="fc-target" data-testid="hint-target-egg-catch-target"
              style={{ left: `${x}%`, top: `${y}%`, transform: "translate(-50%,-50%)", background:"transparent", border:"none" }}
              onClick={() => dispatch({ type:"pop", id:p.id } as EggCatchAction)}
