...
// Egg Catch: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface EggCatchSettings { dummy: boolean; }

export interface EggCatchTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}
...
export type EggCatchAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: EggCatchSettings): EggCatchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: EggCatchState, action: EggCatchAction): EggCatchState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: EggCatchTarget[] = [];
...
export function isTerminal(state: EggCatchState): { score: number } | null {
