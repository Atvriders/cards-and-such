...
export interface MeteorBashSettings { dummy: boolean; }
export interface MeteorBashTarget { id: number; lane: number; ticksLeft: number; }
export interface MeteorBashState {
  rngSeed: number;
  targets: MeteorBashTarget[];
  nextId: number;
...
export type MeteorBashAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: MeteorBashSettings): MeteorBashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: MeteorBashState, action: MeteorBashAction): MeteorBashState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: MeteorBashTarget[] = [];
...
export function isTerminal(state: MeteorBashState): { score: number } | null {
