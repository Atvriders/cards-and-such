...
import "./Game.css";
export function CardCastleBuildGame({ state, dispatch, onGameOver }: GameProps<CardCastleBuildState, CardCastleBuildSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "dealing" && (
        <button data-testid="hint-target-card-castle-build-primary" className="cm-btn" onClick={() => dispatch({ type:"deal" } as CardCastleBuildAction)}>Deal 5</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">Sum: {state.sum} — {state.lastPts > 0 ? `+${state.lastPts}` : "no points"}</div>
          <div className="cm-help">Aces & face cards are stones</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardCastleBuildAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
