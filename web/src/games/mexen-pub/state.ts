...
export const PAYOUTS: number[] = [80,30,5];
export const CHOICES: string[] = ["Mexen (2-1) (+80)","Pair (+30)","Common (+5)"];
export interface MexenPubSettings { dummy: boolean; }
export interface MexenPubState {
  rngSeed: number;
  round: number;
...
export type MexenPubAction =
  | { type: "predict"; choice: number }
  | { type: "next" };
export function initialState(seed: number, _settings: MexenPubSettings): MexenPubState {
  return { rngSeed: seed, round: 1, prediction: null, dice: null, resultIdx: null, score: 0, phase: "predict" };
}
export function reducer(state: MexenPubState, action: MexenPubAction): MexenPubState {
  ...
  if (action.type === "predict") {
    ...
    const resultIdx = (()=>{ if ((a===2&&b===1)||(a===1&&b===2)) return 0; if (a===b) return 1; return 2; })();
...
export function isTerminal(state: MexenPubState): { score: number } | null {
