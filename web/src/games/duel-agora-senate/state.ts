...
export const SUIT_NAMES = ["Senator","Vote","Decree","Censor"] as const;
...
export type DuelAgoraSenateState = DraftState;
export type DuelAgoraSenateAction = DraftAction;
export interface DuelAgoraSenateSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: DuelAgoraSenateSettings): DuelAgoraSenateState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: DuelAgoraSenateState, action: DuelAgoraSenateAction): DuelAgoraSenateState {
  return draftReducer(state, action);
}

export function score(state: DuelAgoraSenateState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: DuelAgoraSenateState): { score: number } | null {
