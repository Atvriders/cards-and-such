...
export interface DuelAgoraSenateState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type DuelAgoraSenateAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: DuelAgoraSenateSettings): DuelAgoraSenateState {
...
export function score(s: DuelAgoraSenateState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: DuelAgoraSenateState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
