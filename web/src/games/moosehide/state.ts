...
export interface MoosehideSettings { dummy: boolean; }

export interface MoosehideState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const colors=h.map(c=>cardSuit(c)%2); const reds = colors.filter(c=>c===0).length; const blacks = colors.length-reds; s += Math.min(reds, blacks)*6; const f = h.filter(c=>cardRank(c)>=11).length; s += f*3; if (s===0) s=1; return s;
}
...
export function reducer(state: MoosehideState, action: MoosehideAction): MoosehideState {
...
export function isTerminal(state: MoosehideState): { score: number } | null {
