...
const SevenCardCribbageGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SevenCardCribbageGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: SevenCardCribbageState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-seven-card-cribbage-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-seven-card-cribbage-secondary"]', pulses: 3 };
  return null;
};
...
export const sevenCardCribbagePlugin: GamePlugin<SevenCardCribbageState, SevenCardCribbageAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: SevenCardCribbageGame,
};
