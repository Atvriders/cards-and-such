...
export const SCORE_WIN = 8;
export const SCORE_TIE = 3;
export interface SevenCardCribbageSettings { dummy: boolean; }
export interface SevenCardCribbageState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type SevenCardCribbageAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SevenCardCribbageSettings): SevenCardCribbageState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: SevenCardCribbageState, action: SevenCardCribbageAction): SevenCardCribbageState {
...
export function isTerminal(state: SevenCardCribbageState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
