...
export function SevenCardCribbageGame({ state, dispatch, onGameOver }: GameProps<SevenCardCribbageState, SevenCardCribbageSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as SevenCardCribbageAction)}>Cut!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SevenCardCribbageAction)}>Next</button>
      </>}
    </div>
  );
}
