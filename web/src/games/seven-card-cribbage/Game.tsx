...
import "./Game.css";
export function SevenCardCribbageGame({ state, dispatch, onGameOver }: GameProps<SevenCardCribbageState, SevenCardCribbageSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-seven-card-cribbage-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as SevenCardCribbageAction)}>Cut!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-seven-card-cribbage-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SevenCardCribbageAction)}>Next</button>
      </>}
    </div>
  );
}
