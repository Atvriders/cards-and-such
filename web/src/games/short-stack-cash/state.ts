...
export interface ShortStackCashSettings { dummy: boolean; }
export interface ShortStackCashState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ShortStackCashAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ShortStackCashSettings): ShortStackCashState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ShortStackCashState, action: ShortStackCashAction): ShortStackCashState {
...
export function isTerminal(state: ShortStackCashState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
