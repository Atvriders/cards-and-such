...
// Lumines Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface LuminesMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface LuminesMiniState {
  rngSeed: number;
  grid: Grid;
...
export function initialState(seed: number, _settings: LuminesMiniSettings): LuminesMiniState {
...
export function reducer(state: LuminesMiniState, action: LuminesMiniAction): LuminesMiniState {
...
export function isTerminal(state: LuminesMiniState): { score: number } | null {
