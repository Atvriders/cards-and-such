...
export interface SvitenSpecialSettings { dummy: boolean; }
export interface SvitenSpecialState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SvitenSpecialAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SvitenSpecialSettings): SvitenSpecialState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SvitenSpecialState, action: SvitenSpecialAction): SvitenSpecialState {
...
export function isTerminal(state: SvitenSpecialState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
