...
const SpaceColony = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SpaceColony as unknown as React.ComponentType<unknown> })));
...
Strategy: Keep all three bars above 25 at all times. Never let two resources drop below 30 simultaneously. Prioritize whichever is lowest. If all three are healthy, top up the one with the lowest reserve headroom. A colony of 10 alive at day 30 with strong resources earns a perfect score. Good luck, commander!`,
  ...
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "done" || p === "gameover" || p === "ended" || p === "finished" || (s as any).gameOver || (s as any).won || (s as any).complete || (s as any).isComplete) return null; return { selector: ".colony-action-btn", pulses: 3 }; },
