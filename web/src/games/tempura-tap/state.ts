...
export interface TempuraTapSettings { dummy: boolean; }
...
export interface TempuraTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type TempuraTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: TempuraTapSettings): TempuraTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: TempuraTapState, action: TempuraTapAction): TempuraTapState {
...
export function isTerminal(state: TempuraTapState): { score: number } | null {
