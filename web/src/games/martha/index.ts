...
Tip: Same-suit building is far more restrictive than alternating colors — sequences of the same suit are rare. Prioritize uncovering face-down cards to create mobility, and use empty columns carefully since only Kings may fill them.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-martha-action"]', pulses: 3 }; },
  component: Martha,
};
