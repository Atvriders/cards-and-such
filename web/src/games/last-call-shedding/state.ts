...
export interface LastCallSheddingSettings { dummy: boolean; }
export interface LastCallSheddingState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type LastCallSheddingAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: LastCallSheddingSettings): LastCallSheddingState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: LastCallSheddingState, action: LastCallSheddingAction): LastCallSheddingState {
...
export function isTerminal(state: LastCallSheddingState): { score: number } | null {
