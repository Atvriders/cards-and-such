...
export const DECK: { name: string; value: number }[] = [
  ...
  { name: "Hero", value: 4 },
  { name: "Champion", value: 6 },
  { name: "Relic", value: 3 },
  { name: "Spell", value: 5 },
];

export interface SummonerWarsGridSettings { dummy: boolean; }
export interface SummonerWarsGridState {
  rngSeed: number;
  round: number;
  hand: number[];
  lastPts: number;
...
export type SummonerWarsGridAction = { type: "draw" } | { type: "next" };
export function initialState(seed: number, _s: SummonerWarsGridSettings): SummonerWarsGridState {
