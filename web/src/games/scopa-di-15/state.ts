...
export interface ScopaDi15Settings { dummy: boolean; }
export interface ScopaDi15State { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type ScopaDi15Action = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ScopaDi15Settings): ScopaDi15State {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: ScopaDi15State, action: ScopaDi15Action): ScopaDi15State {
...
export function isTerminal(state: ScopaDi15State): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
