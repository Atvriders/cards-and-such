...
export function ScopaDi15Game({ state, dispatch, onGameOver }: GameProps<ScopaDi15State, ScopaDi15Settings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as ScopaDi15Action)}>Capture!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ScopaDi15Action)}>Next</button>
      </>}
    </div>
  );
}
