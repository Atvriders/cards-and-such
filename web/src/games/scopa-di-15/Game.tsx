...
import "./Game.css";
export function ScopaDi15Game({ state, dispatch, onGameOver }: GameProps<ScopaDi15State, ScopaDi15Settings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-scopa-di-15-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as ScopaDi15Action)}>Capture!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-scopa-di-15-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ScopaDi15Action)}>Next</button>
      </>}
    </div>
  );
}
