...
const ScopaDi15Game = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ScopaDi15Game as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: ScopaDi15State): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-scopa-di-15-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-scopa-di-15-secondary"]', pulses: 3 };
  return null;
};
...
export const scopaDi15Plugin: GamePlugin<ScopaDi15State, ScopaDi15Action, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: ScopaDi15Game,
};
