...
export interface RoboSnapSettings { dummy: boolean; }
export interface Target { id:number; lane:number; ticksLeft:number; }
export interface RoboSnapState { rngSeed:number; targets:Target[]; nextId:number; ticksRemaining:number; score:number; popped:number; missed:number; phase:"playing"|"done"; }
export type RoboSnapAction = { type:"tick" } | { type:"pop"; id:number };
export function initialState(seed:number,_s:RoboSnapSettings):RoboSnapState { return { rngSeed:seed, targets:[], nextId:1, ticksRemaining:TIMER_TICKS, score:0, popped:0, missed:0, phase:"playing" }; }
export function reducer(state:RoboSnapState, action:RoboSnapAction):RoboSnapState {
...
export function isTerminal(state:RoboSnapState){ return state.phase==="done"?{score:state.score}:null; }
