...
export function Yukon({
  ...
  dispatch,
  onGameOver,
}: GameProps<YukonState, YukonSettings>): JSX.Element {
  const { onDragStart, onDragOver, onDrop } = useDragDrop();

  const handleMove = useCallback(
    (from: string, to: string, count: number) => {
      dispatch({ type: "move", fromPile: from, toPile: to, count } as YukonAction);
    },
    [dispatch],
  );
  ...
  const handleCardClick = useCallback(
    (pileId: string, indexFromTop: number) => {
      ...
      const target = findAutoMove(state.piles, pileId, count, yukonRuleset);
      if (target) {
        dispatch({ type: "move", fromPile: pileId, toPile: target, count } as YukonAction);
      }
    },
    ...
    <div className="yukon">
      <div className="yukon-info">
        ...
        <button
          ...
          onClick={() => dispatch({ type: "auto-move-to-foundation" } as YukonAction)}
        >
          Auto-move
        </button>
      </div>
      ...
      <div className="yukon-tableau-row">
