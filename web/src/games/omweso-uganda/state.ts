...
export interface OmwesoUgandaSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface OmwesoUgandaState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type OmwesoUgandaAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: OmwesoUgandaSettings): OmwesoUgandaState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: OmwesoUgandaState, action: OmwesoUgandaAction): OmwesoUgandaState {
...
export function isTerminal(state: OmwesoUgandaState): { score: number } | null {
