...
export function MiniEmperorGame({ state, dispatch, onGameOver }: GameProps<MiniEmperorState, MiniEmperorSettings>): JSX.Element {
    ...
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.layout.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "remove", index: i } as MiniEmperorAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
