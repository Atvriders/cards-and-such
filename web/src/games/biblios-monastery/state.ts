...
export const SUIT_NAMES = ["Abbot","Monk","Novice","Manuscript"] as const;
...
export type BibliosMonasteryState = DraftState;
export type BibliosMonasteryAction = DraftAction;
...
export function initialState(seed: number, _settings: BibliosMonasterySettings): BibliosMonasteryState {
...
export function score(state: BibliosMonasteryState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: BibliosMonasteryState): { score: number } | null {
