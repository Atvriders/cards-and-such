...
export const SUIT_NAMES: readonly string[] = ["Theology","Astronomy","Botany","History"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface BibliosMonasterySettings { dummy: boolean; }
export interface BibliosMonasteryState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BibliosMonasteryAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BibliosMonasterySettings): BibliosMonasteryState {
...
export function reducer(state: BibliosMonasteryState, action: BibliosMonasteryAction): BibliosMonasteryState {
...
export function score(s: BibliosMonasteryState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BibliosMonasteryState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
