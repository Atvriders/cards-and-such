...
export function BibliosMonasteryGame({ state, dispatch, onGameOver }: GameProps<BibliosMonasteryState, BibliosMonasterySettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Biblios: Monastery</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BibliosMonasteryAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BibliosMonasteryAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
