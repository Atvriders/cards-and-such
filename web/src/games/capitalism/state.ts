...
export interface CapitalismSettings { dummy: boolean; }
export interface CapitalismState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type CapitalismAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: CapitalismSettings): CapitalismState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: CapitalismState, action: CapitalismAction): CapitalismState {
...
export function isTerminal(state: CapitalismState): { score: number } | null {
