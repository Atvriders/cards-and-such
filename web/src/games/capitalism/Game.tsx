...
export function CapitalismGame({ state, dispatch, onGameOver }: GameProps<CapitalismState, CapitalismSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="capitalism-wrap cap-shed"><div className="capitalism-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="capitalism-final">{state.score} pts</div></div></div>;
  return (
    <div className="capitalism-wrap cap-shed">
