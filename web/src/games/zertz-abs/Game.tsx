...
export function ZertzAbsGame({ state, dispatch, onGameOver }: GameProps<ZertzAbsState, ZertzAbsSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="zrz-wrap"><div className="zrz-done"><h2>Done!</h2><p>Correct: {state.correctCount} / {state.questions.length}</p><p style={{ fontSize:"1.8rem",fontWeight:900,color:"#27ae60" }}>{state.score} pts</p></div></div>;
  const q = state.questions[state.currentIndex]!;
  const isResult = state.phase === "result";
  const urgent = state.timeLeft <= 5 && !state.submitted;
  return (
    <div className="zrz-wrap">
      <div className="zrz-header">
        ...
        <span className="zrz-score">{state.score} pts</span>
      </div>
      ...
      <div className="zrz-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as ZertzAbsAction)}><span className="zrz-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
