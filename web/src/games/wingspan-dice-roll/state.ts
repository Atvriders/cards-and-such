...
export interface WingspanDiceRollSettings { dummy: boolean; }
export interface WingspanDiceRollState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type WingspanDiceRollAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: WingspanDiceRollSettings): WingspanDiceRollState {
  return {
    rngSeed: seed,
...
export function reducer(state: WingspanDiceRollState, action: WingspanDiceRollAction): WingspanDiceRollState {
...
export function isTerminal(state: WingspanDiceRollState): { score: number } | null {
