...
export interface StockpileSharesSettings { dummy: boolean; }
export interface StockpileSharesState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type StockpileSharesAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: StockpileSharesSettings): StockpileSharesState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Tip: blue-chip stable","Rumor: merger pending","SEC sniffing around","Quarterly earnings beat","Pump and dump alert","Bear market mood"];

export function reducer(state: StockpileSharesState, action: StockpileSharesAction): StockpileSharesState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      ...
      if (assets >= 1) {
        ...
        cash += market; assets -= 1; evt = "Traded 1 Shares (+$" + market + ")"; delta = market;
      } else { evt = "No Shares to trade"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
...
export function score(s: StockpileSharesState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: StockpileSharesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
