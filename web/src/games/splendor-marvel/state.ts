...
export const SUIT_NAMES = ["Hero","Villain","Mystic","Tech","Cosmic"] as const;
...
export type SplendorMarvelState = DraftState;
export type SplendorMarvelAction = DraftAction;
...
export function initialState(seed: number, _settings: SplendorMarvelSettings): SplendorMarvelState {
...
export function score(state: SplendorMarvelState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SplendorMarvelState): { score: number } | null {
