...
export interface LegionPatienceSettings { dummy: boolean; }

export interface LegionPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type LegionPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank); let sorted=true; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; if (a<b) sorted=false; } if (sorted && r.length>=3) s += 18; const f = h.filter(c=>cardRank(c)>=11).length; s += f*3; if (s===0) s=1; return s;
}
...
export function reducer(state: LegionPatienceState, action: LegionPatienceAction): LegionPatienceState {
...
export function isTerminal(state: LegionPatienceState): { score: number } | null {
