...
const TerziglioGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TerziglioGame as unknown as React.ComponentType<unknown> })));
...
export const terziglioPlugin: GamePlugin<TerziglioState, TerziglioAction, typeof settings> = {
  ...
  description: "Terziglio — solo Tressette.",
  howToPlay: "Terziglio — solo Tressette. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as TerziglioSettings),
  reducer,
  isTerminal,
  hint: (state: TerziglioState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-terzc-${pick.id}"]`, pulses: 3 };
    }
