...
export function SamuraiQuizGame({ state, dispatch, onGameOver }: GameProps<SamuraiQuizState, SamuraiQuizSettings>): JSX.Element {
      ...
      {!state.done && entry ? (<>
        ...
        <div className="hq-choices">
          {entry.choices.map((choice, i) => {
            ...
            return <button key={i} className={cls} disabled={state.selected !== null} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", index:i } as SamuraiQuizAction)}>{choice}</button>;
          })}
        </div>
        {state.selected !== null && <button className="hq-next" onClick={() => dispatch({ type:"next" } as SamuraiQuizAction)}>{state.current + 1 < state.entries.length ? "Next" : "Finish"}</button>}
