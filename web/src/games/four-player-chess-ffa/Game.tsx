    ...
    <div className="fpcffa-wrap">
      ...
      <div className="fpcffa-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button data-testid={`hint-target-four-player-chess-ffa-answer-${i}`} key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as FourPlayerChessFfaAction)}><span className="fpcffa-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
