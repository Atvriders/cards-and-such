// Court Piece — South-Asian call-trump partnership game — 1v1 duel.
...
export type CourtPiecePhase = "playing" | "done";
...
export type CourtPieceAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `court-piece-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: CourtPieceState, trick: readonly { seat: number; card: Card }[]): CourtPieceState {
...
export function reducer(state: CourtPieceState, action: CourtPieceAction): CourtPieceState {
  ...
  let s: CourtPieceState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: CourtPieceState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: CourtPieceState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): CourtPieceState {
...
export function isTerminal(state: CourtPieceState): { score: number } | null {
