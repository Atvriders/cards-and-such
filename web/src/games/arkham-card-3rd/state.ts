...
export interface arkhamCardThirdSettings { dummy: boolean; }
export interface arkhamCardThirdState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type arkhamCardThirdAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: arkhamCardThirdSettings): arkhamCardThirdState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: arkhamCardThirdState, action: arkhamCardThirdAction): arkhamCardThirdState {
...
export function isTerminal(state: arkhamCardThirdState): { score: number } | null {
