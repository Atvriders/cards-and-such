...
export function PaintPopGame({ state, dispatch, onGameOver }: GameProps<PaintPopState, PaintPopSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as PaintPopAction), 750);
    ...
    <div className="fc-wrap">
      ...
      <div className="fc-board">
        {state.targets.map(t => {
          ...
          return (
            <button key={t.id}
              className="fc-target" data-testid="hint-target-paint-pop-target"
              style={{ left: `${x}%`, top: `${y}%`, transform: "translate(-50%,-50%)", background:"transparent", border:"none" }}
              onClick={() => dispatch({ type:"pop", id:t.id } as PaintPopAction)}
              aria-label="paint">🎨</button>
          );
        })}
      </div>
    </div>
