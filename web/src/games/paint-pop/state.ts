...
export interface PaintPopSettings { dummy: boolean; }
...
export interface PaintPopState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PaintPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PaintPopSettings): PaintPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PaintPopState, action: PaintPopAction): PaintPopState {
...
export function isTerminal(state: PaintPopState): { score: number } | null {
