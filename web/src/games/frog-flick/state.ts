...
export interface FrogFlickSettings { dummy: boolean; }
...
export interface FrogFlickState {
  rngSeed: number;
  critters: Critter[];
  nextId: number;
...
export type FrogFlickAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: FrogFlickSettings): FrogFlickState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: FrogFlickState, action: FrogFlickAction): FrogFlickState {
...
export function isTerminal(state: FrogFlickState): { score: number } | null {
