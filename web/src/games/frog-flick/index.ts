...
const FrogFlickGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FrogFlickGame as unknown as React.ComponentType<unknown> })));
...
export const frogFlickPlugin: GamePlugin<FrogFlickState, FrogFlickAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: FrogFlickState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.critters || state.critters.length === 0) return null;
    return { selector: '[data-testid="hint-target-frog-flick-target"]', pulses: 3 };
  },
  component:FrogFlickGame,
};
