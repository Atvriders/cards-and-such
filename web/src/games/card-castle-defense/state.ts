...
export interface CardCastleDefenseSettings { dummy: boolean; }
export interface CardCastleDefenseState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardCastleDefenseAction = { type: "draw" } | { type: "next" };
...
export function cardWins(c: number): boolean { const r = c % 13; return r >= 9 && r <= 11; }
export function initialState(seed: number, _settings: CardCastleDefenseSettings): CardCastleDefenseState {
  return { rngSeed: seed, round: 1, card: null, score: 0, phase: "draw", lastWin: false, lastPts: 0 };
}
export function reducer(state: CardCastleDefenseState, action: CardCastleDefenseAction): CardCastleDefenseState {
...
export function isTerminal(state: CardCastleDefenseState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
