...
import "./Game.css";
export function CardCastleDefenseGame({ state, dispatch, onGameOver }: GameProps<CardCastleDefenseState, CardCastleDefenseSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "draw" && (
        <button data-testid="hint-target-card-castle-defense-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as CardCastleDefenseAction)}>Draw</button>
      )}
      {state.phase === "result" && (
        <>
          <div className="cm-result">{state.lastWin ? `Hit! +${state.lastPts}` : "Miss — 0"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardCastleDefenseAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
