...
import "./Game.css";
export function KansasCityLowballGame({ state, dispatch, onGameOver }: GameProps<KansasCityLowballState, KansasCityLowballSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-kansas-city-lowball-deal" onClick={() => dispatch({ type: "deal" } as KansasCityLowballAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-kansas-city-lowball-next" onClick={() => dispatch({ type: "next" } as KansasCityLowballAction)}>Next</button>
      </>}
    </div>
  );
}
