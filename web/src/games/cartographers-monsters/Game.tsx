...
export function CartographersMonstersGame({ state, dispatch, onGameOver }: GameProps<CartographersMonstersState, CartographersMonstersSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as CartographersMonstersAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as CartographersMonstersAction)}>Skip</button>
      )}
    </div>
  );
}
