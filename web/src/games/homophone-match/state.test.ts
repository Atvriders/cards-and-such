...
describe("HomophoneMatch initialState", () => {
  ...
  it("left items are words from pairs", () => {
  ...
  it("right items are homophones from pairs", () => {
    const s = initialState(5, defaultSettings);
    const homoSet = new Set(s.pairs.map((p) => p.homophone));
    s.rightItems.forEach((h) => expect(homoSet.has(h)).toBe(true));
  });
});

describe("HomophoneMatch reducer", () => {
  ...
  it("correct match increases score and adds to matched", () => {
    ...
    const ri = s.rightItems.indexOf(pair.homophone);
  ...
  it("wrong match sets wrong field", () => {
    ...
    const word = s.leftItems[0]!;
    const pair = s.pairs.find((p) => p.word === word)!;
    const correctRi = s.rightItems.indexOf(pair.homophone);
  ...
  it("matching all pairs sets done", () => {
    ...
    for (let li = 0; li < state.pairs.length; li++) {
      ...
      const ri = state.rightItems.indexOf(pair.homophone);
...
describe("HomophoneMatch isTerminal", () => {
