...
import "./Game.css";
export function MeyerDiceGame({ state, dispatch, onGameOver }: GameProps<MeyerDiceState, MeyerDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-meyer-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as MeyerDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-meyer-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MeyerDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
