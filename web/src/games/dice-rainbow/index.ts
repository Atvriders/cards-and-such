...
const DiceRainbowGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceRainbowGame as unknown as React.ComponentType<unknown> })));
...
export const diceRainbowPlugin: GamePlugin<DiceRainbowState, DiceRainbowAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceRainbowState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-rainbow-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-dice-rainbow-next"]', pulses: 3 };
    return null;
  },
  component:DiceRainbowGame,
};
