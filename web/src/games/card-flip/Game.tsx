...
export function CardFlipGame({ state, dispatch, onGameOver }: GameProps<CardFlipState, CardFlipSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="cf-wrap"><div className="cf-done"><h2>All Matched!</h2><div>Attempts: {state.attempts}</div><div className="cf-final">{t?.score} pts</div></div></div>;
  }
  return (
    <div className="cf-wrap">
      ...
      <div className="cf-grid">
        {state.cards.map((c, i) => (
          <button key={c.id} className={`cf-card${c.revealed ? " revealed" : ""}${c.matched ? " matched" : ""}`} onClick={() => dispatch({ type:"flip", index: i } as CardFlipAction)}>
