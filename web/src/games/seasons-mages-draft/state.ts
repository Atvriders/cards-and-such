...
export const SUIT_NAMES = ["Air","Earth","Fire","Water","Spring"] as const;
...
export type SeasonsMagesDraftState = DraftState;
export type SeasonsMagesDraftAction = DraftAction;
export interface SeasonsMagesDraftSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SeasonsMagesDraftSettings): SeasonsMagesDraftState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SeasonsMagesDraftState, action: SeasonsMagesDraftAction): SeasonsMagesDraftState {
  return draftReducer(state, action);
}

export function score(state: SeasonsMagesDraftState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SeasonsMagesDraftState): { score: number } | null {
