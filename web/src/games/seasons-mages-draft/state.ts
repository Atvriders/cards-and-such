...
export interface SeasonsMagesDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SeasonsMagesDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SeasonsMagesDraftSettings): SeasonsMagesDraftState {
...
export function score(s: SeasonsMagesDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SeasonsMagesDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
