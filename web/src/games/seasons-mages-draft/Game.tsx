...
export function SeasonsMagesDraftGame({ state, dispatch, onGameOver }: GameProps<SeasonsMagesDraftState, SeasonsMagesDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Seasons: Mages</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SeasonsMagesDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SeasonsMagesDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
