...
Strategy: Start with Spaghetti at 28 portions and $6. Save up for one sauce upgrade early, then reinvest into marketing. Scale up batch size as demand grows. Aim for $1800 by day 22 for a perfect score!`,
  ...
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "done" || p === "gameover" || p === "ended" || p === "finished" || (s as any).gameOver || (s as any).won || (s as any).complete || (s as any).isComplete) return null; return { selector: ".pasta-btn", pulses: 3 }; },
  component: PastaShop,
