...
export function FluxxOriginalMiniGame({ state, dispatch, onGameOver }: GameProps<FluxxOriginalMiniState, FluxxOriginalMiniSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="g-fluxorigmini-wrap"><div className="g-fluxorigmini-done bounce-in"><h2>Done!</h2><div>Correct: {state.correctCount} / {state.rounds.length}</div><div className="g-fluxorigmini-final">{state.score} pts</div></div></div>;
  }
  const r = state.rounds[state.currentIndex]!;
  return (
    <div className="g-fluxorigmini-wrap fade-in">
      <div className="g-fluxorigmini-info">Round {state.currentIndex + 1} / {state.rounds.length}</div>
      <div className="g-fluxorigmini-score pulse">{state.score} pts</div>
