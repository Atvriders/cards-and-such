...
export interface PaperclipPinchSettings { dummy: boolean; }
...
export interface PaperclipPinchState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PaperclipPinchAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PaperclipPinchSettings): PaperclipPinchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PaperclipPinchState, action: PaperclipPinchAction): PaperclipPinchState {
...
export function isTerminal(state: PaperclipPinchState): { score: number } | null {
