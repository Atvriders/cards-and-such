...
const S = { dummy: false };
describe("PaperclipPinch", () => {
  it("starts in playing with full timer", () => { const s = initialState(1, S); expect(s.phase).toBe("playing"); expect(s.ticksRemaining).toBeGreaterThanOrEqual(TIMER_TICKS); expect(s.targets).toEqual([]); });
  it("tick adds targets and decrements timer", () => { const s = reducer(initialState(1, S), { type:"tick" }); expect(s.ticksRemaining).toBeLessThanOrEqual(TIMER_TICKS); expect(s.targets.length).toBeGreaterThanOrEqual(1); });
