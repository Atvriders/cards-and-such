...
export const JackboxPack1Quiz_QUESTIONS: QuizQuestion[] = [
  {
...
const CFG = { totalQuestions: Math.min(10, JackboxPack1Quiz_QUESTIONS.length), pool: JackboxPack1Quiz_QUESTIONS };

export interface JackboxPack1QuizSettings { dummy: boolean; }
export type JackboxPack1QuizState = QuizState;
export type JackboxPack1QuizAction = { type: "answer"; choice: number; elapsedMs: number } | { type: "next" };

export function initialState(seed: number, _s: JackboxPack1QuizSettings): JackboxPack1QuizState {
...
export function isTerminal(state: JackboxPack1QuizState): { score: number } | null {
