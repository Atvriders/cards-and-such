...
const GeierGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.GeierGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;

export const geierPlugin: GamePlugin<GeierState, GeierAction, typeof settings> = {
  id: "geier",
  title: "Geier",
  category: "cards",
  players: { min: 1, max: 1, multiplayer: false },
  description: "Geier — Schafkopf variant with Queen trumps.",
  howToPlay: "Geier — Schafkopf variant with Queen trumps. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as GeierSettings),
  reducer,
  isTerminal,
  hint: (state: GeierState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-geierc-${pick.id}"]`, pulses: 3 };
    }
