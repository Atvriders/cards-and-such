...
export interface AnacondaPassPokerSettings { rounds: "10"; }

export interface AnacondaPassPokerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type AnacondaPassPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AnacondaPassPokerSettings): AnacondaPassPokerState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: AnacondaPassPokerState, action: AnacondaPassPokerAction): AnacondaPassPokerState {
...
export function isTerminal(state: AnacondaPassPokerState): { score: number } | null {
