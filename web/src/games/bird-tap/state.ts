...
// Bird Tap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface BirdTapSettings { dummy: boolean; }

export interface BirdTapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface BirdTapState {
  rngSeed: number;
  targets: BirdTapTarget[];
  nextId: number;
...
export type BirdTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BirdTapSettings): BirdTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BirdTapState, action: BirdTapAction): BirdTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: BirdTapTarget[] = [];
...
export function isTerminal(state: BirdTapState): { score: number } | null {
