...
export interface ParksTrailSettings { dummy: boolean; }
export interface ParksTrailState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type ParksTrailAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: ParksTrailSettings): ParksTrailState {
...
export function reducer(state: ParksTrailState, action: ParksTrailAction): ParksTrailState {
...
export function score(s: ParksTrailState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: ParksTrailState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
