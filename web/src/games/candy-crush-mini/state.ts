...
// Candy Crush Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface CandyCrushMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface CandyCrushMiniState {
  rngSeed: number;
  grid: Grid;
...
export type CandyCrushMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: CandyCrushMiniSettings): CandyCrushMiniState {
...
export function reducer(state: CandyCrushMiniState, action: CandyCrushMiniAction): CandyCrushMiniState {
...
export function isTerminal(state: CandyCrushMiniState): { score: number } | null {
