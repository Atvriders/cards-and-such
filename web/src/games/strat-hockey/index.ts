...
const StratHockeyGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.StratHockeyGame as unknown as React.ComponentType<unknown> })));
...
export const stratHockeyPlugin: GamePlugin<StratHockeyState, StratHockeyAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: StratHockeyState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-strat-hockey-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-strat-hockey-next"]', pulses: 3 };
    return null;
  },
