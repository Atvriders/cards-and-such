...
export const SUIT_NAMES: readonly string[] = ["Residential","Commercial","Industrial","Park"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface TheEstatesBidSettings { dummy: boolean; }
export interface TheEstatesBidState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type TheEstatesBidAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: TheEstatesBidSettings): TheEstatesBidState {
...
export function reducer(state: TheEstatesBidState, action: TheEstatesBidAction): TheEstatesBidState {
...
export function score(s: TheEstatesBidState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: TheEstatesBidState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
