...
export function Golf6CardGame({ state, dispatch, onGameOver }: GameProps<Golf6CardState, Golf6CardSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="golf-6-card-wrap g6c-shed"><div className="golf-6-card-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="golf-6-card-final">{state.score} pts</div></div></div>;
  return (
    <div className="golf-6-card-wrap g6c-shed">
