...
export interface Golf6CardSettings { dummy: boolean; }
export interface Golf6CardState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type Golf6CardAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: Golf6CardSettings): Golf6CardState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: Golf6CardState, action: Golf6CardAction): Golf6CardState {
...
export function isTerminal(state: Golf6CardState): { score: number } | null {
