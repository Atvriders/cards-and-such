...
export interface FleetDiceSettings { rounds: "10"; }

export interface FleetDiceState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type FleetDiceAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: FleetDiceSettings): FleetDiceState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: FleetDiceState, action: FleetDiceAction): FleetDiceState {
...
export function isTerminal(state: FleetDiceState): { score: number } | null {
