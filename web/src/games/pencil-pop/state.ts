...
export interface PencilPopSettings { dummy: boolean; }
...
export interface PencilPopState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PencilPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PencilPopSettings): PencilPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PencilPopState, action: PencilPopAction): PencilPopState {
...
export function isTerminal(state: PencilPopState): { score: number } | null {
