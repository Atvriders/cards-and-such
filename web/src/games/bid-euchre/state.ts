// Bid Euchre — Euchre variant with bidding — simplified for solo play with set trump.
...
export type BidEuchrePhase = "playing" | "done";

export interface BidEuchreState {
  rngSeed: number;
...
export type BidEuchreAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `bid-euchre-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: BidEuchreState, trick: readonly { seat: number; card: Card }[]): BidEuchreState {
...
export function reducer(state: BidEuchreState, action: BidEuchreAction): BidEuchreState {
  ...
  let s: BidEuchreState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: BidEuchreState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: BidEuchreState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): BidEuchreState {
...
export function isTerminal(state: BidEuchreState): { score: number } | null {
