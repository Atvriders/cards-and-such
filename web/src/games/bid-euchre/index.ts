...
const BidEuchreGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BidEuchreGame as unknown as React.ComponentType<unknown> })));
...
export const bidEuchrePlugin: GamePlugin<BidEuchreState, BidEuchreAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: BidEuchreState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-bideu-${pick.id}"]`, pulses: 3 };
    }
