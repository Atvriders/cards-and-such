...
const BriscoloneGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BriscoloneGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;

export const briscolonePlugin: GamePlugin<BriscoloneState, BriscoloneAction, typeof settings> = {
  id: "briscolone",
  title: "Briscolone",
  category: "cards",
  players: { min: 1, max: 1, multiplayer: false },
  description: "Briscolone — long Briscola.",
  howToPlay: "Briscolone — long Briscola. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as BriscoloneSettings),
  reducer,
  isTerminal,
  hint: (state: BriscoloneState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-brislc-${pick.id}"]`, pulses: 3 };
    }
