...
export const SIZE = 6;
export const MAX_MOVES = 18;
export interface CanadianCheckers12Settings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface CanadianCheckers12State {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type CanadianCheckers12Action = { type: "place"; idx: number };
export function initialState(seed: number, _settings: CanadianCheckers12Settings): CanadianCheckers12State {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: CanadianCheckers12State, action: CanadianCheckers12Action): CanadianCheckers12State {
...
export function isTerminal(state: CanadianCheckers12State): { score: number } | null {
