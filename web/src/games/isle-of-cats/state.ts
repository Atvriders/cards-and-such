...
export interface IsleOfCatsSettings { dummy: boolean; }
export interface IsleOfCatsState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type IsleOfCatsAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: IsleOfCatsSettings): IsleOfCatsState {
...
export function reducer(state: IsleOfCatsState, action: IsleOfCatsAction): IsleOfCatsState {
...
export function score(s: IsleOfCatsState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: IsleOfCatsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
