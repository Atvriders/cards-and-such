...
export const CARDS_PER_HAND = 9;
export interface DoubleBoardBombPotSettings { dummy: boolean; }
export interface DoubleBoardBombPotState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DoubleBoardBombPotAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DoubleBoardBombPotSettings): DoubleBoardBombPotState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DoubleBoardBombPotState, action: DoubleBoardBombPotAction): DoubleBoardBombPotState {
...
export function isTerminal(state: DoubleBoardBombPotState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
