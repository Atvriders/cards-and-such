...
export interface NachosNowSettings { dummy: boolean; }
...
export interface NachosNowState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type NachosNowAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: NachosNowSettings): NachosNowState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: NachosNowState, action: NachosNowAction): NachosNowState {
...
export function isTerminal(state: NachosNowState): { score: number } | null {
