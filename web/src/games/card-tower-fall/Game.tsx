...
export function CardTowerFallGame({ state, dispatch, onGameOver }: GameProps<CardTowerFallState, CardTowerFallSettings>): JSX.Element {
    ...
    <div className="rg-wrap">
      ...
      {state.phase === "predict" && (
        <div className="rg-row">
          {CHOICES.map(c => (
            <button key={c} className="rg-btn" onClick={() => dispatch({ type:"go", choice:c } as CardTowerFallAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && (
