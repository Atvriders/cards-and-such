...
export interface MangoTapSettings { dummy: boolean; }
export interface MangoTapTarget { id: number; lane: number; ticksLeft: number; }
export interface MangoTapState {
  rngSeed: number;
  targets: MangoTapTarget[];
  nextId: number;
...
export type MangoTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: MangoTapSettings): MangoTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: MangoTapState, action: MangoTapAction): MangoTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: MangoTapTarget[] = [];
...
export function isTerminal(state: MangoTapState): { score: number } | null {
