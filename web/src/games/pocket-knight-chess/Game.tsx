    ...
    <div className="pockknt-wrap">
      ...
      <div className="pockknt-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button data-testid={`hint-target-pocket-knight-chess-answer-${i}`} key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as PocketKnightChessAction)}><span className="pockknt-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
