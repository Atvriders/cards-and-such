...
export function TwoTruthsLiePickGame({ state, dispatch, onGameOver }: GameProps<TwoTruthsLiePickState, TwoTruthsLiePickSettings>): JSX.Element {
    ...
    return (
      <div className={`${P}-wrap`}>
        <div className={`${P}-final`}>
          <h2 className={`${P}-final-title`}>Two Truths and a Lie — Complete</h2>
    ...
    <div className={`${P}-wrap`}>
      ...
      <div className={`${P}-choices`}>
        {q.choices.map((c, i) => {
          ...
          const cls = state.phase === "feedback"
            ...
            : `${P}-choice`;
          return (
            <button data-testid="hint-target-two-truths-lie-pick-primary" key={i} className={cls} disabled={state.phase !== "ask"}
              type="button"
              onClick={() => dispatch({ type: "answer", choice: i, elapsedMs: Date.now() - start } as TwoTruthsLiePickAction)}
            >{c}</button>
          );
        })}
      </div>
      {state.phase === "feedback" && (
        <>
          ...
          <button data-testid="hint-target-two-truths-lie-pick-next" className={`${P}-next`} type="button" onClick={() => dispatch({ type: "next" } as TwoTruthsLiePickAction)}>
