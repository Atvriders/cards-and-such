...
const KingdomsDice = /* @__PURE__ */ lazy(() => import("./KingdomsDice.js").then((mod) => ({ default: mod.KingdomsDice as unknown as React.ComponentType<unknown> })));
...
Strategy: ranks near the average roll (3–4) appear more frequently on your dice. High-value kingdoms with common ranks are the best buys. Watch what the bot is targeting — if it keeps buying low-rank kingdoms, consider blocking with a die even for a small payoff to deny its progress.`,
  ...
  hint: (state) => {
    ...
    if (phase === "rolling") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "rolling-dice") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "preRoll") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "ready") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "playerRoll") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "roll") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "play") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    if (phase === "playing") return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
    return { selector: '[data-testid="hint-target-kingdoms-dice-roll"]', pulses: 3 };
  },
