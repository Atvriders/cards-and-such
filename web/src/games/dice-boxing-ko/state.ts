...
export interface DiceBoxingKoSettings { dummy: boolean; }

export interface DiceBoxingKoState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceBoxingKoAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceBoxingKoSettings): DiceBoxingKoState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceBoxingKoState, action: DiceBoxingKoAction): DiceBoxingKoState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceBoxingKoState> = {};
...
export function isTerminal(state: DiceBoxingKoState): { score: number } | null {
