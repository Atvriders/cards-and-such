...
export interface EzBaccaratPokerSettings { dummy: boolean; }
export interface EzBaccaratPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type EzBaccaratPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: EzBaccaratPokerSettings): EzBaccaratPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: EzBaccaratPokerState, action: EzBaccaratPokerAction): EzBaccaratPokerState {
...
export function isTerminal(state: EzBaccaratPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
