...
import "./Game.css";
export function EzBaccaratPokerGame({ state, dispatch, onGameOver }: GameProps<EzBaccaratPokerState, EzBaccaratPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-ez-baccarat-poker-deal" onClick={() => dispatch({ type: "deal" } as EzBaccaratPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-ez-baccarat-poker-next" onClick={() => dispatch({ type: "next" } as EzBaccaratPokerAction)}>Next</button>
      </>}
    </div>
  );
}
