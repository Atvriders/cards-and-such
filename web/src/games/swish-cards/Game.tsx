...
export function SwishCardsGame({ state, dispatch, onGameOver }: GameProps<SwishCardsState, SwishCardsSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="swshcrd-wrap"><div className="swshcrd-done bounce-in"><h2>Done!</h2><div>Correct: {state.correctCount} / {state.rounds.length}</div><div className="swshcrd-final">{state.score} pts</div></div></div>;
  }
  const r = state.rounds[state.currentIndex]!;
  return (
    <div className="swshcrd-wrap fade-in">
      <div className="swshcrd-info">Round {state.currentIndex + 1} / {state.rounds.length}</div>
      <div className="swshcrd-score pulse">{state.score} pts</div>
