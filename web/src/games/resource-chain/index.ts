...
const ResourceChain = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ResourceChain as unknown as React.ComponentType<unknown> })));
...
Strategy tips: Build a buffer of flour or bread early so you can sell when demand spikes. Don't overspend on seeds before you have the pipeline working. The entire chain costs about 5 coins per bread but earns 14–30 coins — profit depends on demand timing. Reach 400 coins for a perfect score!`,
  ...
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "done" || p === "gameover" || p === "ended" || p === "finished" || (s as any).gameOver || (s as any).won || (s as any).complete || (s as any).isComplete) return null; return { selector: ".rc-btn", pulses: 3 }; },
