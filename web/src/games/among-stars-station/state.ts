...
export interface AmongStarsStationState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type AmongStarsStationAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: AmongStarsStationSettings): AmongStarsStationState {
...
export function score(s: AmongStarsStationState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: AmongStarsStationState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
