...
export const SUIT_NAMES = ["Diplomat","Military","Civic","Trade","Science"] as const;
...
export type AmongStarsStationState = DraftState;
export type AmongStarsStationAction = DraftAction;
export interface AmongStarsStationSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: AmongStarsStationSettings): AmongStarsStationState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: AmongStarsStationState, action: AmongStarsStationAction): AmongStarsStationState {
  return draftReducer(state, action);
}

export function score(state: AmongStarsStationState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: AmongStarsStationState): { score: number } | null {
