...
export function AmongStarsStationGame({ state, dispatch, onGameOver }: GameProps<AmongStarsStationState, AmongStarsStationSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Among the Stars</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as AmongStarsStationAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as AmongStarsStationAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
