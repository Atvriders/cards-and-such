...
export interface LetItRideCasSettings { dummy: boolean; }
export interface LetItRideCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type LetItRideCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: LetItRideCasSettings): LetItRideCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: LetItRideCasState, action: LetItRideCasAction): LetItRideCasState {
...
export function isTerminal(state: LetItRideCasState): { score: number } | null {
