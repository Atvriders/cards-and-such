// Bower — Generic partnership trump trick-taking game.
...
export type BowerPhase = "playing" | "done";

export interface BowerState {
  rngSeed: number;
...
export type BowerAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `bower-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: BowerState, trick: readonly { seat: number; card: Card }[]): BowerState {
...
export function reducer(state: BowerState, action: BowerAction): BowerState {
  ...
  let s: BowerState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: BowerState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: BowerState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): BowerState {
...
export function isTerminal(state: BowerState): { score: number } | null {
