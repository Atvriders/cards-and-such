...
import "./Game.css";
export function CardCliffGame({ state, dispatch, onGameOver }: GameProps<CardCliffState, CardCliffSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      <div className="cm-info">Cliff — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardCliffAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
