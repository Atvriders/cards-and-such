...
export interface SpitInTheOceanSettings { dummy: boolean; }
export interface SpitInTheOceanState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SpitInTheOceanAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SpitInTheOceanSettings): SpitInTheOceanState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SpitInTheOceanState, action: SpitInTheOceanAction): SpitInTheOceanState {
...
export function isTerminal(state: SpitInTheOceanState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
