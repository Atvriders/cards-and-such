...
export function SpitInTheOceanGame({ state, dispatch, onGameOver }: GameProps<SpitInTheOceanState, SpitInTheOceanSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap thmSpitOc">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-spit-in-the-ocean-deal" onClick={() => dispatch({ type: "deal" } as SpitInTheOceanAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-spit-in-the-ocean-next" onClick={() => dispatch({ type: "next" } as SpitInTheOceanAction)}>Next</button>
      </>}
    </div>
  );
}
