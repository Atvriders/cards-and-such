...
export interface GolfNineShedSettings { dummy: boolean; }
export interface GolfNineShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type GolfNineShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: GolfNineShedSettings): GolfNineShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: GolfNineShedState, action: GolfNineShedAction): GolfNineShedState {
...
export function isTerminal(state: GolfNineShedState): { score: number } | null {
