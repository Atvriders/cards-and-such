...
import "./Game.css";
export function DoomlingsCatastropheGame({ state, dispatch, onGameOver }: GameProps<DoomlingsCatastropheState, DoomlingsCatastropheSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.hand.length > 0 && (
        <>
          ...
          {state.catastrophe > 0 && <div className="cm-card clank">Catastrophe (-{state.catastrophe})</div>}
        </>
      )}
      {state.phase === "drawing" && (
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as DoomlingsCatastropheAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
