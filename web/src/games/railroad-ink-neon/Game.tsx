...
export function RailroadInkNeonGame({ state, dispatch, onGameOver }: GameProps<RailroadInkNeonState, RailroadInkNeonSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RailroadInkNeonAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RailroadInkNeonAction)}>Skip</button>
      )}
    </div>
  );
}
