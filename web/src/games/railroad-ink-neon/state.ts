...
export interface RailroadInkNeonSettings { dummy: boolean; }
export interface RailroadInkNeonState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type RailroadInkNeonAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: RailroadInkNeonSettings): RailroadInkNeonState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: RailroadInkNeonState, action: RailroadInkNeonAction): RailroadInkNeonState {
...
export function isTerminal(state: RailroadInkNeonState): { score: number } | null {
