...
const PineconePopGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PineconePopGame as unknown as React.ComponentType<unknown> })));
...
export const pineconePopPlugin: GamePlugin<PineconePopState, PineconePopAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: PineconePopState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-pinecone-pop-target"]', pulses: 3 };
  },
  component:PineconePopGame,
};
