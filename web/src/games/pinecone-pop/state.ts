...
export interface PineconePopSettings { dummy: boolean; }
export interface PineconePopTarget { id: number; lane: number; ticksLeft: number; }
export interface PineconePopState { rngSeed: number; targets: PineconePopTarget[]; nextId: number; ticksRemaining: number; score: number; clicked: number; missed: number; phase: "playing" | "done"; }
export type PineconePopAction = { type: "tick" } | { type: "click"; id: number };
export function initialState(seed: number, _s: PineconePopSettings): PineconePopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, clicked: 0, missed: 0, phase: "playing" };
}
export function reducer(state: PineconePopState, action: PineconePopAction): PineconePopState {
  ...
  if (action.type === "tick") {
    ...
    let nextId = state.nextId; const newT: PineconePopTarget[] = [];
...
export function isTerminal(state: PineconePopState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
