...
export interface DiceTripBetSettings { rounds: "8" | "12"; }
export interface DiceTripBetState {
...
export type DiceTripBetAction = { type: "roll"; amount: number } | { type: "next" };
...
function checkWin(dice: number[]): { win: boolean; label: string } {
  ...
  return {win:trip, label:trip?"Triplet! All match!":"No triple"};
}

export function initialState(seed: number, settings: DiceTripBetSettings): DiceTripBetState {
...
export function reducer(state: DiceTripBetState, action: DiceTripBetAction): DiceTripBetState {
...
export function isTerminal(state: DiceTripBetState): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
