...
export interface PenniesHeavenRSettings { dummy: boolean; }
export interface PenniesHeavenRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type PenniesHeavenRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: PenniesHeavenRSettings): PenniesHeavenRState {
...
export function reducer(state: PenniesHeavenRState, action: PenniesHeavenRAction): PenniesHeavenRState {
...
export function isTerminal(state: PenniesHeavenRState): { score: number } | null {
