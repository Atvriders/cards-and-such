...
export function PenniesHeavenRGame({ state, dispatch, onGameOver }: GameProps<PenniesHeavenRState, PenniesHeavenRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as PenniesHeavenRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PenniesHeavenRAction)}>Next</button>
      </>}
    </div>
  );
}
