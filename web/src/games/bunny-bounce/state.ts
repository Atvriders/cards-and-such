...
export interface BunnyBounceSettings { dummy: boolean; }
...
export interface BunnyBounceState {
  rngSeed: number;
  critters: Critter[];
  nextId: number;
...
export type BunnyBounceAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BunnyBounceSettings): BunnyBounceState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BunnyBounceState, action: BunnyBounceAction): BunnyBounceState {
...
export function isTerminal(state: BunnyBounceState): { score: number } | null {
