...
export interface CardMountainSettings { dummy: boolean; }
export interface CardMountainState {
  rngSeed: number;
  round: number;
  hand: number[];
  sum: number;
...
export type CardMountainAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardMountainSettings): CardMountainState {
  return { rngSeed: seed, round: 1, hand: [], sum: 0, score: 0, phase: "dealing", lastPts: 0 };
}
export function reducer(state: CardMountainState, action: CardMountainAction): CardMountainState {
...
export function isTerminal(state: CardMountainState): { score: number } | null {
