...
import "./Game.css";
export function CardMountainGame({ state, dispatch, onGameOver }: GameProps<CardMountainState, CardMountainSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <div className="cm-help">Lower sum = better climb</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardMountainAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
