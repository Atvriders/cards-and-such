...
export interface RajasDiceCharmersSettings { rounds: "10"; }

export interface RajasDiceCharmersState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type RajasDiceCharmersAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: RajasDiceCharmersSettings): RajasDiceCharmersState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: RajasDiceCharmersState, action: RajasDiceCharmersAction): RajasDiceCharmersState {
...
export function isTerminal(state: RajasDiceCharmersState): { score: number } | null {
