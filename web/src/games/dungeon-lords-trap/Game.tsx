...
export function DungeonLordsTrapGame({ state, dispatch, onGameOver }: GameProps<DungeonLordsTrapState, DungeonLordsTrapSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "drawing" && (
        <button className="cm-btn" onClick={() => dispatch({ type:"draw" } as DungeonLordsTrapAction)}>Draw</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as DungeonLordsTrapAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
