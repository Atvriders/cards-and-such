...
export interface MagpieTaflSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface MagpieTaflState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type MagpieTaflAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: MagpieTaflSettings): MagpieTaflState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: MagpieTaflState, action: MagpieTaflAction): MagpieTaflState {
...
export function isTerminal(state: MagpieTaflState): { score: number } | null {
