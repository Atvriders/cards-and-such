...
export interface ColorettoCardsSettings { dummy: boolean; }
export interface ColorettoCardsState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type ColorettoCardsAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: ColorettoCardsSettings): ColorettoCardsState {
...
export function reducer(state: ColorettoCardsState, action: ColorettoCardsAction): ColorettoCardsState {
...
export function score(s: ColorettoCardsState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: ColorettoCardsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
