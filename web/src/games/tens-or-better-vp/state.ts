...
export interface TensOrBetterVpSettings { dummy: boolean; }
export interface TensOrBetterVpState { rngSeed: number; round: number; hand: number[]; phase: "deal" | "scored" | "done"; score: number; pts: number; result: string; rank: string; }
export type TensOrBetterVpAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TensOrBetterVpSettings): TensOrBetterVpState {
  return { rngSeed: seed, round: 1, hand: [], phase: "deal", score: 0, pts: 0, result: "", rank: "" };
}
export function reducer(state: TensOrBetterVpState, action: TensOrBetterVpAction): TensOrBetterVpState {
...
export function isTerminal(state: TensOrBetterVpState): { score: number } | null {
