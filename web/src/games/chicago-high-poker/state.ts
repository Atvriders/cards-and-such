...
export interface ChicagoHighPokerSettings { dummy: boolean; }
export interface ChicagoHighPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ChicagoHighPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ChicagoHighPokerSettings): ChicagoHighPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ChicagoHighPokerState, action: ChicagoHighPokerAction): ChicagoHighPokerState {
...
export function isTerminal(state: ChicagoHighPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
