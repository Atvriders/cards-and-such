...
export function ChicagoHighPokerGame({ state, dispatch, onGameOver }: GameProps<ChicagoHighPokerState, ChicagoHighPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap thmChiHi">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as ChicagoHighPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ChicagoHighPokerAction)}>Next</button>
      </>}
    </div>
  );
}
