...
export interface PumpkinSmashSettings { dummy: boolean; }
...
export interface PumpkinSmashState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PumpkinSmashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PumpkinSmashSettings): PumpkinSmashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PumpkinSmashState, action: PumpkinSmashAction): PumpkinSmashState {
...
export function isTerminal(state: PumpkinSmashState): { score: number } | null {
