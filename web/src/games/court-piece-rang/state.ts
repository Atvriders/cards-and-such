...
export interface CourtPieceRangSettings { dummy: boolean; }
export interface CourtPieceRangState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type CourtPieceRangAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: CourtPieceRangSettings): CourtPieceRangState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: CourtPieceRangState, action: CourtPieceRangAction): CourtPieceRangState {
...
export function isTerminal(state: CourtPieceRangState): { score: number } | null {
