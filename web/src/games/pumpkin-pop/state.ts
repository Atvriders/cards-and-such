...
export interface PumpkinPopSettings { dummy: boolean; }
...
export interface PumpkinPopState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PumpkinPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _s: PumpkinPopSettings): PumpkinPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PumpkinPopState, action: PumpkinPopAction): PumpkinPopState {
...
export function isTerminal(state: PumpkinPopState): { score: number } | null {
