...
export interface SkittlesRubberSettings { dummy: boolean; }

export interface SkittlesRubberState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type SkittlesRubberAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: SkittlesRubberSettings): SkittlesRubberState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: SkittlesRubberState, action: SkittlesRubberAction): SkittlesRubberState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<SkittlesRubberState> = {};
...
export function isTerminal(state: SkittlesRubberState): { score: number } | null {
