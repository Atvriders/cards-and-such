...
// Cookie Clutch: bakery clicker. 30-second timer.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface CookieClutchSettings { dummy: boolean; }

export interface CookieClutchItem {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface CookieClutchState {
  rngSeed: number;
  items: CookieClutchItem[];
  nextId: number;
...
export type CookieClutchAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CookieClutchSettings): CookieClutchState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CookieClutchState, action: CookieClutchAction): CookieClutchState {
  ...
  if (action.type === "tick") {
    ...
    const newItems: CookieClutchItem[] = [];
...
export function isTerminal(state: CookieClutchState): { score: number } | null {
