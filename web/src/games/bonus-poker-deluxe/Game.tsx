...
import "./Game.css";
export function BonusPokerDeluxeGame({ state, dispatch, onGameOver }: GameProps<BonusPokerDeluxeState, BonusPokerDeluxeSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-bonus-poker-deluxe-deal" onClick={() => dispatch({ type: "deal" } as BonusPokerDeluxeAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-bonus-poker-deluxe-next" onClick={() => dispatch({ type: "next" } as BonusPokerDeluxeAction)}>Next</button>
      </>}
    </div>
  );
}
