...
const BonusPokerDeluxeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BonusPokerDeluxeGame as unknown as React.ComponentType<unknown> })));
...
export const bonusPokerDeluxePlugin: GamePlugin<BonusPokerDeluxeState, BonusPokerDeluxeAction, typeof settings> = {
  ...
  hint: (state: BonusPokerDeluxeState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-bonus-poker-deluxe-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-bonus-poker-deluxe-next"]', pulses: 3 };
    return null;
  },
};
