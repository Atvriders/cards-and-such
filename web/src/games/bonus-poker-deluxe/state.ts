...
export interface BonusPokerDeluxeSettings { dummy: boolean; }
export interface BonusPokerDeluxeState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type BonusPokerDeluxeAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BonusPokerDeluxeSettings): BonusPokerDeluxeState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: BonusPokerDeluxeState, action: BonusPokerDeluxeAction): BonusPokerDeluxeState {
...
export function isTerminal(state: BonusPokerDeluxeState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
