...
export interface CardBid2Settings { rounds: "8" | "12"; }
export interface CardBid2State {
  deck: number[]; pos: number; topCard: number;
  coins: number; round: number; maxRounds: number;
  phase: "betting" | "revealed" | "gameover";
  lastResult: "win" | "lose" | "tie" | null;
  revealedCard: number | null; bid: number;
}
export type CardBid2Action = { type: "bet"; amount: number; dir: "higher" | "lower" } | { type: "next" };
...
export function initialState(seed: number, settings: CardBid2Settings): CardBid2State {
  const rng = mulberry32(seed);
  const deck = makeDeck(rng);
  return { deck, pos: 1, topCard: deck[0]!, coins: 50, bid: 0, round: 1, maxRounds: parseInt(settings.rounds, 10), phase: "betting", lastResult: null, revealedCard: null };
}
export function reducer(state: CardBid2State, action: CardBid2Action): CardBid2State {
...
export function isTerminal(state: CardBid2State): { score: number } | null {
