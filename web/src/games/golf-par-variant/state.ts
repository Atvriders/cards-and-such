...
export interface GolfParVariantSettings { dummy: boolean; }

export interface GolfParVariantState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type GolfParVariantAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { const r = cardRank(c); if (r <= 5) s += 5; else s += 1; }
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: GolfParVariantState, action: GolfParVariantAction): GolfParVariantState {
...
export function isTerminal(state: GolfParVariantState): { score: number } | null {
