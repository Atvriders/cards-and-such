  ...
  return (
    <div className="journal-wrap">
      ...
      <div className="journal-choices">
        {p.choices.map((c, i) => (
          <button key={i} className={`journal-choice${state.selected === i ? " selected" : ""}`} disabled={isResult} onClick={() => dispatch({ type:"choose", choice:i } as AloneAmongStarsCardAction)}>
      ...
      <div className="journal-actions">
        {isResult && <button className="journal-btn next" onClick={() => dispatch({ type:"next" } as AloneAmongStarsCardAction)}>{state.index + 1 >= state.prompts.length ? "Finish" : "Next Entry"}</button>}
      </div>
    </div>
  );
}
