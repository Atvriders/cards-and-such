...
export interface TomatoTossSettings { dummy: boolean; }
export interface TomatoTossItem { id: number; lane: number; ticksLeft: number; }
export interface TomatoTossState {
  rngSeed: number;
  items: TomatoTossItem[];
  nextId: number;
...
export type TomatoTossAction = { type: "tick" } | { type: "click"; id: number };

export function initialState(seed: number, _settings: TomatoTossSettings): TomatoTossState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, clicked: 0, missed: 0, phase: "playing" };
}

export function reducer(state: TomatoTossState, action: TomatoTossAction): TomatoTossState {
...
export function isTerminal(state: TomatoTossState): { score: number } | null {
