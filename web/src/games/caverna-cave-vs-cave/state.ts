...
export interface CavernaCaveVsCaveSettings { dummy: boolean; }
export interface CavernaCaveVsCaveState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type CavernaCaveVsCaveAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: CavernaCaveVsCaveSettings): CavernaCaveVsCaveState {
...
export function reducer(state: CavernaCaveVsCaveState, action: CavernaCaveVsCaveAction): CavernaCaveVsCaveState {
...
export function score(s: CavernaCaveVsCaveState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: CavernaCaveVsCaveState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
