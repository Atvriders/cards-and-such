...
export interface CometClickerSettings { dummy: boolean; }
...
export interface CometClickerState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type CometClickerAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CometClickerSettings): CometClickerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CometClickerState, action: CometClickerAction): CometClickerState {
...
export function isTerminal(state: CometClickerState): { score: number } | null {
