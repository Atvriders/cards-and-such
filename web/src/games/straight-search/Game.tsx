...
export function StraightSearchGame({ state, dispatch, onGameOver }: GameProps<StraightSearchState, StraightSearchSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          <div className="cm-result">{state.straightFlush ? "Straight flush! +250" : state.hadStraight ? "Straight! +200" : "No straight — 0"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as StraightSearchAction)}>{state.draw >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
