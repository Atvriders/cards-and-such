...
// Candy Friends Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface CandyFriendsMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface CandyFriendsMiniState {
  rngSeed: number;
  grid: Grid;
...
export type CandyFriendsMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: CandyFriendsMiniSettings): CandyFriendsMiniState {
...
export function reducer(state: CandyFriendsMiniState, action: CandyFriendsMiniAction): CandyFriendsMiniState {
...
export function isTerminal(state: CandyFriendsMiniState): { score: number } | null {
