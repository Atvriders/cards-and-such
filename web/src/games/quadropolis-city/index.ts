...
export const quadropolisCityPlugin: GamePlugin<QuadropolisCityState, QuadropolisCityAction, typeof settings> = {
  id: "quadropolis-city",
  title: "Quadropolis: City",
  category: "board",
  players: { min: 1, max: 1, multiplayer: false },
  description: "Tile placement: place 12 tiles on a 4x4 grid; score by adjacency.",
  howToPlay: "Quadropolis: City is a tile-placement game on a 4x4 grid. A randomized queue of 12 tiles is generated. Each turn the next tile from the queue is shown; click any empty cell to place it. Tile types are: Tower, Park, Shop, Factory, Harbor. Each orthogonal pair of same-type tiles scores +2. Same-type connected clusters of 3+ score a +4 bonus, clusters of 5+ score an additional +8. Strategy: place tiles next to existing same-type neighbors to grow clusters efficiently. Don't waste placements in isolated corners. The grid has 16 cells but you only place 12 tiles, so plan compact clusters.",
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as QuadropolisCitySettings),
  reducer,
  isTerminal,
  hint: (state: QuadropolisCityState): HintTarget | null => {
    const sel = tileHintSelector(state, "qdc-grid");
