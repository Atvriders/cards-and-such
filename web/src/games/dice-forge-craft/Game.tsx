...
export function diceForgeCraftGame({ state, dispatch, onGameOver }: GameProps<diceForgeCraftState, diceForgeCraftSettings>): JSX.Element {
      ...
      <div className="rw-grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 44px)` }}>
        {state.cells.map((filled, i) => (
          <button
            ...
            onClick={() => dispatch({ type: "mark", index: i } as diceForgeCraftAction)}
          >{filled ? state.cellValues[i] : ""}</button>
        ))}
      </div>
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as diceForgeCraftAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
