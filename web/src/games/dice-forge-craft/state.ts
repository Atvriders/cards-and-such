...
export interface diceForgeCraftSettings { dummy: boolean; }
export interface diceForgeCraftState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type diceForgeCraftAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: diceForgeCraftSettings): diceForgeCraftState {
  return {
    rngSeed: seed,
...
export function reducer(state: diceForgeCraftState, action: diceForgeCraftAction): diceForgeCraftState {
...
export function isTerminal(state: diceForgeCraftState): { score: number } | null {
