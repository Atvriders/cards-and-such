...
export interface MichiganNewmarketSettings { dummy: boolean; }
export interface MichiganNewmarketState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type MichiganNewmarketAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MichiganNewmarketSettings): MichiganNewmarketState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: MichiganNewmarketState, action: MichiganNewmarketAction): MichiganNewmarketState {
...
export function isTerminal(state: MichiganNewmarketState): { score: number } | null {
