...
const CincinnatiLamebrains = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CincinnatiLamebrains as unknown as React.ComponentType<unknown> })));
const settings = {
...
export const cincinnatiLamebrainsPlugin: GamePlugin<CincinnatiLamebrainsState, CincinnatiLamebrainsAction, typeof settings> = {
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-cincinnati-lamebrains-deal"]', pulses: 3 };
    if (state.phase === "dealt") return { selector: '[data-testid="hint-target-cincinnati-lamebrains-next"]', pulses: 3 };
    return null;
