// Auto-generated backgammon-family state for plakoto.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const POINTS = 24;
export const CHECKERS_PER_SIDE = 15;
export const HIT_ENABLED = false;
export const PIN_ENABLED = true;
...
const START_LAYOUT: ReadonlyArray<readonly [number, number]> = [[0,15]];

export type Side = "P" | "C";
export interface PlakotoSettings { dummy: boolean }

export interface PlakotoState {
  rngSeed: number;
...
export function initialState(seed: number, _s: PlakotoSettings): PlakotoState {
...
export function legalMoves(state: PlakotoState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: PlakotoState, side: Side, from: number, pips: number): PlakotoState | null {
  const next: PlakotoState = {
    ...state,
...
function checkWin(s: PlakotoState): PlakotoState {
...
function cpuTurn(state: PlakotoState): PlakotoState {
  let s = state;
...
export function reducer(state: PlakotoState, action: PlakotoAction): PlakotoState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: PlakotoState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: PlakotoState): { score: number } | null {
