...
export interface CribbageCrashSettings { dummy: boolean; }

export interface CribbageCrashState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type CribbageCrashAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: CribbageCrashSettings): CribbageCrashState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: CribbageCrashState, action: CribbageCrashAction): CribbageCrashState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<CribbageCrashState> = {};
...
export function isTerminal(state: CribbageCrashState): { score: number } | null {
