...
// Rocket Pop: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface RocketPopSettings { dummy: boolean; }

export interface RocketPopTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface RocketPopState {
  rngSeed: number;
  targets: RocketPopTarget[];
  nextId: number;
...
export type RocketPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: RocketPopSettings): RocketPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: RocketPopState, action: RocketPopAction): RocketPopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: RocketPopTarget[] = [];
...
export function isTerminal(state: RocketPopState): { score: number } | null {
