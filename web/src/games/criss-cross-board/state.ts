...
export interface CrissCrossBoardSettings { rounds: "10"; }

export interface CrissCrossBoardState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type CrissCrossBoardAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CrissCrossBoardSettings): CrissCrossBoardState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: CrissCrossBoardState, action: CrissCrossBoardAction): CrissCrossBoardState {
...
export function isTerminal(state: CrissCrossBoardState): { score: number } | null {
