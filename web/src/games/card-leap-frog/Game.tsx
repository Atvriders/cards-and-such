...
import "./Game.css";
export function CardLeapFrogGame({ state, dispatch, onGameOver }: GameProps<CardLeapFrogState, CardLeapFrogSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "result" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardLeapFrogAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
