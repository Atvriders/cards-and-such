    ...
    <div className="palindromequiz-wrap">
      ...
      <div className="palindromequiz-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type: "select", choice: i } as PalindromeQuizAction)}><span className="palindromequiz-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
