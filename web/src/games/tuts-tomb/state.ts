...
export interface TutsTombSettings { dummy: boolean; }

export interface TutsTombState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const ranks=h.map(cardRank); for (let i=0;i<ranks.length;i++){ for (let j=i+1;j<ranks.length;j++){ const a=ranks[i] as number; const b=ranks[j] as number; if (a+b===13) s += 8; } } if (s===0) s=1; return s;
}
...
export function reducer(state: TutsTombState, action: TutsTombAction): TutsTombState {
...
export function isTerminal(state: TutsTombState): { score: number } | null {
