...
// Latte Leap: targets drift across the screen in 6 lanes. Click each before it disappears for 10 points. 30-tick timer.
export const TIMER_TICKS = 30;
export const LANES = 6;
export const POINTS = 10;
export interface LatteLeapSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface LatteLeapState { rngSeed: number; targets: Target[]; nextId: number; ticksRemaining: number; score: number; clicked: number; missed: number; phase: "playing" | "done"; }
export type LatteLeapAction = { type: "tick" } | { type: "click"; id: number };
export function initialState(seed: number, _s: LatteLeapSettings): LatteLeapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, clicked: 0, missed: 0, phase: "playing" };
}
export function reducer(state: LatteLeapState, action: LatteLeapAction): LatteLeapState {
...
export function isTerminal(state: LatteLeapState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
