// Schieber Jass — Partnership Swiss Jass with pass-trump option — 1v1 duel here.
...
export type SchieberJassPhase = "playing" | "done";

export interface SchieberJassState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: SchieberJassPhase;
  playerLeads: boolean;
  message: string;
}

export type SchieberJassAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `schieber-jass-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: SchieberJassState, trick: readonly { seat: number; card: Card }[]): SchieberJassState {
...
export function reducer(state: SchieberJassState, action: SchieberJassAction): SchieberJassState {
  ...
  let s: SchieberJassState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: SchieberJassState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: SchieberJassState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): SchieberJassState {
...
export function isTerminal(state: SchieberJassState): { score: number } | null {
