...
const SchieberJassGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SchieberJassGame as unknown as React.ComponentType<unknown> })));
...
export const schieberJassPlugin: GamePlugin<SchieberJassState, SchieberJassAction, typeof settings> = {
  ...
  description: "Schieber Jass — partnership Jass.",
  howToPlay: "Schieber Jass — partnership Jass. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as SchieberJassSettings),
  reducer,
  isTerminal,
  hint: (state: SchieberJassState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-schjc-${pick.id}"]`, pulses: 3 };
    }
