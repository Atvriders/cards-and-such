...
export interface WelcomeToSuburbSettings { dummy: boolean; }
export interface WelcomeToSuburbState {
  rngSeed: number;
...
export type WelcomeToSuburbAction =
  | { type: "roll" }
  | { type: "place"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: WelcomeToSuburbSettings): WelcomeToSuburbState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: WelcomeToSuburbState, action: WelcomeToSuburbAction): WelcomeToSuburbState {
...
export function isTerminal(state: WelcomeToSuburbState): { score: number } | null {
