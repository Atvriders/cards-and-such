    ...
    return (
      <div className={`${P}-wrap fade-in`}>
        <div className={`${P}-final bounce-in`}>
          <h2 className={`${P}-final-title`}>Telestrations Quiz — Complete</h2>
    ...
    <div className={`${P}-wrap fade-in`}>
        ...
        {q.choices.map((c, i) => {
          ...
          return (
            <button
              ...
              onClick={() => dispatch({ type: "answer", choice: i, elapsedMs: Date.now() - start } as TelestrationsQuizAction)}
            >{c}</button>
          );
        })}
      </div>
      {state.phase === "feedback" && (
        <>
          ...
          <button className={`${P}-next`} type="button" onClick={() => dispatch({ type: "next" } as TelestrationsQuizAction)}>
