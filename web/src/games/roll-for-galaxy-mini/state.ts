...
export interface RollForGalaxyMiniSettings { dummy: boolean; }
export interface RollForGalaxyMiniState {
  rngSeed: number;
  round: number;
  rolls: number[];
  bonus: number;
...
export type RollForGalaxyMiniAction = { type: "roll" } | { type: "next" };
export function initialState(seed: number, _s: RollForGalaxyMiniSettings): RollForGalaxyMiniState {
...
export function reducer(state: RollForGalaxyMiniState, action: RollForGalaxyMiniAction): RollForGalaxyMiniState {
...
export function isTerminal(state: RollForGalaxyMiniState): { score: number } | null {
