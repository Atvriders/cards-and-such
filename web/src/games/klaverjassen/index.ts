...
const KlaverjassenGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.KlaverjassenGame as unknown as React.ComponentType<unknown> })));
...
export const klaverjassenPlugin: GamePlugin<KlaverjassenState, KlaverjassenAction, typeof settings> = {
  ...
  description: "Klaverjassen — Dutch Jass with trumps.",
  howToPlay: "Klaverjassen — Dutch Jass with trumps. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as KlaverjassenSettings),
  reducer,
  isTerminal,
  hint: (state: KlaverjassenState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-klavc-${pick.id}"]`, pulses: 3 };
    }
