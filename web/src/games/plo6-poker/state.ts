...
export interface Plo6PokerSettings { dummy: boolean; }
export interface Plo6PokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type Plo6PokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: Plo6PokerSettings): Plo6PokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: Plo6PokerState, action: Plo6PokerAction): Plo6PokerState {
...
export function isTerminal(state: Plo6PokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
