// Auto-generated race-game state for senet-mini.
...
export const SPECIALS: Record<number, Special> = {14: {type:"safe"},15: {type:"snake", to:6},16: {type:"safe"},17: {type:"safe"}};

export type Side = "P" | "C";
export interface SenetMiniSettings { dummy: boolean }

export interface SenetMiniState {
  rngSeed: number;
...
export function initialState(seed: number, _s: SenetMiniSettings): SenetMiniState {
  return {
    rngSeed: seed,
...
function checkWin(s: SenetMiniState): SenetMiniState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: SenetMiniState): SenetMiniState {
  let s = state;
...
export function legalMovesForP(state: SenetMiniState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: SenetMiniState, action: SenetMiniAction): SenetMiniState {
...
export function isTerminal(state: SenetMiniState): { score: number } | null {
