...
export function LiverpoolRGame({ state, dispatch, onGameOver }: GameProps<LiverpoolRState, LiverpoolRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as LiverpoolRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as LiverpoolRAction)}>Next</button>
      </>}
    </div>
  );
}
