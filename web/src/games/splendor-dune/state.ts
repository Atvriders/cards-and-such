...
export interface SplendorDuneState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type SplendorDuneAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: SplendorDuneSettings): SplendorDuneState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Spice harvest","Sandworm sighting","Fremen pact","Harkonnen raid","Trade route opens","Imperium edict","Shai-Hulud nears","Melange demand"];
...
export function reducer(state: SplendorDuneState, action: SplendorDuneAction): SplendorDuneState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Mentat (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
    ...
    if (pay > 0) evt += " | Mentat earned +$" + pay;
...
export function score(s: SplendorDuneState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: SplendorDuneState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
