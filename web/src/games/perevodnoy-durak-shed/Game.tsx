...
export function PerevodnoyDurakShedGame({ state, dispatch, onGameOver }: GameProps<PerevodnoyDurakShedState, PerevodnoyDurakShedSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as PerevodnoyDurakShedAction)}>Play Round</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PerevodnoyDurakShedAction)}>Next</button>
      </>}
    </div>
  );
}
