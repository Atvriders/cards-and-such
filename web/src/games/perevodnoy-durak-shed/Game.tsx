...
export function PerevodnoyDurakShedGame({ state, dispatch, onGameOver }: GameProps<PerevodnoyDurakShedState, PerevodnoyDurakShedSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap"><h3>Perevodnoy Durak</h3><div className="dm-done bounce-in"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
