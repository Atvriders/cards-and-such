...
export function RobinsonCrusoeCoopGame({ state, dispatch, onGameOver }: GameProps<RobinsonCrusoeCoopState, RobinsonCrusoeCoopSettings>): JSX.Element {
  ...
  const theme = "Shipwreck Survival";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      <div className="coop-info">🏝️ {theme} — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as RobinsonCrusoeCoopAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as RobinsonCrusoeCoopAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
