...
const VoliMariasGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.VoliMariasGame as unknown as React.ComponentType<unknown> })));
...
export const voliMariasPlugin: GamePlugin<VoliMariasState, VoliMariasAction, typeof settings> = {
  ...
  description: "Volí Mariáš — voluntary Mariáš.",
  howToPlay: "Volí Mariáš — voluntary Mariáš. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as VoliMariasSettings),
  reducer,
  isTerminal,
  hint: (state: VoliMariasState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-voli-m-${pick.id}"]`, pulses: 3 };
    }
