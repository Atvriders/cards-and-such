...
export function SecondChanceGridGame({ state, dispatch, onGameOver }: GameProps<SecondChanceGridState, SecondChanceGridSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as SecondChanceGridAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as SecondChanceGridAction)}>Skip</button>
      )}
    </div>
  );
}
