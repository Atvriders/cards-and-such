...
export function PodkidnoyDurakShedGame({ state, dispatch, onGameOver }: GameProps<PodkidnoyDurakShedState, PodkidnoyDurakShedSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap"><h3>Podkidnoy Durak</h3><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as PodkidnoyDurakShedAction)}>Play Round</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PodkidnoyDurakShedAction)}>Next</button>
      </>}
    </div>
  );
}
