...
export function RollingAmericaGame({ state, dispatch, onGameOver }: GameProps<RollingAmericaState, RollingAmericaSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RollingAmericaAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RollingAmericaAction)}>Skip</button>
      )}
    </div>
  );
}
