...
export interface DoubleBonusVpSettings { rounds: "10"; }

export interface DoubleBonusVpState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type DoubleBonusVpAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DoubleBonusVpSettings): DoubleBonusVpState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: DoubleBonusVpState, action: DoubleBonusVpAction): DoubleBonusVpState {
...
export function isTerminal(state: DoubleBonusVpState): { score: number } | null {
