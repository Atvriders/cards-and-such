...
export const SUIT_NAMES = ["Egypt","Greek","Roman","Phoenician","Mesopotamian"] as const;
...
export type DuelPantheonGodsState = DraftState;
export type DuelPantheonGodsAction = DraftAction;
export interface DuelPantheonGodsSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: DuelPantheonGodsSettings): DuelPantheonGodsState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: DuelPantheonGodsState, action: DuelPantheonGodsAction): DuelPantheonGodsState {
  return draftReducer(state, action);
}

export function score(state: DuelPantheonGodsState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: DuelPantheonGodsState): { score: number } | null {
