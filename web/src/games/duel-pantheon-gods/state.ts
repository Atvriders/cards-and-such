...
export interface DuelPantheonGodsState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type DuelPantheonGodsAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: DuelPantheonGodsSettings): DuelPantheonGodsState {
...
export function score(s: DuelPantheonGodsState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: DuelPantheonGodsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
