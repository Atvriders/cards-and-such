...
export function DuelPantheonGodsGame({ state, dispatch, onGameOver }: GameProps<DuelPantheonGodsState, DuelPantheonGodsSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">7 Wonders Duel: Pantheon</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as DuelPantheonGodsAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as DuelPantheonGodsAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
