...
const med20 = { difficulty: "medium" as const, questions: "20" as const };
const hard10 = { difficulty: "hard" as const, questions: "10" as const };

describe("OrderOfOps initialState", () => {
  ...
  it("same seed is deterministic", () => {
    const s1 = initialState(77, easy10);
    const s2 = initialState(77, easy10);
    expect(s1.questions[0]!.answer).toBe(s2.questions[0]!.answer);
  });

  it("generates medium questions", () => {
    const s = initialState(5, med20);
  ...
  it("generates hard questions", () => {
    const s = initialState(3, hard10);
    expect(s.questions.length).toBe(10);
  });
});

describe("OrderOfOps reducer", () => {
  it("type updates typed field", () => {
    const s = initialState(1, easy10);
    const s2 = reducer(s, { type: "type", text: "25" });
    expect(s2.typed).toBe("25");
  });
  ...
  it("wrong answer scores 0", () => {
    const s = initialState(1, easy10);
    const wrong = s.questions[0]!.answer + 9999;
...
describe("OrderOfOps isTerminal", () => {
