...
describe("PretzelPluck", () => {
  ...
  it("tick spawns pretzels", () => { const s = reducer(initialState(1, S), { type:"tick" }); expect(s.pretzels.length).toBeGreaterThanOrEqual(1); });
  it("pop removes pretzel and adds 10 points", () => {
    let s = initialState(1, S);
    s = reducer(s, { type:"tick" });
    const id = s.pretzels[0]!.id;
