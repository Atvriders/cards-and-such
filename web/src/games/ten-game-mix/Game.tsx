...
import "./Game.css";
export function TenGameMixGame({ state, dispatch, onGameOver }: GameProps<TenGameMixState, TenGameMixSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-ten-game-mix-deal" onClick={() => dispatch({ type: "deal" } as TenGameMixAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-ten-game-mix-next" onClick={() => dispatch({ type: "next" } as TenGameMixAction)}>Next</button>
      </>}
    </div>
  );
}
