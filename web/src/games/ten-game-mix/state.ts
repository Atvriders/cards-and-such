...
export interface TenGameMixSettings { dummy: boolean; }
export interface TenGameMixState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TenGameMixAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TenGameMixSettings): TenGameMixState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TenGameMixState, action: TenGameMixAction): TenGameMixState {
...
export function isTerminal(state: TenGameMixState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
