...
import type { ThreeCheckState, ThreeCheckSettings, ChessCoord, ChessMove } from "./state.js";
import type { ThreeCheckAction } from "./state.js";
...
export function ThreeCheckGame({
  state,
  dispatch,
  onGameOver,
}: GameProps<ThreeCheckState, ThreeCheckSettings>): JSX.Element {
  ...
  function handleClick(r: number, c: number) {
    ...
    if (selected && legalTargets.has(key)) {
      dispatch({ type: "move", from: selected, to: coord } as ThreeCheckAction);
      setSelected(null);
      setLegalTargets(new Set());
      return;
    }
  ...
  function squareClass(r: number, c: number): string {
    const light = (r + c) % 2 === 0;
    const base = light ? "tc-sq-light" : "tc-sq-dark";
    const key = `${r},${c}`;
  ...
  const wChecks = state.checksGiven.white;
  const bChecks = state.checksGiven.black;

  function checkDots(n: number, color: string) {
    return Array.from({ length: 3 }, (_, i) => (
      <span key={i} style={{ width: 14, height: 14, borderRadius: "50%", background: i < n ? color : "#ddd", display: "inline-block", margin: "0 2px" }} />
    ));
  }
  ...
  return (
    <div className="tc-wrap">
      <div className="tc-info">
        <span>Three-Check Chess</span>
        <span>You: White | Bot: Black ({state.settings.opponent})</span>
      </div>
      <div className="tc-checks">
        <div>Your checks: {checkDots(wChecks, "#27ae60")} ({wChecks}/3)</div>
        <div>Bot checks: {checkDots(bChecks, "#e74c3c")} ({bChecks}/3)</div>
      </div>
      <div className={`tc-status ${statusClass}`}>{statusText}</div>
        ...
        {Array.from({ length: 8 }, (_, r) =>
          Array.from({ length: 8 }, (_, c) => {
            ...
            return (
              <div
                key={`${r}-${c}`}
                className={`tc-cell ${squareClass(r, c)}`}
              ...
              >
                {piece && <span className="tc-piece">{PIECE_UNICODE[piece.color][piece.type]}</span>}
      ...
      {state.promotionPending && (
        <div className="tc-promo-overlay">
          <div className="tc-promo-box">
            <h3>Promote pawn</h3>
            <div className="tc-promo-choices">
              {(["queen","rook","bishop","knight"] as PieceType[]).map(pt => (
                <button key={pt} className="tc-promo-btn"
                  onClick={() => dispatch({ type: "promote", piece: pt as "queen"|"rook"|"bishop"|"knight" } as ThreeCheckAction)}>
