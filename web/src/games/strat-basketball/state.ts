...
export interface StratBasketballSettings { dummy: boolean; }

export interface StratBasketballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type StratBasketballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: StratBasketballSettings): StratBasketballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: StratBasketballState, action: StratBasketballAction): StratBasketballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<StratBasketballState> = {};
...
export function isTerminal(state: StratBasketballState): { score: number } | null {
