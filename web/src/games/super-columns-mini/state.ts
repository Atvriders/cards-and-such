...
// Super Columns Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface SuperColumnsMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface SuperColumnsMiniState {
  rngSeed: number;
  grid: Grid;
...
export type SuperColumnsMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: SuperColumnsMiniSettings): SuperColumnsMiniState {
...
export function reducer(state: SuperColumnsMiniState, action: SuperColumnsMiniAction): SuperColumnsMiniState {
...
export function isTerminal(state: SuperColumnsMiniState): { score: number } | null {
