...
export interface CarltonPatienceSettings { dummy: boolean; }

export interface CarltonPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type CarltonPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const dia = h.filter(c=>cardSuit(c)===2).length; s += dia*8; const f = h.filter(c=>cardRank(c)>=11).length; s += f*3; if (s===0) s=1; return s;
}
...
export function reducer(state: CarltonPatienceState, action: CarltonPatienceAction): CarltonPatienceState {
...
export function isTerminal(state: CarltonPatienceState): { score: number } | null {
