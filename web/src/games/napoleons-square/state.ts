...
export interface NapoleonsSquareSettings { dummy: boolean; }

export interface NapoleonsSquareState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type NapoleonsSquareAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const ranks=h.map(cardRank); const counts:Record<number,number>={}; for (const r of ranks) counts[r]=(counts[r]||0)+1; const trips = Object.values(counts).filter(v=>v>=3).length; s += trips*15; const pairs = Object.values(counts).filter(v=>v===2).length; s += pairs*7; if (s===0) s=1; return s;
}
...
export function reducer(state: NapoleonsSquareState, action: NapoleonsSquareAction): NapoleonsSquareState {
...
export function isTerminal(state: NapoleonsSquareState): { score: number } | null {
