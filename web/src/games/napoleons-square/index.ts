...
const NapoleonsSquareGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.NapoleonsSquareGame as unknown as React.ComponentType<unknown> })));
...
export const napoleonsSquarePlugin: GamePlugin<NapoleonsSquareState, NapoleonsSquareAction, typeof settings> = {
  ...
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as unknown as NapoleonsSquareSettings),
  reducer,
  isTerminal,
  hint: (state: NapoleonsSquareState): HintTarget | null => {
    const FOUNDATION_IDS = ["f1","f2","f3","f4","f5","f6","f7","f8"];
    const TABLEAU_IDS = ["t1","t2","t3","t4","t5","t6","t7","t8","t9","t10","t11","t12"];
