...
import "./Game.css";
export function EightGameMixGame({ state, dispatch, onGameOver }: GameProps<EightGameMixState, EightGameMixSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-eight-game-mix-deal" onClick={() => dispatch({ type: "deal" } as EightGameMixAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-eight-game-mix-next" onClick={() => dispatch({ type: "next" } as EightGameMixAction)}>Next</button>
      </>}
    </div>
  );
}
