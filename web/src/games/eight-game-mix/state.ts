...
export interface EightGameMixSettings { dummy: boolean; }
export interface EightGameMixState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type EightGameMixAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: EightGameMixSettings): EightGameMixState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: EightGameMixState, action: EightGameMixAction): EightGameMixState {
...
export function isTerminal(state: EightGameMixState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
