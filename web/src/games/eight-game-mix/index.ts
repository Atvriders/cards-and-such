...
const EightGameMixGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.EightGameMixGame as unknown as React.ComponentType<unknown> })));
...
export const eightGameMixPlugin: GamePlugin<EightGameMixState, EightGameMixAction, typeof settings> = {
  ...
  reducer,isTerminal,  hint: (state: EightGameMixState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-eight-game-mix-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-eight-game-mix-next"]', pulses: 3 };
    return null;
  },
  component:EightGameMixGame,
};
