...
export interface AnacondaPokerSettings { dummy: boolean; }
export interface AnacondaPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type AnacondaPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AnacondaPokerSettings): AnacondaPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: AnacondaPokerState, action: AnacondaPokerAction): AnacondaPokerState {
...
export function isTerminal(state: AnacondaPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
