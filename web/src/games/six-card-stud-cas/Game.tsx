...
export function SixCardStudCasGame({ state, dispatch, onGameOver }: GameProps<SixCardStudCasState, SixCardStudCasSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap thm6StudCas"><div className="dm-done"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap thm6StudCas">
      ...
      {state.phase === "see" && <div className="dm-row">
        <button data-testid="hint-target-six-card-stud-cas-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as SixCardStudCasAction)}>Play</button>
      ...
      {state.phase === "scored" && <>
        ...
        <button data-testid="hint-target-six-card-stud-cas-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SixCardStudCasAction)}>Next</button>
      </>}
    </div>
  );
}
