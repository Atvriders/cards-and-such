...
// Cave Runner: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface CaveRunnerSettings { dummy: boolean; }

export interface CaveRunnerTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface CaveRunnerState {
  rngSeed: number;
  targets: CaveRunnerTarget[];
  nextId: number;
...
export type CaveRunnerAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: CaveRunnerSettings): CaveRunnerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: CaveRunnerState, action: CaveRunnerAction): CaveRunnerState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CaveRunnerTarget[] = [];
...
export function isTerminal(state: CaveRunnerState): { score: number } | null {
