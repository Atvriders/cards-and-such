    ...
    <div className="torchq-wrap">
      ...
      <div className="torchq-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", choice:i } as ToroidalChessQuizAction)}><span className="torchq-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
