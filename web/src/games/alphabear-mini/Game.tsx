...
export function AlphabearMiniGame({ state, dispatch, onGameOver }: GameProps<AlphabearMiniState, AlphabearMiniSettings>): JSX.Element {
    ...
    <div className="word-wrap">
      ...
      <div className="word-choices">
        {r.choices.map((choice, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type: "select", choice: i } as AlphabearMiniAction)}>
      ...
      <div className="word-actions">
        {!isResult && (
          <button className="word-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as AlphabearMiniAction)}>Submit</button>
        )}
        {isResult && (
          <button className="word-btn next" onClick={() => dispatch({ type: "next" } as AlphabearMiniAction)}>
