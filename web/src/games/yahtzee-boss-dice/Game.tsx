...
import "./Game.css";
export function YahtzeeBossDiceGame({ state, dispatch, onGameOver }: GameProps<YahtzeeBossDiceState, YahtzeeBossDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-yahtzee-boss-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as YahtzeeBossDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-yahtzee-boss-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as YahtzeeBossDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
