...
export interface PapayaPopSettings { dummy: boolean; }
...
export interface PapayaPopState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PapayaPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PapayaPopSettings): PapayaPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PapayaPopState, action: PapayaPopAction): PapayaPopState {
...
export function isTerminal(state: PapayaPopState): { score: number } | null {
