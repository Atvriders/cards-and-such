...
export interface GipfOriginalSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface GipfOriginalState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type GipfOriginalAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: GipfOriginalSettings): GipfOriginalState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: GipfOriginalState, action: GipfOriginalAction): GipfOriginalState {
...
export function isTerminal(state: GipfOriginalState): { score: number } | null {
