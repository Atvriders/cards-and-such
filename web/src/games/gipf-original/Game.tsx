...
import "./Game.css";
export function GipfOriginalGame({ state, dispatch, onGameOver }: GameProps<GipfOriginalState, GipfOriginalSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") {
    const msg = state.result === "P" ? "You won!" : state.result === "C" ? "CPU won!" : "Draw";
    return <div className="ab-wrap"><h3>Gipf Original</h3><div className="ab-done"><h2>{msg}</h2><div className="ab-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="ab-wrap">
      ...
      <div className="ab-board" style={{ gridTemplateColumns: `repeat(${SIZE}, 1fr)` }}>
        {Array.from({ length: SIZE * SIZE }).map((_, i) => {
          ...
          return <button key={i} className={`ab-cell ${cls}`} disabled={v !== null} onClick={() => dispatch({ type:"place", idx:i } as GipfOriginalAction)}>{v ?? ""}</button>;
        })}
      </div>
    </div>
  );
