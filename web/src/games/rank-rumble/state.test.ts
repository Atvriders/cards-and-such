...
import { initialState, reducer, isTerminal, TOTAL_ROUNDS, maxSameRank } from "./state.js";
const S = { dummy: false };
describe("RankRumble", () => {
  ...
  it("maxSameRank handles 4-of-a-kind", () => {
    expect(maxSameRank([0, 13, 26, 39])).toBe(4);
    expect(maxSameRank([0, 1, 2, 3])).toBe(1);
  });
});
