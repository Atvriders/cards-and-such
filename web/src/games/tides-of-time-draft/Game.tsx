...
export function TidesOfTimeGame({ state, dispatch, onGameOver }: GameProps<TidesOfTimeState, TidesOfTimeSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Tides of Time</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as TidesOfTimeAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as TidesOfTimeAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
