...
export const SUIT_NAMES = ["Royalty","Religion","Military","Culture","Mystic"] as const;
...
export type TidesOfTimeState = DraftState;
export type TidesOfTimeAction = DraftAction;
export interface TidesOfTimeSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: TidesOfTimeSettings): TidesOfTimeState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: TidesOfTimeState, action: TidesOfTimeAction): TidesOfTimeState {
  return draftReducer(state, action);
}

export function score(state: TidesOfTimeState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: TidesOfTimeState): { score: number } | null {
