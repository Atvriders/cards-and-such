...
export interface TidesOfTimeState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type TidesOfTimeAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: TidesOfTimeSettings): TidesOfTimeState {
...
export function score(s: TidesOfTimeState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: TidesOfTimeState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
