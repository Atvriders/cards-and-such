...
export interface HoneymoonBridgeState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type HoneymoonBridgeAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: HoneymoonBridgeSettings): HoneymoonBridgeState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: HoneymoonBridgeState, action: HoneymoonBridgeAction): HoneymoonBridgeState {
...
export function isTerminal(state: HoneymoonBridgeState): { score: number } | null {
