...
const HoneymoonBridgeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.HoneymoonBridgeGame as unknown as React.ComponentType<unknown> })));
...
export const honeymoonBridgePlugin: GamePlugin<HoneymoonBridgeState, HoneymoonBridgeAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: HoneymoonBridgeState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-honeyb-${pick.id}"]`, pulses: 3 };
    }
