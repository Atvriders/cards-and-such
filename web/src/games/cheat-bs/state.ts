...
export interface CheatBsSettings { rounds: "10"; }

export interface CheatBsRound {
...
export type CheatBsAction =
  | { type: "decide"; decision: "trust" | "callBluff" }
  | { type: "next" };

const ITEMS: string[] = ["Two","Three","Four","Five","Six","Seven","Eight","Nine","Ten","Jack","Queen","King","Ace"];
...
export function initialState(seed: number, _settings: CheatBsSettings): CheatBsState {
  const rng = mulberry32(seed);
  const rounds: CheatBsRound[] = [];
  for (let i = 0; i < 10; i++) {
