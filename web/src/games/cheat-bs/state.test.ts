...
import { initialState, reducer, isTerminal, CheatBs_CFG } from "./state.js";
...
describe("cheat-bs", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(CheatBs_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < CheatBs_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < CheatBs_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
