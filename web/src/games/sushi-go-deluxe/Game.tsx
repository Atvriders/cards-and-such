...
export function SushiGoDeluxeGame({ state, dispatch, onGameOver }: GameProps<SushiGoDeluxeState, SushiGoDeluxeSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Sushi Go Deluxe</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SushiGoDeluxeAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SushiGoDeluxeAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
