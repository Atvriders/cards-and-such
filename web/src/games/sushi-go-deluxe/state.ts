...
export type SushiGoDeluxeAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SushiGoDeluxeSettings): SushiGoDeluxeState {
...
export function score(s: SushiGoDeluxeState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SushiGoDeluxeState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
