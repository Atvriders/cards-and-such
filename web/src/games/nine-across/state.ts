...
export interface NineAcrossSettings { dummy: boolean; }

export interface NineAcrossState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const total = h.reduce((a,c)=>a+cardRank(c),0); if (total===9 || total%9===0) s += 12; const ranks=h.map(cardRank); const counts:Record<number,number>={}; for (const r of ranks) counts[r]=(counts[r]||0)+1; const pairs = Object.values(counts).filter(v=>v>=2).length; s += pairs*5; if (s===0) s=1; return s;
}
...
export function reducer(state: NineAcrossState, action: NineAcrossAction): NineAcrossState {
...
export function isTerminal(state: NineAcrossState): { score: number } | null {
