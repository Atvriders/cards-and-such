...
const FiveOPokerGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FiveOPokerGame as unknown as React.ComponentType<unknown> })));
...
export const fiveOPokerPlugin: GamePlugin<FiveOPokerState, FiveOPokerAction, typeof settings> = {
  ...
  hint: (state: FiveOPokerState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-five-o-poker-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-five-o-poker-next"]', pulses: 3 };
    return null;
  },
};
