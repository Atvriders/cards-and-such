...
export interface FiveOPokerSettings { dummy: boolean; }
export interface FiveOPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FiveOPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FiveOPokerSettings): FiveOPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FiveOPokerState, action: FiveOPokerAction): FiveOPokerState {
...
export function isTerminal(state: FiveOPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
