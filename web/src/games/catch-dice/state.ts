...
export const PAYOUTS: number[] = [18,9,30];
export const CHOICES: string[] = ["Easy 2-5 (+18)","Medium 6-9 (+9)","Hard 10-12 (+30)"];
export interface CatchDiceSettings { dummy: boolean; }
export interface CatchDiceState {
  rngSeed: number;
  round: number;
...
export type CatchDiceAction =
  | { type: "predict"; choice: number }
  | { type: "next" };
export function initialState(seed: number, _settings: CatchDiceSettings): CatchDiceState {
  return { rngSeed: seed, round: 1, prediction: null, dice: null, resultIdx: null, score: 0, phase: "predict" };
}
export function reducer(state: CatchDiceState, action: CatchDiceAction): CatchDiceState {
  ...
  if (action.type === "predict") {
    ...
    const resultIdx = (()=>{ const s=a+b; if (s<=5) return 0; if (s<=9) return 1; return 2; })();
...
export function isTerminal(state: CatchDiceState): { score: number } | null {
