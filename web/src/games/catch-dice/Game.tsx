...
import "./Game.css";
export function CatchDiceGame({ state, dispatch, onGameOver }: GameProps<CatchDiceState, CatchDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-catch-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as CatchDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-catch-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as CatchDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
