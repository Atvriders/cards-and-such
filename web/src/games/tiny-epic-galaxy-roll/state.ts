...
export interface TinyEpicGalaxyRollSettings { rounds: "10"; }

export interface TinyEpicGalaxyRollState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type TinyEpicGalaxyRollAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: TinyEpicGalaxyRollSettings): TinyEpicGalaxyRollState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: TinyEpicGalaxyRollState, action: TinyEpicGalaxyRollAction): TinyEpicGalaxyRollState {
...
export function isTerminal(state: TinyEpicGalaxyRollState): { score: number } | null {
