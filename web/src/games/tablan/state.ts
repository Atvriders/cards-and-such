...
export interface TablanSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface TablanState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type TablanAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: TablanSettings): TablanState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: TablanState, action: TablanAction): TablanState {
...
export function isTerminal(state: TablanState): { score: number } | null {
