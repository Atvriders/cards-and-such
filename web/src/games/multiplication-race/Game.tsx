...
export function MultiplicationRaceGame({ state, dispatch, onGameOver }: GameProps<MultiplicationRaceState, MultiplicationRaceSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as MultiplicationRaceAction), 1000);
  ...
  return (
    <div className="mr-wrap">
      ...
      <div className="mr-choices">
        {q.choices.map((c, i) => (
          <button key={i} className="mr-choice" onClick={() => dispatch({ type: "answer", choice: i } as MultiplicationRaceAction)}>{c}</button>
        ))}
      </div>
    </div>
  );
