...
export interface GolfParSettings { dummy: boolean; }

export interface GolfParState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export function reducer(state: GolfParState, action: GolfParAction): GolfParState {
...
export function isTerminal(state: GolfParState): { score: number } | null {
