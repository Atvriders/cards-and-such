...
export interface SpiritOfForestSettings { dummy: boolean; }
export interface SpiritOfForestState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type SpiritOfForestAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: SpiritOfForestSettings): SpiritOfForestState {
...
export function reducer(state: SpiritOfForestState, action: SpiritOfForestAction): SpiritOfForestState {
...
export function score(s: SpiritOfForestState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: SpiritOfForestState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
