...
export interface ScalaFortyRSettings { dummy: boolean; }
export interface ScalaFortyRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type ScalaFortyRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: ScalaFortyRSettings): ScalaFortyRState {
...
export function reducer(state: ScalaFortyRState, action: ScalaFortyRAction): ScalaFortyRState {
...
export function isTerminal(state: ScalaFortyRState): { score: number } | null {
