...
const ScopaDiQuindiciGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ScopaDiQuindiciGame as unknown as React.ComponentType<unknown> })));
...
export const scopaDiQuindiciPlugin: GamePlugin<ScopaDiQuindiciState, ScopaDiQuindiciAction, typeof settings> = {
  ...
  description: "Scopa di Quindici — 15-fishing variant.",
  howToPlay: "Scopa di Quindici — 15-fishing variant. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as ScopaDiQuindiciSettings),
  reducer,
  isTerminal,
  hint: (state: ScopaDiQuindiciState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-sq15c-${pick.id}"]`, pulses: 3 };
    }
