...
export function SpeedPairsGame({ state, dispatch, onGameOver }: GameProps<SpeedPairsState, SpeedPairsSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="spdpair-wrap"><div className="spdpair-done"><h2>Done!</h2><div>Correct: {state.correctCount} / {state.rounds.length}</div><div className="spdpair-final">{state.score} pts</div></div></div>;
  }
  const r = state.rounds[state.currentIndex]!;
  return (
    <div className="spdpair-wrap">
      <div className="spdpair-info">Round {state.currentIndex + 1} / {state.rounds.length}</div>
      <div className="spdpair-score">{state.score} pts</div>
