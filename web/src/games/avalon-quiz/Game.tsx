...
export function AvalonQuizGame({ state, dispatch, onGameOver }: GameProps<AvalonQuizState, AvalonQuizSettings>): JSX.Element {
    ...
    <div className="quiz-wrap">
      ...
      <div className="quiz-choices">
        {q.choices.map((choice, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type: "select", choice: i } as AvalonQuizAction)}>
      ...
      <div className="quiz-actions">
        {!isResult && (
          <button className="quiz-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as AvalonQuizAction)}>Submit</button>
        )}
        {isResult && (
          <button className="quiz-btn next" onClick={() => dispatch({ type: "next" } as AvalonQuizAction)}>
