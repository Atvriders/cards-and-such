...
export interface MagicMazeMaxSecuritySettings { dummy: boolean; }
export interface MagicMazeMaxSecurityState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type MagicMazeMaxSecurityAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: MagicMazeMaxSecuritySettings): MagicMazeMaxSecurityState {
