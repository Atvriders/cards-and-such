...
export function DiceFlush({ state, dispatch, onGameOver }: GameProps<DiceFlushState, DiceFlushSettings>): JSX.Element {
  ...
  return <div className="dice-game-wrap">
    <div className="dice-game-header"><span>Round {state.round}/{state.maxRounds}</span><span>Total: {state.totalScore} | Rerolls: {state.maxRerolls-state.rerolls} left</span></div>
    ...
    {!isResult&&<div style={{display:"flex",gap:"12px"}}>
      <button className="dice-btn" disabled={state.rerolls>=state.maxRerolls} onClick={()=>dispatch({type:"roll"} as DiceFlushAction)}>Reroll Unkept</button>
      <button className="dice-btn" style={{background:"#27ae60"}} onClick={()=>dispatch({type:"score"} as DiceFlushAction)}>Score ({pending} pts)</button>
    </div>}
    {isResult&&<div>
      <p style={{fontWeight:700,fontSize:"1.3rem"}}>Round score: {state.score} pts</p>
      <button className="dice-btn" onClick={()=>dispatch({type:"next"} as DiceFlushAction)}>Next Round</button>
    </div>}
  </div>;
}
