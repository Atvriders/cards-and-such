...
const ApophisSoliGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ApophisSoliGame as unknown as React.ComponentType<unknown> })));
...
export const apophisSoliPlugin: GamePlugin<ApophisSoliState, ApophisSoliAction, typeof settings> = {
  ...
  hint: (state: ApophisSoliState): HintTarget | null => {
    ...
    if (king) return { selector: `[data-testid="hint-target-apophis-soli-pyramid-${king.row}-${king.col}"]`, pulses: 3 };
    for (const a of avail) {
      for (const b of avail) {
        if (a === b) continue;
        if (a.rank + b.rank === 13) {
          return { selector: `[data-testid="hint-target-apophis-soli-pyramid-${a.row}-${a.col}"]`, pulses: 3 };
        }
      }
    }
    ...
    if (wasteTop) {
      ...
      if (m) return { selector: `[data-testid="hint-target-apophis-soli-pyramid-${m.row}-${m.col}"]`, pulses: 3 };
    }
    if (state.stock.length > 0) return { selector: '[data-testid="hint-target-apophis-soli-draw"]', pulses: 3 };
    if (state.redealsRemaining > 0) return { selector: '[data-testid="hint-target-apophis-soli-redeal"]', pulses: 3 };
    return null;
  },
  reducer,
  isTerminal,
