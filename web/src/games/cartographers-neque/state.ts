...
export interface cartographersNequeSettings { dummy: boolean; }
export interface cartographersNequeState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type cartographersNequeAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: cartographersNequeSettings): cartographersNequeState {
  return {
    rngSeed: seed,
...
export function reducer(state: cartographersNequeState, action: cartographersNequeAction): cartographersNequeState {
...
export function isTerminal(state: cartographersNequeState): { score: number } | null {
