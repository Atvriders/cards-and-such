...
export function GoBoomGame({ state, dispatch, onGameOver }: GameProps<GoBoomState, GoBoomSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as GoBoomAction)}>Boom!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as GoBoomAction)}>Next</button>
      </>}
    </div>
  );
}
