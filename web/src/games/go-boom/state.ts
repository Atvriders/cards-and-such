...
export interface GoBoomSettings { dummy: boolean; }
export interface GoBoomState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type GoBoomAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: GoBoomSettings): GoBoomState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: GoBoomState, action: GoBoomAction): GoBoomState {
...
export function isTerminal(state: GoBoomState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
