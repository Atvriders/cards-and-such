...
export interface CardShoeStoreSettings { dummy: boolean; }
export interface CardShoeStoreState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardShoeStoreAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardShoeStoreSettings): CardShoeStoreState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardShoeStoreState, action: CardShoeStoreAction): CardShoeStoreState {
...
export function isTerminal(state: CardShoeStoreState): { score: number } | null {
