...
import "./Game.css";
export function CardShoeStoreGame({ state, dispatch, onGameOver }: GameProps<CardShoeStoreState, CardShoeStoreSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "drawing" && (
        <button data-testid="hint-target-card-shoe-store-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as CardShoeStoreAction)}>Draw Card</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardShoeStoreAction)}>{state.drawCount >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
