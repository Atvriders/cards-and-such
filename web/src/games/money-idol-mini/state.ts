...
// Money Idol Mini: 6x6 match-3 with 60s timer.
...
export interface MoneyIdolMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface MoneyIdolMiniState {
  rngSeed: number;
  grid: Grid;
...
export type MoneyIdolMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: MoneyIdolMiniSettings): MoneyIdolMiniState {
...
export function reducer(state: MoneyIdolMiniState, action: MoneyIdolMiniAction): MoneyIdolMiniState {
...
export function isTerminal(state: MoneyIdolMiniState): { score: number } | null {
