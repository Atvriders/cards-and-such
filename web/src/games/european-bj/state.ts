...
export interface EuropeanBjState { rngSeed: number; round: number; you: number[]; dealer: number[]; phase: "play" | "scored" | "done"; score: number; pts: number; result: string; yourTotal: number; dealerTotal: number; }
export type EuropeanBjAction = { type: "hit" } | { type: "stand" } | { type: "next" };
