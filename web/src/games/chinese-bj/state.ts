...
export interface ChineseBjSettings { dummy: boolean; }
export interface ChineseBjState { rngSeed: number; round: number; hand: number[]; total: number; phase: "play" | "scored" | "done"; score: number; pts: number; result: string; }
export type ChineseBjAction = { type: "hit" } | { type: "stand" } | { type: "next" };
...
export function initialState(seed: number, _s: ChineseBjSettings): ChineseBjState {
...
export function reducer(state: ChineseBjState, action: ChineseBjAction): ChineseBjState {
...
export function isTerminal(state: ChineseBjState): { score: number } | null {
