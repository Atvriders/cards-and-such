...
export interface CandyGrabSettings { duration: "20" | "30" | "45"; }
export interface FallingItem { id: number; x: number; y: number; speed: number; points: number; }
export interface CandyGrabState {
...
export type CandyGrabAction = { type: "catch"; id: number } | { type: "tick" } | { type: "spawn" };

export function initialState(seed: number, settings: CandyGrabSettings): CandyGrabState {
...
export function reducer(state: CandyGrabState, action: CandyGrabAction): CandyGrabState {
...
export function isTerminal(state: CandyGrabState): { score: number } | null {
