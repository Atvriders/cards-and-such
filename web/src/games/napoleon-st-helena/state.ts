...
export interface NapoleonStHelenaSettings { dummy: boolean; }

export interface NapoleonStHelenaState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type NapoleonStHelenaAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank).sort((a,b)=>a-b); let run=1, best=1; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; if (a===b+1){run++; if(run>best)best=run;} else if (a!==b) run=1; } s += best*5; const suits = new Set(h.map(cardSuit)); if (suits.size===1) s += 12; if (s===0) s=1; return s;
}
...
export function reducer(state: NapoleonStHelenaState, action: NapoleonStHelenaAction): NapoleonStHelenaState {
...
export function isTerminal(state: NapoleonStHelenaState): { score: number } | null {
