// Bid Whist — Partnership Whist with bidding for trump — trimmed to a duel.
...
export type BidWhistPhase = "playing" | "done";
...
export type BidWhistAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `bid-whist-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: BidWhistState, trick: readonly { seat: number; card: Card }[]): BidWhistState {
...
export function reducer(state: BidWhistState, action: BidWhistAction): BidWhistState {
  ...
  let s: BidWhistState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: BidWhistState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: BidWhistState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): BidWhistState {
...
export function isTerminal(state: BidWhistState): { score: number } | null {
