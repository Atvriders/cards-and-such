...
export function JewelQuestArcadeGame({ state, dispatch, onGameOver }: GameProps<JewelQuestArcadeState, JewelQuestArcadeSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as JewelQuestArcadeAction), 1000);
    ...
    <div className="m3-wrap">
      ...
      <div className="m3-grid">
        {state.grid.map((row, r) => row.map((g, c) => {
          ...
          return (
            <button data-testid="hint-target-jewel-quest-arcade-action" key={`${r}-${c}`} className={`m3-cell${sel ? " sel" : ""}`}
              onClick={() => dispatch({ type: "select", row: r, col: c } as JewelQuestArcadeAction)}
