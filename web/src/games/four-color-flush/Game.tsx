...
export function FourColorFlushGame({ state, dispatch, onGameOver }: GameProps<FourColorFlushState, FourColorFlushSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          <div className="cm-result">{state.hasFlush ? "Four-color flush! +100" : "No flush — 0"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as FourColorFlushAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
