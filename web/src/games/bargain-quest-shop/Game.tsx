...
import "./Game.css";
export function BargainQuestShopGame({ state, dispatch, onGameOver }: GameProps<BargainQuestShopState, BargainQuestShopSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "drawing" && (
        <button data-testid="hint-target-bargain-quest-shop-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as BargainQuestShopAction)}>Draw</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as BargainQuestShopAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
