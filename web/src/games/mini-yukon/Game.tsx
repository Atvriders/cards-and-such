...
export function MiniYukonGame({ state, dispatch, onGameOver }: GameProps<MiniYukonState, MiniYukonSettings>): JSX.Element {
    ...
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.layout.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "remove", index: i } as MiniYukonAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
