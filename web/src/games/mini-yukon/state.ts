...
export interface MiniYukonSettings { dummy: boolean; }

export interface MiniYukonState {
  rngSeed: number;
...
export type MiniYukonAction = { type: "remove"; index: number } | { type: "noop" };
...
export function initialState(seed: number, _settings: MiniYukonSettings): MiniYukonState {
...
export function reducer(state: MiniYukonState, action: MiniYukonAction): MiniYukonState {
...
export function isTerminal(state: MiniYukonState): { score: number } | null {
