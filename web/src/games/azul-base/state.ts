...
export const TYPE_NAMES = ["Blue","Yellow","Red","Black","Cyan"] as const;
...
export type AzulBaseState = TileState;
export type AzulBaseAction = TileAction;
export interface AzulBaseSettings { dummy: boolean }

export function typeName(t: number): string { return TYPE_NAMES[t] ?? "?"; }

export function initialState(seed: number, _s: AzulBaseSettings): AzulBaseState {
  void mulberry32;
  return initialTileState(seed, CONFIG);
}

export function reducer(state: AzulBaseState, action: AzulBaseAction): AzulBaseState {
  return tileReducer(state, action);
}

export function isTerminal(state: AzulBaseState): { score: number } | null {
