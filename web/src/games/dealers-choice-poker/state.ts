...
export interface DealersChoicePokerSettings { dummy: boolean; }
export interface DealersChoicePokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DealersChoicePokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DealersChoicePokerSettings): DealersChoicePokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DealersChoicePokerState, action: DealersChoicePokerAction): DealersChoicePokerState {
...
export function isTerminal(state: DealersChoicePokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
