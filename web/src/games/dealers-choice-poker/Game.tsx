...
import "./Game.css";
export function DealersChoicePokerGame({ state, dispatch, onGameOver }: GameProps<DealersChoicePokerState, DealersChoicePokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-dealers-choice-poker-deal" onClick={() => dispatch({ type: "deal" } as DealersChoicePokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-dealers-choice-poker-next" onClick={() => dispatch({ type: "next" } as DealersChoicePokerAction)}>Next</button>
      </>}
    </div>
  );
}
