...
export interface PonnukiSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface PonnukiState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type PonnukiAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: PonnukiSettings): PonnukiState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: PonnukiState, action: PonnukiAction): PonnukiState {
...
export function isTerminal(state: PonnukiState): { score: number } | null {
