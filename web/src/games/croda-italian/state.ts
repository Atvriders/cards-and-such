...
export interface CrodaItalianSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface CrodaItalianState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type CrodaItalianAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: CrodaItalianSettings): CrodaItalianState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: CrodaItalianState, action: CrodaItalianAction): CrodaItalianState {
...
export function isTerminal(state: CrodaItalianState): { score: number } | null {
