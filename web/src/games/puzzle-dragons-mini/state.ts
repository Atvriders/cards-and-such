...
// Puzzle Dragons Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface PuzzleDragonsMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface PuzzleDragonsMiniState {
  rngSeed: number;
  grid: Grid;
...
export type PuzzleDragonsMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: PuzzleDragonsMiniSettings): PuzzleDragonsMiniState {
...
export function reducer(state: PuzzleDragonsMiniState, action: PuzzleDragonsMiniAction): PuzzleDragonsMiniState {
...
export function isTerminal(state: PuzzleDragonsMiniState): { score: number } | null {
