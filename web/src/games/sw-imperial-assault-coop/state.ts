...
export interface swImperialAssaultCoopSettings { dummy: boolean; }
export interface swImperialAssaultCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type swImperialAssaultCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: swImperialAssaultCoopSettings): swImperialAssaultCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: swImperialAssaultCoopState, action: swImperialAssaultCoopAction): swImperialAssaultCoopState {
...
export function isTerminal(state: swImperialAssaultCoopState): { score: number } | null {
