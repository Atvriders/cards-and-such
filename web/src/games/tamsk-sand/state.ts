...
export interface TamskSandSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface TamskSandState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type TamskSandAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: TamskSandSettings): TamskSandState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: TamskSandState, action: TamskSandAction): TamskSandState {
...
export function isTerminal(state: TamskSandState): { score: number } | null {
