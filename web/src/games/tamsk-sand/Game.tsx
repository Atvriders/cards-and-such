...
export function TamskSandGame({ state, dispatch, onGameOver }: GameProps<TamskSandState, TamskSandSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    ...
    return <div className="ab-wrap"><h3>Tamsk Sand</h3><div className="ab-done"><h2>{msg}</h2><div className="ab-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="ab-wrap">
      ...
      <div className="ab-board" style={{ gridTemplateColumns: `repeat(${SIZE}, 1fr)` }}>
        {Array.from({ length: SIZE * SIZE }).map((_, i) => {
          ...
          return <button key={i} className={`ab-cell ${cls}`} disabled={v !== null} onClick={() => dispatch({ type:"place", idx:i } as TamskSandAction)}>{v ?? ""}</button>;
        })}
      </div>
    </div>
  );
