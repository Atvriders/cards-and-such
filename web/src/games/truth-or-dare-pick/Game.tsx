...
export function TruthOrDarePickGame({ state, dispatch, onGameOver }: GameProps<TruthOrDarePickState, TruthOrDarePickSettings>): JSX.Element {
    ...
    return (
      <div className={`${P}-wrap`}>
        <div className={`${P}-final`}>
          <h2 className={`${P}-final-title`}>Truth or Dare — Complete</h2>
    ...
    <div className={`${P}-wrap`}>
      ...
      <div className={`${P}-choices`}>
        {q.choices.map((c, i) => {
          ...
          const cls = state.phase === "feedback"
            ...
            : `${P}-choice`;
          return (
            <button data-testid="hint-target-truth-or-dare-pick-primary" key={i} className={cls} disabled={state.phase !== "ask"}
              type="button"
              onClick={() => dispatch({ type: "answer", choice: i, elapsedMs: Date.now() - start } as TruthOrDarePickAction)}
            >{c}</button>
          );
        })}
      </div>
      {state.phase === "feedback" && (
        <>
          ...
          <button data-testid="hint-target-truth-or-dare-pick-next" className={`${P}-next`} type="button" onClick={() => dispatch({ type: "next" } as TruthOrDarePickAction)}>
