...
export function RadishRushGame({ state, dispatch, onGameOver }: GameProps<RadishRushState, RadishRushSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="fc-wrap"><div className="fc-done"><h2>Time's Up!</h2><div>Picked: {state.popped} / Missed: {state.missed}</div><div className="fc-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="fc-wrap">
      <div className="fc-header">
        <span className="fc-info">Picked: {state.popped}</span>
