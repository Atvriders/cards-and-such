...
export interface AmericanToadSettings { dummy: boolean; }

export interface AmericanToadState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type AmericanToadAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { if (cardRank(c) === 13) s += 10; else s += 1; }
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: AmericanToadState, action: AmericanToadAction): AmericanToadState {
...
export function isTerminal(state: AmericanToadState): { score: number } | null {
