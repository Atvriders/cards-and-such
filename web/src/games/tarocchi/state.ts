...
export interface TarocchiSettings { dummy: boolean; }
export interface TarocchiState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type TarocchiAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: TarocchiSettings): TarocchiState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: TarocchiState, action: TarocchiAction): TarocchiState {
...
export function isTerminal(state: TarocchiState): { score: number } | null {
