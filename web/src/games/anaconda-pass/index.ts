...
const AnacondaPass = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.AnacondaPass as unknown as React.ComponentType<unknown> })));
const settings = {
...
export const anacondaPassPlugin: GamePlugin<AnacondaPassState, AnacondaPassAction, typeof settings> = {
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-anaconda-pass-deal"]', pulses: 3 };
    if (state.phase === "dealt") return { selector: '[data-testid="hint-target-anaconda-pass-next"]', pulses: 3 };
    return null;
  }, component: AnacondaPass,
};
