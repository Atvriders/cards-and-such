...
export interface AnacondaPassSettings { rounds: "10"; }

export interface AnacondaPassState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type AnacondaPassAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AnacondaPassSettings): AnacondaPassState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: AnacondaPassState, action: AnacondaPassAction): AnacondaPassState {
...
export function isTerminal(state: AnacondaPassState): { score: number } | null {
