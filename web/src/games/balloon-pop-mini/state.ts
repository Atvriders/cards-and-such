...
export interface BalloonPopMiniSettings { dummy: boolean; }
export interface BalloonPopMiniItem {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface BalloonPopMiniState {
  rngSeed: number;
  items: BalloonPopMiniItem[];
  nextId: number;
...
export type BalloonPopMiniAction =
  | { type: "tick" }
  | { type: "pop"; id: number };
export function initialState(seed: number, _settings: BalloonPopMiniSettings): BalloonPopMiniState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: BalloonPopMiniState, action: BalloonPopMiniAction): BalloonPopMiniState {
  ...
  if (action.type === "tick") {
    ...
    const newItems: BalloonPopMiniItem[] = [];
...
export function isTerminal(state: BalloonPopMiniState): { score: number } | null {
