...
export function PointSaladVegGame({ state, dispatch, onGameOver }: GameProps<PointSaladVegState, PointSaladVegSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Point Salad</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as PointSaladVegAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as PointSaladVegAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
