...
export interface PointSaladVegState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type PointSaladVegAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: PointSaladVegSettings): PointSaladVegState {
...
export function score(s: PointSaladVegState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: PointSaladVegState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
