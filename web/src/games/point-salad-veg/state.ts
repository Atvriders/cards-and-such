...
export const SUIT_NAMES = ["Carrot","Cabbage","Onion","Pepper","Tomato","Lettuce"] as const;
...
export type PointSaladVegState = DraftState;
export type PointSaladVegAction = DraftAction;
export interface PointSaladVegSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: PointSaladVegSettings): PointSaladVegState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: PointSaladVegState, action: PointSaladVegAction): PointSaladVegState {
  return draftReducer(state, action);
}

export function score(state: PointSaladVegState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: PointSaladVegState): { score: number } | null {
