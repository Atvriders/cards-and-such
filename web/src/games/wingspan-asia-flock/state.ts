...
export const SUIT_NAMES: readonly string[] = ["Forest","Wetland","Grassland","Mountain"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface WingspanAsiaFlockSettings { dummy: boolean; }
export interface WingspanAsiaFlockState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WingspanAsiaFlockAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WingspanAsiaFlockSettings): WingspanAsiaFlockState {
...
export function reducer(state: WingspanAsiaFlockState, action: WingspanAsiaFlockAction): WingspanAsiaFlockState {
...
export function score(s: WingspanAsiaFlockState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WingspanAsiaFlockState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
