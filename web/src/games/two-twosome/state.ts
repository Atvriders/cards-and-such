...
export interface TwoTwosomeSettings { dummy: boolean; }
export interface TwoTwosomeState { rngSeed: number; drawn: number; hand: number[]; score: number; matches: number; phase: "playing" | "done"; lastCard: number | null; }
export type TwoTwosomeAction = { type: "draw" };
...
export function initialState(seed: number, _s: TwoTwosomeSettings): TwoTwosomeState {
  return { rngSeed:seed, drawn:0, hand:[], score:0, matches:0, phase:"playing", lastCard:null };
}
function scoreCard(card: number, _state: TwoTwosomeState): { points: number; matched: boolean } {
  if (rankIndex(card) === 0) return { points: 15, matched: true };
  return { points: 0, matched: false };
}
export function reducer(state: TwoTwosomeState, action: TwoTwosomeAction): TwoTwosomeState {
...
export function isTerminal(state: TwoTwosomeState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
