...
export function SambaCanastaRGame({ state, dispatch, onGameOver }: GameProps<SambaCanastaRState, SambaCanastaRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as SambaCanastaRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SambaCanastaRAction)}>Next</button>
      </>}
    </div>
  );
}
