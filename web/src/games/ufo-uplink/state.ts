...
export interface UfoUplinkSettings { dummy: boolean; }
...
export interface UfoUplinkState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type UfoUplinkAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: UfoUplinkSettings): UfoUplinkState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}

export function reducer(state: UfoUplinkState, action: UfoUplinkAction): UfoUplinkState {
...
export function isTerminal(state: UfoUplinkState): { score: number } | null {
