...
export interface SawSnapSettings { dummy: boolean; }
...
export type SawSnapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: SawSnapSettings): SawSnapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: SawSnapState, action: SawSnapAction): SawSnapState {
...
export function isTerminal(state: SawSnapState): { score: number } | null {
