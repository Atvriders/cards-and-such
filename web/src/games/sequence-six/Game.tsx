...
import "./Game.css";
export function SequenceSixGame({ state, dispatch, onGameOver }: GameProps<SequenceSixState, SequenceSixSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-sequence-six-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as SequenceSixAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-sequence-six-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SequenceSixAction)}>Next</button>
        </>
      )}
    </div>
  );
