...
const DiceDiscGolfGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceDiscGolfGame as unknown as React.ComponentType<unknown> })));
...
export const diceDiscGolfPlugin: GamePlugin<DiceDiscGolfState, DiceDiscGolfAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceDiscGolfState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-disc-golf-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-disc-golf-next"]', pulses: 3 };
    return null;
  },
