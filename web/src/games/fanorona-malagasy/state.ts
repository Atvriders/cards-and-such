...
export interface FanoronaMalagasySettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface FanoronaMalagasyState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type FanoronaMalagasyAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: FanoronaMalagasySettings): FanoronaMalagasyState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: FanoronaMalagasyState, action: FanoronaMalagasyAction): FanoronaMalagasyState {
...
export function isTerminal(state: FanoronaMalagasyState): { score: number } | null {
