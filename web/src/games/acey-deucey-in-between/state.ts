...
export interface AceyDeuceyInBetweenSettings { rounds: "10"; }

export interface AceyDeuceyInBetweenState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type AceyDeuceyInBetweenAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AceyDeuceyInBetweenSettings): AceyDeuceyInBetweenState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: AceyDeuceyInBetweenState, action: AceyDeuceyInBetweenAction): AceyDeuceyInBetweenState {
...
export function isTerminal(state: AceyDeuceyInBetweenState): { score: number } | null {
