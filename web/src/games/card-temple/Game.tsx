...
import "./Game.css";
export function CardTempleGame({ state, dispatch, onGameOver }: GameProps<CardTempleState, CardTempleSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      <div className="cm-info">Temple — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardTempleAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
