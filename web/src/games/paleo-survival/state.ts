...
export interface PaleoSurvivalSettings { dummy: boolean; }
export interface PaleoSurvivalState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type PaleoSurvivalAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: PaleoSurvivalSettings): PaleoSurvivalState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Hunt successful","Storm rolls in","Mammoth tracks","Cave painted","Sickness","Tribesmate joins","Berries plentiful","Fire goes out"];

export function reducer(state: PaleoSurvivalState, action: PaleoSurvivalAction): PaleoSurvivalState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Tool (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Tribesmate (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Sold 1 Tool (+$" + market + ")"; delta = market;
      } else { evt = "No Tool to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
    ...
    if (pay > 0) evt += " | Tribesmate earned +$" + pay;
...
export function score(s: PaleoSurvivalState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: PaleoSurvivalState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
