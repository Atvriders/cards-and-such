...
export interface SwitchSheddingSettings { dummy: boolean; }
export interface SwitchSheddingState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SwitchSheddingAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SwitchSheddingSettings): SwitchSheddingState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SwitchSheddingState, action: SwitchSheddingAction): SwitchSheddingState {
...
export function isTerminal(state: SwitchSheddingState): { score: number } | null {
