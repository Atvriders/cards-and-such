...
function checkWin(b: Cell[], mark: "P" | "C"): boolean {
  ...
  for (let i = 0; i < b.length; i++) {
    if (b[i] !== mark || seenAll.has(i)) continue;
    let cTop = false, cLeft = false, cRight = false;
    const stack = [i];
    seenAll.add(i);
    while (stack.length > 0) {
      const j = stack.pop()!;
      const r2 = Math.floor(j / SIZE), col = j % SIZE;
      if (r2 === 0) cTop = true;
      if (col === 0) cLeft = true;
      if (col === SIZE - 1) cRight = true;
      for (const [dr, dc] of [[-1,0],[1,0],[0,-1],[0,1]] as const) {
        const rr = r2 + dr, cc = col + dc;
        if (rr < 0 || rr >= SIZE || cc < 0 || cc >= SIZE) continue;
        const k = rr * SIZE + cc;
        if (b[k] === mark && !seenAll.has(k)) { seenAll.add(k); stack.push(k); }
      }
    }
    if (cTop && cLeft && cRight) return true;
  }
  return false;
}
...
export function reducer(state: GameState, action: GameAction): GameState {
  ...
  if (checkWin(nb, "P")) {
    const pCount = nb.filter(x => x === "P").length;
    return { ...state, board: nb, moves, result: "P", score: state.score + 100 + pCount * 3, phase: "done" };
  }
