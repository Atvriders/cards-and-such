...
export interface PoolRummyRSettings { dummy: boolean; }
export interface PoolRummyRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type PoolRummyRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: PoolRummyRSettings): PoolRummyRState {
...
export function reducer(state: PoolRummyRState, action: PoolRummyRAction): PoolRummyRState {
...
export function isTerminal(state: PoolRummyRState): { score: number } | null {
