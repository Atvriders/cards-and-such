...
export interface WelcomeToClassicSettings { dummy: boolean; }
export interface WelcomeToClassicState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type WelcomeToClassicAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: WelcomeToClassicSettings): WelcomeToClassicState {
  return {
    rngSeed: seed,
...
export function reducer(state: WelcomeToClassicState, action: WelcomeToClassicAction): WelcomeToClassicState {
...
export function isTerminal(state: WelcomeToClassicState): { score: number } | null {
