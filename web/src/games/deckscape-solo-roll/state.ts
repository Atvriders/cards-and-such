...
export interface DeckscapeSoloRollSettings { rounds: "10"; }

export interface DeckscapeSoloRollState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type DeckscapeSoloRollAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: DeckscapeSoloRollSettings): DeckscapeSoloRollState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: DeckscapeSoloRollState, action: DeckscapeSoloRollAction): DeckscapeSoloRollState {
...
export function isTerminal(state: DeckscapeSoloRollState): { score: number } | null {
