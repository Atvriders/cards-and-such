...
const RubiconBeziqueGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RubiconBeziqueGame as unknown as React.ComponentType<unknown> })));
...
export const rubiconBeziquePlugin: GamePlugin<RubiconBeziqueState, RubiconBeziqueAction, typeof settings> = {
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-rubicon-bezique-primary"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-rubicon-bezique-next"]', pulses: 3 };
    return null;
  }, component: RubiconBeziqueGame,
};
