...
export interface deadOfWinterCoopSettings { dummy: boolean; }
export interface deadOfWinterCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type deadOfWinterCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: deadOfWinterCoopSettings): deadOfWinterCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: deadOfWinterCoopState, action: deadOfWinterCoopAction): deadOfWinterCoopState {
...
export function isTerminal(state: deadOfWinterCoopState): { score: number } | null {
