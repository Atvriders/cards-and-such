...
export function DiceTreasureGame({ state, dispatch, onGameOver }: GameProps<DiceTreasureState, DiceTreasureSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap dice-treasure-theme">
      ...
      {state.phase === "roll" && (
        <button className="dm-btn" data-testid="hint-target-dice-treasure-roll" onClick={() => dispatch({ type:"roll" } as DiceTreasureAction)}>Roll</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">{state.lastPts > 0 ? `+${state.lastPts}` : "No points"}</div>
          <button className="dm-btn alt" data-testid="hint-target-dice-treasure-next" onClick={() => dispatch({ type:"next" } as DiceTreasureAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
