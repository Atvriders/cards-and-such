...
const DiceTreasureGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceTreasureGame as unknown as React.ComponentType<unknown> })));
...
export const diceTreasurePlugin: GamePlugin<DiceTreasureState, DiceTreasureAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceTreasureState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "roll") return { selector: '[data-testid="hint-target-dice-treasure-roll"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-dice-treasure-next"]', pulses: 3 };
    return null;
  },
  component:DiceTreasureGame,
};
