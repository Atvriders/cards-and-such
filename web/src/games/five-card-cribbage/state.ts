...
export interface FiveCardCribbageSettings { dummy: boolean; }
export interface FiveCardCribbageState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type FiveCardCribbageAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: FiveCardCribbageSettings): FiveCardCribbageState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: FiveCardCribbageState, action: FiveCardCribbageAction): FiveCardCribbageState {
...
export function isTerminal(state: FiveCardCribbageState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
