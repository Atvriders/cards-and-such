...
const FiveCardCribbageGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FiveCardCribbageGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: FiveCardCribbageState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-five-card-cribbage-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-five-card-cribbage-secondary"]', pulses: 3 };
  return null;
};
...
export const fiveCardCribbagePlugin: GamePlugin<FiveCardCribbageState, FiveCardCribbageAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: FiveCardCribbageGame,
};
