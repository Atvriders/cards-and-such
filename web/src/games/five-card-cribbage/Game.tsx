...
import "./Game.css";
export function FiveCardCribbageGame({ state, dispatch, onGameOver }: GameProps<FiveCardCribbageState, FiveCardCribbageSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-five-card-cribbage-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as FiveCardCribbageAction)}>Cut!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-five-card-cribbage-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as FiveCardCribbageAction)}>Next</button>
      </>}
    </div>
  );
}
