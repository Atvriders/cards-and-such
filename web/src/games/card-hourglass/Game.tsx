...
export function CardHourglassGame({ state, dispatch, onGameOver }: GameProps<CardHourglassState, CardHourglassSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "dealing" && (
        <button data-testid="hint-target-card-hourglass-primary" className="cm-btn" onClick={() => dispatch({ type:"deal" } as CardHourglassAction)}>Deal</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">{state.lastPts > 0 ? `+${state.lastPts}` : "0"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardHourglassAction)}>Next</button>
        </>
      )}
    </div>
  );
