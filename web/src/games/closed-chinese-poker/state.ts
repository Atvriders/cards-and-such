...
export interface ClosedChinesePokerSettings { dummy: boolean; }
export interface ClosedChinesePokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ClosedChinesePokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ClosedChinesePokerSettings): ClosedChinesePokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ClosedChinesePokerState, action: ClosedChinesePokerAction): ClosedChinesePokerState {
...
export function isTerminal(state: ClosedChinesePokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
