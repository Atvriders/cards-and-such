...
export function ClosedChinesePokerGame({ state, dispatch, onGameOver }: GameProps<ClosedChinesePokerState, ClosedChinesePokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap thmCCP">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as ClosedChinesePokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ClosedChinesePokerAction)}>Next</button>
      </>}
    </div>
  );
}
