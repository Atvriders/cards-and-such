...
export interface CourchevelPokerSettings { dummy: boolean; }
export interface CourchevelPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type CourchevelPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: CourchevelPokerSettings): CourchevelPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: CourchevelPokerState, action: CourchevelPokerAction): CourchevelPokerState {
...
export function isTerminal(state: CourchevelPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
