...
const ZoolorettoTruckGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ZoolorettoTruckGame as unknown as React.ComponentType<unknown> })));
...
export const zoolorettoTruckPlugin: GamePlugin<ZoolorettoTruckState, ZoolorettoTruckAction, typeof settings> = {
  ...
  isTerminal,
  hint: (state: any) => {
    if (state.phase === "choosing") return { selector: '[data-testid="hint-target-zooloretto-truck-primary"]', pulses: 3 };
    if (state.phase === "resolved") return { selector: '[data-testid="hint-target-zooloretto-truck-next"]', pulses: 3 };
    return null;
  },
