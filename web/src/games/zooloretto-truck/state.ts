...
export interface ZoolorettoTruckSettings { dummy: boolean; }
export interface ZoolorettoTruckState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type ZoolorettoTruckAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: ZoolorettoTruckSettings): ZoolorettoTruckState {
...
export function reducer(state: ZoolorettoTruckState, action: ZoolorettoTruckAction): ZoolorettoTruckState {
...
export function score(s: ZoolorettoTruckState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: ZoolorettoTruckState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
