...
// Triple Town Merge: 5x5 merge board. Place a tile; three-of-a-kind in adjacent cells merges into a tier-up tile.

export const SIZE = 5;
export const MAX_TIER = 7;
export const MAX_MOVES = 60;

export interface TripleTownMergeSettings { dummy: boolean; }
export type Cell = number; // 0 = empty, 1..MAX_TIER tiers
export type Grid = Cell[][];

export interface TripleTownMergeState {
  rngSeed: number;
  grid: Grid;
  next: Cell;
  score: number;
...
export type TripleTownMergeAction = { type: "place"; row: number; col: number };
...
export function initialState(seed: number, _settings: TripleTownMergeSettings): TripleTownMergeState {
...
export function reducer(state: TripleTownMergeState, action: TripleTownMergeAction): TripleTownMergeState {
...
export function isTerminal(state: TripleTownMergeState): { score: number } | null {
