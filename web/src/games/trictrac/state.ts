// Auto-generated backgammon-family state for trictrac.
...
export interface TrictracSettings { dummy: boolean }

export interface TrictracState {
  rngSeed: number;
...
export function initialState(seed: number, _s: TrictracSettings): TrictracState {
...
export function legalMoves(state: TrictracState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: TrictracState, side: Side, from: number, pips: number): TrictracState | null {
  const next: TrictracState = {
    ...state,
...
function checkWin(s: TrictracState): TrictracState {
...
function cpuTurn(state: TrictracState): TrictracState {
  let s = state;
...
export function reducer(state: TrictracState, action: TrictracAction): TrictracState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: TrictracState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: TrictracState): { score: number } | null {
