...
Strategy: kings occupy large "exclusion zones," so place them in widely separated corners or edges first. Knights have unusual attack patterns — two placed knights rarely conflict unless they are close, but watch for king–knight conflicts carefully.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-kings-and-knights-action"]', pulses: 3 }; },
