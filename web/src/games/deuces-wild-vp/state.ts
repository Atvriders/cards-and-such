...
export interface DeucesWildVpSettings { dummy: boolean; }
export interface DeucesWildVpState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DeucesWildVpAction = { type: "deal" } | { type: "next" };
...
function rateFiveCards(hand5: number[]): { rank: string; pts: number } {
  ...
  // deuces (rank index 0) are wild
  const wilds = ranks.filter(r=>r===0).length; const nonWild = ranks.filter(r=>r!==0);
...
export function initialState(seed: number, _s: DeucesWildVpSettings): DeucesWildVpState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DeucesWildVpState, action: DeucesWildVpAction): DeucesWildVpState {
...
export function isTerminal(state: DeucesWildVpState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
