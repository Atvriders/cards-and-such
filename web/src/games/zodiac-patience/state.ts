...
export interface ZodiacPatienceSettings { dummy: boolean; }

export interface ZodiacPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type ZodiacPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const total = h.reduce((a,c)=>a+cardRank(c),0); if (total===12 || total===24) s += 16; const suits = new Set(h.map(cardSuit)); s += (5-suits.size)*3; if (s===0) s=1; return s;
}
...
export function reducer(state: ZodiacPatienceState, action: ZodiacPatienceAction): ZodiacPatienceState {
...
export function isTerminal(state: ZodiacPatienceState): { score: number } | null {
