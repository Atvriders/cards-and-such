...
export interface FreeBetBjSettings { dummy: boolean; }
export interface FreeBetBjState { rngSeed: number; round: number; hand: number[]; total: number; phase: "play" | "scored" | "done"; score: number; pts: number; result: string; }
export type FreeBetBjAction = { type: "hit" } | { type: "stand" } | { type: "next" };
...
export function initialState(seed: number, _s: FreeBetBjSettings): FreeBetBjState {
...
export function reducer(state: FreeBetBjState, action: FreeBetBjAction): FreeBetBjState {
...
export function isTerminal(state: FreeBetBjState): { score: number } | null {
