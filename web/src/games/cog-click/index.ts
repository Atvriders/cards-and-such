...
const CogClickGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CogClickGame as unknown as React.ComponentType<unknown> })));
...
export const cogClickPlugin: GamePlugin<CogClickState, CogClickAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: CogClickState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-cog-click-target"]', pulses: 3 };
  },
  component:CogClickGame,
};
