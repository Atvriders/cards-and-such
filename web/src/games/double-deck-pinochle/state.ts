// Double Deck Pinochle — Pinochle with 80 cards — simplified to a head-to-head trick duel.
...
export type DoubleDeckPinochlePhase = "playing" | "done";

export interface DoubleDeckPinochleState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: DoubleDeckPinochlePhase;
  playerLeads: boolean;
  message: string;
}

export type DoubleDeckPinochleAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (let copy = 0; copy < 2; copy++) {
    for (const s of SUITS) for (const r of RANKS) {
      cards.push({ suit: s, rank: r, id: `double-deck-pinochle-${copy}-${s}${r}` });
    }
  }
  return cards;
}
...
function applyTrick(state: DoubleDeckPinochleState, trick: readonly { seat: number; card: Card }[]): DoubleDeckPinochleState {
...
export function reducer(state: DoubleDeckPinochleState, action: DoubleDeckPinochleAction): DoubleDeckPinochleState {
  ...
  let s: DoubleDeckPinochleState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: DoubleDeckPinochleState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: DoubleDeckPinochleState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): DoubleDeckPinochleState {
...
export function isTerminal(state: DoubleDeckPinochleState): { score: number } | null {
