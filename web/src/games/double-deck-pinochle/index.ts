...
const DoubleDeckPinochleGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DoubleDeckPinochleGame as unknown as React.ComponentType<unknown> })));
...
export const doubleDeckPinochlePlugin: GamePlugin<DoubleDeckPinochleState, DoubleDeckPinochleAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DoubleDeckPinochleState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-ddpinc-${pick.id}"]`, pulses: 3 };
    }
