...
import "./Game.css";
export function FourCardPokerGame({ state, dispatch, onGameOver }: GameProps<FourCardPokerState, FourCardPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-four-card-poker-deal" onClick={() => dispatch({ type: "deal" } as FourCardPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-four-card-poker-next" onClick={() => dispatch({ type: "next" } as FourCardPokerAction)}>Next</button>
      </>}
    </div>
  );
}
