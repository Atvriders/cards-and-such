...
export interface FourCardPokerSettings { dummy: boolean; }
export interface FourCardPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FourCardPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FourCardPokerSettings): FourCardPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FourCardPokerState, action: FourCardPokerAction): FourCardPokerState {
...
export function isTerminal(state: FourCardPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
