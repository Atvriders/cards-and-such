...
export function ArkhamLcgFellowGame({ state, dispatch, onGameOver }: GameProps<ArkhamLcgFellowState, ArkhamLcgFellowSettings>): JSX.Element {
  ...
  const theme = "Investigating Mythos";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as ArkhamLcgFellowAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as ArkhamLcgFellowAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
