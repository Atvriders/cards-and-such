...
export interface CaramelCatchSettings { dummy: boolean; }
...
export interface CaramelCatchState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type CaramelCatchAction =
  | { type: "tick" }
  | { type: "catch"; id: number };

export function initialState(seed: number, _settings: CaramelCatchSettings): CaramelCatchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, caught: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CaramelCatchState, action: CaramelCatchAction): CaramelCatchState {
...
export function isTerminal(state: CaramelCatchState): { score: number } | null {
