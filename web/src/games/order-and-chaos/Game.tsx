...
export function OrderAndChaosGame({ state, dispatch, onGameOver }: GameProps<GameState, GameSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    ...
    return (
      <div className="oac-wrap">
        <div className="oac-done"><h2>{msg}</h2><div className="oac-final">{state.score} pts</div></div>
      </div>
    );
  }
  return (
    <div className="oac-wrap">
      ...
      <div className="oac-board" style={{ gridTemplateColumns: `repeat(${SIZE}, 1fr)` }}>
        {Array.from({ length: SIZE * SIZE }).map((_, i) => {
          ...
          return (
            <button key={i} className={`oac-cell ${cls}`} disabled={disabled} onClick={() => dispatch({ type: "place", idx: i } as GameAction)}>
              {v ?? ""}
            </button>
          );
        })}
