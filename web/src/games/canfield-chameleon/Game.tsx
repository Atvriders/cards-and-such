...
export function CanfieldChameleonGame({ state, dispatch, onGameOver }: GameProps<CanfieldChameleonState, CanfieldChameleonSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap fade-in">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button title="Play card" key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as CanfieldChameleonAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-canfield-chameleon-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as CanfieldChameleonAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as CanfieldChameleonAction)}>Discard Hand</button>
      </div>
