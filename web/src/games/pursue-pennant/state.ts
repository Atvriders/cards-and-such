...
export interface PursuePennantSettings { dummy: boolean; }

export interface PursuePennantState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type PursuePennantAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: PursuePennantSettings): PursuePennantState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: PursuePennantState, action: PursuePennantAction): PursuePennantState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<PursuePennantState> = {};
...
export function isTerminal(state: PursuePennantState): { score: number } | null {
