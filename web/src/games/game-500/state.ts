...
export interface Game500Settings { dummy: boolean; }
export interface Game500State { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type Game500Action = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: Game500Settings): Game500State {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: Game500State, action: Game500Action): Game500State {
...
export function isTerminal(state: Game500State): { score: number } | null {
