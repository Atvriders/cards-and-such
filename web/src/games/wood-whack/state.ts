...
export interface WoodWhackSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface WoodWhackState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type WoodWhackAction = { type: "tick" } | { type: "hit"; id: number };
export function initialState(seed: number, _settings: WoodWhackSettings): WoodWhackState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}
export function reducer(state: WoodWhackState, action: WoodWhackAction): WoodWhackState {
...
export function isTerminal(state: WoodWhackState): { score: number } | null {
