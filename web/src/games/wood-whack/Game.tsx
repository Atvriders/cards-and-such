...
import "./Game.css";
export function WoodWhackGame({ state, dispatch, onGameOver }: GameProps<WoodWhackState, WoodWhackSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as WoodWhackAction), 750);
    ...
    <div className="ck-wrap">
      ...
      <div className="ck-board">
        {state.targets.map(p => {
          ...
          return (
            <button key={p.id}
              className="ck-target" data-testid="hint-target-wood-whack-target"
              style={{ left: `${x}%`, top: `${y}%`, transform: "translate(-50%,-50%)", background:"transparent", border:"none" }}
              onClick={() => dispatch({ type:"hit", id:p.id } as WoodWhackAction)}
