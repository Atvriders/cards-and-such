...
export function MichiganRumRGame({ state, dispatch, onGameOver }: GameProps<MichiganRumRState, MichiganRumRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as MichiganRumRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MichiganRumRAction)}>Next</button>
      </>}
    </div>
  );
}
