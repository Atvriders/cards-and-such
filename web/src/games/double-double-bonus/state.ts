...
export interface DoubleDoubleBonusSettings { dummy: boolean; }
export interface DoubleDoubleBonusState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DoubleDoubleBonusAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DoubleDoubleBonusSettings): DoubleDoubleBonusState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DoubleDoubleBonusState, action: DoubleDoubleBonusAction): DoubleDoubleBonusState {
...
export function isTerminal(state: DoubleDoubleBonusState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
