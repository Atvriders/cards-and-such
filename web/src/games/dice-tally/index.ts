...
const DiceTallyGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceTallyGame as unknown as React.ComponentType<unknown> })));
...
export const diceTallyPlugin: GamePlugin<DiceTallyState, DiceTallyAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: DiceTallyState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "rolling") {
      return { selector: '[data-testid="hint-target-dice-tally-roll"]', pulses: 3 };
    }
    return { selector: '[data-testid="hint-target-dice-tally-next"]', pulses: 3 };
  },
  component:DiceTallyGame,
};
