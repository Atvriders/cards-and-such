...
export interface DiceRussianPyramidSettings { dummy: boolean; }

export interface DiceRussianPyramidState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceRussianPyramidAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceRussianPyramidSettings): DiceRussianPyramidState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceRussianPyramidState, action: DiceRussianPyramidAction): DiceRussianPyramidState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceRussianPyramidState> = {};
    const pot = dice[0]! >= 4;
    if (!pot) {
      pts = 0;
      extra.breakRun = 0;
...
export function isTerminal(state: DiceRussianPyramidState): { score: number } | null {
