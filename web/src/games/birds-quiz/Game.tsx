...
export function BirdsQuiz({ state, dispatch, onGameOver }: GameProps<BirdsQuizState, BirdsQuizSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as BirdsQuizAction), 1000);
      ...
      <div className="trivia-actions">
        {!isResult && (
          <button className="trivia-btn submit" disabled={state.selected === null}
            onClick={() => dispatch({ type: "submit" } as BirdsQuizAction)}>Submit</button>
        )}
        {isResult && (
          <button className="trivia-btn next"
            onClick={() => dispatch({ type: "next" } as BirdsQuizAction)}>
