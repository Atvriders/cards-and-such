...
import { initialState, reducer, isTerminal, DeadlyDowagers_CFG } from "./state.js";
...
describe("deadly-dowagers", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(DeadlyDowagers_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < DeadlyDowagers_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < DeadlyDowagers_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
