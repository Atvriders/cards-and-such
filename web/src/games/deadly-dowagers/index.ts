...
import { DeadlyDowagers_CFG, initialState, reducer, isTerminal } from "./state.js";
import { deductionHintSelector } from "../_shared/deduction-engine.js";
const DeadlyDowagersGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DeadlyDowagersGame as unknown as React.ComponentType<unknown> })));
...
export const deadlyDowagersPlugin: GamePlugin<DeadlyDowagersState, DeadlyDowagersAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DeadlyDowagersState): HintTarget | null => {
    const sel = deductionHintSelector(state, DeadlyDowagers_CFG);
