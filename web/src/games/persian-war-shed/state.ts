...
export interface PersianWarShedSettings { dummy: boolean; }
export interface PersianWarShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type PersianWarShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: PersianWarShedSettings): PersianWarShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: PersianWarShedState, action: PersianWarShedAction): PersianWarShedState {
...
export function isTerminal(state: PersianWarShedState): { score: number } | null {
