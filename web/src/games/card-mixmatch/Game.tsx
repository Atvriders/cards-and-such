...
export function CardMixmatchGame({ state, dispatch, onGameOver }: GameProps<CardMixmatchState, CardMixmatchSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardMixmatchAction)}>Next</button>
        </>
      )}
    </div>
  );
