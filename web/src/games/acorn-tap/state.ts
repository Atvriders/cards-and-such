...
export interface AcornTapSettings { dummy: boolean; }
...
export interface AcornTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type AcornTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: AcornTapSettings): AcornTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: AcornTapState, action: AcornTapAction): AcornTapState {
...
export function isTerminal(state: AcornTapState): { score: number } | null {
