...
const FrenchTarotGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FrenchTarotGame as unknown as React.ComponentType<unknown> })));
...
export const frenchTarotPlugin: GamePlugin<FrenchTarotState, FrenchTarotAction, typeof settings> = {
  ...
  description: "French Tarot — 78-card deck adapted.",
  howToPlay: "French Tarot — 78-card deck adapted. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as FrenchTarotSettings),
  reducer,
  isTerminal,
  hint: (state: FrenchTarotState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-frtarc-${pick.id}"]`, pulses: 3 };
    }
