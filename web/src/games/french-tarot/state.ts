...
export interface FrenchTarotSettings { dummy: boolean; }
export interface FrenchTarotState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type FrenchTarotAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: FrenchTarotSettings): FrenchTarotState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: FrenchTarotState, action: FrenchTarotAction): FrenchTarotState {
...
export function isTerminal(state: FrenchTarotState): { score: number } | null {
