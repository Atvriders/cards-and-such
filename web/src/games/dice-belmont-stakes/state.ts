...
export interface DiceBelmontStakesSettings { dummy: boolean; }

export interface DiceBelmontStakesState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceBelmontStakesAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceBelmontStakesSettings): DiceBelmontStakesState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceBelmontStakesState, action: DiceBelmontStakesAction): DiceBelmontStakesState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceBelmontStakesState> = {};
...
export function isTerminal(state: DiceBelmontStakesState): { score: number } | null {
