...
export interface BananaBashSettings { dummy: boolean; }
...
export interface BananaBashState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type BananaBashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BananaBashSettings): BananaBashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BananaBashState, action: BananaBashAction): BananaBashState {
...
export function isTerminal(state: BananaBashState): { score: number } | null {
