...
export function LotrLcgFellowGame({ state, dispatch, onGameOver }: GameProps<LotrLcgFellowState, LotrLcgFellowSettings>): JSX.Element {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as LotrLcgFellowAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as LotrLcgFellowAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
