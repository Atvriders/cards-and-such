...
export const TYPE_NAMES = ["Inn","Hot Spring","Temple","Souvenir","Vista"] as const;
...
export type TokaidoState = TileState;
export type TokaidoAction = TileAction;
export interface TokaidoSettings { dummy: boolean }

export function typeName(t: number): string { return TYPE_NAMES[t] ?? "?"; }

export function initialState(seed: number, _s: TokaidoSettings): TokaidoState {
  void mulberry32;
  return initialTileState(seed, CONFIG);
}

export function reducer(state: TokaidoState, action: TokaidoAction): TokaidoState {
  return tileReducer(state, action);
}

export function isTerminal(state: TokaidoState): { score: number } | null {
