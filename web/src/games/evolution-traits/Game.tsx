...
export function EvolutionTraitsGame({ state, dispatch, onGameOver }: GameProps<EvolutionTraitsState, EvolutionTraitsSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Evolution: Traits Draft</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as EvolutionTraitsAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as EvolutionTraitsAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
