...
export const SUIT_NAMES: readonly string[] = ["Carnivore","Herbivore","Defensive","Pack"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface EvolutionTraitsSettings { dummy: boolean; }
export interface EvolutionTraitsState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type EvolutionTraitsAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: EvolutionTraitsSettings): EvolutionTraitsState {
...
export function reducer(state: EvolutionTraitsState, action: EvolutionTraitsAction): EvolutionTraitsState {
...
export function score(s: EvolutionTraitsState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: EvolutionTraitsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
