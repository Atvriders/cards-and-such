...
export const SUIT_NAMES = ["Carnivore","Herbivore","Climbing","Burrow","Pack"] as const;
...
export type EvolutionTraitsState = DraftState;
export type EvolutionTraitsAction = DraftAction;
...
export function initialState(seed: number, _settings: EvolutionTraitsSettings): EvolutionTraitsState {
...
export function score(state: EvolutionTraitsState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: EvolutionTraitsState): { score: number } | null {
