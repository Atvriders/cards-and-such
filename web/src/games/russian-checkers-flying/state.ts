...
export interface RussianCheckersFlyingSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface RussianCheckersFlyingState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type RussianCheckersFlyingAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: RussianCheckersFlyingSettings): RussianCheckersFlyingState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: RussianCheckersFlyingState, action: RussianCheckersFlyingAction): RussianCheckersFlyingState {
...
export function isTerminal(state: RussianCheckersFlyingState): { score: number } | null {
