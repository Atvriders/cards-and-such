...
export function DiceCircusGame({ state, dispatch, onGameOver }: GameProps<DiceCircusState, DiceCircusSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {OPTIONS.map(o => (
            <button data-testid="hint-target-dice-circus-roll" key={o.label} className="dm-btn" onClick={() => dispatch({ type:"predict", choice:o.label } as DiceCircusAction)}>{o.label} (+{o.points})</button>
          ))}
        </div>
      )}
      {state.phase === "result" && (
        <>
          ...
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceCircusAction)}>Next</button>
        </>
      )}
    </div>
  );
