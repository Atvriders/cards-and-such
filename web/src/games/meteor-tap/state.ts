...
export interface MeteorTapSettings { dummy: boolean; }
...
export interface MeteorTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type MeteorTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: MeteorTapSettings): MeteorTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: MeteorTapState, action: MeteorTapAction): MeteorTapState {
...
export function isTerminal(state: MeteorTapState): { score: number } | null {
