...
export const TOTAL_TILES = 10;
export const NUM_TYPES = 4;
export interface PatchworkExpressGameSettings { dummy: boolean; }
export interface PatchworkExpressGameState {
  rngSeed: number;
  cells: number[];
  queue: number[];
  placed: number;
...
export type PatchworkExpressGameAction = { type: "place"; index: number };
...
export function initialState(seed: number, _s: PatchworkExpressGameSettings): PatchworkExpressGameState {
...
export function reducer(state: PatchworkExpressGameState, action: PatchworkExpressGameAction): PatchworkExpressGameState {
...
export function isTerminal(state: PatchworkExpressGameState): { score: number } | null {
