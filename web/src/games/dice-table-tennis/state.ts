...
export const POINT_VALUE = 8;
export const OPP_PENALTY = 4;
export const REMAIN_BONUS = 3;
export const BASE_SCORE = 70;

export interface DiceTableTennisSettings { dummy: boolean; }

export interface DiceTableTennisState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  myPoints: number;
...
export type DiceTableTennisAction = { type: "roll" } | { type: "next" };

function evalRoll(dice: number[]): number {
  if (dice[0]===dice[1] && dice[0]!>=4) return 1; if (dice[0]===dice[1] && dice[0]!<=3) return -1; return 0;
}

export function initialState(seed: number, _settings: DiceTableTennisSettings): DiceTableTennisState {
  return { rngSeed: seed, round: 1, dice: null, myPoints: 0, oppPoints: 0, lastDelta: 0, phase: "rolling" };
}

export function reducer(state: DiceTableTennisState, action: DiceTableTennisAction): DiceTableTennisState {
...
export function isTerminal(state: DiceTableTennisState): { score: number } | null {
