...
export function TenPlayDrawGame({ state, dispatch, onGameOver }: GameProps<TenPlayDrawState, TenPlayDrawSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as TenPlayDrawAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as TenPlayDrawAction)}>Next</button>
      </>}
    </div>
  );
}
