...
export interface TenPlayDrawSettings { dummy: boolean; }
export interface TenPlayDrawState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TenPlayDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TenPlayDrawSettings): TenPlayDrawState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TenPlayDrawState, action: TenPlayDrawAction): TenPlayDrawState {
...
export function isTerminal(state: TenPlayDrawState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
