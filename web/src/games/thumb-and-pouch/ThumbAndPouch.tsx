...
import { thumbRuleset } from "./state.js";
import { Pile } from "../../engines/tableau/Pile.js";
import { useDragDrop } from "../../engines/tableau/useDragDrop.js";
import { findAutoMove } from "../../engines/tableau/index.js";
import "./ThumbAndPouch.css";

export function ThumbAndPouch({
  state,
  dispatch,
  onGameOver,
}: GameProps<ThumbAndPouchState, Record<string, never>>): JSX.Element {
  const { onDragStart, onDragOver, onDrop } = useDragDrop();

  const handleMove = useCallback(
    (from: string, to: string, count: number) => {
      dispatch({ type: "move", fromPile: from, toPile: to, count } as ThumbAndPouchAction);
    },
    [dispatch],
  );
  ...
  const handleCardClick = useCallback(
    (pileId: string, indexFromTop: number) => {
      ...
      const target = findAutoMove(state.piles, pileId, count, thumbRuleset);
      if (target) {
        dispatch({ type: "move", fromPile: pileId, toPile: target, count } as ThumbAndPouchAction);
      }
    },
  ...
  const handleStockClick = useCallback(() => {
    ...
    if (stock && stock.cards.length === 0 && waste && waste.cards.length > 0) {
      dispatch({ type: "recycle" } as ThumbAndPouchAction);
    } else {
      dispatch({ type: "draw" } as ThumbAndPouchAction);
    }
  }, [state.piles, dispatch]);
  ...
  return (
    <div className="thumb-and-pouch">
      <div className="tap-info">
        <span>Moves: {state.movesMade}</span>
        <span>Score: {state.score}</span>
        <button
          className="auto-move-btn"
          onClick={() => dispatch({ type: "auto-move-to-foundation" } as ThumbAndPouchAction)}
        >
          Auto-move
        </button>
      </div>
