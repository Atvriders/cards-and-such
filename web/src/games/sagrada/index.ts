...
import { SagradaGame } from "./Game.js";
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
export const sagradaPlugin: GamePlugin<SagradaState, SagradaAction, typeof settings> = {
  id: "sagrada",
  title: "Sagrada",
  category: "board",
  players: { min: 1, max: 1, multiplayer: false },
  description: "Stained-glass dice draft; colour and shade restrictions on placement.",
  howToPlay: `Sagrada is a single-player tile-placement puzzle. Stained-glass dice draft; colour and shade restrictions on placement. Tiles draw from a randomized queue, and your job is to place each one onto a small grid in a way that maximises end-of-game scoring.

How it works: Place 16 dice on a 4x4 window. Adjacent same-colour dice are forbidden — every illegal placement costs -2. Otherwise +1 per tile, +3 per row of all unique colours.
...
The shuffle is fully seeded; identical seeds produce identical queues, allowing for puzzle replay and head-to-head comparison. There is no opponent — this is a pure puzzle. Aim for high adjacency density to score well above baseline.`,
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as SagradaSettings),
  reducer,
  isTerminal,
  component: SagradaGame,
};
