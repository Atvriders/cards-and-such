...
export function RaidersNorthSeaGame({ state, dispatch, onGameOver }: GameProps<RaidersNorthSeaState, RaidersNorthSeaSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Raiders of the North Sea</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as RaidersNorthSeaAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as RaidersNorthSeaAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
