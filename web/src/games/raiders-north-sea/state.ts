...
export const SUIT_NAMES = ["Warrior","Berserker","Cleric","Shopkeep","Crew"] as const;
...
export type RaidersNorthSeaState = DraftState;
export type RaidersNorthSeaAction = DraftAction;
...
export function initialState(seed: number, _settings: RaidersNorthSeaSettings): RaidersNorthSeaState {
...
export function score(state: RaidersNorthSeaState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: RaidersNorthSeaState): { score: number } | null {
