...
export const SUIT_NAMES: readonly string[] = ["Berserker","Skald","Shieldmaiden","Jarl"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface RaidersNorthSeaSettings { dummy: boolean; }
export interface RaidersNorthSeaState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type RaidersNorthSeaAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: RaidersNorthSeaSettings): RaidersNorthSeaState {
...
export function reducer(state: RaidersNorthSeaState, action: RaidersNorthSeaAction): RaidersNorthSeaState {
...
export function score(s: RaidersNorthSeaState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: RaidersNorthSeaState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
