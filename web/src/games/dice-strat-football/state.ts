...
export interface DiceStratFootballSettings { dummy: boolean; }

export interface DiceStratFootballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceStratFootballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceStratFootballSettings): DiceStratFootballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceStratFootballState, action: DiceStratFootballAction): DiceStratFootballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceStratFootballState> = {};
...
export function isTerminal(state: DiceStratFootballState): { score: number } | null {
