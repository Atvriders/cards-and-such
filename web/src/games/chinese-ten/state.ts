...
export interface ChineseTenSettings { dummy: boolean; }
export interface ChineseTenState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type ChineseTenAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ChineseTenSettings): ChineseTenState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: ChineseTenState, action: ChineseTenAction): ChineseTenState {
...
export function isTerminal(state: ChineseTenState): { score: number } | null {
