...
export function ChineseTenGame({ state, dispatch, onGameOver }: GameProps<ChineseTenState, ChineseTenSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="chinese-ten-wrap ct-shed"><div className="chinese-ten-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="chinese-ten-final">{state.score} pts</div></div></div>;
  return (
    <div className="chinese-ten-wrap ct-shed">
      ...
      {state.phase === "scored" && <>
        ...
        <button data-testid="hint-target-chinese-ten-next" className="chinese-ten-btn alt" onClick={() => dispatch({ type: "next" } as ChineseTenAction)}>Next</button>
      </>}
    </div>
  );
}
