...
export interface IronCrossPokerSettings { dummy: boolean; }
export interface IronCrossPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type IronCrossPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: IronCrossPokerSettings): IronCrossPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: IronCrossPokerState, action: IronCrossPokerAction): IronCrossPokerState {
...
export function isTerminal(state: IronCrossPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
