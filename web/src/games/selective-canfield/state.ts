...
export interface SelectiveCanfieldSettings { dummy: boolean; }

export interface SelectiveCanfieldState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type SelectiveCanfieldAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const target = 7; const matches = h.filter(c=>cardRank(c)===target).length; s += matches*15; const adj = h.filter(c=>Math.abs(cardRank(c)-target)===1).length; s += adj*5; if (s===0) s=1; return s;
}
...
export function reducer(state: SelectiveCanfieldState, action: SelectiveCanfieldAction): SelectiveCanfieldState {
...
export function isTerminal(state: SelectiveCanfieldState): { score: number } | null {
