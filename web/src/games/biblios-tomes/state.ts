...
export const SUIT_NAMES = ["Manuscript","Pigment","Forbidden","Scribe","Holy"] as const;
...
export type BibliosTomesState = DraftState;
export type BibliosTomesAction = DraftAction;
export interface BibliosTomesSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: BibliosTomesSettings): BibliosTomesState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: BibliosTomesState, action: BibliosTomesAction): BibliosTomesState {
  return draftReducer(state, action);
}

export function score(state: BibliosTomesState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: BibliosTomesState): { score: number } | null {
