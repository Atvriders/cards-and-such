...
export interface BibliosTomesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BibliosTomesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BibliosTomesSettings): BibliosTomesState {
...
export function score(s: BibliosTomesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BibliosTomesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
