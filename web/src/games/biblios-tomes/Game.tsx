...
export function BibliosTomesGame({ state, dispatch, onGameOver }: GameProps<BibliosTomesState, BibliosTomesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Biblios Tomes</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BibliosTomesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BibliosTomesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
