...
export interface BarbuSettings { dummy: boolean; }
export interface BarbuState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type BarbuAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: BarbuSettings): BarbuState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: BarbuState, action: BarbuAction): BarbuState {
...
export function isTerminal(state: BarbuState): { score: number } | null {
