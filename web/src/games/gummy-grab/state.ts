...
export interface GummyGrabSettings { dummy: boolean; }
export interface GummyGrabState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type GummyGrabAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: GummyGrabSettings): GummyGrabState {
...
export function reducer(state: GummyGrabState, action: GummyGrabAction): GummyGrabState {
...
export function isTerminal(state: GummyGrabState): { score: number } | null {
