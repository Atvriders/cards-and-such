// Scopone — 4-handed Scopa simplified to a head-to-head capture-trick duel.
...
export type ScoponePhase = "playing" | "done";

export interface ScoponeState {
  rngSeed: number;
...
export type ScoponeAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `scopone-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: ScoponeState, trick: readonly { seat: number; card: Card }[]): ScoponeState {
...
export function reducer(state: ScoponeState, action: ScoponeAction): ScoponeState {
  ...
  let s: ScoponeState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: ScoponeState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: ScoponeState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): ScoponeState {
...
export function isTerminal(state: ScoponeState): { score: number } | null {
