...
export interface blazeFireworkSettings { dummy: boolean; }
export interface blazeFireworkState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type blazeFireworkAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: blazeFireworkSettings): blazeFireworkState {
  return {
    rngSeed: seed,
...
export function reducer(state: blazeFireworkState, action: blazeFireworkAction): blazeFireworkState {
...
export function isTerminal(state: blazeFireworkState): { score: number } | null {
