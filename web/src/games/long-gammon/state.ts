// Auto-generated backgammon-family state for long-gammon.
...
export interface LongSettings { dummy: boolean }
...
export function legalMoves(state: LongState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: LongState, side: Side, from: number, pips: number): LongState | null {
  const next: LongState = {
    ...state,
...
function checkWin(s: LongState): LongState {
...
function cpuTurn(state: LongState): LongState {
  let s = state;
...
export function reducer(state: LongState, action: LongAction): LongState {
  ...
  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    ...
    const next: LongState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
