...
// Simon Pattern: stimuli appear; react when condition is met.
...
export interface SimonPatternSettings { dummy: boolean; }

export interface SimonPatternState {
  rngSeed: number;
...
export type SimonPatternAction =
  | { type: "tick" }
  | { type: "react" };

export function initialState(seed: number, _settings: SimonPatternSettings): SimonPatternState {
...
export function reducer(state: SimonPatternState, action: SimonPatternAction): SimonPatternState {
...
export function isTerminal(state: SimonPatternState): { score: number } | null {
