...
export interface DameoFreelingSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface DameoFreelingState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type DameoFreelingAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: DameoFreelingSettings): DameoFreelingState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: DameoFreelingState, action: DameoFreelingAction): DameoFreelingState {
...
export function isTerminal(state: DameoFreelingState): { score: number } | null {
