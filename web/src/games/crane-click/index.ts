...
const CraneClickGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CraneClickGame as unknown as React.ComponentType<unknown> })));
...
export const craneClickPlugin: GamePlugin<CraneClickState, CraneClickAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: CraneClickState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-crane-click-target"]', pulses: 3 };
  },
  component:CraneClickGame,
};
