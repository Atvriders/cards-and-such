...
export interface CraneClickSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface CraneClickState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type CraneClickAction = { type: "tick" } | { type: "hit"; id: number };
export function initialState(seed: number, _settings: CraneClickSettings): CraneClickState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}
export function reducer(state: CraneClickState, action: CraneClickAction): CraneClickState {
...
export function isTerminal(state: CraneClickState): { score: number } | null {
