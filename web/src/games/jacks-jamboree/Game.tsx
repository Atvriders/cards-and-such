...
export function JacksJamboreeGame({ state, dispatch, onGameOver }: GameProps<JacksJamboreeState, JacksJamboreeSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as JacksJamboreeAction)}>{state.draw >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
