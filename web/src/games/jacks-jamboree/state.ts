...
// Score by drawing Jacks; +100 per match across 10 draws.

export const TOTAL_DRAWS = 10;
export const HAND_SIZE = 5;
export const TARGET_RANK = 9; // 0..12 = 2..A; J=9, Q=10, K=11, 10=8
export const POINTS_PER = 100;

export interface JacksJamboreeSettings { dummy: boolean; }

export interface JacksJamboreeState {
  rngSeed: number;
  draw: number;
  hand: number[];
  matchedIdx: number[];
...
export type JacksJamboreeAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: JacksJamboreeSettings): JacksJamboreeState {
  return { rngSeed: seed, draw: 1, hand: [], matchedIdx: [], score: 0, phase: "dealing", lastPts: 0 };
}

export function reducer(state: JacksJamboreeState, action: JacksJamboreeAction): JacksJamboreeState {
...
export function isTerminal(state: JacksJamboreeState): { score: number } | null {
