...
// Apple Toss: Apples fall from the top. Catch them by clicking before they hit the ground.
// Missed apples cost a life. 3 lives. 30 seconds.

export interface AppleTossSettings { duration: "20" | "30" | "45"; }

export interface FallingItem { id: number; x: number; y: number; speed: number; points: number; }

export interface AppleTossState {
  items: FallingItem[];
  score: number;
  caught: number;
  missed: number;
...
export type AppleTossAction = { type: "catch"; id: number } | { type: "tick" } | { type: "spawn" };

export function initialState(seed: number, settings: AppleTossSettings): AppleTossState {
...
export function reducer(state: AppleTossState, action: AppleTossAction): AppleTossState {
  ...
  if (action.type === "spawn") {
    ...
    const pts = speed >= 9 ? 20 : 10;
    const newItem: FallingItem = { id: state.nextId, x: Math.floor(rng() * 85) + 5, y: 5, speed, points: pts };
...
export function isTerminal(state: AppleTossState): { score: number } | null {
