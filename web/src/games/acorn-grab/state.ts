...
export interface AcornGrabSettings { dummy: boolean; }
export interface AcornGrabTarget { id: number; lane: number; ticksLeft: number; }
export interface AcornGrabState { rngSeed: number; targets: AcornGrabTarget[]; nextId: number; ticksRemaining: number; score: number; clicked: number; missed: number; phase: "playing" | "done"; }
export type AcornGrabAction = { type: "tick" } | { type: "click"; id: number };
export function initialState(seed: number, _s: AcornGrabSettings): AcornGrabState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, clicked: 0, missed: 0, phase: "playing" };
}
export function reducer(state: AcornGrabState, action: AcornGrabAction): AcornGrabState {
  ...
  if (action.type === "tick") {
    ...
    let nextId = state.nextId; const newT: AcornGrabTarget[] = [];
...
export function isTerminal(state: AcornGrabState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
