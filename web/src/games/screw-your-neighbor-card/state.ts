...
export interface ScrewYourNeighborCardSettings { rounds: "10"; }

export interface ScrewYourNeighborCardState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type ScrewYourNeighborCardAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ScrewYourNeighborCardSettings): ScrewYourNeighborCardState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: ScrewYourNeighborCardState, action: ScrewYourNeighborCardAction): ScrewYourNeighborCardState {
...
export function isTerminal(state: ScrewYourNeighborCardState): { score: number } | null {
