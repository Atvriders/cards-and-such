...
export function BlindHookeySoliGame({ state, dispatch, onGameOver }: GameProps<BlindHookeySoliState, BlindHookeySoliSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as BlindHookeySoliAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-blind-hookey-soli-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as BlindHookeySoliAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as BlindHookeySoliAction)}>Discard Hand</button>
      </div>
