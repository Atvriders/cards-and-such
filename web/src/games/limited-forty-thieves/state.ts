...
export const RATING_BANDS = [0,40,80,120] as const;

export interface LimitedFortyThievesSettings { dummy: boolean; }

export interface LimitedFortyThievesState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type LimitedFortyThievesAction =
  | { type: "keep" }
...
function variantBonus(hand: number[]): number {
  ...
  const lows = ranks.filter((r) => r <= 5).length; return lows >= 3 ? lows * 2 : 0;
}
...
export function initialState(seed: number, _s: LimitedFortyThievesSettings): LimitedFortyThievesState {
...
export function reducer(state: LimitedFortyThievesState, action: LimitedFortyThievesAction): LimitedFortyThievesState {
...
export function isTerminal(state: LimitedFortyThievesState): { score: number } | null {
