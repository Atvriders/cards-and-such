...
export function TractorGame({ state, dispatch, onGameOver }: GameProps<TractorState, TractorSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="tractor-wrap"><div className="tractor-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="tractor-final">{state.score} pts</div></div></div>;
  return (
