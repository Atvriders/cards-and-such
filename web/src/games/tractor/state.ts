...
export interface TractorSettings { dummy: boolean; }
export interface TractorState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type TractorAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: TractorSettings): TractorState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: TractorState, action: TractorAction): TractorState {
...
export function isTerminal(state: TractorState): { score: number } | null {
