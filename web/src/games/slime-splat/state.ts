...
// Slime Splat: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface SlimeSplatSettings { dummy: boolean; }

export interface SlimeSplatTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface SlimeSplatState {
  rngSeed: number;
  targets: SlimeSplatTarget[];
  nextId: number;
...
export type SlimeSplatAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: SlimeSplatSettings): SlimeSplatState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: SlimeSplatState, action: SlimeSplatAction): SlimeSplatState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: SlimeSplatTarget[] = [];
...
export function isTerminal(state: SlimeSplatState): { score: number } | null {
