...
describe("TangerineToss", () => {
  ...
  it("tick spawns tangerines", () => { const s = reducer(initialState(1, S), { type:"tick" }); expect(s.tangerines.length).toBeGreaterThanOrEqual(1); });
  it("pop removes tangerine and adds 10 points", () => {
    let s = initialState(1, S);
    s = reducer(s, { type:"tick" });
    const id = s.tangerines[0]!.id;
