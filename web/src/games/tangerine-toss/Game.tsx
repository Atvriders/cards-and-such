...
export function TangerineTossGame({ state, dispatch, onGameOver }: GameProps<TangerineTossState, TangerineTossSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="fc-wrap"><div className="fc-done"><h2>Time's Up!</h2><div>Tossed: {state.popped} / Missed: {state.missed}</div><div className="fc-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="fc-wrap">
      <div className="fc-header">
        <span className="fc-info">Tossed: {state.popped}</span>
