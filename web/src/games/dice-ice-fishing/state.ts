...
export interface DiceIceFishingSettings { dummy: boolean; }

export interface DiceIceFishingState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceIceFishingAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceIceFishingSettings): DiceIceFishingState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceIceFishingState, action: DiceIceFishingAction): DiceIceFishingState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceIceFishingState> = {};
...
export function isTerminal(state: DiceIceFishingState): { score: number } | null {
