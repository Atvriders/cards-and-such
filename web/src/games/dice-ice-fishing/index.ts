...
const DiceIceFishingGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceIceFishingGame as unknown as React.ComponentType<unknown> })));
...
export const diceIceFishingPlugin: GamePlugin<DiceIceFishingState, DiceIceFishingAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceIceFishingState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-ice-fishing-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-ice-fishing-next"]', pulses: 3 };
    return null;
  },
