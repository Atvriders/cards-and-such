...
export interface CleverSpringSettings { dummy: boolean; }
export interface CleverSpringState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type CleverSpringAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: CleverSpringSettings): CleverSpringState {
  return {
    rngSeed: seed,
...
export function reducer(state: CleverSpringState, action: CleverSpringAction): CleverSpringState {
...
export function isTerminal(state: CleverSpringState): { score: number } | null {
