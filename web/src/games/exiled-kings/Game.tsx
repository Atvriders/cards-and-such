...
export function ExiledKingsGame({ state, dispatch, onGameOver }: GameProps<ExiledKingsState, ExiledKingsSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap fade-in">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as ExiledKingsAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as ExiledKingsAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as ExiledKingsAction)}>Discard Hand</button>
      </div>
