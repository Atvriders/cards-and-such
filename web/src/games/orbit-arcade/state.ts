...
// Orbit Arcade: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface OrbitArcadeSettings { dummy: boolean; }

export interface OrbitArcadeTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface OrbitArcadeState {
  rngSeed: number;
  targets: OrbitArcadeTarget[];
  nextId: number;
...
export type OrbitArcadeAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: OrbitArcadeSettings): OrbitArcadeState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: OrbitArcadeState, action: OrbitArcadeAction): OrbitArcadeState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: OrbitArcadeTarget[] = [];
...
export function isTerminal(state: OrbitArcadeState): { score: number } | null {
