...
export function CardLadderGame({ state, dispatch, onGameOver }: GameProps<CardLadderState, CardLadderSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="cm-wrap"><div className="cm-done bounce-in"><h2>Done!</h2><div>Best ladder: {state.bestRun}</div><div className="cm-final">{state.score} pts</div></div></div>;
  }
