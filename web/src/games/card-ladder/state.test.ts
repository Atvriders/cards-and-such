...
const S = { dummy: false };
describe("Card Ladder", () => {
  it("starts in drawing phase", () => { const s=initialState(1,S); expect(s.phase).toBe("drawing"); expect(s.draws).toBe(0); });
  it("first draw sets run to 1", () => { const s=reducer(initialState(1,S),{type:"draw"}); expect(s.currentRun).toBe(1); });
