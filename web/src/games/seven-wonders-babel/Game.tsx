...
export function SevenWondersBabelGame({ state, dispatch, onGameOver }: GameProps<SevenWondersBabelState, SevenWondersBabelSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">7 Wonders: Babel</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SevenWondersBabelAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SevenWondersBabelAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
