...
export const SUIT_NAMES = ["Tower","Law","Garden","Tongue"] as const;
...
export type SevenWondersBabelState = DraftState;
export type SevenWondersBabelAction = DraftAction;
...
export function initialState(seed: number, _settings: SevenWondersBabelSettings): SevenWondersBabelState {
...
export function score(state: SevenWondersBabelState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersBabelState): { score: number } | null {
