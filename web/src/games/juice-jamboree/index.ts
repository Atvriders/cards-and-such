...
const JuiceJamboreeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.JuiceJamboreeGame as unknown as React.ComponentType<unknown> })));
...
export const juiceJamboreePlugin: GamePlugin<JuiceJamboreeState, JuiceJamboreeAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: JuiceJamboreeState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-juice-jamboree-target"]', pulses: 3 };
  },
  component:JuiceJamboreeGame,
};
