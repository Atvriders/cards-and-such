...
export function CardPaddleGame({ state, dispatch, onGameOver }: GameProps<CardPaddleState, CardPaddleSettings>): JSX.Element {
    ...
    <div className="rg-wrap">
      ...
      {state.phase === "result" && (
        <>
          ...
          <button className="rg-btn alt" onClick={() => dispatch({ type:"next" } as CardPaddleAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
