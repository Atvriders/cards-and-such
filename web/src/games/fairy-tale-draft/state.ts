...
export interface FairyTaleDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type FairyTaleDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: FairyTaleDraftSettings): FairyTaleDraftState {
...
export function score(s: FairyTaleDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: FairyTaleDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
