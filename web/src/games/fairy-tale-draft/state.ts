...
export const SUIT_NAMES = ["Spell","Sword","Shield","Crown"] as const;
...
export type FairyTaleDraftState = DraftState;
export type FairyTaleDraftAction = DraftAction;
export interface FairyTaleDraftSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: FairyTaleDraftSettings): FairyTaleDraftState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: FairyTaleDraftState, action: FairyTaleDraftAction): FairyTaleDraftState {
  return draftReducer(state, action);
}

export function score(state: FairyTaleDraftState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: FairyTaleDraftState): { score: number } | null {
