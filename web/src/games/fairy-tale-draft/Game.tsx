...
export function FairyTaleDraftGame({ state, dispatch, onGameOver }: GameProps<FairyTaleDraftState, FairyTaleDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Fairy Tale Draft</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as FairyTaleDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as FairyTaleDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
