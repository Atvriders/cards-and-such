...
export function CardPharmacyGame({ state, dispatch, onGameOver }: GameProps<CardPharmacyState, CardPharmacySettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "decide" && state.pair && (
        <>
          ...
          <div className="cm-row">
            <button className="cm-btn" onClick={() => dispatch({ type:"take" } as CardPharmacyAction)}>Take</button>
            <button className="cm-btn alt" onClick={() => dispatch({ type:"skip" } as CardPharmacyAction)}>Skip</button>
          </div>
        </>
      )}
