...
export function PandemicHotZoneNaGame({ state, dispatch, onGameOver }: GameProps<PandemicHotZoneNaState, PandemicHotZoneNaSettings>): JSX.Element {
  ...
  const theme = "North America Outbreak";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as PandemicHotZoneNaAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as PandemicHotZoneNaAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
