...
import { initialState, reducer, isTerminal, ThirteenClues_CFG } from "./state.js";

const S = { dummy: false };

describe("thirteen-clues", () => {
  it("starts in guess phase with empty history", () => {
    const s = initialState(1, S);
    expect(s.phase).toBe("guess");
    expect(s.guesses.length).toBe(0);
    expect(s.answer.length).toBe(ThirteenClues_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < ThirteenClues_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < ThirteenClues_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
