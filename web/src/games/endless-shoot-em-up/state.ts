...
// Endless Shoot 'em Up: endless runner. Player has 3 lanes; tap to move; obstacles spawn from right; collision = game over.

export const LANES = 3;
export const LANE_LENGTH = 12; // x positions, 0 (player) to 11 (spawn point)
export const TICK_MS = 100;

export interface EndlessShootEmUpSettings { dummy: boolean; }

export interface Obstacle { id: number; lane: number; x: number; }

export interface EndlessShootEmUpState {
  rngSeed: number;
  playerLane: number;
  obstacles: Obstacle[];
  nextId: number;
...
export type EndlessShootEmUpAction =
  | { type: "tick" }
  | { type: "lane"; dir: -1 | 1 }
...
export function isTerminal(state: EndlessShootEmUpState): { score: number } | null {
