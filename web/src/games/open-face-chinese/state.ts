...
export interface OpenFaceChineseSettings { dummy: boolean; }
export interface OpenFaceChineseState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type OpenFaceChineseAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: OpenFaceChineseSettings): OpenFaceChineseState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: OpenFaceChineseState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
