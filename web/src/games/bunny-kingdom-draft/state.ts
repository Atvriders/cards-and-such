...
export const SUIT_NAMES = ["Carrot","Mushroom","Wood","Water"] as const;
...
export type BunnyKingdomState = DraftState;
export type BunnyKingdomAction = DraftAction;
export interface BunnyKingdomSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: BunnyKingdomSettings): BunnyKingdomState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: BunnyKingdomState, action: BunnyKingdomAction): BunnyKingdomState {
  return draftReducer(state, action);
}

export function score(state: BunnyKingdomState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: BunnyKingdomState): { score: number } | null {
