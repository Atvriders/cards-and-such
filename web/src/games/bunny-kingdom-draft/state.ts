...
export interface BunnyKingdomState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BunnyKingdomAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BunnyKingdomSettings): BunnyKingdomState {
...
export function score(s: BunnyKingdomState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BunnyKingdomState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
