...
export function BunnyKingdomGame({ state, dispatch, onGameOver }: GameProps<BunnyKingdomState, BunnyKingdomSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Bunny Kingdom</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BunnyKingdomAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BunnyKingdomAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
