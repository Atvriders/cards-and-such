// Auto-generated backgammon-family state for nackgammon.
...
const START_LAYOUT: ReadonlyArray<readonly [number, number]> = [[0,2], [11,4], [16,3], [18,4], [23,2]];

export type Side = "P" | "C";
export interface NackSettings { dummy: boolean }

export interface NackState {
  rngSeed: number;
...
export function initialState(seed: number, _s: NackSettings): NackState {
...
export function legalMoves(state: NackState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: NackState, side: Side, from: number, pips: number): NackState | null {
  const next: NackState = {
    ...state,
...
function checkWin(s: NackState): NackState {
...
function cpuTurn(state: NackState): NackState {
  let s = state;
...
export function reducer(state: NackState, action: NackAction): NackState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: NackState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: NackState): { score: number } | null {
