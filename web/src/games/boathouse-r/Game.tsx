...
export function BoathouseRGame({ state, dispatch, onGameOver }: GameProps<BoathouseRState, BoathouseRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as BoathouseRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as BoathouseRAction)}>Next</button>
      </>}
    </div>
  );
}
