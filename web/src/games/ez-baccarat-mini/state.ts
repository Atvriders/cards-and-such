...
export interface EzBaccaratMiniSettings { dummy: boolean; }
export interface EzBaccaratMiniState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type EzBaccaratMiniAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: EzBaccaratMiniSettings): EzBaccaratMiniState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: EzBaccaratMiniState, action: EzBaccaratMiniAction): EzBaccaratMiniState {
...
export function isTerminal(state: EzBaccaratMiniState): { score: number } | null {
