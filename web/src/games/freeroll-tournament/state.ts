...
export interface FreerollTournamentSettings { dummy: boolean; }
export interface FreerollTournamentState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FreerollTournamentAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FreerollTournamentSettings): FreerollTournamentState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FreerollTournamentState, action: FreerollTournamentAction): FreerollTournamentState {
...
export function isTerminal(state: FreerollTournamentState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
