...
export interface ChessboardSettings { dummy: boolean; }
...
function scoreHand(h: number[]): number {
  let s = 0; const colors = new Set(h.map(c=>cardSuit(c)%2)); if (colors.size===1) s += 14; const f = h.filter(c=>cardRank(c)>=11).length; s += f*5; if (s===0) s=1; return s;
}
...
export function reducer(state: ChessboardState, action: ChessboardAction): ChessboardState {
...
export function isTerminal(state: ChessboardState): { score: number } | null {
