...
export function RailroadInkRedGame({ state, dispatch, onGameOver }: GameProps<RailroadInkRedState, RailroadInkRedSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RailroadInkRedAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RailroadInkRedAction)}>Skip</button>
      )}
    </div>
  );
}
