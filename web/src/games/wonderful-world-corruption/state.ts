...
export interface WonderfulWorldCorruptionState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WonderfulWorldCorruptionAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WonderfulWorldCorruptionSettings): WonderfulWorldCorruptionState {
...
export function score(s: WonderfulWorldCorruptionState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WonderfulWorldCorruptionState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
