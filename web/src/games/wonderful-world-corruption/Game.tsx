...
export function WonderfulWorldCorruptionGame({ state, dispatch, onGameOver }: GameProps<WonderfulWorldCorruptionState, WonderfulWorldCorruptionSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Wonderful World: Corruption</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as WonderfulWorldCorruptionAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as WonderfulWorldCorruptionAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
