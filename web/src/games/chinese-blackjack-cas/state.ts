...
export interface ChineseBlackjackCasSettings { dummy: boolean; }
export interface ChineseBlackjackCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type ChineseBlackjackCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ChineseBlackjackCasSettings): ChineseBlackjackCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: ChineseBlackjackCasState, action: ChineseBlackjackCasAction): ChineseBlackjackCasState {
...
export function isTerminal(state: ChineseBlackjackCasState): { score: number } | null {
