...
export interface CribbageMugginsVarSettings { dummy: boolean; }

export interface CribbageMugginsVarState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type CribbageMugginsVarAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: CribbageMugginsVarSettings): CribbageMugginsVarState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: CribbageMugginsVarState, action: CribbageMugginsVarAction): CribbageMugginsVarState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<CribbageMugginsVarState> = {};
...
export function isTerminal(state: CribbageMugginsVarState): { score: number } | null {
