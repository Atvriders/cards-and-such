...
export function CardCupGame({ state, dispatch, onGameOver }: GameProps<CardCupState, CardCupSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "choose" && (<div className="cm-row"><button data-testid="hint-target-card-cup-primary" className="cm-btn" onClick={() => dispatch({ type:"pick", cup:"low" } as CardCupAction)}>Low Cup</button><button className="cm-btn" onClick={() => dispatch({ type:"pick", cup:"mid" } as CardCupAction)}>Mid Cup</button><button className="cm-btn" onClick={() => dispatch({ type:"pick", cup:"high" } as CardCupAction)}>High Cup</button></div>)}
