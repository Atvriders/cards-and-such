...
export function DiceBazaarGame({ state, dispatch, onGameOver }: GameProps<DiceBazaarState, DiceBazaarSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      <div className="dm-info">Bazaar — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "rolling" && (
        <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as DiceBazaarAction)}>Roll 5 Dice</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">Sum: {state.sum} — {state.lastPts > 0 ? `+${state.lastPts}` : "no points"}</div>
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceBazaarAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
