...
export function PatienceRoyalCotillionGame({ state, dispatch, onGameOver }: GameProps<PatienceRoyalCotillionState, PatienceRoyalCotillionSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as PatienceRoyalCotillionAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-patience-royal-cotillion-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as PatienceRoyalCotillionAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as PatienceRoyalCotillionAction)}>Discard Hand</button>
      </div>
