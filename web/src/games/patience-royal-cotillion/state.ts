...
export const RATING_BANDS = [0,32,72,112] as const;

export interface PatienceRoyalCotillionSettings { dummy: boolean; }

export interface PatienceRoyalCotillionState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type PatienceRoyalCotillionAction =
  | { type: "keep" }
...
function variantBonus(hand: number[]): number {
  ...
  const odds = ranks.filter((r) => r % 2 === 1).length; return odds >= 3 ? odds * 2 : 0;
}
...
export function initialState(seed: number, _s: PatienceRoyalCotillionSettings): PatienceRoyalCotillionState {
...
export function reducer(state: PatienceRoyalCotillionState, action: PatienceRoyalCotillionAction): PatienceRoyalCotillionState {
...
export function isTerminal(state: PatienceRoyalCotillionState): { score: number } | null {
