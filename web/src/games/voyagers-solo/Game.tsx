...
export function VoyagersSolo({ state, dispatch, onGameOver }: GameProps<VoyagersSoloState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      <div className="g-controls">
        {state.phase === "ready" && <button className="g-btn" onClick={() => dispatch({ type: "play" } as VoyagersSoloAction)}>Play Hand</button>}
        {state.phase === "played" && <button className="g-btn" onClick={() => dispatch({ type: "next" } as VoyagersSoloAction)}>Next</button>}
      </div>
    </div>
  );
}
