...
export interface VoyagersSoloSettings { rounds: "10"; }

export interface VoyagersSoloState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type VoyagersSoloAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: VoyagersSoloSettings): VoyagersSoloState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: VoyagersSoloState, action: VoyagersSoloAction): VoyagersSoloState {
...
export function isTerminal(state: VoyagersSoloState): { score: number } | null {
