...
// Pixel Pop: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface PixelPopSettings { dummy: boolean; }

export interface PixelPopTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface PixelPopState {
  rngSeed: number;
  targets: PixelPopTarget[];
  nextId: number;
...
export type PixelPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PixelPopSettings): PixelPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PixelPopState, action: PixelPopAction): PixelPopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: PixelPopTarget[] = [];
...
export function isTerminal(state: PixelPopState): { score: number } | null {
