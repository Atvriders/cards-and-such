...
export interface SpitOceanWildSettings { rounds: "10"; }

export interface SpitOceanWildState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type SpitOceanWildAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SpitOceanWildSettings): SpitOceanWildState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: SpitOceanWildState, action: SpitOceanWildAction): SpitOceanWildState {
...
export function isTerminal(state: SpitOceanWildState): { score: number } | null {
