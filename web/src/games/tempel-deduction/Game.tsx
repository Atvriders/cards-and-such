...
export function TempelDeductionGame({ state, dispatch, onGameOver }: GameProps<TempelDeductionState, TempelDeductionSettings>): JSX.Element {
    ...
    <div className="quiz-wrap">
      ...
      <div className="quiz-choices">
        {q.choices.map((choice, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type: "select", choice: i } as TempelDeductionAction)}>
      ...
      <div className="quiz-actions">
        {!isResult && (
          <button className="quiz-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as TempelDeductionAction)}>Submit</button>
        )}
        {isResult && (
          <button className="quiz-btn next" onClick={() => dispatch({ type: "next" } as TempelDeductionAction)}>
