...
// Snowflake Tap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface SnowflakeTapSettings { dummy: boolean; }

export interface SnowflakeTapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface SnowflakeTapState {
  rngSeed: number;
  targets: SnowflakeTapTarget[];
  nextId: number;
...
export type SnowflakeTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: SnowflakeTapSettings): SnowflakeTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: SnowflakeTapState, action: SnowflakeTapAction): SnowflakeTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: SnowflakeTapTarget[] = [];
...
export function isTerminal(state: SnowflakeTapState): { score: number } | null {
