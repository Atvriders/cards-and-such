...
export interface FusionPokerClSettings { rounds: "10"; }

export interface FusionPokerClState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type FusionPokerClAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FusionPokerClSettings): FusionPokerClState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: FusionPokerClState, action: FusionPokerClAction): FusionPokerClState {
...
export function isTerminal(state: FusionPokerClState): { score: number } | null {
