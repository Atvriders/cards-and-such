// Omnibus Hearts — Hearts variant with a bonus for capturing the Ten of Diamonds.
...
export type OmnibusHeartsPhase = "playing" | "done";

export interface OmnibusHeartsState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: OmnibusHeartsPhase;
  playerLeads: boolean;
  message: string;
}

export type OmnibusHeartsAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `omnibus-hearts-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: OmnibusHeartsState, trick: readonly { seat: number; card: Card }[]): OmnibusHeartsState {
...
export function reducer(state: OmnibusHeartsState, action: OmnibusHeartsAction): OmnibusHeartsState {
  ...
  let s: OmnibusHeartsState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: OmnibusHeartsState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: OmnibusHeartsState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): OmnibusHeartsState {
...
export function isTerminal(state: OmnibusHeartsState): { score: number } | null {
