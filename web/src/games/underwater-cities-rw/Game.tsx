...
export function UnderwaterCitiesRw({ state, dispatch, onGameOver }: GameProps<UnderwaterCitiesRwState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
      ...
      <div className="g-controls">
        {state.phase === "ready" && <button className="g-btn" onClick={() => dispatch({ type: "play" } as UnderwaterCitiesRwAction)}>Play Hand</button>}
        {state.phase === "played" && <button className="g-btn" onClick={() => dispatch({ type: "next" } as UnderwaterCitiesRwAction)}>Next</button>}
      </div>
    </div>
  );
}
