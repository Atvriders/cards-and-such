...
export interface CribbageSquaresSoliSettings { dummy: boolean; }
export interface CribbageSquaresSoliState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type CribbageSquaresSoliAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: CribbageSquaresSoliSettings): CribbageSquaresSoliState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: CribbageSquaresSoliState, action: CribbageSquaresSoliAction): CribbageSquaresSoliState {
...
export function isTerminal(state: CribbageSquaresSoliState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
