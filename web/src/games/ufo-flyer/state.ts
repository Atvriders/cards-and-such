...
// UFO Flyer: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface UfoFlyerSettings { dummy: boolean; }

export interface UfoFlyerTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface UfoFlyerState {
  rngSeed: number;
  targets: UfoFlyerTarget[];
  nextId: number;
...
export type UfoFlyerAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: UfoFlyerSettings): UfoFlyerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: UfoFlyerState, action: UfoFlyerAction): UfoFlyerState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: UfoFlyerTarget[] = [];
...
export function isTerminal(state: UfoFlyerState): { score: number } | null {
