...
// Puzzle Quest Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface PuzzleQuestMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface PuzzleQuestMiniState {
  rngSeed: number;
  grid: Grid;
...
export type PuzzleQuestMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: PuzzleQuestMiniSettings): PuzzleQuestMiniState {
...
export function reducer(state: PuzzleQuestMiniState, action: PuzzleQuestMiniAction): PuzzleQuestMiniState {
...
export function isTerminal(state: PuzzleQuestMiniState): { score: number } | null {
