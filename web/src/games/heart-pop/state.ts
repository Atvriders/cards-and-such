...
// Heart Pop: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface HeartPopSettings { dummy: boolean; }

export interface HeartPopTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface HeartPopState {
  rngSeed: number;
  targets: HeartPopTarget[];
  nextId: number;
...
export type HeartPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: HeartPopSettings): HeartPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: HeartPopState, action: HeartPopAction): HeartPopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: HeartPopTarget[] = [];
...
export function isTerminal(state: HeartPopState): { score: number } | null {
