...
export function TeenPattiCasGame({ state, dispatch, onGameOver }: GameProps<TeenPattiCasState, TeenPattiCasSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      <div className="dm-info">Round {state.round} / {TOTAL_ROUNDS} — Your hand: {state.rank}</div>
      ...
      {state.phase === "see" && <div className="dm-row">
        <button className="dm-btn" onClick={() => dispatch({ type: "play" } as TeenPattiCasAction)}>Play</button>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "fold" } as TeenPattiCasAction)}>Fold</button>
      </div>}
      {state.phase === "scored" && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as TeenPattiCasAction)}>Next</button>
      </>}
    </div>
  );
}
