// Auto-generated backgammon-family state for tavli-greek-race.
...
export interface TavliGrSettings { dummy: boolean }

export interface TavliGrState {
  rngSeed: number;
...
export function initialState(seed: number, _s: TavliGrSettings): TavliGrState {
...
export function legalMoves(state: TavliGrState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: TavliGrState, side: Side, from: number, pips: number): TavliGrState | null {
  const next: TavliGrState = {
    ...state,
...
function checkWin(s: TavliGrState): TavliGrState {
...
function cpuTurn(state: TavliGrState): TavliGrState {
  let s = state;
...
export function reducer(state: TavliGrState, action: TavliGrAction): TavliGrState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: TavliGrState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: TavliGrState): { score: number } | null {
