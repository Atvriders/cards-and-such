...
export const SUIT_NAMES: readonly string[] = ["Drone","Forager","Guard","Queen"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface ApiaryBeesSettings { dummy: boolean; }
export interface ApiaryBeesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type ApiaryBeesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: ApiaryBeesSettings): ApiaryBeesState {
...
export function reducer(state: ApiaryBeesState, action: ApiaryBeesAction): ApiaryBeesState {
...
export function score(s: ApiaryBeesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: ApiaryBeesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
