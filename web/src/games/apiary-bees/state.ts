...
export const SUIT_NAMES = ["Worker","Drone","Queen","Honey","Pollen"] as const;
...
export type ApiaryBeesState = DraftState;
export type ApiaryBeesAction = DraftAction;
...
export function initialState(seed: number, _settings: ApiaryBeesSettings): ApiaryBeesState {
...
export function score(state: ApiaryBeesState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: ApiaryBeesState): { score: number } | null {
