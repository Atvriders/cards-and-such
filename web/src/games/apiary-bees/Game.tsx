...
export function ApiaryBeesGame({ state, dispatch, onGameOver }: GameProps<ApiaryBeesState, ApiaryBeesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Apiary: Bees</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as ApiaryBeesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as ApiaryBeesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
