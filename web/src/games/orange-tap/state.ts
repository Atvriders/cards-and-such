...
export interface OrangeTapSettings { dummy: boolean; }
...
export interface OrangeTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type OrangeTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: OrangeTapSettings): OrangeTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: OrangeTapState, action: OrangeTapAction): OrangeTapState {
...
export function isTerminal(state: OrangeTapState): { score: number } | null {
