...
const PaddleBallJuggleGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PaddleBallJuggleGame as unknown as React.ComponentType<unknown> })));
...
Score points for every paddle bounce plus 5 bonus points per power-up caught. The game ends when all lives are exhausted (each dropped ball costs one life). Can you juggle four or five balls at once?`,
  ...
  reducer, isTerminal, hint: (state: PaddleBallState): HintTarget | null => (!state.over ? { selector: ".arcade-btn", pulses: 3 } : null), component: PaddleBallJuggleGame,
};
