...
export interface BananaPeelSettings { duration: "20" | "30" | "45"; }
export interface FallingItem { id: number; x: number; y: number; speed: number; points: number; }
export interface BananaPeelState {
...
export type BananaPeelAction = { type: "catch"; id: number } | { type: "tick" } | { type: "spawn" };

export function initialState(seed: number, settings: BananaPeelSettings): BananaPeelState {
...
export function reducer(state: BananaPeelState, action: BananaPeelAction): BananaPeelState {
...
export function isTerminal(state: BananaPeelState): { score: number } | null {
