...
export function QwixxBonusGame({ state, dispatch, onGameOver }: GameProps<QwixxBonusState, QwixxBonusSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return (
      <div className="rw-wrap">
        <h3 className="rw-title">Qwixx: Bonus Dice</h3>
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as QwixxBonusAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as QwixxBonusAction)}>Skip</button>
      )}
    </div>
  );
}
