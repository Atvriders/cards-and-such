    ...
    <div className="ded-wrap">
      ...
      <div className="ded-options">
        {p.options.map((opt, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={state.resolved} onClick={() => dispatch({ type: "select", index: i } as FifteenPuzzleAction)}>
              {opt}
            </button>
          );
        })}
      ...
      <div className="ded-actions">
        {!state.resolved && (
          <button className="ded-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as FifteenPuzzleAction)}>Accuse!</button>
        )}
        {state.resolved && (
          <button className="ded-btn next" onClick={() => dispatch({ type: "next" } as FifteenPuzzleAction)}>
