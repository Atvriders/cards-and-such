...
// Mole Mash: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface MoleMashSettings { dummy: boolean; }

export interface MoleMashTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface MoleMashState {
  rngSeed: number;
  targets: MoleMashTarget[];
  nextId: number;
...
export type MoleMashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: MoleMashSettings): MoleMashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: MoleMashState, action: MoleMashAction): MoleMashState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: MoleMashTarget[] = [];
...
export function isTerminal(state: MoleMashState): { score: number } | null {
