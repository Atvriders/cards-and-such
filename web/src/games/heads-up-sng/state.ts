...
export interface HeadsUpSngSettings { dummy: boolean; }
export interface HeadsUpSngState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HeadsUpSngAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HeadsUpSngSettings): HeadsUpSngState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HeadsUpSngState, action: HeadsUpSngAction): HeadsUpSngState {
...
export function isTerminal(state: HeadsUpSngState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
