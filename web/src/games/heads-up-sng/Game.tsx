...
import "./Game.css";
export function HeadsUpSngGame({ state, dispatch, onGameOver }: GameProps<HeadsUpSngState, HeadsUpSngSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-heads-up-sng-deal" onClick={() => dispatch({ type: "deal" } as HeadsUpSngAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-heads-up-sng-next" onClick={() => dispatch({ type: "next" } as HeadsUpSngAction)}>Next</button>
      </>}
    </div>
  );
}
