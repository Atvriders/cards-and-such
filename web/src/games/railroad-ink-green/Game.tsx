...
export function RailroadInkGreenGame({ state, dispatch, onGameOver }: GameProps<RailroadInkGreenState, RailroadInkGreenSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RailroadInkGreenAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RailroadInkGreenAction)}>Skip</button>
      )}
    </div>
  );
}
