...
export interface RailroadInkGreenSettings { dummy: boolean; }
export interface RailroadInkGreenState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type RailroadInkGreenAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: RailroadInkGreenSettings): RailroadInkGreenState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: RailroadInkGreenState, action: RailroadInkGreenAction): RailroadInkGreenState {
...
export function isTerminal(state: RailroadInkGreenState): { score: number } | null {
