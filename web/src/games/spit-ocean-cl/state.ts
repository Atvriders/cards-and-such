...
export interface SpitOceanClSettings { rounds: "10"; }

export interface SpitOceanClState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type SpitOceanClAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SpitOceanClSettings): SpitOceanClState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: SpitOceanClState, action: SpitOceanClAction): SpitOceanClState {
...
export function isTerminal(state: SpitOceanClState): { score: number } | null {
