...
export interface CoralClickSettings { dummy: boolean; }
export interface Critter { id: number; lane: number; ticksLeft: number; }
export interface CoralClickState { rngSeed: number; critters: Critter[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type CoralClickAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: CoralClickSettings): CoralClickState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: CoralClickState, action: CoralClickAction): CoralClickState {
...
export function isTerminal(s: CoralClickState): { score: number } | null { return s.phase==="done" ? { score: s.score } : null; }
