...
export function ScumGame({ state, dispatch, onGameOver }: GameProps<ScumState, ScumSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="scum-wrap sc-shed"><div className="scum-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="scum-final">{state.score} pts</div></div></div>;
  return (
    <div className="scum-wrap sc-shed">
