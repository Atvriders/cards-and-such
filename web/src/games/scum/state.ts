...
export interface ScumSettings { dummy: boolean; }
export interface ScumState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type ScumAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ScumSettings): ScumState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: ScumState, action: ScumAction): ScumState {
...
export function isTerminal(state: ScumState): { score: number } | null {
