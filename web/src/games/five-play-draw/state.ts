...
export interface FivePlayDrawSettings { dummy: boolean; }
export interface FivePlayDrawState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FivePlayDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FivePlayDrawSettings): FivePlayDrawState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FivePlayDrawState, action: FivePlayDrawAction): FivePlayDrawState {
...
export function isTerminal(state: FivePlayDrawState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
