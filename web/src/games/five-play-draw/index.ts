...
const FivePlayDrawGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FivePlayDrawGame as unknown as React.ComponentType<unknown> })));
...
export const fivePlayDrawPlugin: GamePlugin<FivePlayDrawState, FivePlayDrawAction, typeof settings> = {
  ...
  reducer,isTerminal,  hint: (state: FivePlayDrawState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-five-play-draw-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-five-play-draw-next"]', pulses: 3 };
    return null;
  },
  component:FivePlayDrawGame,
};
