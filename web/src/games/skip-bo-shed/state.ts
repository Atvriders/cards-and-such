...
export interface SkipBoShedSettings { dummy: boolean; }
export interface SkipBoShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SkipBoShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SkipBoShedSettings): SkipBoShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SkipBoShedState, action: SkipBoShedAction): SkipBoShedState {
...
export function isTerminal(state: SkipBoShedState): { score: number } | null {
