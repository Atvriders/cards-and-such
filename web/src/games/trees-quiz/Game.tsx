...
export function TreesQuiz({ state, dispatch, onGameOver }: GameProps<TreesQuizState, TreesQuizSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as TreesQuizAction), 1000);
      ...
      <div className="trivia-actions">
        {!isResult && (
          <button className="trivia-btn submit" disabled={state.selected === null}
            onClick={() => dispatch({ type: "submit" } as TreesQuizAction)}>Submit</button>
        )}
        {isResult && (
          <button className="trivia-btn next"
            onClick={() => dispatch({ type: "next" } as TreesQuizAction)}>
