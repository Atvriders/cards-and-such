...
const FarmSupplyGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FarmSupplyGame as unknown as React.ComponentType<unknown> })));
...
export const farmSupplyPlugin: GamePlugin<FarmSupplyState, FarmSupplyAction, typeof settings> = {
  ...
  isTerminal,
  hint: (state: any) => {
    if (state.phase === "choosing") return { selector: '[data-testid="hint-target-farm-supply-primary"]', pulses: 3 };
    if (state.phase === "resolved") return { selector: '[data-testid="hint-target-farm-supply-next"]', pulses: 3 };
    return null;
  },
