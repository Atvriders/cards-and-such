...
export interface FarmSupplySettings { dummy: boolean; }
export interface FarmSupplyState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type FarmSupplyAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: FarmSupplySettings): FarmSupplyState {
...
export function reducer(state: FarmSupplyState, action: FarmSupplyAction): FarmSupplyState {
...
export function score(s: FarmSupplyState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: FarmSupplyState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
