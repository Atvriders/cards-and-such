...
export interface QuoitsScotsSettings { dummy: boolean; }

export interface QuoitsScotsState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type QuoitsScotsAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: QuoitsScotsSettings): QuoitsScotsState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: QuoitsScotsState, action: QuoitsScotsAction): QuoitsScotsState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<QuoitsScotsState> = {};
...
export function isTerminal(state: QuoitsScotsState): { score: number } | null {
