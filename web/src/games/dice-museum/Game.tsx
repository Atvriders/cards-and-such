...
export function DiceMuseumGame({ state, dispatch, onGameOver }: GameProps<DiceMuseumState, DiceMuseumSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      <div className="dm-info">Museum — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "rolling" && (
        <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as DiceMuseumAction)}>Roll 5 Dice</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">Sum: {state.sum} — {state.lastPts > 0 ? `+${state.lastPts}` : "no points"}</div>
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceMuseumAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
