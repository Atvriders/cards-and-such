...
const ScopaDAssiGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ScopaDAssiGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: ScopaDAssiState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-scopa-d-assi-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-scopa-d-assi-secondary"]', pulses: 3 };
  return null;
};
...
export const scopaDAssiPlugin: GamePlugin<ScopaDAssiState, ScopaDAssiAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: ScopaDAssiGame,
};
