...
export function ScopaDAssiGame({ state, dispatch, onGameOver }: GameProps<ScopaDAssiState, ScopaDAssiSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as ScopaDAssiAction)}>Asso!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ScopaDAssiAction)}>Next</button>
      </>}
    </div>
  );
}
