...
export const SCORE_WIN = 13;
export const SCORE_TIE = 4;
export interface ScopaDAssiSettings { dummy: boolean; }
export interface ScopaDAssiState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type ScopaDAssiAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ScopaDAssiSettings): ScopaDAssiState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: ScopaDAssiState, action: ScopaDAssiAction): ScopaDAssiState {
  ...
  // Ace bonus: +5 if your card is an Ace (rank 12)
  if (rankOf(you) === 12) pts += 5;
...
export function isTerminal(state: ScopaDAssiState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
