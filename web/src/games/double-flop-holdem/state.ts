...
export interface DoubleFlopHoldemSettings { dummy: boolean; }
export interface DoubleFlopHoldemState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DoubleFlopHoldemAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DoubleFlopHoldemSettings): DoubleFlopHoldemState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DoubleFlopHoldemState, action: DoubleFlopHoldemAction): DoubleFlopHoldemState {
...
export function isTerminal(state: DoubleFlopHoldemState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
