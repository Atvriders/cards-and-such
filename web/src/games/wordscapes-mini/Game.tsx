...
export function WordscapesMiniGame({ state, dispatch, onGameOver }: GameProps<WordscapesMiniState, WordscapesMiniSettings>): JSX.Element {
    ...
    <div className="word-wrap">
      ...
      <div className="word-choices">
        {r.choices.map((choice, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type: "select", choice: i } as WordscapesMiniAction)}>
      ...
      <div className="word-actions">
        {!isResult && (
          <button className="word-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as WordscapesMiniAction)}>Submit</button>
        )}
        {isResult && (
          <button className="word-btn next" onClick={() => dispatch({ type: "next" } as WordscapesMiniAction)}>
