...
import "./Game.css";
export function RebuyTournamentGame({ state, dispatch, onGameOver }: GameProps<RebuyTournamentState, RebuyTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-rebuy-tournament-deal" onClick={() => dispatch({ type: "deal" } as RebuyTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-rebuy-tournament-next" onClick={() => dispatch({ type: "next" } as RebuyTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
