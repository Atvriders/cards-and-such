...
export function RebuyTournamentGame({ state, dispatch, onGameOver }: GameProps<RebuyTournamentState, RebuyTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as RebuyTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as RebuyTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
