// Auto-generated backgammon-family state for tavli-portes-race.
...
export interface TavliPortSettings { dummy: boolean }

export interface TavliPortState {
  rngSeed: number;
...
export function initialState(seed: number, _s: TavliPortSettings): TavliPortState {
...
export function legalMoves(state: TavliPortState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: TavliPortState, side: Side, from: number, pips: number): TavliPortState | null {
  const next: TavliPortState = {
    ...state,
...
function checkWin(s: TavliPortState): TavliPortState {
...
function cpuTurn(state: TavliPortState): TavliPortState {
  let s = state;
...
export function reducer(state: TavliPortState, action: TavliPortAction): TavliPortState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: TavliPortState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: TavliPortState): { score: number } | null {
