...
describe("RhymeFinder initialState", () => {
  ...
  it("is deterministic under same seed", () => {
    ...
    expect(s1.entries.map((e) => e.prompt)).toEqual(s2.entries.map((e) => e.prompt));
  });

  it("prompt and answer are non-empty strings", () => {
    const s = initialState(42, defaultSettings);
    for (const entry of s.entries) {
...
describe("RhymeFinder isTerminal", () => {
