...
// Score by drawing Queens; +100 per match across 10 draws.

export const TOTAL_DRAWS = 10;
export const HAND_SIZE = 5;
export const TARGET_RANK = 10; // 0..12 = 2..A; J=9, Q=10, K=11, 10=8
export const POINTS_PER = 100;

export interface QueensQuestSettings { dummy: boolean; }

export interface QueensQuestState {
  rngSeed: number;
  draw: number;
  hand: number[];
  matchedIdx: number[];
...
export type QueensQuestAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: QueensQuestSettings): QueensQuestState {
  return { rngSeed: seed, draw: 1, hand: [], matchedIdx: [], score: 0, phase: "dealing", lastPts: 0 };
}

export function reducer(state: QueensQuestState, action: QueensQuestAction): QueensQuestState {
...
export function isTerminal(state: QueensQuestState): { score: number } | null {
