...
export const DIE_MAX = 8;

export interface QwixxExtremeSettings { dummy: boolean; }
export interface QwixxExtremeState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type QwixxExtremeAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: QwixxExtremeSettings): QwixxExtremeState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: QwixxExtremeState, action: QwixxExtremeAction): QwixxExtremeState {
...
export function isTerminal(state: QwixxExtremeState): { score: number } | null {
