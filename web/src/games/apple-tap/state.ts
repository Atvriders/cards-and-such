...
export interface AppleTapSettings { dummy: boolean; }
...
export interface AppleTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type AppleTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: AppleTapSettings): AppleTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: AppleTapState, action: AppleTapAction): AppleTapState {
...
export function isTerminal(state: AppleTapState): { score: number } | null {
