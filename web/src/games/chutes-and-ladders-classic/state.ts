// Auto-generated race-game state for chutes-and-ladders-classic.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const TRACK_LEN = 100;
export const PAWNS_PER_SIDE = 1;
...
export const SPECIALS: Record<number, Special> = {1: {type:"ladder", to:38},4: {type:"ladder", to:14},9: {type:"ladder", to:31},21: {type:"ladder", to:42},28: {type:"ladder", to:84},36: {type:"ladder", to:44},51: {type:"ladder", to:67},71: {type:"ladder", to:91},80: {type:"ladder", to:100},16: {type:"snake", to:6},47: {type:"snake", to:26},49: {type:"snake", to:11},56: {type:"snake", to:53},62: {type:"snake", to:19},64: {type:"snake", to:60},87: {type:"snake", to:24},93: {type:"snake", to:73},95: {type:"snake", to:75},98: {type:"snake", to:78}};

export type Side = "P" | "C";
export interface ChutesSettings { dummy: boolean }
...
export function initialState(seed: number, _s: ChutesSettings): ChutesState {
  return {
    rngSeed: seed,
...
function checkWin(s: ChutesState): ChutesState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: ChutesState): ChutesState {
  let s = state;
...
export function legalMovesForP(state: ChutesState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: ChutesState, action: ChutesAction): ChutesState {
...
export function isTerminal(state: ChutesState): { score: number } | null {
