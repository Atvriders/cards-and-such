...
export interface MexicanPokerSettings { dummy: boolean; }
export interface MexicanPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type MexicanPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: MexicanPokerSettings): MexicanPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: MexicanPokerState, action: MexicanPokerAction): MexicanPokerState {
...
export function isTerminal(state: MexicanPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
