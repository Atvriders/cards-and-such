...
export function BloodRageVikingsGame({ state, dispatch, onGameOver }: GameProps<BloodRageVikingsState, BloodRageVikingsSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Blood Rage: Vikings</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BloodRageVikingsAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BloodRageVikingsAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
