...
export const SUIT_NAMES = ["Bear","Wolf","Boar","Raven","Serpent"] as const;
...
export type BloodRageVikingsState = DraftState;
export type BloodRageVikingsAction = DraftAction;
export interface BloodRageVikingsSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: BloodRageVikingsSettings): BloodRageVikingsState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: BloodRageVikingsState, action: BloodRageVikingsAction): BloodRageVikingsState {
  return draftReducer(state, action);
}

export function score(state: BloodRageVikingsState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: BloodRageVikingsState): { score: number } | null {
