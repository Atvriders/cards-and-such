...
export interface BloodRageVikingsState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BloodRageVikingsAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BloodRageVikingsSettings): BloodRageVikingsState {
...
export function score(s: BloodRageVikingsState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BloodRageVikingsState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
