...
describe("four-hundred initialState", () => {
  it("deals correct number of cards to each player", () => {
    const s = initialState(42);
    expect(s.playerHand.length).toBe(6);
    expect(s.botHand.length).toBe(6);
  });
...
describe("four-hundred legalPlays", () => {
  it("all cards legal when leading", () => {
    const s = initialState(42);
    expect(legalPlays(s.playerHand, s.currentTrick).length).toBeGreaterThanOrEqual(6);
  });
});

describe("four-hundred reducer", () => {
...
describe("four-hundred full game", () => {
