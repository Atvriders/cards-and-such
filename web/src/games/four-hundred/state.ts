// Four Hundred — Euchre cousin played to 400 points — simplified single-deal duel.
...
export type FourHundredPhase = "playing" | "done";

export interface FourHundredState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: FourHundredPhase;
  playerLeads: boolean;
  message: string;
}

export type FourHundredAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `four-hundred-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: FourHundredState, trick: readonly { seat: number; card: Card }[]): FourHundredState {
...
export function reducer(state: FourHundredState, action: FourHundredAction): FourHundredState {
  ...
  let s: FourHundredState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: FourHundredState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: FourHundredState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): FourHundredState {
...
export function isTerminal(state: FourHundredState): { score: number } | null {
