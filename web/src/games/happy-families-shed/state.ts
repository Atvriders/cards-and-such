...
export interface HappyFamiliesShedSettings { dummy: boolean; }
export interface HappyFamiliesShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type HappyFamiliesShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: HappyFamiliesShedSettings): HappyFamiliesShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: HappyFamiliesShedState, action: HappyFamiliesShedAction): HappyFamiliesShedState {
  if (state.phase === "done") return state;
  if (action.type === "play") {
    if (state.phase !== "ready") return state;
    const rng = mulberry32(state.rngSeed);
    // Sim a round: player plays optimally, CPU plays random; outcome is random with mild skill skew
...
export function isTerminal(state: HappyFamiliesShedState): { score: number } | null {
