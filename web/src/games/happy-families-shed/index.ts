...
const HappyFamiliesShedGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.HappyFamiliesShedGame as unknown as React.ComponentType<unknown> })));
...
export const happyFamiliesShedPlugin: GamePlugin<HappyFamiliesShedState, HappyFamiliesShedAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: HappyFamiliesShedState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-happy-families-shed-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-happy-families-shed-next"]', pulses: 3 };
    return null;
  },
  component: HappyFamiliesShedGame,
};
