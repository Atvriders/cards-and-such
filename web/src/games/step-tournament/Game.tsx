...
import "./Game.css";
export function StepTournamentGame({ state, dispatch, onGameOver }: GameProps<StepTournamentState, StepTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-step-tournament-deal" onClick={() => dispatch({ type: "deal" } as StepTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-step-tournament-next" onClick={() => dispatch({ type: "next" } as StepTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
