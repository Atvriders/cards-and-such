...
export interface StepTournamentSettings { dummy: boolean; }
export interface StepTournamentState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type StepTournamentAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: StepTournamentSettings): StepTournamentState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: StepTournamentState, action: StepTournamentAction): StepTournamentState {
...
export function isTerminal(state: StepTournamentState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
