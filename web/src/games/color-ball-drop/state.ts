...
// Color Ball Drop: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface ColorBallDropSettings { dummy: boolean; }

export interface ColorBallDropTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface ColorBallDropState {
  rngSeed: number;
  targets: ColorBallDropTarget[];
  nextId: number;
...
export type ColorBallDropAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: ColorBallDropSettings): ColorBallDropState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: ColorBallDropState, action: ColorBallDropAction): ColorBallDropState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: ColorBallDropTarget[] = [];
...
export function isTerminal(state: ColorBallDropState): { score: number } | null {
