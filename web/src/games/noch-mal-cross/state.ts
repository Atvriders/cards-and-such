...
export interface NochMalCrossSettings { dummy: boolean; }
export interface NochMalCrossState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type NochMalCrossAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: NochMalCrossSettings): NochMalCrossState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: NochMalCrossState, action: NochMalCrossAction): NochMalCrossState {
...
export function isTerminal(state: NochMalCrossState): { score: number } | null {
