...
export function CortexChallenge2Game({ state, dispatch, onGameOver }: GameProps<CortexChallenge2State, CortexChallenge2Settings>): JSX.Element {
  ...
  return (
    <div className="crtxch2-wrap fade-in">
      ...
      {!state.submitted && <button className="crtxch2-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as CortexChallenge2Action)}>Submit</button>}
      {state.submitted && <button className="crtxch2-btn next" onClick={() => dispatch({ type: "next" } as CortexChallenge2Action)}>{state.currentIndex + 1 >= state.rounds.length ? "Finish" : "Next"}</button>}
    </div>
  );
}
