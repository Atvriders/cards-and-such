...
export interface DungeonRollDelveSettings { dummy: boolean; }
export interface DungeonRollDelveState {
  rngSeed: number;
  round: number;
  rolls: number[];
  bonus: number;
...
export type DungeonRollDelveAction = { type: "roll" } | { type: "next" };
export function initialState(seed: number, _s: DungeonRollDelveSettings): DungeonRollDelveState {
...
export function reducer(state: DungeonRollDelveState, action: DungeonRollDelveAction): DungeonRollDelveState {
...
export function isTerminal(state: DungeonRollDelveState): { score: number } | null {
