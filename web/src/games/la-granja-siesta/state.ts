...
export interface LaGranjaSiestaSettings { dummy: boolean; }
export interface LaGranjaSiestaState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type LaGranjaSiestaAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: LaGranjaSiestaSettings): LaGranjaSiestaState {
  return {
    rngSeed: seed,
...
export function reducer(state: LaGranjaSiestaState, action: LaGranjaSiestaAction): LaGranjaSiestaState {
...
export function isTerminal(state: LaGranjaSiestaState): { score: number } | null {
