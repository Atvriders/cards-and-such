...
import "./Game.css";
export function SevensDiceGame({ state, dispatch, onGameOver }: GameProps<SevensDiceState, SevensDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-sevens-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as SevensDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-sevens-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SevensDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
