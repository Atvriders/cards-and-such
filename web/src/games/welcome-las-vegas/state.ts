...
export interface WelcomeLasVegasSettings { dummy: boolean; }
export interface WelcomeLasVegasState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type WelcomeLasVegasAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: WelcomeLasVegasSettings): WelcomeLasVegasState {
  return {
    rngSeed: seed,
...
export function reducer(state: WelcomeLasVegasState, action: WelcomeLasVegasAction): WelcomeLasVegasState {
...
export function isTerminal(state: WelcomeLasVegasState): { score: number } | null {
