...
export interface CardSnakeLineSettings { dummy: boolean; }
export interface CardSnakeLineState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardSnakeLineAction = { type: "draw" } | { type: "next" };
...
export function cardWins(c: number): boolean { const r = c % 13; const isRedColor = c >= 13 && c < 39; return (r % 2 === 0) === isRedColor; }
export function initialState(seed: number, _settings: CardSnakeLineSettings): CardSnakeLineState {
  return { rngSeed: seed, round: 1, card: null, score: 0, phase: "draw", lastWin: false, lastPts: 0 };
}
export function reducer(state: CardSnakeLineState, action: CardSnakeLineAction): CardSnakeLineState {
...
export function isTerminal(state: CardSnakeLineState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
