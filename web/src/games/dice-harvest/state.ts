...
export type DiceHarvestAction = { type: "roll" } | { type: "next" };
export function isWin(a: number, b: number): boolean { return a + b >= 9; }
...
export function isTerminal(state: DiceHarvestState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
