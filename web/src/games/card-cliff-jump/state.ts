...
export interface CardCliffJumpSettings { dummy: boolean; }
export interface CardCliffJumpState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardCliffJumpAction = { type: "draw" } | { type: "next" };
...
export function cardWins(c: number): boolean { const r = c % 13; return r >= 9; }
export function initialState(seed: number, _settings: CardCliffJumpSettings): CardCliffJumpState {
  return { rngSeed: seed, round: 1, card: null, score: 0, phase: "draw", lastWin: false, lastPts: 0 };
}
export function reducer(state: CardCliffJumpState, action: CardCliffJumpAction): CardCliffJumpState {
...
export function isTerminal(state: CardCliffJumpState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
