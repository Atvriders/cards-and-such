...
export interface KingsQueensSettings { dummy: boolean; }

export interface KingsQueensState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const kings = h.filter(c=>cardRank(c)===13).length; const queens = h.filter(c=>cardRank(c)===12).length; s += Math.min(kings, queens)*15; s += (kings+queens)*3; if (s===0) s=1; return s;
}
...
export function reducer(state: KingsQueensState, action: KingsQueensAction): KingsQueensState {
...
export function isTerminal(state: KingsQueensState): { score: number } | null {
