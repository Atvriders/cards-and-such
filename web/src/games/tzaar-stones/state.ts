...
export interface TzaarStonesSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface TzaarStonesState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type TzaarStonesAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: TzaarStonesSettings): TzaarStonesState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: TzaarStonesState, action: TzaarStonesAction): TzaarStonesState {
...
export function isTerminal(state: TzaarStonesState): { score: number } | null {
