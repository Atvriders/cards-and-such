...
// UFO Zap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface UfoZapSettings { dummy: boolean; }

export interface UfoZapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface UfoZapState {
  rngSeed: number;
  targets: UfoZapTarget[];
  nextId: number;
...
export type UfoZapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: UfoZapSettings): UfoZapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: UfoZapState, action: UfoZapAction): UfoZapState {
...
export function isTerminal(state: UfoZapState): { score: number } | null {
