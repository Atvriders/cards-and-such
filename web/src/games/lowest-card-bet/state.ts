...
export interface LowestCardBetSettings { rounds: "10"|"20" }
export interface LowestCardBetState {
...
export type LowestCardBetAction = { type:"bet"; amount:number } | { type:"next" };
...
export function initialState(seed: number, settings: LowestCardBetSettings): LowestCardBetState {
...
export function reducer(state: LowestCardBetState, action: LowestCardBetAction): LowestCardBetState {
  if(state.phase==="gameover") return state;
  switch(action.type){
    case "bet": {
      if(state.phase!=="betting") return state;
      const bet=Math.min(action.amount,state.chips);
      const curVal=cardValue(state.currentCard);const nxtVal=cardValue(state.nextCard);
      ...
      return {...state,bet,chips:newChips,lastResult:result,phase:isLast?"gameover":"reveal"};
    }
    case "next": {
      if(state.phase!=="reveal") return state;
      const newPos=state.pos+1;
      if(state.round>=state.maxRounds||newPos+1>=state.deck.length) return {...state,phase:"gameover"};
...
export function isTerminal(state: LowestCardBetState): { score:number }|null { return state.phase==="gameover"?{score:state.chips}:null; }
