    ...
    return (
      <div className={`${P}-wrap`}>
        <div className={`${P}-final`}>
          <h2 className={`${P}-final-title`}>Apples Big Picture Quiz — Complete</h2>
    ...
    <div className={`${P}-wrap`}>
        ...
        {q.choices.map((c, i) => {
          ...
          return (
            <button
              ...
              onClick={() => dispatch({ type: "answer", choice: i, elapsedMs: Date.now() - start } as ApplesBigPictureQuizAction)}
            >{c}</button>
          );
        })}
      </div>
      {state.phase === "feedback" && (
        <>
          ...
          <button className={`${P}-next`} type="button" onClick={() => dispatch({ type: "next" } as ApplesBigPictureQuizAction)}>
