...
export interface KittyWhistSettings { dummy: boolean; }
export interface KittyWhistState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type KittyWhistAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: KittyWhistSettings): KittyWhistState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: KittyWhistState, action: KittyWhistAction): KittyWhistState {
...
export function isTerminal(state: KittyWhistState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
