...
export function KittyWhistGame({ state, dispatch, onGameOver }: GameProps<KittyWhistState, KittyWhistSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as KittyWhistAction)}>Trick!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as KittyWhistAction)}>Next</button>
      </>}
    </div>
  );
}
