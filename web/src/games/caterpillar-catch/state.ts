...
export interface CaterpillarCatchSettings { dummy: boolean; }
...
export interface CaterpillarCatchState {
  rngSeed: number;
  bugs: Bug[];
  nextId: number;
...
export type CaterpillarCatchAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CaterpillarCatchSettings): CaterpillarCatchState {
  return { rngSeed: seed, bugs: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CaterpillarCatchState, action: CaterpillarCatchAction): CaterpillarCatchState {
...
export function isTerminal(state: CaterpillarCatchState): { score: number } | null {
