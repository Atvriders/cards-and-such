...
export const ASSET_COST = 45;
export const HIRE_COST = 65;

export interface HotelAcquisitionSettings { dummy: boolean; }
export interface HotelAcquisitionState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type HotelAcquisitionAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: HotelAcquisitionSettings): HotelAcquisitionState {
  return { rngSeed: seed, turn: 1, cash: 250, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Market shift","Demand spike","Supply glut","Trade pact","Toll levy","Tax break","Boom day","Slow day"];

export function reducer(state: HotelAcquisitionState, action: HotelAcquisitionAction): HotelAcquisitionState {
  ...
  if (state.phase === "choosing") {
    ...
    const div = assets * Math.max(1, Math.floor(45 * 0.18));
    cash += div;
    const pay = workers * Math.max(1, Math.floor(65 * 0.18));
    cash += pay;
...
export function score(s: HotelAcquisitionState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: HotelAcquisitionState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
