...
const MarbleRoll = /* @__PURE__ */ lazy(() => import("./MarbleRoll.js").then((mod) => ({ default: mod.MarbleRoll as unknown as React.ComponentType<unknown> })));
...
Tips: Don't over-tilt — short bursts of input work better than holding a direction. Watch the marble's speed and anticipate where it will be in half a second. The hole is larger than it looks, so aim for the general vicinity and let the marble roll in naturally.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver) return null; return { selector: '[data-testid="hint-target-marble-roll-action"]', pulses: 3 }; },
