...
import type { DanceStylesQuizState, DanceStylesQuizAction, DanceStylesQuizSettings } from "./state.js";
import { isTerminal } from "./state.js";
import "./Game.css";

export function DanceStylesQuiz({
  state,
  dispatch,
  onGameOver,
}: GameProps<DanceStylesQuizState, DanceStylesQuizSettings>): JSX.Element {
    ...
    <div className="qz-wrap">
      ...
      {!state.done && entry ? (
        <>
            ...
            {entry.choices.map((choice, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => dispatch({ type: "select", index: i } as DanceStylesQuizAction)}
                >
                  {choice}
                </button>
              );
          ...
          {state.selected !== null && (
            <button className="qz-next" onClick={() => dispatch({ type: "next" } as DanceStylesQuizAction)}>
