// Rook — Classic trick-taking game — set-trump duel here.
...
export type RookPhase = "playing" | "done";
...
export type RookAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `rook-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: RookState, trick: readonly { seat: number; card: Card }[]): RookState {
...
export function reducer(state: RookState, action: RookAction): RookState {
  ...
  let s: RookState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: RookState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: RookState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): RookState {
...
export function isTerminal(state: RookState): { score: number } | null {
