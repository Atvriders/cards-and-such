...
const MadrassoTrickGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.MadrassoTrickGame as unknown as React.ComponentType<unknown> })));
...
export const madrassoTrickPlugin: GamePlugin<MadrassoTrickState, MadrassoTrickAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: MadrassoTrickState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-madrc-${pick.id}"]`, pulses: 3 };
    }
