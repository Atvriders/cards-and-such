...
export interface SeahorseSpinSettings { dummy: boolean; }
export interface Critter { id: number; lane: number; ticksLeft: number; }
export interface SeahorseSpinState { rngSeed: number; critters: Critter[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type SeahorseSpinAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: SeahorseSpinSettings): SeahorseSpinState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: SeahorseSpinState, action: SeahorseSpinAction): SeahorseSpinState {
...
export function isTerminal(s: SeahorseSpinState): { score: number } | null { return s.phase==="done" ? { score: s.score } : null; }
