...
export interface FiveCardStudClassicSettings { rounds: "10"; }

export interface FiveCardStudClassicState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type FiveCardStudClassicAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FiveCardStudClassicSettings): FiveCardStudClassicState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: FiveCardStudClassicState, action: FiveCardStudClassicAction): FiveCardStudClassicState {
...
export function isTerminal(state: FiveCardStudClassicState): { score: number } | null {
