...
export interface GumballGrabSettings { dummy: boolean; }
...
export interface GumballGrabState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type GumballGrabAction =
  | { type: "tick" }
  | { type: "catch"; id: number };

export function initialState(seed: number, _settings: GumballGrabSettings): GumballGrabState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, caught: 0, missed: 0, phase: "playing" };
}

export function reducer(state: GumballGrabState, action: GumballGrabAction): GumballGrabState {
...
export function isTerminal(state: GumballGrabState): { score: number } | null {
