...
const GumballGrabGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.GumballGrabGame as unknown as React.ComponentType<unknown> })));
...
export const gumballGrabPlugin: GamePlugin<GumballGrabState, GumballGrabAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: GumballGrabState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-gumball-grab-target"]', pulses: 3 };
  },
  component:GumballGrabGame,
};
