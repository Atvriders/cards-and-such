...
describe("three-shuffles-and-a-draw", () => {
  it("deals the correct total card count", () => {
    const s = initialState(1, S);
    const total = s.piles.reduce((sum, p) => sum + p.cards.length, 0);
    // Variants without a stock leave some cards undealt; just confirm
    // the deal stays within deck bounds.
    expect(total).toBeGreaterThan(0);
    expect(total).toBeLessThanOrEqual(52 * cfg.copies);
  });
