...
export interface IronCrossRevealSettings { rounds: "10"; }

export interface IronCrossRevealState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type IronCrossRevealAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: IronCrossRevealSettings): IronCrossRevealState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: IronCrossRevealState, action: IronCrossRevealAction): IronCrossRevealState {
...
export function isTerminal(state: IronCrossRevealState): { score: number } | null {
