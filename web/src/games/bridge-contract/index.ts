...
const BridgeContractGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BridgeContractGame as unknown as React.ComponentType<unknown> })));
...
export const bridgeContractPlugin: GamePlugin<BridgeContractState, BridgeContractAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: BridgeContractState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-bridgec-${pick.id}"]`, pulses: 3 };
    }
