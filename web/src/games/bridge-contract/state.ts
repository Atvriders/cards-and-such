...
export interface BridgeContractSettings { dummy: boolean; }
export interface BridgeContractState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type BridgeContractAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: BridgeContractSettings): BridgeContractState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: BridgeContractState, action: BridgeContractAction): BridgeContractState {
...
export function isTerminal(state: BridgeContractState): { score: number } | null {
