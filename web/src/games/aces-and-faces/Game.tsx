...
import "./Game.css";
export function AcesAndFacesGame({ state, dispatch, onGameOver }: GameProps<AcesAndFacesState, AcesAndFacesSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-aces-and-faces-deal" onClick={() => dispatch({ type: "deal" } as AcesAndFacesAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-aces-and-faces-next" onClick={() => dispatch({ type: "next" } as AcesAndFacesAction)}>Next</button>
      </>}
    </div>
  );
}
