...
const AcesAndFacesGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.AcesAndFacesGame as unknown as React.ComponentType<unknown> })));
...
export const acesAndFacesPlugin: GamePlugin<AcesAndFacesState, AcesAndFacesAction, typeof settings> = {
  ...
  reducer,isTerminal,  hint: (state: AcesAndFacesState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-aces-and-faces-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-aces-and-faces-next"]', pulses: 3 };
    return null;
  },
  component:AcesAndFacesGame,
};
