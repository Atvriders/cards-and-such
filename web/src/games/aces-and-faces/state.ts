...
export interface AcesAndFacesSettings { dummy: boolean; }
export interface AcesAndFacesState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type AcesAndFacesAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AcesAndFacesSettings): AcesAndFacesState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: AcesAndFacesState, action: AcesAndFacesAction): AcesAndFacesState {
...
export function isTerminal(state: AcesAndFacesState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
