...
export interface BadeucyPokerSettings { dummy: boolean; }
export interface BadeucyPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type BadeucyPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BadeucyPokerSettings): BadeucyPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: BadeucyPokerState, action: BadeucyPokerAction): BadeucyPokerState {
...
export function isTerminal(state: BadeucyPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
