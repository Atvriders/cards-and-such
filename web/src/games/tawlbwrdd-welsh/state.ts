...
export interface TawlbwrddWelshSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface TawlbwrddWelshState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type TawlbwrddWelshAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: TawlbwrddWelshSettings): TawlbwrddWelshState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: TawlbwrddWelshState, action: TawlbwrddWelshAction): TawlbwrddWelshState {
...
export function isTerminal(state: TawlbwrddWelshState): { score: number } | null {
