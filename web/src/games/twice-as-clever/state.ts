...
export interface TwiceAsCleverSettings { rounds: "10"; }

export interface TwiceAsCleverState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type TwiceAsCleverAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: TwiceAsCleverSettings): TwiceAsCleverState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: TwiceAsCleverState, action: TwiceAsCleverAction): TwiceAsCleverState {
...
export function isTerminal(state: TwiceAsCleverState): { score: number } | null {
