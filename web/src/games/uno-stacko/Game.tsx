...
export function UnoStackoGame({ state, dispatch, onGameOver }: GameProps<UnoStackoState, UnoStackoSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="uno-stacko-wrap"><div className="uno-stacko-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="uno-stacko-final">{state.score} pts</div></div></div>;
  return (
    <div className="uno-stacko-wrap">
