...
const UnoStackoGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.UnoStackoGame as unknown as React.ComponentType<unknown> })));
...
export const unoStackoPlugin: GamePlugin<UnoStackoState, UnoStackoAction, typeof settings> = {
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-uno-stacko-primary"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-uno-stacko-next"]', pulses: 3 };
    return null;
  }, component: UnoStackoGame,
};
