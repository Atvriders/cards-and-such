...
export interface UnoStackoSettings { dummy: boolean; }
export interface UnoStackoState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type UnoStackoAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: UnoStackoSettings): UnoStackoState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: UnoStackoState, action: UnoStackoAction): UnoStackoState {
...
export function isTerminal(state: UnoStackoState): { score: number } | null {
