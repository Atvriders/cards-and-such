...
export interface ManilaPokerSettings { dummy: boolean; }
export interface ManilaPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ManilaPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ManilaPokerSettings): ManilaPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ManilaPokerState, action: ManilaPokerAction): ManilaPokerState {
...
export function isTerminal(state: ManilaPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
