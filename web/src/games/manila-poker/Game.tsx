...
export function ManilaPokerGame({ state, dispatch, onGameOver }: GameProps<ManilaPokerState, ManilaPokerSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap thmManila"><div className="dm-done"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap thmManila">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as ManilaPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ManilaPokerAction)}>Next</button>
      </>}
    </div>
  );
}
