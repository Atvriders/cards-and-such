...
export interface RotoPokerSettings { dummy: boolean; }
export interface RotoPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type RotoPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: RotoPokerSettings): RotoPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: RotoPokerState, action: RotoPokerAction): RotoPokerState {
...
export function isTerminal(state: RotoPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
