...
import "./Game.css";
export function RotoPokerGame({ state, dispatch, onGameOver }: GameProps<RotoPokerState, RotoPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-roto-poker-deal" onClick={() => dispatch({ type: "deal" } as RotoPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-roto-poker-next" onClick={() => dispatch({ type: "next" } as RotoPokerAction)}>Next</button>
      </>}
    </div>
  );
}
