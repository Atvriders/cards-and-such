...
// Match-3 Hex Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface Match3HexMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface Match3HexMiniState {
  rngSeed: number;
  grid: Grid;
...
export type Match3HexMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: Match3HexMiniSettings): Match3HexMiniState {
...
export function reducer(state: Match3HexMiniState, action: Match3HexMiniAction): Match3HexMiniState {
...
export function isTerminal(state: Match3HexMiniState): { score: number } | null {
