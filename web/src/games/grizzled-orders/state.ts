...
export interface GrizzledOrdersSettings { dummy: boolean; }
export interface GrizzledOrdersState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type GrizzledOrdersAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: GrizzledOrdersSettings): GrizzledOrdersState {
