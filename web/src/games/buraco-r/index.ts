...
const BuracoRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BuracoRGame as unknown as React.ComponentType<unknown> })));
...
export const buracoRPlugin: GamePlugin<BuracoRState, BuracoRAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: BuracoRState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "play") return { selector: '[data-testid="hint-target-buraco-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-buraco-r-next"]', pulses: 3 };
    return null;
  },
  component: BuracoRGame,
};
