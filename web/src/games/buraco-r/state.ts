...
export interface BuracoRSettings { dummy: boolean; }
export interface BuracoRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type BuracoRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: BuracoRSettings): BuracoRState {
...
export function reducer(state: BuracoRState, action: BuracoRAction): BuracoRState {
...
export function isTerminal(state: BuracoRState): { score: number } | null {
