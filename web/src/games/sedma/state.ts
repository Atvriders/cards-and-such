// Sedma — Czech 7-takes-all capture game — simplified head-to-head trick duel.
...
export type SedmaPhase = "playing" | "done";
...
export type SedmaAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `sedma-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: SedmaState, trick: readonly { seat: number; card: Card }[]): SedmaState {
...
export function reducer(state: SedmaState, action: SedmaAction): SedmaState {
  ...
  let s: SedmaState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: SedmaState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: SedmaState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): SedmaState {
...
export function isTerminal(state: SedmaState): { score: number } | null {
