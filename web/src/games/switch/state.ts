...
export interface SwitchSettings { dummy: boolean; }
export interface SwitchState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SwitchAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SwitchSettings): SwitchState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SwitchState, action: SwitchAction): SwitchState {
...
export function isTerminal(state: SwitchState): { score: number } | null {
