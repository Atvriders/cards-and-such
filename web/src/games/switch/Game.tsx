...
export function SwitchGame({ state, dispatch, onGameOver }: GameProps<SwitchState, SwitchSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="switch-wrap sw-shed"><div className="switch-done bounce-in"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="switch-final">{state.score} pts</div></div></div>;
  return (
    <div className="switch-wrap sw-shed fade-in">
      <div className="switch-info">Round {state.round} / {TOTAL_ROUNDS} — W{state.wins} L{state.losses}</div>
      <div className="switch-score pulse">{state.score} pts</div>
