...
export function SwitchGame({ state, dispatch, onGameOver }: GameProps<SwitchState, SwitchSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="switch-wrap"><div className="switch-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="switch-final">{state.score} pts</div></div></div>;
  return (
