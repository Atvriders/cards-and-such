import { describe, it, expect } from "vitest";
import { initialState, reducer, isTerminal, TOTAL_ROUNDS } from "./state.js";
const S = { dummy: false };
describe("TriadTower", () => {
  it("starts in dealing phase", () => { const s=initialState(1,S); expect(s.phase).toBe("dealing"); expect(s.score).toBe(0); });
  it("deal produces 3 cards", () => { const s=reducer(initialState(1,S), { type:"deal" }); expect(s.hand.length).toBe(3); });
  it("score is 0 or 30 after deal", () => { const s=reducer(initialState(1,S), { type:"deal" }); expect([0,30]).toContain(s.score); });
  it("isTerminal null while playing", () => { expect(isTerminal(initialState(1,S))).toBeNull(); });
  it("game ends after all rounds", () => {
    let s=initialState(1,S);
    for(let i=0;i<TOTAL_ROUNDS;i++){ s=reducer(s,{type:"deal"}); if(s.phase==="scored") s=reducer(s,{type:"next"}); }
    expect(s.phase).toBe("done");
  });
});
