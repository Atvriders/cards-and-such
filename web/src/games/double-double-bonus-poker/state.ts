...
export interface DoubleDoubleBonusPokerSettings { rounds: "10"; }

export interface DoubleDoubleBonusPokerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type DoubleDoubleBonusPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DoubleDoubleBonusPokerSettings): DoubleDoubleBonusPokerState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: DoubleDoubleBonusPokerState, action: DoubleDoubleBonusPokerAction): DoubleDoubleBonusPokerState {
...
export function isTerminal(state: DoubleDoubleBonusPokerState): { score: number } | null {
