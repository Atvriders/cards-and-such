...
// Merge Mansion Mini: 5x5 merge board. Place a tile; three-of-a-kind in adjacent cells merges into a tier-up tile.

export const SIZE = 5;
export const MAX_TIER = 7;
export const MAX_MOVES = 60;

export interface MergeMansionMiniSettings { dummy: boolean; }
export type Cell = number; // 0 = empty, 1..MAX_TIER tiers
export type Grid = Cell[][];

export interface MergeMansionMiniState {
  rngSeed: number;
  grid: Grid;
  next: Cell;
  score: number;
...
export type MergeMansionMiniAction = { type: "place"; row: number; col: number };
...
export function initialState(seed: number, _settings: MergeMansionMiniSettings): MergeMansionMiniState {
...
export function reducer(state: MergeMansionMiniState, action: MergeMansionMiniAction): MergeMansionMiniState {
...
export function isTerminal(state: MergeMansionMiniState): { score: number } | null {
