// Madrasso — Venetian 4-player partnership game — simplified to a 1v1 duel.
...
export type MadrassoPhase = "playing" | "done";

export interface MadrassoState {
  rngSeed: number;
...
export type MadrassoAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `madrasso-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: MadrassoState, trick: readonly { seat: number; card: Card }[]): MadrassoState {
...
export function reducer(state: MadrassoState, action: MadrassoAction): MadrassoState {
  ...
  let s: MadrassoState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: MadrassoState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: MadrassoState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): MadrassoState {
...
export function isTerminal(state: MadrassoState): { score: number } | null {
