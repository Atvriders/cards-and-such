...
export interface IDoubtItSettings { dummy: boolean; }
export interface IDoubtItState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type IDoubtItAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: IDoubtItSettings): IDoubtItState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: IDoubtItState, action: IDoubtItAction): IDoubtItState {
...
export function isTerminal(state: IDoubtItState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
