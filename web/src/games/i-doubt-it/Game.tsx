...
export function IDoubtItGame({ state, dispatch, onGameOver }: GameProps<IDoubtItState, IDoubtItSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap idi-shed">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as IDoubtItAction)}>Doubt!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as IDoubtItAction)}>Next</button>
      </>}
    </div>
  );
}
