...
import "./Game.css";
export function IDoubtItGame({ state, dispatch, onGameOver }: GameProps<IDoubtItState, IDoubtItSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") {
    return <div className="dm-wrap idi-shed"><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses} T: {state.ties}</div><div className="dm-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="dm-wrap idi-shed">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-i-doubt-it-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as IDoubtItAction)}>Doubt!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-i-doubt-it-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as IDoubtItAction)}>Next</button>
      </>}
    </div>
  );
}
