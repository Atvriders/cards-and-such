...
const IDoubtItGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.IDoubtItGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: IDoubtItState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-i-doubt-it-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-i-doubt-it-secondary"]', pulses: 3 };
  return null;
};
...
export const iDoubtItPlugin: GamePlugin<IDoubtItState, IDoubtItAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: IDoubtItGame,
};
