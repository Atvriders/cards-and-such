...
import "./Game.css";
export function CardPuzzleGame({ state, dispatch, onGameOver }: GameProps<CardPuzzleState, CardPuzzleSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "result" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardPuzzleAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
