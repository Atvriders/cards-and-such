...
export interface KiwiClickerSettings { dummy: boolean; }
...
export interface KiwiClickerState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type KiwiClickerAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: KiwiClickerSettings): KiwiClickerState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: KiwiClickerState, action: KiwiClickerAction): KiwiClickerState {
...
export function isTerminal(state: KiwiClickerState): { score: number } | null {
