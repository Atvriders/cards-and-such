...
export const TOTAL_TILES = 13;
export const NUM_TYPES = 4;
export interface BlokusClassicSettings { dummy: boolean; }
export interface BlokusClassicState {
  rngSeed: number;
  cells: number[];
  queue: number[];
  placed: number;
...
export type BlokusClassicAction = { type: "place"; index: number };
...
export function initialState(seed: number, _s: BlokusClassicSettings): BlokusClassicState {
...
export function reducer(state: BlokusClassicState, action: BlokusClassicAction): BlokusClassicState {
...
export function isTerminal(state: BlokusClassicState): { score: number } | null {
