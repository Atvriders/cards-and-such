...
export interface TowerOfLondonSettings { dummy: boolean; }

export interface TowerOfLondonState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type TowerOfLondonAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const suits = new Set(h.map(cardSuit)); s += (4-suits.size)*5; const ranks=h.map(cardRank); const counts:Record<number,number>={}; for (const r of ranks) counts[r]=(counts[r]||0)+1; const pairs = Object.values(counts).filter(v=>v>=2).length; s += pairs*8; if (s===0) s=1; return s;
}
...
export function reducer(state: TowerOfLondonState, action: TowerOfLondonAction): TowerOfLondonState {
...
export function isTerminal(state: TowerOfLondonState): { score: number } | null {
