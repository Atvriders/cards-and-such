...
export interface CandyCaneGrabSettings { dummy: boolean; }
export interface CandyCaneGrabState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type CandyCaneGrabAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: CandyCaneGrabSettings): CandyCaneGrabState {
...
export function reducer(state: CandyCaneGrabState, action: CandyCaneGrabAction): CandyCaneGrabState {
...
export function isTerminal(state: CandyCaneGrabState): { score: number } | null {
