...
export function CalculationSolitaireGame({ state, dispatch, onGameOver }: GameProps<CalculationSolitaireState, CalculationSolitaireSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as CalculationSolitaireAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
