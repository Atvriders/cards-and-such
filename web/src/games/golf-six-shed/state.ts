...
export interface GolfSixShedSettings { dummy: boolean; }
export interface GolfSixShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type GolfSixShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: GolfSixShedSettings): GolfSixShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: GolfSixShedState, action: GolfSixShedAction): GolfSixShedState {
...
export function isTerminal(state: GolfSixShedState): { score: number } | null {
