...
export function GolfSixShedGame({ state, dispatch, onGameOver }: GameProps<GolfSixShedState, GolfSixShedSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as GolfSixShedAction)}>Play Round</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as GolfSixShedAction)}>Next</button>
      </>}
    </div>
  );
}
