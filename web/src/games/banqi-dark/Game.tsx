...
export function AbsGame({ state, dispatch, onGameOver }: GameProps<AbsState, AbsSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    ...
    return <div className="ab-wrap"><h3>Banqi (Dark Chess)</h3><div className="ab-done bounce-in"><h2>{msg}</h2><div className="ab-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="ab-wrap fade-in">
      <h3>Banqi (Dark Chess)</h3>
      ...
      <div className="ab-board" style={{ gridTemplateColumns: `repeat(${SIZE}, 1fr)` }}>
        {Array.from({ length: SIZE * SIZE }).map((_, i) => {
          ...
          return <button title="Select cell" key={i} className={`ab-cell ${cls}`} disabled={v !== null} onClick={() => dispatch({ type:"place", idx:i } as AbsAction)}>{v ?? ""}</button>;
        })}
      </div>
    </div>
  );
