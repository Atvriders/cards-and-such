...
export function ThrowThrowBurritoQuiz({ state, dispatch, onGameOver }: GameProps<ThrowThrowBurritoQuizState, { rounds: "10" }>): JSX.Element {
  ...
  return (
    <div className="g-wrap">
        ...
        {q.a.map((opt, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={state.phase !== "ready"}
              data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type: "answer", choice: i } as ThrowThrowBurritoQuizAction)}>
              {opt}
            </button>
          );
        })}
      ...
      {state.phase === "answered" && (
        <button className="g-btn" onClick={() => dispatch({ type: "next" } as ThrowThrowBurritoQuizAction)}>Next</button>
      )}
    </div>
  );
}
