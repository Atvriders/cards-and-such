...
import "./Game.css";
export function CardGlacierGame({ state, dispatch, onGameOver }: GameProps<CardGlacierState, CardGlacierSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      <div className="cm-info">Glacier — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardGlacierAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
