...
const TaffyTapGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TaffyTapGame as unknown as React.ComponentType<unknown> })));
...
export const taffyTapPlugin: GamePlugin<TaffyTapState, TaffyTapAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: TaffyTapState) => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: ".taffytap-target", pulses: 3 };
  },
  component:TaffyTapGame,
};
