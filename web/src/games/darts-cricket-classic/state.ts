...
export interface DartsCricketClassicSettings { dummy: boolean; }

export interface DartsCricketClassicState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DartsCricketClassicAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DartsCricketClassicSettings): DartsCricketClassicState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DartsCricketClassicState, action: DartsCricketClassicAction): DartsCricketClassicState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DartsCricketClassicState> = {};
...
export function isTerminal(state: DartsCricketClassicState): { score: number } | null {
