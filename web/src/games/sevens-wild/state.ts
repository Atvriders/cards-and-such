...
export interface SevensWildSettings { dummy: boolean; }
export interface SevensWildState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SevensWildAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SevensWildSettings): SevensWildState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SevensWildState, action: SevensWildAction): SevensWildState {
...
export function isTerminal(state: SevensWildState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
