...
import "./Game.css";
export function SevensWildGame({ state, dispatch, onGameOver }: GameProps<SevensWildState, SevensWildSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-sevens-wild-deal" onClick={() => dispatch({ type: "deal" } as SevensWildAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-sevens-wild-next" onClick={() => dispatch({ type: "next" } as SevensWildAction)}>Next</button>
      </>}
    </div>
  );
}
