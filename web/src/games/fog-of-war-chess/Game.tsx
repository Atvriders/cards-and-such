...
export function FogOfWarChessGame({ state, dispatch, onGameOver }: GameProps<FogOfWarChessState, FogOfWarChessSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="fogch-wrap"><div className="fogch-done"><h2>Done!</h2><p>Correct: {state.correctCount} / {state.questions.length}</p><p style={{ fontSize:"1.8rem",fontWeight:900,color:"#27ae60" }}>{state.score} pts</p></div></div>;
  const q = state.questions[state.currentIndex]!;
  const isResult = state.phase === "result";
  const urgent = state.timeLeft <= 5 && !state.submitted;
  return (
    <div className="fogch-wrap">
      <div className="fogch-header">
        ...
        <span className="fogch-score">{state.score} pts</span>
      </div>
      ...
      <div className="fogch-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as FogOfWarChessAction)}><span className="fogch-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
