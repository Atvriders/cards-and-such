// Differenzler Jass — Bidding Jass game where you predict your trick total.
...
export type DifferenzlerJassPhase = "playing" | "done";

export interface DifferenzlerJassState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: DifferenzlerJassPhase;
  playerLeads: boolean;
  message: string;
}

export type DifferenzlerJassAction = { type: "play"; cardId: string };

function buildDeck(): Card[] {
  const cards: Card[] = [];
  const RANKS: Rank[] = [1, 6, 7, 8, 9, 10, 11, 12, 13];
  const SUITS: Suit[] = ["♠", "♥", "♦", "♣"];
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `differenzler-jass-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: DifferenzlerJassState, trick: readonly { seat: number; card: Card }[]): DifferenzlerJassState {
...
export function reducer(state: DifferenzlerJassState, action: DifferenzlerJassAction): DifferenzlerJassState {
  ...
  let s: DifferenzlerJassState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: DifferenzlerJassState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: DifferenzlerJassState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): DifferenzlerJassState {
...
export function isTerminal(state: DifferenzlerJassState): { score: number } | null {
