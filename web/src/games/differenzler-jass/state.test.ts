...
describe("differenzler-jass initialState", () => {
  it("deals correct number of cards to each player", () => {
    const s = initialState(42);
    expect(s.playerHand.length).toBe(9);
    expect(s.botHand.length).toBe(9);
  });
...
describe("differenzler-jass legalPlays", () => {
  it("all cards legal when leading", () => {
    const s = initialState(42);
    expect(legalPlays(s.playerHand, s.currentTrick).length).toBeGreaterThanOrEqual(9);
  });
});

describe("differenzler-jass reducer", () => {
...
describe("differenzler-jass full game", () => {
