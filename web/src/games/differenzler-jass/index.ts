...
const DifferenzlerJassGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DifferenzlerJassGame as unknown as React.ComponentType<unknown> })));
...
export const differenzlerJassPlugin: GamePlugin<DifferenzlerJassState, DifferenzlerJassAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DifferenzlerJassState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-difjc-${pick.id}"]`, pulses: 3 };
    }
