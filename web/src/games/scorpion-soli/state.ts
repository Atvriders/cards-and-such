...
export interface ScorpionSoliSettings { dummy: boolean; }

export interface ScorpionSoliState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type ScorpionSoliAction =
  | { type: "keep" }
...
function scoreHand(hand: number[]): { score: number; reason: string } {
  ...
  // Run-heavy variant
  if (best >= 3) { score += best * best * 2; reasons.push(`run${best}`); }
  if (pairs >= 1) { score += pairs * 8; reasons.push(`${pairs} pair`); }
  if (flush) { score += 14; reasons.push("flush"); }
  if (faces >= 1) { score += faces * 5; reasons.push(`${faces} face`); }
  if (score === 0) score = 1;
  return { score, reason: reasons.join(",") || "weak" };
}

export function initialState(seed: number, _s: ScorpionSoliSettings): ScorpionSoliState {
...
export function reducer(state: ScorpionSoliState, action: ScorpionSoliAction): ScorpionSoliState {
...
export function isTerminal(state: ScorpionSoliState): { score: number } | null {
