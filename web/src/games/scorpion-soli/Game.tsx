...
export function ScorpionSoliGame({ state, dispatch, onGameOver }: GameProps<ScorpionSoliState, ScorpionSoliSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as ScorpionSoliAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-scorpion-soli-keep" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as ScorpionSoliAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as ScorpionSoliAction)}>Discard Hand</button>
      </div>
