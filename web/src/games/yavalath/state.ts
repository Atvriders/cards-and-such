...
export interface YavalathSettings { dummy: boolean; }
...
export type YavalathAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: YavalathSettings): YavalathState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: YavalathState, action: YavalathAction): YavalathState {
...
export function isTerminal(state: YavalathState): { score: number } | null {
