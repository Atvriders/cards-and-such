...
export interface HandFootRSettings { dummy: boolean; }
export interface HandFootRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type HandFootRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: HandFootRSettings): HandFootRState {
...
export function reducer(state: HandFootRState, action: HandFootRAction): HandFootRState {
...
export function isTerminal(state: HandFootRState): { score: number } | null {
