...
export function HandFootRGame({ state, dispatch, onGameOver }: GameProps<HandFootRState, HandFootRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as HandFootRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as HandFootRAction)}>Next</button>
      </>}
    </div>
  );
}
