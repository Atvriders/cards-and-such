...
// Cloud Pop: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface CloudPopSettings { dummy: boolean; }

export interface CloudPopTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface CloudPopState {
  rngSeed: number;
  targets: CloudPopTarget[];
  nextId: number;
...
export type CloudPopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CloudPopSettings): CloudPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CloudPopState, action: CloudPopAction): CloudPopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CloudPopTarget[] = [];
...
export function isTerminal(state: CloudPopState): { score: number } | null {
