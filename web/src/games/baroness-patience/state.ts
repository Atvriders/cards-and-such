...
export interface BaronessPatienceSettings { dummy: boolean; }

export interface BaronessPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type BaronessPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const ranks=h.map(cardRank); for (let i=0;i<ranks.length;i++){ for (let j=i+1;j<ranks.length;j++){ const a=ranks[i] as number; const b=ranks[j] as number; if (a+b===13) s += 7; } } if (s===0) s=1; return s;
}
...
export function reducer(state: BaronessPatienceState, action: BaronessPatienceAction): BaronessPatienceState {
...
export function isTerminal(state: BaronessPatienceState): { score: number } | null {
