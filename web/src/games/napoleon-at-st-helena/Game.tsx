...
export function NapoleonAtStHelenaGame({ state, dispatch, onGameOver }: GameProps<NapoleonAtStHelenaState, NapoleonAtStHelenaSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as NapoleonAtStHelenaAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as NapoleonAtStHelenaAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as NapoleonAtStHelenaAction)}>Discard Hand</button>
      </div>
