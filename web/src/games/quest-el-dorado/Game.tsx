...
import "./Game.css";
export function QuestElDoradoGame({ state, dispatch, onGameOver }: GameProps<QuestElDoradoState, QuestElDoradoSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as QuestElDoradoAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
