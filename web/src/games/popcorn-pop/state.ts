...
export interface PopcornPopSettings { dummy: boolean; }
...
export interface PopcornPopState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PopcornPopAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PopcornPopSettings): PopcornPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PopcornPopState, action: PopcornPopAction): PopcornPopState {
...
export function isTerminal(state: PopcornPopState): { score: number } | null {
