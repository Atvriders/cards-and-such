...
// Threes Puzzle: 4x4 sliding tiles. Arrow keys slide+merge tiles. Reach 384 to win.

export const SIZE = 4;
export const TARGET = 384;

export interface ThreesPuzzleSettings { dummy: boolean; }
export type Cell = number; // 0 = empty, else power-of-2
export type Grid = Cell[][];

export interface ThreesPuzzleState {
  rngSeed: number;
  grid: Grid;
  score: number;
  moves: number;
...
export type ThreesPuzzleAction = { type: "slide"; dir: Direction };
...
export function initialState(seed: number, _settings: ThreesPuzzleSettings): ThreesPuzzleState {
...
export function reducer(state: ThreesPuzzleState, action: ThreesPuzzleAction): ThreesPuzzleState {
  ...
  if (action.type === "slide") {
    ...
    if (best >= TARGET && (state.phase as string) !== "won") phase = "won";
...
export function isTerminal(state: ThreesPuzzleState): { score: number } | null {
