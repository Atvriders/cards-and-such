...
export interface IdiotsDelightSettings { dummy: boolean; }

export interface IdiotsDelightState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type IdiotsDelightAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const aces = h.filter(c=>cardRank(c)===1).length; s += aces*15; const f = h.filter(c=>cardRank(c)>=11).length; s += f*2; if (s===0) s=1; return s;
}
...
export function reducer(state: IdiotsDelightState, action: IdiotsDelightAction): IdiotsDelightState {
...
export function isTerminal(state: IdiotsDelightState): { score: number } | null {
