...
const ValentineMatchGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ValentineMatchGame as unknown as React.ComponentType<unknown> })));
const settings = {
  pairs: {
    kind: "enum" as const,
    label: "Pairs",
...
Settings: choose 6, 8, or 10 pairs.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver) return null; return { selector: '[data-testid="hint-target-valentine-match-action"]', pulses: 3 }; },
