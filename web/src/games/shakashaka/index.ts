...
const Shakashaka = /* @__PURE__ */ lazy(() => import("./Shakashaka.js").then((mod) => ({ default: mod.Shakashaka as unknown as React.ComponentType<unknown> })));
...
Strategy: start with 0-clued black cells (clear all their neighbors) and 4-clued cells (force triangles in all neighbors). Then analyze which triangle orientations are forced by the need to form rectangles in the white regions — rectangles cannot have jagged or L-shaped boundaries. Work inward from constrained edges.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-shakashaka-action"]', pulses: 3 }; },
