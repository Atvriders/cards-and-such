...
export interface CardArenaMiniSettings { dummy: boolean; }
export interface CardArenaMiniState { rngSeed: number; deck: number[]; drawn: number[]; score: number; phase: "drawing" | "done"; }
export type CardArenaMiniAction = { type: "draw" } | { type: "reset" };
...
export function scoreCard(c: number): number {
  const r=c%13; return r>=9?15:0;  // J,Q,K,A face=15
}
export function initialState(seed: number, _settings: CardArenaMiniSettings): CardArenaMiniState {
  const rng=mulberry32(seed); const deck=shuffle(rng);
  return { rngSeed: seed, deck, drawn: [], score: 0, phase: "drawing" };
}
export function reducer(state: CardArenaMiniState, action: CardArenaMiniAction): CardArenaMiniState {
...
export function isTerminal(state: CardArenaMiniState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
