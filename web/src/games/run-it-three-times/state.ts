...
export interface RunItThreeTimesSettings { rounds: "10"; }

export interface RunItThreeTimesState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type RunItThreeTimesAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: RunItThreeTimesSettings): RunItThreeTimesState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: RunItThreeTimesState, action: RunItThreeTimesAction): RunItThreeTimesState {
...
export function isTerminal(state: RunItThreeTimesState): { score: number } | null {
