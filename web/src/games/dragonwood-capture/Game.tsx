...
import "./Game.css";
export function DragonwoodCaptureGame({ state, dispatch, onGameOver }: GameProps<DragonwoodCaptureState, DragonwoodCaptureSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="dm-btn" data-testid="hint-target-dragonwood-capture-roll" onClick={() => dispatch({ type:"roll" } as DragonwoodCaptureAction)}>Roll</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">+{state.lastPts} pts</div>
          <button className="dm-btn alt" data-testid="hint-target-dragonwood-capture-next" onClick={() => dispatch({ type:"next" } as DragonwoodCaptureAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
