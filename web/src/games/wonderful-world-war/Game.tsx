...
export function WonderfulWorldWarGame({ state, dispatch, onGameOver }: GameProps<WonderfulWorldWarState, WonderfulWorldWarSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Wonderful World: War or Peace</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as WonderfulWorldWarAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as WonderfulWorldWarAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
