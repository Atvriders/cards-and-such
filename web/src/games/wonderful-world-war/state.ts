...
export interface WonderfulWorldWarState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WonderfulWorldWarAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WonderfulWorldWarSettings): WonderfulWorldWarState {
...
export function score(s: WonderfulWorldWarState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WonderfulWorldWarState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
