...
export function HammerTapGame({ state, dispatch, onGameOver }: GameProps<HammerTapState, HammerTapSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as HammerTapAction), 750);
        ...
        {state.targets.map(t => {
          ...
          return (
            <button key={t.id}
              ...
              onClick={() => dispatch({ type:"pop", id:t.id } as HammerTapAction)}
