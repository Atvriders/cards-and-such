...
export interface HammerTapSettings { dummy: boolean; }
...
export interface HammerTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type HammerTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: HammerTapSettings): HammerTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: HammerTapState, action: HammerTapAction): HammerTapState {
...
export function isTerminal(state: HammerTapState): { score: number } | null {
