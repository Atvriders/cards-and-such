...
export interface SpadesSolitaireSettings { dummy: boolean; }

export interface SpadesSolitaireState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type SpadesSolitaireAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { if (cardSuit(c) === 0) s += 6; else s += 1; }
  if (s === 0) s = 1; return s;
}

export function initialState(seed: number, _s: SpadesSolitaireSettings): SpadesSolitaireState {
...
export function reducer(state: SpadesSolitaireState, action: SpadesSolitaireAction): SpadesSolitaireState {
...
export function isTerminal(state: SpadesSolitaireState): { score: number } | null {
