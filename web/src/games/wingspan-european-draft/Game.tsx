...
export function WingspanEuropeanDraftGame({ state, dispatch, onGameOver }: GameProps<WingspanEuropeanDraftState, WingspanEuropeanDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Wingspan: European</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as WingspanEuropeanDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as WingspanEuropeanDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
