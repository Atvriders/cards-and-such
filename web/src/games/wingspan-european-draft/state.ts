...
export const SUIT_NAMES = ["Forest","Grassland","Wetland","Mountain"] as const;
...
export type WingspanEuropeanDraftState = DraftState;
export type WingspanEuropeanDraftAction = DraftAction;
export interface WingspanEuropeanDraftSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: WingspanEuropeanDraftSettings): WingspanEuropeanDraftState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: WingspanEuropeanDraftState, action: WingspanEuropeanDraftAction): WingspanEuropeanDraftState {
  return draftReducer(state, action);
}

export function score(state: WingspanEuropeanDraftState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: WingspanEuropeanDraftState): { score: number } | null {
