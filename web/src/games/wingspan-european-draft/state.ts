...
export interface WingspanEuropeanDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WingspanEuropeanDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WingspanEuropeanDraftSettings): WingspanEuropeanDraftState {
...
export function score(s: WingspanEuropeanDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WingspanEuropeanDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
