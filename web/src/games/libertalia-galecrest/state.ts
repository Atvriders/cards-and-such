...
export const SUIT_NAMES: readonly string[] = ["Captain","Quartermaster","Gunner","Cook"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface LibertaliaGalecrestSettings { dummy: boolean; }
export interface LibertaliaGalecrestState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type LibertaliaGalecrestAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: LibertaliaGalecrestSettings): LibertaliaGalecrestState {
...
export function reducer(state: LibertaliaGalecrestState, action: LibertaliaGalecrestAction): LibertaliaGalecrestState {
...
export function score(s: LibertaliaGalecrestState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: LibertaliaGalecrestState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
