...
export function LibertaliaGalecrestGame({ state, dispatch, onGameOver }: GameProps<LibertaliaGalecrestState, LibertaliaGalecrestSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Libertalia: Galecrest</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as LibertaliaGalecrestAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as LibertaliaGalecrestAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
