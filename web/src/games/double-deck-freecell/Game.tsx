...
export function DoubleDeckFreecellGame({ state, dispatch, onGameOver }: GameProps<DoubleDeckFreecellState, DoubleDeckFreecellSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as DoubleDeckFreecellAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-double-deck-freecell-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as DoubleDeckFreecellAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as DoubleDeckFreecellAction)}>Discard Hand</button>
      </div>
