...
export interface DoubleDeckFreecellSettings { dummy: boolean; }

export interface DoubleDeckFreecellState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type DoubleDeckFreecellAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const suits = h.map(cardSuit);
  for (let i = 1; i < suits.length; i++) { const a = suits[i]!; const b = suits[i-1]!; const ar = (a===1||a===2); const br = (b===1||b===2); if (ar !== br) s += 4; }
  s += h.length;
  if (s === 0) s = 1; return s;
}

export function initialState(seed: number, _s: DoubleDeckFreecellSettings): DoubleDeckFreecellState {
...
export function reducer(state: DoubleDeckFreecellState, action: DoubleDeckFreecellAction): DoubleDeckFreecellState {
...
export function isTerminal(state: DoubleDeckFreecellState): { score: number } | null {
