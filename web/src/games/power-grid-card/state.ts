...
export interface PowerGridCardSettings { dummy: boolean; }
export interface PowerGridCardState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type PowerGridCardAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: PowerGridCardSettings): PowerGridCardState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Coal price up","Uranium scarce","Renewables blossom","Auction surplus","Cities expand","Resource shortage","Plant retired","New plant flips"];

export function reducer(state: PowerGridCardState, action: PowerGridCardAction): PowerGridCardState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Plant (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (assets >= 1) {
        ...
        cash += market; assets -= 1; evt = "Sold 1 Plant (+$" + market + ")"; delta = market;
      } else { evt = "No Plant to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
...
export function score(s: PowerGridCardState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: PowerGridCardState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
