// Go Stop — Korean Go-Stop simplified to a trick-style duel.
...
export type GoStopPhase = "playing" | "done";
...
export type GoStopAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `go-stop-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: GoStopState, trick: readonly { seat: number; card: Card }[]): GoStopState {
...
export function reducer(state: GoStopState, action: GoStopAction): GoStopState {
  ...
  let s: GoStopState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: GoStopState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: GoStopState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): GoStopState {
...
export function isTerminal(state: GoStopState): { score: number } | null {
