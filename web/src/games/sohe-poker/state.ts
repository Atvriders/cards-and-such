...
export interface SohePokerSettings { dummy: boolean; }
export interface SohePokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SohePokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SohePokerSettings): SohePokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SohePokerState, action: SohePokerAction): SohePokerState {
...
export function isTerminal(state: SohePokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
