...
import "./Game.css";
export function SohePokerGame({ state, dispatch, onGameOver }: GameProps<SohePokerState, SohePokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-sohe-poker-deal" onClick={() => dispatch({ type: "deal" } as SohePokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-sohe-poker-next" onClick={() => dispatch({ type: "next" } as SohePokerAction)}>Next</button>
      </>}
    </div>
  );
}
