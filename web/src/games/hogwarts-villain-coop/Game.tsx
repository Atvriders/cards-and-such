...
export function HogwartsVillainCoopGame({ state, dispatch, onGameOver }: GameProps<HogwartsVillainCoopState, HogwartsVillainCoopSettings>): JSX.Element {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as HogwartsVillainCoopAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as HogwartsVillainCoopAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
