    ...
    <div className="trivia-wrap">
      ...
      <div className="trivia-choices">
        {qq.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as CeeLoQuizAction)}><span className="trivia-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
