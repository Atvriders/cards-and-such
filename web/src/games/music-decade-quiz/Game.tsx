...
export function MusicDecadeQuiz({ state, dispatch, onGameOver }: GameProps<MusicDecadeQuizState, MusicDecadeQuizSettings>): JSX.Element {
  ...
  return (
    <div className="trivia-wrap fade-in">
      ...
      {isResult && <div className={`trivia-feedback ${state.selected===q.correct?"correct":"wrong"}`}>
        {state.selected===q.correct?"Correct! +100 pts":`Wrong! Answer: ${q.choices[q.correct]}`}
      </div>}
      <div className="trivia-actions">
        {!isResult && <button className="trivia-btn submit" disabled={state.selected===null} onClick={()=>dispatch({type:"submit"} as MusicDecadeQuizAction)}>Submit</button>}
        {isResult && <button className="trivia-btn next" onClick={()=>dispatch({type:"next"} as MusicDecadeQuizAction)}>{state.currentIndex+1>=state.questions.length?"Finish":"Next"}</button>}
      </div>
    </div>
  );
}
