...
const YambDiceGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.YambDiceGame as unknown as React.ComponentType<unknown> })));
...
export const yambDicePlugin: GamePlugin<YambDiceState, YambDiceAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: YambDiceState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "roll") return { selector: '[data-testid="hint-target-yamb-dice-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-yamb-dice-next"]', pulses: 3 };
    return null;
  },
  component:YambDiceGame,
};
