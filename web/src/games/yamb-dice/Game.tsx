...
export function YambDiceGame({ state, dispatch, onGameOver }: GameProps<YambDiceState, YambDiceSettings>): JSX.Element {
  ...
  return (
    <div className="ym-wrap">
      ...
      {state.phase === "roll" && <button className="ym-btn" onClick={() => dispatch({ type:"roll" } as YambDiceAction)}>Roll</button>}
      {state.phase === "result" && <button className="ym-btn alt" onClick={() => dispatch({ type:"next" } as YambDiceAction)}>Next</button>}
    </div>
  );
}
