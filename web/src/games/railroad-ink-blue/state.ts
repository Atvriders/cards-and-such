...
export interface RailroadInkBlueSettings { dummy: boolean; }
export interface RailroadInkBlueState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type RailroadInkBlueAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: RailroadInkBlueSettings): RailroadInkBlueState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: RailroadInkBlueState, action: RailroadInkBlueAction): RailroadInkBlueState {
...
export function isTerminal(state: RailroadInkBlueState): { score: number } | null {
