...
export function RailroadInkBlueGame({ state, dispatch, onGameOver }: GameProps<RailroadInkBlueState, RailroadInkBlueSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RailroadInkBlueAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RailroadInkBlueAction)}>Skip</button>
      )}
    </div>
  );
}
