...
export interface ThreeCardPokerCasSettings { dummy: boolean; }
export interface ThreeCardPokerCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type ThreeCardPokerCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ThreeCardPokerCasSettings): ThreeCardPokerCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: ThreeCardPokerCasState, action: ThreeCardPokerCasAction): ThreeCardPokerCasState {
...
export function isTerminal(state: ThreeCardPokerCasState): { score: number } | null {
