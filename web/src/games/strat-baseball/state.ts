...
export interface StratBaseballSettings { dummy: boolean; }

export interface StratBaseballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type StratBaseballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: StratBaseballSettings): StratBaseballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: StratBaseballState, action: StratBaseballAction): StratBaseballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<StratBaseballState> = {};
...
export function isTerminal(state: StratBaseballState): { score: number } | null {
