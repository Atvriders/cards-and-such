...
export interface PontoonCasState {
  rngSeed: number;
  round: number;
  you: number[];
  dealer: number[];
...
export type PontoonCasAction = { type: "hit" } | { type: "stand" } | { type: "double" } | { type: "surrender" } | { type: "next" };
...
export function reducer(state: PontoonCasState, action: PontoonCasAction): PontoonCasState {
  ...
  if (action.type === "double" && state.phase === "play" && state.you.length === 2) {
    ...
    const r = settleHand(you, dealer, { bjPays: 2.0 });
    const pts = r.pts * 2;
    const isLast = state.round >= TOTAL_ROUNDS;
  ...
  if (action.type === "stand" && state.phase === "play") {
    ...
    const r = settleHand(state.you, dealer, { bjPays: 2.0 });
    const isLast = state.round >= TOTAL_ROUNDS;
