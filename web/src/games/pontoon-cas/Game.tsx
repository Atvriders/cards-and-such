...
export function PontoonCasGame({ state, dispatch, onGameOver }: GameProps<PontoonCasState, PontoonCasSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap"><h3>Pontoon</h3><div className="dm-done"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as PontoonCasAction)}>Play</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PontoonCasAction)}>Next</button>
      </>}
    </div>
  );
}
