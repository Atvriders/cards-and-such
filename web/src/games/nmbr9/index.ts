...
const Nmbr9Game = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.Nmbr9Game as unknown as React.ComponentType<unknown> })));
...
export const nmbr9Plugin: GamePlugin<Nmbr9State, Nmbr9Action, typeof settings> = {
  ...
  howToPlay: "NMBR 9 is a tile-placement game on a 5x5 grid. A randomized queue of 16 tiles is generated. Each turn the next tile from the queue is shown; click any empty cell to place it. Tile types are: 1, 2, 3, 4, 5. Each orthogonal pair of same-type tiles scores +2. Same-type connected clusters of 3+ score a +4 bonus, clusters of 5+ score an additional +8. Strategy: place tiles next to existing same-type neighbors to grow clusters efficiently. Don't waste placements in isolated corners. The grid has 25 cells but you only place 16 tiles, so plan compact clusters.",
  settings,
