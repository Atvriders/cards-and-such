...
describe("SalamiSlice", () => {
  ...
  it("tick spawns salamis", () => { const s = reducer(initialState(1, S), { type:"tick" }); expect(s.salamis.length).toBeGreaterThanOrEqual(1); });
  it("pop removes salami and adds 10 points", () => {
    let s = initialState(1, S);
    s = reducer(s, { type:"tick" });
    const id = s.salamis[0]!.id;
