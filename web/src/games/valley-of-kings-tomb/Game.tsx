...
import "./Game.css";
export function ValleyOfKingsTombGame({ state, dispatch, onGameOver }: GameProps<ValleyOfKingsTombState, ValleyOfKingsTombSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as ValleyOfKingsTombAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
