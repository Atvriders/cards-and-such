...
function checkWin(b: Cell[], mark: "P" | "C"): boolean {
  ...
  const stack: number[] = [];
  for (let c2 = 0; c2 < SIZE; c2++) if (b[c2] === mark) { stack.push(c2); visited.add(c2); }
  while (stack.length > 0) {
    const i = stack.pop()!;
    const r2 = Math.floor(i / SIZE), col = i % SIZE;
    if (r2 === SIZE - 1) return true;
    for (const [dr, dc] of [[-1,0],[1,0],[0,-1],[0,1]] as const) {
      const rr = r2 + dr, cc = col + dc;
      if (rr < 0 || rr >= SIZE || cc < 0 || cc >= SIZE) continue;
      const ii = rr * SIZE + cc;
      if (b[ii] === mark && !visited.has(ii)) { visited.add(ii); stack.push(ii); }
    }
  }
  return false;
}
