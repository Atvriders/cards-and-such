...
export interface YokohamaDiceSettings { dummy: boolean; }
export interface YokohamaDiceState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type YokohamaDiceAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: YokohamaDiceSettings): YokohamaDiceState {
  return {
    rngSeed: seed,
...
export function reducer(state: YokohamaDiceState, action: YokohamaDiceAction): YokohamaDiceState {
...
export function isTerminal(state: YokohamaDiceState): { score: number } | null {
