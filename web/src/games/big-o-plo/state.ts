...
export interface BigOPloSettings { dummy: boolean; }
export interface BigOPloState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type BigOPloAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BigOPloSettings): BigOPloState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: BigOPloState, action: BigOPloAction): BigOPloState {
...
export function isTerminal(state: BigOPloState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
