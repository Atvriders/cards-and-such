...
export interface PineapplePopSettings { dummy: boolean; }
export interface PineapplePopTarget { id: number; lane: number; ticksLeft: number; }
export interface PineapplePopState {
  rngSeed: number;
  targets: PineapplePopTarget[];
  nextId: number;
...
export type PineapplePopAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: PineapplePopSettings): PineapplePopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: PineapplePopState, action: PineapplePopAction): PineapplePopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: PineapplePopTarget[] = [];
...
export function isTerminal(state: PineapplePopState): { score: number } | null {
