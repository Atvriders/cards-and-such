...
export function SpinachSpinGame({ state, dispatch, onGameOver }: GameProps<SpinachSpinState, SpinachSpinSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="spinachspin-wrap"><div className="spinachspin-done bounce-in"><h2>Time's Up!</h2><div>tapped: {state.tapped} / Missed: {state.missed}</div><div className="spinachspin-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="spinachspin-wrap">
      <div className="spinachspin-header">
        ...
        <span className="spinachspin-score pulse">{state.score} pts</span>
      </div>
