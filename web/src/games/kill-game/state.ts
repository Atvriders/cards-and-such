...
export interface KillGameSettings { dummy: boolean; }
export interface KillGameState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type KillGameAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: KillGameSettings): KillGameState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: KillGameState, action: KillGameAction): KillGameState {
...
export function isTerminal(state: KillGameState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
