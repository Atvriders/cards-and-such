...
import "./Game.css";
export function KillGameGame({ state, dispatch, onGameOver }: GameProps<KillGameState, KillGameSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-kill-game-deal" onClick={() => dispatch({ type: "deal" } as KillGameAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-kill-game-next" onClick={() => dispatch({ type: "next" } as KillGameAction)}>Next</button>
      </>}
    </div>
  );
}
