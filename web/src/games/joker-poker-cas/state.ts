...
export interface JokerPokerCasSettings { dummy: boolean; }
export interface JokerPokerCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type JokerPokerCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: JokerPokerCasSettings): JokerPokerCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: JokerPokerCasState, action: JokerPokerCasAction): JokerPokerCasState {
...
export function isTerminal(state: JokerPokerCasState): { score: number } | null {
