...
import "./Game.css";
export function JokerPokerCasGame({ state, dispatch, onGameOver }: GameProps<JokerPokerCasState, JokerPokerCasSettings>): JSX.Element {
  const t = isTerminal(state); useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") return <div className="dm-wrap"><h3>Joker Poker (Casino)</h3><div className="dm-done"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap">
      <h3>Joker Poker (Casino)</h3>
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as JokerPokerCasAction)}>Play</button>}
      {state.phase === "scored" && <>
