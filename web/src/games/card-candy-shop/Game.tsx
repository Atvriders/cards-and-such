...
import "./Game.css";
export function CardCandyShopGame({ state, dispatch, onGameOver }: GameProps<CardCandyShopState, CardCandyShopSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "drawing" && (
        <button data-testid="hint-target-card-candy-shop-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as CardCandyShopAction)}>Draw Card</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardCandyShopAction)}>{state.drawCount >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
