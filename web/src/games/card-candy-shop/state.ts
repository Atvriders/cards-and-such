...
export interface CardCandyShopSettings { dummy: boolean; }
export interface CardCandyShopState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardCandyShopAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardCandyShopSettings): CardCandyShopState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardCandyShopState, action: CardCandyShopAction): CardCandyShopState {
...
export function isTerminal(state: CardCandyShopState): { score: number } | null {
