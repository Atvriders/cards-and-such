...
export interface HarvestDiceGardenSettings { dummy: boolean; }
export interface HarvestDiceGardenState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type HarvestDiceGardenAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: HarvestDiceGardenSettings): HarvestDiceGardenState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: HarvestDiceGardenState, action: HarvestDiceGardenAction): HarvestDiceGardenState {
...
export function isTerminal(state: HarvestDiceGardenState): { score: number } | null {
