...
const HighPointCrapsGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.HighPointCrapsGame as unknown as React.ComponentType<unknown> })));
...
export const highPointCrapsPlugin: GamePlugin<HighPointCrapsState, HighPointCrapsAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: HighPointCrapsState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "roll") return { selector: '[data-testid="hint-target-high-point-craps-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-high-point-craps-next"]', pulses: 3 };
    return null;
  },
  component:HighPointCrapsGame,
};
