...
import "./Game.css";
export function CalLowballGame({ state, dispatch, onGameOver }: GameProps<CalLowballState, CalLowballSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-cal-lowball-deal" onClick={() => dispatch({ type: "deal" } as CalLowballAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-cal-lowball-next" onClick={() => dispatch({ type: "next" } as CalLowballAction)}>Next</button>
      </>}
    </div>
  );
}
