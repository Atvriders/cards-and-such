...
import "./Game.css";
export function DudakDiceGame({ state, dispatch, onGameOver }: GameProps<DudakDiceState, DudakDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-dudak-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as DudakDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-dudak-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DudakDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
