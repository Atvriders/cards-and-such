...
export interface DicePursuePennantSettings { dummy: boolean; }

export interface DicePursuePennantState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DicePursuePennantAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DicePursuePennantSettings): DicePursuePennantState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DicePursuePennantState, action: DicePursuePennantAction): DicePursuePennantState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DicePursuePennantState> = {};
...
export function isTerminal(state: DicePursuePennantState): { score: number } | null {
