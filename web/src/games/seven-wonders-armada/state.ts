...
export interface SevenWondersArmadaState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersArmadaAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersArmadaSettings): SevenWondersArmadaState {
...
export function score(s: SevenWondersArmadaState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersArmadaState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
