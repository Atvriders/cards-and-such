...
export const SUIT_NAMES = ["Naval","Trade","Explore","Pirate"] as const;
...
export type SevenWondersArmadaState = DraftState;
export type SevenWondersArmadaAction = DraftAction;
export interface SevenWondersArmadaSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SevenWondersArmadaSettings): SevenWondersArmadaState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SevenWondersArmadaState, action: SevenWondersArmadaAction): SevenWondersArmadaState {
  return draftReducer(state, action);
}

export function score(state: SevenWondersArmadaState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersArmadaState): { score: number } | null {
