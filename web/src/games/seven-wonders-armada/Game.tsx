...
export function SevenWondersArmadaGame({ state, dispatch, onGameOver }: GameProps<SevenWondersArmadaState, SevenWondersArmadaSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">7 Wonders: Armada</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SevenWondersArmadaAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SevenWondersArmadaAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
