...
import "./Game.css";
export function CardBridgeBuildGame({ state, dispatch, onGameOver }: GameProps<CardBridgeBuildState, CardBridgeBuildSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <div className="cm-help">Ascending order = strong bridge</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardBridgeBuildAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
