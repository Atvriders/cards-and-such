...
export interface HanafudaKoiKoiState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type HanafudaKoiKoiAction = { type: "play" } | { type: "next" };
...
export function isTerminal(state: HanafudaKoiKoiState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
