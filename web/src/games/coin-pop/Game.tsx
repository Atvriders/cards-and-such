...
export function CoinPop({ state, dispatch, onGameOver }: GameProps<CoinPopState, CoinPopSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as CoinPopAction), 600);
    spawnRef.current = setInterval(() => dispatch({ type: "spawn" } as CoinPopAction), 2000);
