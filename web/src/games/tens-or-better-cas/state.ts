...
export interface TensOrBetterCasSettings { dummy: boolean; }
export interface TensOrBetterCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type TensOrBetterCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: TensOrBetterCasSettings): TensOrBetterCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: TensOrBetterCasState, action: TensOrBetterCasAction): TensOrBetterCasState {
...
export function isTerminal(state: TensOrBetterCasState): { score: number } | null {
