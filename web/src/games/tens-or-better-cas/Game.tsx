...
import "./Game.css";
export function TensOrBetterCasGame({ state, dispatch, onGameOver }: GameProps<TensOrBetterCasState, TensOrBetterCasSettings>): JSX.Element {
  const t = isTerminal(state); useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") return <div className="dm-wrap"><h3>Tens or Better (Casino)</h3><div className="dm-done"><h2>Done!</h2><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap">
      <h3>Tens or Better (Casino)</h3>
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as TensOrBetterCasAction)}>Play</button>}
      {state.phase === "scored" && <>
