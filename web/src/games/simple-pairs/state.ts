...
export interface SimplePairsSettings { dummy: boolean; }

export interface SimplePairsState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const ranks=h.map(cardRank); const counts:Record<number,number>={}; for (const r of ranks) counts[r]=(counts[r]||0)+1; const pairs = Object.values(counts).filter(v=>v>=2).length; s += pairs*14; if (s===0) s=1; return s;
}
...
export function reducer(state: SimplePairsState, action: SimplePairsAction): SimplePairsState {
...
export function isTerminal(state: SimplePairsState): { score: number } | null {
