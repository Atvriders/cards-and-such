...
export interface WaspWhipSettings { dummy: boolean; }
...
export interface WaspWhipState {
  rngSeed: number;
  bugs: Bug[];
  nextId: number;
...
export type WaspWhipAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: WaspWhipSettings): WaspWhipState {
  return { rngSeed: seed, bugs: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: WaspWhipState, action: WaspWhipAction): WaspWhipState {
...
export function isTerminal(state: WaspWhipState): { score: number } | null {
