...
const BadeuceyDrawGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.BadeuceyDrawGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: BadeuceyDrawState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-badeucey-draw-secondary"]', pulses: 3 };
  return null;
};
...
export const badeuceyDrawPlugin: GamePlugin<BadeuceyDrawState, BadeuceyDrawAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: BadeuceyDrawGame,
};
