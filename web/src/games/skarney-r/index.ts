...
const SkarneyRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SkarneyRGame as unknown as React.ComponentType<unknown> })));
...
export const skarneyRPlugin: GamePlugin<GState, GAction, typeof settings> = {
  ...
  hint: (state: GState): HintTarget | null => {
    ...
    if (state.phase === "play") return { selector: '[data-testid="hint-target-skarney-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-skarney-r-next"]', pulses: 3 };
    return null;
  },
  component: SkarneyRGame,
};
