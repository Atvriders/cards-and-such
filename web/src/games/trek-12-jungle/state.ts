...
export interface Trek12JungleSettings { dummy: boolean; }
export interface Trek12JungleState {
  rngSeed: number;
...
export type Trek12JungleAction =
  | { type: "roll" }
  | { type: "place"; index: number; op: "sum" | "diff" | "max" | "min" }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: Trek12JungleSettings): Trek12JungleState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: Trek12JungleState, action: Trek12JungleAction): Trek12JungleState {
...
export function isTerminal(state: Trek12JungleState): { score: number } | null {
