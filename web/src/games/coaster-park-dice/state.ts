...
export interface CoasterParkDiceSettings { rounds: "10"; }

export interface CoasterParkDiceState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type CoasterParkDiceAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: CoasterParkDiceSettings): CoasterParkDiceState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: CoasterParkDiceState, action: CoasterParkDiceAction): CoasterParkDiceState {
...
export function isTerminal(state: CoasterParkDiceState): { score: number } | null {
