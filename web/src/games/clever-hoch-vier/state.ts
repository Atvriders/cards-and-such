...
export interface CleverHochVierSettings { dummy: boolean; }
export interface CleverHochVierState {
  rngSeed: number;
...
export type CleverHochVierAction =
  | { type: "roll" }
...
export function initialState(seed: number, _s: CleverHochVierSettings): CleverHochVierState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: CleverHochVierState, action: CleverHochVierAction): CleverHochVierState {
...
export function isTerminal(state: CleverHochVierState): { score: number } | null {
