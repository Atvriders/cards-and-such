...
import "./Game.css";
export function MangoPokerGame({ state, dispatch, onGameOver }: GameProps<MangoPokerState, MangoPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-mango-poker-deal" onClick={() => dispatch({ type: "deal" } as MangoPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-mango-poker-next" onClick={() => dispatch({ type: "next" } as MangoPokerAction)}>Next</button>
      </>}
    </div>
  );
}
