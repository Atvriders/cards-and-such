...
export interface MangoPokerSettings { dummy: boolean; }
export interface MangoPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type MangoPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: MangoPokerSettings): MangoPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: MangoPokerState, action: MangoPokerAction): MangoPokerState {
...
export function isTerminal(state: MangoPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
