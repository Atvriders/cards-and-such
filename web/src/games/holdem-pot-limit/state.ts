...
export interface HoldemPotLimitSettings { dummy: boolean; }
export interface HoldemPotLimitState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HoldemPotLimitAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HoldemPotLimitSettings): HoldemPotLimitState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: HoldemPotLimitState, action: HoldemPotLimitAction): HoldemPotLimitState {
...
export function isTerminal(state: HoldemPotLimitState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
