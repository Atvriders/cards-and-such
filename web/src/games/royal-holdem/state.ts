...
export interface RoyalHoldemSettings { dummy: boolean; }
export interface RoyalHoldemState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type RoyalHoldemAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: RoyalHoldemSettings): RoyalHoldemState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: RoyalHoldemState, action: RoyalHoldemAction): RoyalHoldemState {
...
export function isTerminal(state: RoyalHoldemState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
