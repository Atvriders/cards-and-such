...
export interface CardFlowerPickupSettings { dummy: boolean; }
export interface CardFlowerPickupState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardFlowerPickupAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardFlowerPickupSettings): CardFlowerPickupState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardFlowerPickupState, action: CardFlowerPickupAction): CardFlowerPickupState {
...
export function isTerminal(state: CardFlowerPickupState): { score: number } | null {
