...
export function DiceCookingGame({ state, dispatch, onGameOver }: GameProps<DiceCookingState, DiceCookingSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="dco-wrap"><div className="dco-done"><h2>Done!</h2><div className="dco-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="dco-wrap">
      <div className="dco-info">Round {state.round + 1} / {TOTAL_ROUNDS}</div>
      <div className="dco-score">{state.score} pts</div>
      {state.lastRoll > 0 && (
        <>
