...
export interface BrandubhSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface BrandubhState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type BrandubhAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: BrandubhSettings): BrandubhState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: BrandubhState, action: BrandubhAction): BrandubhState {
...
export function isTerminal(state: BrandubhState): { score: number } | null {
