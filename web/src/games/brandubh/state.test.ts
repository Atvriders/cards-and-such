...
describe("Brandubh", () => {
  ...
  it("place puts P piece on board", () => { const s=reducer(initialState(1,S), { type:"place", idx:0 }); expect(s.board[0]).toBe("P"); });
  ...
  it("placing on full cell is rejected", () => { let s=reducer(initialState(1,S), { type:"place", idx:0 }); s=reducer(s, { type:"place", idx:0 }); expect(s.board[0]).toBe("P"); });
