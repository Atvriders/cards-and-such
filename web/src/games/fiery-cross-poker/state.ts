...
export interface FieryCrossPokerSettings { rounds: "10"; }

export interface FieryCrossPokerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type FieryCrossPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FieryCrossPokerSettings): FieryCrossPokerState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: FieryCrossPokerState, action: FieryCrossPokerAction): FieryCrossPokerState {
...
export function isTerminal(state: FieryCrossPokerState): { score: number } | null {
