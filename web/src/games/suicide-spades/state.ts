...
export interface SuicideSpadesSettings { dummy: boolean; }
export interface SuicideSpadesState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type SuicideSpadesAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: SuicideSpadesSettings): SuicideSpadesState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: SuicideSpadesState, action: SuicideSpadesAction): SuicideSpadesState {
...
export function isTerminal(state: SuicideSpadesState): { score: number } | null {
