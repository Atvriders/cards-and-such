// Auto-generated backgammon-family state for ludus-12.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const POINTS = 12;
export const CHECKERS_PER_SIDE = 5;
...
const START_LAYOUT: ReadonlyArray<readonly [number, number]> = [[0,5]];
...
export function initialState(seed: number, _s: Ludus12Settings): Ludus12State {
...
export function legalMoves(state: Ludus12State, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: Ludus12State, side: Side, from: number, pips: number): Ludus12State | null {
  const next: Ludus12State = {
    ...state,
...
function checkWin(s: Ludus12State): Ludus12State {
...
function cpuTurn(state: Ludus12State): Ludus12State {
  let s = state;
...
export function reducer(state: Ludus12State, action: Ludus12Action): Ludus12State {
  ...
  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    ...
    const next: Ludus12State = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
