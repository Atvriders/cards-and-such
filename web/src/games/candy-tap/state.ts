...
// Candy Tap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface CandyTapSettings { dummy: boolean; }

export interface CandyTapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface CandyTapState {
  rngSeed: number;
  targets: CandyTapTarget[];
  nextId: number;
...
export type CandyTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CandyTapSettings): CandyTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CandyTapState, action: CandyTapAction): CandyTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CandyTapTarget[] = [];
...
export function isTerminal(state: CandyTapState): { score: number } | null {
