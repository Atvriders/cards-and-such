...
function applyMove(b: Cell[], idx: number, mark: "P" | "C"): { board: Cell[]; captures: number } {
  ...
  lines.push(Array.from({length: SIZE}, (_, i) => [r, i] as [number, number]));
  lines.push(Array.from({length: SIZE}, (_, i) => [i, c] as [number, number]));
  if (r === c) lines.push(Array.from({length: SIZE}, (_, i) => [i, i] as [number, number]));
  if (r + c === SIZE - 1) lines.push(Array.from({length: SIZE}, (_, i) => [i, SIZE - 1 - i] as [number, number]));
  let mill = false;
  for (const line of lines) {
    let run = 0;
    for (const [rr, cc] of line) {
      if (nb[rr * SIZE + cc] === mark) { run++; if (run >= 3) { mill = true; break; } } else run = 0;
    }
    if (mill) break;
  }
  if (mill) {
    const oppCells: number[] = [];
    for (let i = 0; i < nb.length; i++) if (nb[i] === opp) oppCells.push(i);
    if (oppCells.length > 0) { nb[oppCells[0]!] = null; captures++; }
  }
