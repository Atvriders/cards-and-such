...
export interface DrPepperWildSettings { rounds: "10"; }

export interface DrPepperWildState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type DrPepperWildAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DrPepperWildSettings): DrPepperWildState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: DrPepperWildState, action: DrPepperWildAction): DrPepperWildState {
...
export function isTerminal(state: DrPepperWildState): { score: number } | null {
