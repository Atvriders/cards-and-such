...
export interface MonopolyDealMiniSettings { dummy: boolean; }
export interface MonopolyDealMiniState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type MonopolyDealMiniAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: MonopolyDealMiniSettings): MonopolyDealMiniState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Tenant moves in","Eviction notice","Mortgage paid off","Renovation complete","Rent doubled card","Dealership opens"];

export function reducer(state: MonopolyDealMiniState, action: MonopolyDealMiniAction): MonopolyDealMiniState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Properties (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Tenant (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Traded 1 Properties (+$" + market + ")"; delta = market;
      } else { evt = "No Properties to trade"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
...
export function score(s: MonopolyDealMiniState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: MonopolyDealMiniState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
