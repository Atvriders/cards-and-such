...
// Candy Jelly Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface CandyJellyMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface CandyJellyMiniState {
  rngSeed: number;
  grid: Grid;
...
export type CandyJellyMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: CandyJellyMiniSettings): CandyJellyMiniState {
...
export function reducer(state: CandyJellyMiniState, action: CandyJellyMiniAction): CandyJellyMiniState {
...
export function isTerminal(state: CandyJellyMiniState): { score: number } | null {
