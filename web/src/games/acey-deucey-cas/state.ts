...
export interface AceyDeuceyCasSettings { dummy: boolean; }
export interface AceyDeuceyCasState { rngSeed: number; round: number; left: number | null; right: number | null; middle: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type AceyDeuceyCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: AceyDeuceyCasSettings): AceyDeuceyCasState {
  return { rngSeed: seed, round: 1, left: null, right: null, middle: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: AceyDeuceyCasState, action: AceyDeuceyCasAction): AceyDeuceyCasState {
...
export function isTerminal(state: AceyDeuceyCasState): { score: number } | null {
