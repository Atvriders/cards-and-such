...
const JassGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.JassGame as unknown as React.ComponentType<unknown> })));
...
export const jassPlugin: GamePlugin<JassState, JassAction, typeof settings> = {
  ...
  description: "Jass — Swiss trick-taking.",
  howToPlay: "Jass — Swiss trick-taking. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as JassSettings),
  reducer,
  isTerminal,
  hint: (state: JassState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-jassc-${pick.id}"]`, pulses: 3 };
    }
