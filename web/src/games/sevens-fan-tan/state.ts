...
export interface SevensFanTanSettings { dummy: boolean; }
export interface SevensFanTanState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SevensFanTanAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SevensFanTanSettings): SevensFanTanState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SevensFanTanState, action: SevensFanTanAction): SevensFanTanState {
...
export function isTerminal(state: SevensFanTanState): { score: number } | null {
