...
export interface AnteOnlyGameSettings { dummy: boolean; }
export interface AnteOnlyGameState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type AnteOnlyGameAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AnteOnlyGameSettings): AnteOnlyGameState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: AnteOnlyGameState, action: AnteOnlyGameAction): AnteOnlyGameState {
...
export function isTerminal(state: AnteOnlyGameState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
