...
import "./Game.css";
export function AnteOnlyGameGame({ state, dispatch, onGameOver }: GameProps<AnteOnlyGameState, AnteOnlyGameSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-ante-only-game-deal" onClick={() => dispatch({ type: "deal" } as AnteOnlyGameAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-ante-only-game-next" onClick={() => dispatch({ type: "next" } as AnteOnlyGameAction)}>Next</button>
      </>}
    </div>
  );
}
