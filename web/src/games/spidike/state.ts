...
export interface SpidikeSettings { dummy: boolean; }

export interface SpidikeState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export function reducer(state: SpidikeState, action: SpidikeAction): SpidikeState {
...
export function isTerminal(state: SpidikeState): { score: number } | null {
