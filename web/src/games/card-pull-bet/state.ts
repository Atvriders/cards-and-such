...
export interface CardPullBetSettings { rounds: "8" | "12"; }
export interface CardPullBetState { deck: number[]; pos: number; topCard: number; coins: number; round: number; maxRounds: number; phase: "betting" | "revealed" | "gameover"; lastResult: "win" | "lose" | "tie" | null; revealedCard: number | null; bid: number; }
export type CardPullBetAction = { type: "bet"; amount: number; dir: "higher" | "lower" } | { type: "next" };
...
export function initialState(seed: number, settings: CardPullBetSettings): CardPullBetState {
  const rng=mulberry32(seed); const deck=makeDeck(rng);
  return {deck,pos:1,topCard:deck[0]!,coins:80,bid:0,round:1,maxRounds:parseInt(settings.rounds,10),phase:"betting",lastResult:null,revealedCard:null};
}
export function reducer(state: CardPullBetState, action: CardPullBetAction): CardPullBetState {
...
export function isTerminal(state: CardPullBetState): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
