...
export interface CleverDiceSettings { dummy: boolean; }
export interface CleverDiceState {
  rngSeed: number;
  cells: boolean[];
...
export type CleverDiceAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: CleverDiceSettings): CleverDiceState {
  return {
    rngSeed: seed,
...
export function reducer(state: CleverDiceState, action: CleverDiceAction): CleverDiceState {
...
export function isTerminal(state: CleverDiceState): { score: number } | null {
