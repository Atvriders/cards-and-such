...
export interface FourFoursSettings { dummy: boolean; }

export interface FourFoursState {
  rngSeed: number;
  draw: number;
  card: number | null;
  hits: number;
...
export type FourFoursAction = { type: "draw" } | { type: "next" };
...
export function isHit(c: number): boolean { return rankIndex(c) === 2; }

export function initialState(seed: number, _settings: FourFoursSettings): FourFoursState {
  return { rngSeed: seed, draw: 1, card: null, hits: 0, score: 0, phase: "drawing", lastWasHit: false };
}

export function reducer(state: FourFoursState, action: FourFoursAction): FourFoursState {
...
export function isTerminal(state: FourFoursState): { score: number } | null {
