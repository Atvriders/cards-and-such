...
export function MiniPokerGame({ state, dispatch, onGameOver }: GameProps<MiniPokerState, MiniPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as MiniPokerAction)}>Deal 5 cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MiniPokerAction)}>Next</button>
      </>}
    </div>
  );
}
