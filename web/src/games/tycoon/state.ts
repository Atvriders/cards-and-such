...
export interface TycoonSettings { dummy: boolean; }
export interface TycoonState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type TycoonAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: TycoonSettings): TycoonState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: TycoonState, action: TycoonAction): TycoonState {
...
export function isTerminal(state: TycoonState): { score: number } | null {
