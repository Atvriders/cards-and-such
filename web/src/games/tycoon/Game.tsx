...
export function TycoonGame({ state, dispatch, onGameOver }: GameProps<TycoonState, TycoonSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="tycoon-wrap ty-shed"><div className="tycoon-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="tycoon-final">{state.score} pts</div></div></div>;
  return (
    <div className="tycoon-wrap ty-shed">
