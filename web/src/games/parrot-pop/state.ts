...
export interface ParrotPopSettings { dummy: boolean; }
export interface ParrotPopTarget { id: number; lane: number; ticksLeft: number; }
export interface ParrotPopState {
  rngSeed: number;
  targets: ParrotPopTarget[];
  nextId: number;
...
export type ParrotPopAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: ParrotPopSettings): ParrotPopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: ParrotPopState, action: ParrotPopAction): ParrotPopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: ParrotPopTarget[] = [];
...
export function isTerminal(state: ParrotPopState): { score: number } | null {
