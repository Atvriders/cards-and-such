...
export interface FusionPokerSettings { dummy: boolean; }
export interface FusionPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FusionPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FusionPokerSettings): FusionPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FusionPokerState, action: FusionPokerAction): FusionPokerState {
...
export function isTerminal(state: FusionPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
