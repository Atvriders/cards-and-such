...
const SokoPokerGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SokoPokerGame as unknown as React.ComponentType<unknown> })));
...
export const sokoPokerPlugin: GamePlugin<SokoPokerState, SokoPokerAction, typeof settings> = {
  ...
  reducer, isTerminal,   hint: (state: SokoPokerState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-soko-poker-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-soko-poker-next"]', pulses: 3 };
    return null;
  },
  component:SokoPokerGame,
};
