...
export interface SokoPokerSettings { dummy: boolean; }
export interface SokoPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SokoPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SokoPokerSettings): SokoPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SokoPokerState, action: SokoPokerAction): SokoPokerState {
...
export function isTerminal(state: SokoPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
