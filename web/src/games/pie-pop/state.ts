...
// Pie Pop: bakery clicker. 30-second timer.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface PiePopSettings { dummy: boolean; }

export interface PiePopItem {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface PiePopState {
  rngSeed: number;
  items: PiePopItem[];
  nextId: number;
...
export type PiePopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PiePopSettings): PiePopState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PiePopState, action: PiePopAction): PiePopState {
...
export function isTerminal(state: PiePopState): { score: number } | null {
