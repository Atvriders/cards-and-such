...
// Star Shoot: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface StarShootSettings { dummy: boolean; }

export interface StarShootTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface StarShootState {
  rngSeed: number;
  targets: StarShootTarget[];
  nextId: number;
...
export type StarShootAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: StarShootSettings): StarShootState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: StarShootState, action: StarShootAction): StarShootState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: StarShootTarget[] = [];
...
export function isTerminal(state: StarShootState): { score: number } | null {
