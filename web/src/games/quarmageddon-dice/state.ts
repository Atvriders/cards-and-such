...
export interface QuarmageddonDiceSettings { rounds: "10"; }

export interface QuarmageddonDiceState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type QuarmageddonDiceAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: QuarmageddonDiceSettings): QuarmageddonDiceState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: QuarmageddonDiceState, action: QuarmageddonDiceAction): QuarmageddonDiceState {
...
export function isTerminal(state: QuarmageddonDiceState): { score: number } | null {
