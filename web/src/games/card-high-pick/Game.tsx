...
export function CardHighPickGame({ state, dispatch, onGameOver }: GameProps<CardHighPickState, CardHighPickSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      <p>Pick the HIGHEST ranked card!</p>
      <div className="cm-cards">
        {state.hand.map((c, i) => {
          const isPickable = state.phase === "picking";
          const red = Math.floor(c/13)===1||Math.floor(c/13)===2;
          return <div key={i} className={`cm-card ${red?"red":""} ${!isPickable?"disabled":""}`} onClick={() => isPickable && dispatch({ type:"pick", index:i } as CardHighPickAction)}>{rankName(c)}{suitName(c)}</div>;
        })}
      </div>
      {state.phase === "result" && <>
        <div className="cm-result">+{state.lastPts} pts</div>
        <button data-testid="hint-target-card-high-pick-primary" className="cm-btn" onClick={() => dispatch({ type:"next" } as CardHighPickAction)}>{state.round >= state.maxRounds ? "Finish" : "Next"}</button>
      </>}
    </div>
  );
}
