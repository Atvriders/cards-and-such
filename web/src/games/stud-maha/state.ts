...
export interface StudMahaSettings { dummy: boolean; }
export interface StudMahaState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type StudMahaAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: StudMahaSettings): StudMahaState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: StudMahaState, action: StudMahaAction): StudMahaState {
...
export function isTerminal(state: StudMahaState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
