...
export interface StarTapperSettings { dummy: boolean; }
...
export interface StarTapperState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type StarTapperAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: StarTapperSettings): StarTapperState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: StarTapperState, action: StarTapperAction): StarTapperState {
...
export function isTerminal(state: StarTapperState): { score: number } | null {
