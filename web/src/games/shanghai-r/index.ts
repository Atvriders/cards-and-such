...
const ShanghaiRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ShanghaiRGame as unknown as React.ComponentType<unknown> })));
...
export const shanghaiRPlugin: GamePlugin<ShanghaiRState, ShanghaiRAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: ShanghaiRState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "play") return { selector: '[data-testid="hint-target-shanghai-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-shanghai-r-next"]', pulses: 3 };
    return null;
  },
  component: ShanghaiRGame,
};
