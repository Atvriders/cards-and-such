...
export interface ShanghaiRSettings { dummy: boolean; }
export interface ShanghaiRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type ShanghaiRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: ShanghaiRSettings): ShanghaiRState {
...
export function reducer(state: ShanghaiRState, action: ShanghaiRAction): ShanghaiRState {
...
export function isTerminal(state: ShanghaiRState): { score: number } | null {
