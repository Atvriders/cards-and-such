...
export interface DuchessLuynesSettings { dummy: boolean; }

export interface DuchessLuynesState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type DuchessLuynesAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const reds = h.filter(c=>cardSuit(c)===1||cardSuit(c)===2).length; s += reds*4; const f = h.filter(c=>cardRank(c)>=11).length; s += f*3; if (s===0) s=1; return s;
}
...
export function reducer(state: DuchessLuynesState, action: DuchessLuynesAction): DuchessLuynesState {
...
export function isTerminal(state: DuchessLuynesState): { score: number } | null {
