...
export function CartographersBaseGame({ state, dispatch, onGameOver }: GameProps<CartographersBaseState, CartographersBaseSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as CartographersBaseAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as CartographersBaseAction)}>Skip</button>
      )}
    </div>
  );
}
