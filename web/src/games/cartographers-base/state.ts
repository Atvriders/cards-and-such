...
export interface CartographersBaseSettings { dummy: boolean; }
export interface CartographersBaseState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type CartographersBaseAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: CartographersBaseSettings): CartographersBaseState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: CartographersBaseState, action: CartographersBaseAction): CartographersBaseState {
...
export function isTerminal(state: CartographersBaseState): { score: number } | null {
