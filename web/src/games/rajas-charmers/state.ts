...
export interface RajasCharmersSettings { dummy: boolean; }
export interface RajasCharmersState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type RajasCharmersAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: RajasCharmersSettings): RajasCharmersState {
  return {
    rngSeed: seed,
...
export function reducer(state: RajasCharmersState, action: RajasCharmersAction): RajasCharmersState {
...
export function isTerminal(state: RajasCharmersState): { score: number } | null {
