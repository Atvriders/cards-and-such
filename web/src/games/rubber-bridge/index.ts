...
const RubberBridgeGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RubberBridgeGame as unknown as React.ComponentType<unknown> })));
...
export const rubberBridgePlugin: GamePlugin<RubberBridgeState, RubberBridgeAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: RubberBridgeState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-rubberb-${pick.id}"]`, pulses: 3 };
    }
