...
export interface BeatNeighbourShedSettings { dummy: boolean; }
export interface BeatNeighbourShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type BeatNeighbourShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: BeatNeighbourShedSettings): BeatNeighbourShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: BeatNeighbourShedState, action: BeatNeighbourShedAction): BeatNeighbourShedState {
...
export function isTerminal(state: BeatNeighbourShedState): { score: number } | null {
