...
import "./Game.css";
export function BeatNeighbourShedGame({ state, dispatch, onGameOver }: GameProps<BeatNeighbourShedState, BeatNeighbourShedSettings>): JSX.Element {
  const t = isTerminal(state); useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") return <div className="dm-wrap bnsh-shed"><h3>Beat Your Neighbour</h3><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap bnsh-shed">
      <h3>Beat Your Neighbour</h3>
