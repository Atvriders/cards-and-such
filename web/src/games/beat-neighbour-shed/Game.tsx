...
export function BeatNeighbourShedGame({ state, dispatch, onGameOver }: GameProps<BeatNeighbourShedState, BeatNeighbourShedSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap"><h3>Beat Your Neighbour</h3><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
