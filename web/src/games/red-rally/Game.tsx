...
export function RedRallyGame({ state, dispatch, onGameOver }: GameProps<RedRallyState, RedRallySettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "shown" && (
        <>
          <div className="cm-result">{state.lastWasHit ? `Red! +${POINTS_PER_HIT}` : `Black — 0`}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type: "next" } as RedRallyAction)}>Next</button>
        </>
      )}
    </div>
  );
