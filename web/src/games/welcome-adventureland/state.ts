...
export interface welcomeAdventurelandSettings { dummy: boolean; }
export interface welcomeAdventurelandState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type welcomeAdventurelandAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: welcomeAdventurelandSettings): welcomeAdventurelandState {
  return {
    rngSeed: seed,
...
export function reducer(state: welcomeAdventurelandState, action: welcomeAdventurelandAction): welcomeAdventurelandState {
...
export function isTerminal(state: welcomeAdventurelandState): { score: number } | null {
