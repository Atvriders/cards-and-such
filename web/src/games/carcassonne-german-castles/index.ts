...
export const carcassonneGermanCastlesPlugin: GamePlugin<CarcassonneGermanCastlesState, CarcassonneGermanCastlesAction, typeof settings> = {
  ...
  description: "Large double-tile castles replace cities; special scoring rules.",
  howToPlay: `Carcassonne: German Castles is a single-player tile-placement puzzle. Large double-tile castles replace cities; special scoring rules. Tiles draw from a randomized queue, and your job is to place each one onto a small grid in a way that maximises end-of-game scoring.

How it works: German castles dominate: every adjacent same-type pair scores +2 (instead of +1). Castles benefit from coverage, so cluster aggressively.
