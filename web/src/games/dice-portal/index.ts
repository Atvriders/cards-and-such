...
const DicePortalGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DicePortalGame as unknown as React.ComponentType<unknown> })));
...
export const dicePortalPlugin: GamePlugin<DicePortalState, DicePortalAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DicePortalState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "roll") return { selector: '[data-testid="hint-target-dice-portal-roll"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-dice-portal-next"]', pulses: 3 };
    return null;
  },
  component:DicePortalGame,
};
