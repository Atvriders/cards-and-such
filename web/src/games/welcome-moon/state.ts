...
export interface WelcomeMoonSettings { dummy: boolean; }
export interface WelcomeMoonState {
  rngSeed: number;
...
export type WelcomeMoonAction =
  | { type: "roll" }
  | { type: "place"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: WelcomeMoonSettings): WelcomeMoonState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: WelcomeMoonState, action: WelcomeMoonAction): WelcomeMoonState {
...
export function isTerminal(state: WelcomeMoonState): { score: number } | null {
