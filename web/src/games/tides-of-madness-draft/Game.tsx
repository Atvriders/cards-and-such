...
export function TidesOfMadnessDraftGame({ state, dispatch, onGameOver }: GameProps<TidesOfMadnessDraftState, TidesOfMadnessDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Tides of Madness</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as TidesOfMadnessDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as TidesOfMadnessDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
