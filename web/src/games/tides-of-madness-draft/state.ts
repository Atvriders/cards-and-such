...
export interface TidesOfMadnessDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type TidesOfMadnessDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: TidesOfMadnessDraftSettings): TidesOfMadnessDraftState {
...
export function score(s: TidesOfMadnessDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: TidesOfMadnessDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
