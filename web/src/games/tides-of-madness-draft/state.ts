...
export const SUIT_NAMES = ["Cult","Tome","Symbol","Eldritch"] as const;
...
export type TidesOfMadnessDraftState = DraftState;
export type TidesOfMadnessDraftAction = DraftAction;
export interface TidesOfMadnessDraftSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: TidesOfMadnessDraftSettings): TidesOfMadnessDraftState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: TidesOfMadnessDraftState, action: TidesOfMadnessDraftAction): TidesOfMadnessDraftState {
  return draftReducer(state, action);
}

export function score(state: TidesOfMadnessDraftState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: TidesOfMadnessDraftState): { score: number } | null {
