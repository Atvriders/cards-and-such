import type { GamePlugin, SettingsOf } from "../../platform/game-plugin/types.js";
import type { QueendominoState, QueendominoAction, QueendominoSettings } from "./state.js";
import { initialState, reducer, isTerminal } from "./state.js";
import { QueendominoGame } from "./Game.js";
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
export const queendominoPlugin: GamePlugin<QueendominoState, QueendominoAction, typeof settings> = {
  id: "queendomino",
  title: "Queendomino",
  category: "board",
  players: { min: 1, max: 1, multiplayer: false },
  description: "Kingdomino plus buildings; towers dominate territories.",
  howToPlay: `Queendomino is a single-player tile-placement puzzle. Kingdomino plus buildings; towers dominate territories. Tiles draw from a randomized queue, and your job is to place each one onto a small grid in a way that maximises end-of-game scoring.

How it works: 16 tiles on a 4x4 grid (overflow goes anywhere unfilled). Towers (any tile adjacent to 3+ same type) pay a +5 territory bonus on top of standard adjacency.

On each turn, the next tile is shown above the grid. Click any empty cell to drop the tile in place. Once placed, a tile cannot be moved or removed — every choice is permanent. Plan ahead by considering not just the current tile but the full queue of upcoming pieces.

After 16 placements, the game ends and your final score is computed from the board layout. The scoring rule rewards adjacency between matching tile types, so clustering same-coloured tiles is the dominant strategy.

The shuffle is fully seeded; identical seeds produce identical queues, allowing for puzzle replay and head-to-head comparison. There is no opponent — this is a pure puzzle. Aim for high adjacency density to score well above baseline.`,
  settings,
  initialState: (seed: number, s: S) => initialState(seed, s as QueendominoSettings),
  reducer,
  isTerminal,
  component: QueendominoGame,
};
