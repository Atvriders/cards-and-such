...
export interface MaskMashSettings { dummy: boolean; }
export interface MaskMashItem {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface MaskMashState {
  rngSeed: number;
  items: MaskMashItem[];
  nextId: number;
...
export type MaskMashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };
export function initialState(seed: number, _settings: MaskMashSettings): MaskMashState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: MaskMashState, action: MaskMashAction): MaskMashState {
...
export function isTerminal(state: MaskMashState): { score: number } | null {
