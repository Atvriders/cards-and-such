...
export interface ChineseKlondikeSettings { dummy: boolean; }

export interface ChineseKlondikeState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type ChineseKlondikeAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r=h.map(cardRank); let evens=0, odds=0; for (const x of r){ if (x%2===0) evens++; else odds++; } s += Math.min(evens, odds)*5; const f = h.filter(c=>cardRank(c)>=11).length; s += f*3; if (s===0) s=1; return s;
}
...
export function reducer(state: ChineseKlondikeState, action: ChineseKlondikeAction): ChineseKlondikeState {
...
export function isTerminal(state: ChineseKlondikeState): { score: number } | null {
