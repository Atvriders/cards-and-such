...
export interface FlopPokerCasSettings { dummy: boolean; }
export interface FlopPokerCasState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FlopPokerCasAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FlopPokerCasSettings): FlopPokerCasState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FlopPokerCasState, action: FlopPokerCasAction): FlopPokerCasState {
...
export function isTerminal(state: FlopPokerCasState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
