...
import "./Game.css";
export function FlopPokerCasGame({ state, dispatch, onGameOver }: GameProps<FlopPokerCasState, FlopPokerCasSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "scored" && <>
        ...
        <button data-testid="hint-target-flop-poker-cas-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as FlopPokerCasAction)}>Next</button>
      </>}
    </div>
  );
}
