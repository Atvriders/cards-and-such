...
const NightBaseballGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.NightBaseballGame as unknown as React.ComponentType<unknown> })));
...
export const nightBaseballPlugin: GamePlugin<NightBaseballState, NightBaseballAction, typeof settings> = {
  ...
  reducer, isTerminal,   hint: (state: NightBaseballState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-night-baseball-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-night-baseball-next"]', pulses: 3 };
    return null;
  },
  component:NightBaseballGame,
};
