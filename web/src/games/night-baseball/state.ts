...
export interface NightBaseballSettings { dummy: boolean; }
export interface NightBaseballState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type NightBaseballAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: NightBaseballSettings): NightBaseballState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: NightBaseballState, action: NightBaseballAction): NightBaseballState {
...
export function isTerminal(state: NightBaseballState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
