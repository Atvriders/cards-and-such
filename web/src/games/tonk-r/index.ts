...
const TonkRGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TonkRGame as unknown as React.ComponentType<unknown> })));
...
export const tonkRPlugin: GamePlugin<TonkRState, TonkRAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: TonkRState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "play") return { selector: '[data-testid="hint-target-tonk-r-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-tonk-r-next"]', pulses: 3 };
    return null;
  },
  component: TonkRGame,
};
