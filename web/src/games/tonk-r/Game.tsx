...
export function TonkRGame({ state, dispatch, onGameOver }: GameProps<TonkRState, TonkRSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="tnkr-wrap"><div className="tnkr-done bounce-in"><h2>Done!</h2><div className="tnkr-final">{state.score} pts</div></div></div>;
  return (
    <div className="tnkr-wrap fade-in">
      <div className="tnkr-info">Round {state.round} / {TOTAL_ROUNDS}</div>
      <div className="tnkr-score pulse">{state.score} pts</div>
