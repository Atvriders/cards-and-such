...
export function TonkRGame({ state, dispatch, onGameOver }: GameProps<TonkRState, TonkRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as TonkRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as TonkRAction)}>Next</button>
      </>}
    </div>
  );
}
