...
export interface TonkRSettings { dummy: boolean; }
export interface TonkRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type TonkRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: TonkRSettings): TonkRState {
...
export function reducer(state: TonkRState, action: TonkRAction): TonkRState {
...
export function isTerminal(state: TonkRState): { score: number } | null {
