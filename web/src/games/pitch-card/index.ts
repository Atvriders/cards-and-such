...
const PitchGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PitchGame as unknown as React.ComponentType<unknown> })));
...
export const pitchCardPlugin: GamePlugin<PitchState, PitchAction, typeof settings> = {
  ...
  description: "Pitch — bidding trick game.",
  howToPlay: "Pitch — bidding trick game. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as PitchSettings),
  reducer,
  isTerminal,
  hint: (state: PitchState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-pitchc-${pick.id}"]`, pulses: 3 };
    }
