...
export interface PitchCardSettings { dummy: boolean; }
export interface PitchCardState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type PitchCardAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: PitchCardSettings): PitchCardState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: PitchCardState, action: PitchCardAction): PitchCardState {
...
export function isTerminal(state: PitchCardState): { score: number } | null {
