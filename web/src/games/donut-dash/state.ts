...
// Donut Dash: bakery clicker. 25-second timer.

export const TIMER_TICKS = 25;
export const LANES = 6;

export interface DonutDashSettings { dummy: boolean; }

export interface DonutDashItem {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface DonutDashState {
  rngSeed: number;
  items: DonutDashItem[];
  nextId: number;
...
export type DonutDashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: DonutDashSettings): DonutDashState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: DonutDashState, action: DonutDashAction): DonutDashState {
...
export function isTerminal(state: DonutDashState): { score: number } | null {
