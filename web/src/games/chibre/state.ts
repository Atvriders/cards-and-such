// Chibre — Western Swiss partnership Jass — simplified to a head-to-head duel.
...
export type ChibrePhase = "playing" | "done";

export interface ChibreState {
  rngSeed: number;
...
export type ChibreAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `chibre-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: ChibreState, trick: readonly { seat: number; card: Card }[]): ChibreState {
...
export function reducer(state: ChibreState, action: ChibreAction): ChibreState {
  ...
  let s: ChibreState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: ChibreState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: ChibreState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): ChibreState {
...
export function isTerminal(state: ChibreState): { score: number } | null {
