...
export function SplendorCitiesGame({ state, dispatch, onGameOver }: GameProps<SplendorCitiesState, SplendorCitiesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Splendor: Cities</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SplendorCitiesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SplendorCitiesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
