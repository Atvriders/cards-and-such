...
export const SUIT_NAMES: readonly string[] = ["Diamond","Sapphire","Emerald","Ruby"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface SplendorCitiesSettings { dummy: boolean; }
export interface SplendorCitiesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SplendorCitiesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SplendorCitiesSettings): SplendorCitiesState {
...
export function reducer(state: SplendorCitiesState, action: SplendorCitiesAction): SplendorCitiesState {
...
export function score(s: SplendorCitiesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SplendorCitiesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
