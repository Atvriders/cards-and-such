...
export interface BrickBashSettings { dummy: boolean; }
...
export type BrickBashAction = { type: "tick" } | { type: "hit"; id: number };
export function initialState(seed: number, _settings: BrickBashSettings): BrickBashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}
export function reducer(state: BrickBashState, action: BrickBashAction): BrickBashState {
...
export function isTerminal(state: BrickBashState): { score: number } | null {
