...
const DiceAddBet = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceAddBet as unknown as React.ComponentType<unknown> })));
...
Use Settings to choose 10 or 20 rounds. Manage your bankroll carefully — large bets can be thrilling but also devastating. Can you grow your stake?`,
  ...
  hint: (state: DiceAddBetState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "betting") return { selector: '[data-testid="hint-target-diceaddbet-bet"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-diceaddbet-next"]', pulses: 3 };
    return null;
  },
};
