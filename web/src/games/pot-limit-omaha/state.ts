...
export interface PotLimitOmahaSettings { dummy: boolean; }
export interface PotLimitOmahaState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type PotLimitOmahaAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: PotLimitOmahaSettings): PotLimitOmahaState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: PotLimitOmahaState, action: PotLimitOmahaAction): PotLimitOmahaState {
...
export function isTerminal(state: PotLimitOmahaState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
