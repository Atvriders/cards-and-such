...
export interface BlackHoleSoliSettings { dummy: boolean; }

export interface BlackHoleSoliState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type BlackHoleSoliAction =
  | { type: "keep" }
...
function scoreHand(hand: number[]): { score: number; reason: string } {
  ...
  // Flush-heavy variant
  if (flush) { score += 24; reasons.push("flush"); }
  if (pairs >= 1) { score += pairs * 9; reasons.push(`${pairs} pair`); }
...
export function initialState(seed: number, _s: BlackHoleSoliSettings): BlackHoleSoliState {
...
export function reducer(state: BlackHoleSoliState, action: BlackHoleSoliAction): BlackHoleSoliState {
...
export function isTerminal(state: BlackHoleSoliState): { score: number } | null {
