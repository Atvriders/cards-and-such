...
export function BlackHoleSoliGame({ state, dispatch, onGameOver }: GameProps<BlackHoleSoliState, BlackHoleSoliSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as BlackHoleSoliAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
