    ...
    <div className="gcinema-wrap">
      ...
      <div className="gcinema-grid">
        {r.choices.map((n, i) => {
          ...
          return <button data-testid={`hint-target-cinematrix-yr-answer-${i}`} key={i} className={cls} disabled={state.submitted} onClick={() => dispatch({ type: "select", choice: i } as CinematrixYrAction)}>{n}</button>;
        })}
      </div>
