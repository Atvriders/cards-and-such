...
export const TOTAL_DRAWS = 20;
export const MATCH_SIZE = 4;
export const MATCH_POINTS = 100;
export interface CardQuadQuestSettings { dummy: boolean; }
export interface CardQuadQuestState { rngSeed: number; drawIdx: number; rankCounts: number[]; matches: number; score: number; phase: "playing" | "done"; lastCard: number | null; lastMatched: boolean; }
export type CardQuadQuestAction = { type: "draw" };
...
export function initialState(seed: number, _s: CardQuadQuestSettings): CardQuadQuestState {
...
export function reducer(state: CardQuadQuestState, action: CardQuadQuestAction): CardQuadQuestState {
...
export function isTerminal(s: CardQuadQuestState): { score: number } | null { return s.phase==="done" ? { score: s.score } : null; }
