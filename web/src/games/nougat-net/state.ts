...
export interface NougatNetSettings { dummy: boolean; }
...
export interface NougatNetState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type NougatNetAction =
  | { type: "tick" }
  | { type: "catch"; id: number };

export function initialState(seed: number, _settings: NougatNetSettings): NougatNetState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, caught: 0, missed: 0, phase: "playing" };
}

export function reducer(state: NougatNetState, action: NougatNetAction): NougatNetState {
...
export function isTerminal(state: NougatNetState): { score: number } | null {
