...
export interface ButterflyNetSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface ButterflyNetState { rngSeed: number; targets: Target[]; nextId: number; ticksRemaining: number; score: number; popped: number; missed: number; phase: "playing" | "done"; }
export type ButterflyNetAction = { type:"tick" } | { type:"pop"; id: number };
export function initialState(seed:number, _s:ButterflyNetSettings):ButterflyNetState {
  return { rngSeed:seed, targets:[], nextId:1, ticksRemaining:TIMER_TICKS, score:0, popped:0, missed:0, phase:"playing" };
}
export function reducer(state:ButterflyNetState, action:ButterflyNetAction):ButterflyNetState {
...
export function isTerminal(state:ButterflyNetState):{score:number}|null { return state.phase==="done"?{score:state.score}:null; }
