...
const ButterflyNetGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ButterflyNetGame as unknown as React.ComponentType<unknown> })));
...
export const butterflyNetPlugin: GamePlugin<ButterflyNetState, ButterflyNetAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: ButterflyNetState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-butterfly-net-target"]', pulses: 3 };
  },
  component:ButterflyNetGame,
};
