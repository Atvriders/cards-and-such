...
export const CARDS_PER_HAND = 17;
export interface PineappleOfcSettings { dummy: boolean; }
export interface PineappleOfcState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type PineappleOfcAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: PineappleOfcSettings): PineappleOfcState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: PineappleOfcState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
