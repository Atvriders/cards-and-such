...
export interface MagicMazeCoopSettings { dummy: boolean; }
export interface MagicMazeCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type MagicMazeCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: MagicMazeCoopSettings): MagicMazeCoopState {
