...
export interface RainbowCanfieldSettings { dummy: boolean; }

export interface RainbowCanfieldState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type RainbowCanfieldAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const suits = new Set(h.map(cardSuit)); s += suits.size*4; const f = h.filter(c=>cardRank(c)>=11).length; s += f*3; if (s===0) s=1; return s;
}
...
export function reducer(state: RainbowCanfieldState, action: RainbowCanfieldAction): RainbowCanfieldState {
...
export function isTerminal(state: RainbowCanfieldState): { score: number } | null {
