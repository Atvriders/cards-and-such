...
export interface EuropeanBjCasSettings { dummy: boolean }
export interface EuropeanBjCasState {
  rngSeed: number;
  round: number;
  you: number[];
  dealer: number[];
...
export type EuropeanBjCasAction = { type: "hit" } | { type: "stand" } | { type: "double" } | { type: "surrender" } | { type: "next" };
...
export function initialState(seed: number, _s: EuropeanBjCasSettings): EuropeanBjCasState {
...
export function reducer(state: EuropeanBjCasState, action: EuropeanBjCasAction): EuropeanBjCasState {
  ...
  if (action.type === "double" && state.phase === "play" && state.you.length === 2) {
    ...
    if (yt <= 21) dealer = dealerPlay(state.dealer, rng, used, false);
  ...
  if (action.type === "stand" && state.phase === "play") {
    const { rng, nextSeed } = makeRng(state.rngSeed);
    const used = new Set<number>([...state.you, ...state.dealer]);
    const dealer = dealerPlay(state.dealer, rng, used, false);
...
export function isTerminal(state: EuropeanBjCasState): { score: number } | null {
