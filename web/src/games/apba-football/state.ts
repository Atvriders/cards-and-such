...
export interface ApbaFootballSettings { dummy: boolean; }

export interface ApbaFootballState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type ApbaFootballAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: ApbaFootballSettings): ApbaFootballState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: ApbaFootballState, action: ApbaFootballAction): ApbaFootballState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<ApbaFootballState> = {};
...
export function isTerminal(state: ApbaFootballState): { score: number } | null {
