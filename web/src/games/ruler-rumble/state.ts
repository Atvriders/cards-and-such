...
export interface RulerRumbleSettings { dummy: boolean; }
...
export interface RulerRumbleState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type RulerRumbleAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: RulerRumbleSettings): RulerRumbleState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: RulerRumbleState, action: RulerRumbleAction): RulerRumbleState {
...
export function isTerminal(state: RulerRumbleState): { score: number } | null {
