...
export interface StraddleGameSettings { dummy: boolean; }
export interface StraddleGameState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type StraddleGameAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: StraddleGameSettings): StraddleGameState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: StraddleGameState, action: StraddleGameAction): StraddleGameState {
...
export function isTerminal(state: StraddleGameState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
