...
import "./Game.css";
export function StraddleGameGame({ state, dispatch, onGameOver }: GameProps<StraddleGameState, StraddleGameSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-straddle-game-deal" onClick={() => dispatch({ type: "deal" } as StraddleGameAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-straddle-game-next" onClick={() => dispatch({ type: "next" } as StraddleGameAction)}>Next</button>
      </>}
    </div>
  );
}
