...
describe("PlanetsQuiz",()=>{
  it("creates 10 questions",()=>{expect(s0().questions.length).toBe(10);});
  it("starts playing",()=>{expect(s0().phase).toBe("playing");});
  it("is deterministic",()=>{expect(initialState(8,{questions:"10"}).questions.map(q=>q.question)).toEqual(initialState(8,{questions:"10"}).questions.map(q=>q.question));});
