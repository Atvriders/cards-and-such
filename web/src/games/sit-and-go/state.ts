...
export interface SitAndGoSettings { dummy: boolean; }
export interface SitAndGoState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SitAndGoAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SitAndGoSettings): SitAndGoState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SitAndGoState, action: SitAndGoAction): SitAndGoState {
...
export function isTerminal(state: SitAndGoState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
