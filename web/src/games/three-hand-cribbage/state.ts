...
export interface ThreeHandCribbageSettings { dummy: boolean; }
export interface ThreeHandCribbageState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type ThreeHandCribbageAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ThreeHandCribbageSettings): ThreeHandCribbageState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: ThreeHandCribbageState, action: ThreeHandCribbageAction): ThreeHandCribbageState {
...
export function isTerminal(state: ThreeHandCribbageState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
