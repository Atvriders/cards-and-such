...
import "./Game.css";
export function ThreeHandCribbageGame({ state, dispatch, onGameOver }: GameProps<ThreeHandCribbageState, ThreeHandCribbageSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-three-hand-cribbage-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as ThreeHandCribbageAction)}>Cut!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-three-hand-cribbage-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ThreeHandCribbageAction)}>Next</button>
      </>}
    </div>
  );
}
