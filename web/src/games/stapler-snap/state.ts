...
export interface StaplerSnapSettings { dummy: boolean; }
...
export interface StaplerSnapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type StaplerSnapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: StaplerSnapSettings): StaplerSnapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: StaplerSnapState, action: StaplerSnapAction): StaplerSnapState {
...
export function isTerminal(state: StaplerSnapState): { score: number } | null {
