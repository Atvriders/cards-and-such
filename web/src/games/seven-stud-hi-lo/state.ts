...
export interface SevenStudHiLoSettings { dummy: boolean; }
export interface SevenStudHiLoState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SevenStudHiLoAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SevenStudHiLoSettings): SevenStudHiLoState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: SevenStudHiLoState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
