...
export interface BirdShootSettings { duration: "20" | "30" | "45"; }
export interface FallingItem { id: number; x: number; y: number; speed: number; points: number; }
export interface BirdShootState {
...
export type BirdShootAction = { type: "catch"; id: number } | { type: "tick" } | { type: "spawn" };

export function initialState(seed: number, settings: BirdShootSettings): BirdShootState {
...
export function reducer(state: BirdShootState, action: BirdShootAction): BirdShootState {
...
export function isTerminal(state: BirdShootState): { score: number } | null {
