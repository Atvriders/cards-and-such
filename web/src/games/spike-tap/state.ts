...
// Spike Tap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface SpikeTapSettings { dummy: boolean; }

export interface SpikeTapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface SpikeTapState {
  rngSeed: number;
  targets: SpikeTapTarget[];
  nextId: number;
...
export type SpikeTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: SpikeTapSettings): SpikeTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: SpikeTapState, action: SpikeTapAction): SpikeTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: SpikeTapTarget[] = [];
...
export function isTerminal(state: SpikeTapState): { score: number } | null {
