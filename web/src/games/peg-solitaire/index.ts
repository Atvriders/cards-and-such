...
const PegSolitaire = /* @__PURE__ */ lazy(() => import("./PegSolitaire.js").then((mod) => ({ default: mod.PegSolitaire as unknown as React.ComponentType<unknown> })));
...
Tips: try to maintain symmetry early in the game. Work from the edges inward. Avoid stranding pegs in corners where they cannot be jumped.`,
  ...
  hint: (state: PegSolitaireState): HintTarget | null => {
    if (state.won) return null;
    for (let i = 0; i < state.cells.length; i++) {
      if (state.cells[i] !== "peg" || !state.valid[i]) continue;
      if (getLegalJumps(state.cells, state.valid, i).length > 0) {
        return { selector: `[data-testid="hint-target-peg-solitaire-${i}"]`, pulses: 3 };
      }
    }
    return null;
  },
