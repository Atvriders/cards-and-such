  ...
  return (
    <div className="carcic-wrap">
      ...
      <div className="carcic-meta">
        ...
        <div className="carcic-meta-item"><span>Score</span><b>{state.score}</b></div>
        <div className="carcic-meta-item"><span>Phase</span><b>{state.phase}</b></div>
      </div>
      {state.phase !== "done" && next >= 0 && (
        <div className="carcic-next">
          Next tile:
          <span className="carcic-next-tile" style={{ background: PALETTE[next % PALETTE.length] }}>{typeName(next)}</span>
        </div>
      )}
      <div className="carcic-grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 44px)` }}>
        {state.cells.map((v, i) => (
          <button
            key={i}
            className={"carcic-cell " + (v < 0 ? "carcic-empty" : "carcic-filled")}
            style={v >= 0 ? { background: PALETTE[v % PALETTE.length], color: "#fff" } : undefined}
            onClick={() => v < 0 && state.phase !== "done" && dispatch({ type: "place", index: i } as CarcassonneInnsCathedralsAction)}
      ...
      <div className="carcic-legend">
        {TYPE_NAMES.map((n, i) => (
          <span key={i} className="carcic-leg" style={{ background: PALETTE[i % PALETTE.length] }}>{n}</span>
        ))}
      </div>
      <div className="carcic-rules">Place tiles adjacent to same-type tiles for +2 each. Clusters of 3+ score +4 bonus, 5+ +8.</div>
    </div>
  );
}
