...
export interface CardStatueSettings { dummy: boolean; }

export interface CardStatueState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardStatueAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardStatueSettings): CardStatueState {
  return { rngSeed: seed, round: 1, card: null, score: 0, lastPts: 0, phase: "draw" };
}

export function reducer(state: CardStatueState, action: CardStatueAction): CardStatueState {
...
export function isTerminal(state: CardStatueState): { score: number } | null {
