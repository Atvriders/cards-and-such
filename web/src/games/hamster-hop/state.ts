...
export interface HamsterHopSettings { dummy: boolean; }
export interface HamsterHopTarget { id: number; lane: number; ticksLeft: number; }
export interface HamsterHopState {
  rngSeed: number;
  targets: HamsterHopTarget[];
  nextId: number;
...
export type HamsterHopAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: HamsterHopSettings): HamsterHopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: HamsterHopState, action: HamsterHopAction): HamsterHopState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: HamsterHopTarget[] = [];
...
export function isTerminal(state: HamsterHopState): { score: number } | null {
