...
export function CloneSudokuMiniGame({ state, dispatch, onGameOver }: GameProps<CloneSudokuMiniState, CloneSudokuMiniSettings>): JSX.Element {
    ...
    <div className="ded-wrap">
      ...
      <div className="ded-options">
        {p.options.map((opt, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={state.resolved} onClick={() => dispatch({ type: "select", index: i } as CloneSudokuMiniAction)}>
              {opt}
            </button>
          );
        })}
      ...
      <div className="ded-actions">
        {!state.resolved && (
          <button className="ded-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as CloneSudokuMiniAction)}>Accuse!</button>
        )}
        {state.resolved && (
          <button className="ded-btn next" onClick={() => dispatch({ type: "next" } as CloneSudokuMiniAction)}>
