...
export interface LadyOfTheManorSettings { dummy: boolean; }

export interface LadyOfTheManorState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type LadyOfTheManorAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { const r = cardRank(c); if (r >= 11) s += 6; else if (r === 1) s += 5; else s += 1; }
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: LadyOfTheManorState, action: LadyOfTheManorAction): LadyOfTheManorState {
...
export function isTerminal(state: LadyOfTheManorState): { score: number } | null {
