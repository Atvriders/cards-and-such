...
// Pip Purse: Each round, deal 5 cards. Sum their PIP values.
// 2..10 = face value, J=11, Q=12, K=13, A=1.
// Score: if sum < 20, +50 - sum*2 (so smaller is better, capped at 0).
// 8 rounds total.

export const TOTAL_ROUNDS = 8;

export interface PipPurseSettings { dummy: boolean; }

export interface PipPurseState {
  rngSeed: number;
  round: number;
  hand: number[]; // 5 cards
  sum: number;
...
export type PipPurseAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: PipPurseSettings): PipPurseState {
  return { rngSeed: seed, round: 1, hand: [], sum: 0, score: 0, phase: "dealing", lastPts: 0 };
}

export function reducer(state: PipPurseState, action: PipPurseAction): PipPurseState {
  ...
  if (action.type === "deal") {
    ...
    let pts = 0;
    if (sum < 20) pts = Math.max(0, 50 - sum * 2);
...
export function isTerminal(state: PipPurseState): { score: number } | null {
