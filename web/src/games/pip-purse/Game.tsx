...
export function PipPurseGame({ state, dispatch, onGameOver }: GameProps<PipPurseState, PipPurseSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          <div className="cm-result">Sum: {state.sum} — {state.lastPts > 0 ? `+${state.lastPts}` : "no points (≥ 20)"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as PipPurseAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
