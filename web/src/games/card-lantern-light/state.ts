...
export interface CardLanternLightSettings { dummy: boolean; }

export interface CardLanternLightState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardLanternLightAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardLanternLightSettings): CardLanternLightState {
  return { rngSeed: seed, round: 1, card: null, score: 0, lastPts: 0, phase: "draw" };
}

export function reducer(state: CardLanternLightState, action: CardLanternLightAction): CardLanternLightState {
...
export function isTerminal(state: CardLanternLightState): { score: number } | null {
