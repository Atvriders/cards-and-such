...
const Samba = /* @__PURE__ */ lazy(() => import("./Samba.js").then((mod) => ({ default: mod.Samba as unknown as React.ComponentType<unknown> })));
...
Controls: Draw from stock or click discard. Select cards in your hand, then click Meld to place them. Click a single selected card then Discard to end your turn.`,
  ...
  reducer,
  isTerminal,
  hint: (state: SambaState): HintTarget | null => {
    if (isTerminal(state)) return null;
    return { selector: '[data-testid="hint-target-samba-primary"]', pulses: 3 };
  },
  component: Samba,
};
