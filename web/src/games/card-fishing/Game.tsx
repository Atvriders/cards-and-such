...
export function CardFishingGame({ state, dispatch, onGameOver }: GameProps<CardFishingState, CardFishingSettings>): JSX.Element {
    ...
    <div className="rg-wrap">
      ...
      {state.phase === "result" && (
        <>
          ...
          <button className="rg-btn alt" onClick={() => dispatch({ type:"next" } as CardFishingAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
