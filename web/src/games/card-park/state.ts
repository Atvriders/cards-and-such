...
export interface CardParkSettings { dummy: boolean; }
export interface CardParkState { rngSeed: number; deck: number[]; drawn: number[]; score: number; phase: "drawing" | "done"; }
export type CardParkAction = { type: "draw" } | { type: "reset" };
...
export function scoreCard(c: number): number {
  return Math.floor(c/13)===2?10:0;
}
export function initialState(seed: number, _settings: CardParkSettings): CardParkState {
  const rng=mulberry32(seed); const deck=shuffle(rng);
  return { rngSeed: seed, deck, drawn: [], score: 0, phase: "drawing" };
}
export function reducer(state: CardParkState, action: CardParkAction): CardParkState {
...
export function isTerminal(state: CardParkState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
