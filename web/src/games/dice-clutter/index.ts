...
const DiceClutterGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceClutterGame as unknown as React.ComponentType<unknown> })));
...
export const diceClutterPlugin: GamePlugin<DiceClutterState, DiceClutterAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceClutterState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-clutter-roll"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-dice-clutter-next"]', pulses: 3 };
    return null;
  },
  component:DiceClutterGame,
};
