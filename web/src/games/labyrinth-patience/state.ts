...
export interface LabyrinthPatienceSettings { dummy: boolean; }

export interface LabyrinthPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type LabyrinthPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank); let altCount=1, best=1; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; const c=(i>=2)?(r[i-2] as number):-999; if ((a>b && (i<2 || b<c)) || (a<b && (i<2 || b>c))){ altCount++; if(altCount>best)best=altCount; } else altCount=1; } s += best*4; if (s===0) s=1; return s;
}
...
export function reducer(state: LabyrinthPatienceState, action: LabyrinthPatienceAction): LabyrinthPatienceState {
...
export function isTerminal(state: LabyrinthPatienceState): { score: number } | null {
