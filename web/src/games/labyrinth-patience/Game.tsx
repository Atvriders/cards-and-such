...
export function LabyrinthPatienceGame({ state, dispatch, onGameOver }: GameProps<LabyrinthPatienceState, LabyrinthPatienceSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as LabyrinthPatienceAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as LabyrinthPatienceAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as LabyrinthPatienceAction)}>Discard Hand</button>
      </div>
