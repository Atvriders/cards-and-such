...
import type { SpicesQuizState, SpicesQuizSettings } from "./state.js";
import { isTerminal } from "./state.js";
import "./SpicesQuiz.css";

export function SpicesQuiz({ state, dispatch, onGameOver }: GameProps<SpicesQuizState, SpicesQuizSettings>): JSX.Element {
    ...
    <div className="sq-wrap">
      ...
      {!state.done && entry ? (
        <>
          ...
          {state.selected !== null && (
            <button className="sq-next" onClick={() => dispatch({ type: "next" })}>
        ...
        <div className="sq-done">
          <h2>Quiz Complete!</h2>
          <div className="sq-final">Score: {state.score} / {state.entries.length * 10}</div>
        </div>
      )}
    </div>
  );
