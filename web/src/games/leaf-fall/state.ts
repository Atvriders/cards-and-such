...
// Leaf Fall: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface LeafFallSettings { dummy: boolean; }

export interface LeafFallTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface LeafFallState {
  rngSeed: number;
  targets: LeafFallTarget[];
  nextId: number;
...
export type LeafFallAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: LeafFallSettings): LeafFallState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LeafFallState, action: LeafFallAction): LeafFallState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: LeafFallTarget[] = [];
...
export function isTerminal(state: LeafFallState): { score: number } | null {
