...
export function RainbowSolitaireGame({ state, dispatch, onGameOver }: GameProps<RainbowSolitaireState, RainbowSolitaireSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as RainbowSolitaireAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
      <div className="sol-actions">
        <button data-testid="hint-target-rainbow-solitaire-primary" className="sol-btn sol-btn-keep" onClick={() => dispatch({ type: "keep" } as RainbowSolitaireAction)}>Keep & Score</button>
        <button className="sol-btn sol-btn-disc" onClick={() => dispatch({ type: "discard", index: 0 } as RainbowSolitaireAction)}>Discard Hand</button>
      </div>
