        ...
        <>
          ...
          <div className="obscard-grid">
            {r.choices.map((n, i) => {
              ...
              return <button data-testid={`hint-target-observer-card-answer-${i}`} key={i} className={cls} disabled={state.submitted} onClick={() => dispatch({ type: "select", choice: i } as ObserverCardAction)}>{n}</button>;
            })}
          </div>
