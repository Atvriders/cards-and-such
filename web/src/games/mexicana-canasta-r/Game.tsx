...
export function MexicanaCanastaRGame({ state, dispatch, onGameOver }: GameProps<MexicanaCanastaRState, MexicanaCanastaRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as MexicanaCanastaRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MexicanaCanastaRAction)}>Next</button>
      </>}
    </div>
  );
}
