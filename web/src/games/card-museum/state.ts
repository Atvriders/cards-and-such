...
export interface CardMuseumSettings { dummy: boolean; }
export interface CardMuseumState { rngSeed: number; deck: number[]; drawn: number[]; score: number; phase: "drawing" | "done"; }
export type CardMuseumAction = { type: "draw" } | { type: "reset" };
...
export function scoreCard(c: number): number {
  return Math.floor(c/13)===0?10:0;
}
export function initialState(seed: number, _settings: CardMuseumSettings): CardMuseumState {
  const rng=mulberry32(seed); const deck=shuffle(rng);
  return { rngSeed: seed, deck, drawn: [], score: 0, phase: "drawing" };
}
export function reducer(state: CardMuseumState, action: CardMuseumAction): CardMuseumState {
...
export function isTerminal(state: CardMuseumState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
