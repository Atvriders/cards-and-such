...
export interface ChowahaPokerSettings { dummy: boolean; }
export interface ChowahaPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ChowahaPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ChowahaPokerSettings): ChowahaPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ChowahaPokerState, action: ChowahaPokerAction): ChowahaPokerState {
...
export function isTerminal(state: ChowahaPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
