    ...
    <div className="dm-wrap">
      ...
      <div className="dm-board" style={{ background:"linear-gradient(180deg,#3a5f3a,#1f3f1f)" }}>
        {state.targets.map(t => {
          ...
          return <button key={t.id} className="dm-target" style={{ left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)", color:"#fff" }} onClick={() => dispatch({ type:"click", id:t.id } as MushroomMashAction)} aria-label="mushroom-mash">🍄</button>;
        })}
      </div>
    </div>
  );
