    ...
    <div className="rkq-wrap">
      ...
      <div className="rkq-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", choice:i } as RacingKingsQuizAction)}><span className="rkq-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
