...
export interface VideoPokerJacksSettings { dummy: boolean; }
export interface VideoPokerJacksState { rngSeed: number; round: number; hand: number[]; phase: "deal" | "scored" | "done"; score: number; pts: number; result: string; rank: string; }
export type VideoPokerJacksAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: VideoPokerJacksSettings): VideoPokerJacksState {
  return { rngSeed: seed, round: 1, hand: [], phase: "deal", score: 0, pts: 0, result: "", rank: "" };
}
export function reducer(state: VideoPokerJacksState, action: VideoPokerJacksAction): VideoPokerJacksState {
...
export function isTerminal(state: VideoPokerJacksState): { score: number } | null {
