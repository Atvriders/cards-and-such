...
export function VideoPokerJacksGame({ state, dispatch, onGameOver }: GameProps<VideoPokerJacksState, VideoPokerJacksSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as VideoPokerJacksAction)}>Deal</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as VideoPokerJacksAction)}>Next</button>
      </>}
    </div>
  );
}
