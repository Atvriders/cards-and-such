...
import type { ArmenianDraughtsState, ArmenianDraughtsSettings, ArmenianDraughtsAction } from "./state.js";
import { isTerminal, getLegalMoves } from "./state.js";
import "./Game.css";

export function ArmenianDraughts({
  state,
  dispatch,
  onGameOver,
}: GameProps<ArmenianDraughtsState, ArmenianDraughtsSettings>): JSX.Element {
  ...
  function handleCellClick(pos: number) {
    ...
    if (validTargets.has(pos)) {
      dispatch({ type: "move", to: pos } satisfies ArmenianDraughtsAction);
    } else {
      const p = state.board[pos];
      if (p && p.color === "white") {
        dispatch({ type: "select", pos } satisfies ArmenianDraughtsAction);
      }
    }
  }
  ...
  return (
    <div className="armdraughts-game">
      <div className="armdraughts-counts">
        <span>White: {wCount}</span>
        <span>Black: {bCount}</span>
      </div>
      <div className={`armdraughts-status ${statusClass}`}>{statusText}</div>
      <div className="armdraughts-board" style={{ width: CELL * 8, height: CELL * 8 }}>
        {Array.from({ length: 64 }, (_, i) => {
          ...
          let cellClass = "armdraughts-cell";
          ...
          return (
            ...
            >
              {piece && (
                <div className={`armdraughts-piece ${piece.color}${piece.king ? " king" : ""}`}>
                  {piece.king ? "♛" : "●"}
                </div>
              )}
            </div>
