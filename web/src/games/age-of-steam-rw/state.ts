...
export interface AgeOfSteamRwSettings { dummy: boolean; }
export interface AgeOfSteamRwState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type AgeOfSteamRwAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: AgeOfSteamRwSettings): AgeOfSteamRwState {
  return {
    rngSeed: seed,
...
export function reducer(state: AgeOfSteamRwState, action: AgeOfSteamRwAction): AgeOfSteamRwState {
...
export function isTerminal(state: AgeOfSteamRwState): { score: number } | null {
