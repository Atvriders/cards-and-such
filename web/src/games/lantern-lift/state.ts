...
export interface LanternLiftSettings { dummy: boolean; }
...
export interface LanternLiftState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type LanternLiftAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _s: LanternLiftSettings): LanternLiftState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LanternLiftState, action: LanternLiftAction): LanternLiftState {
...
export function isTerminal(state: LanternLiftState): { score: number } | null {
