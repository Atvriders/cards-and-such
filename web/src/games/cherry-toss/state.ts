...
export interface CherryTossSettings { dummy: boolean; }
export interface CherryTossState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type CherryTossAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: CherryTossSettings): CherryTossState {
...
export function reducer(state: CherryTossState, action: CherryTossAction): CherryTossState {
...
export function isTerminal(state: CherryTossState): { score: number } | null {
