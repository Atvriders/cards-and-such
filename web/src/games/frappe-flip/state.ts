...
// Frappe Flip: targets drift across the screen in 6 lanes. Click each before it disappears for 10 points. 25-tick timer.
export const TIMER_TICKS = 25;
export const LANES = 6;
export const POINTS = 10;
export interface FrappeFlipSettings { dummy: boolean; }
export interface Target { id: number; lane: number; ticksLeft: number; }
export interface FrappeFlipState { rngSeed: number; targets: Target[]; nextId: number; ticksRemaining: number; score: number; clicked: number; missed: number; phase: "playing" | "done"; }
export type FrappeFlipAction = { type: "tick" } | { type: "click"; id: number };
export function initialState(seed: number, _s: FrappeFlipSettings): FrappeFlipState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, clicked: 0, missed: 0, phase: "playing" };
}
export function reducer(state: FrappeFlipState, action: FrappeFlipAction): FrappeFlipState {
...
export function isTerminal(state: FrappeFlipState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
