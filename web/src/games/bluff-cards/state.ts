...
export interface BluffCardsSettings { dummy: boolean; }
export interface BluffCardsState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type BluffCardsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: BluffCardsSettings): BluffCardsState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: BluffCardsState, action: BluffCardsAction): BluffCardsState {
...
export function isTerminal(state: BluffCardsState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
