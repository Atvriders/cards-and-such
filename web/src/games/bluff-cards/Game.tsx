...
import "./Game.css";
export function BluffCardsGame({ state, dispatch, onGameOver }: GameProps<BluffCardsState, BluffCardsSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap bf-shed">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-bluff-cards-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as BluffCardsAction)}>Stake!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as BluffCardsAction)}>Next</button>
      </>}
    </div>
  );
}
