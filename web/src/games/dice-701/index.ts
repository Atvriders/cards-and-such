...
const Dice701Game = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.Dice701Game as unknown as React.ComponentType<unknown> })));
...
export const dice701Plugin: GamePlugin<Dice701State, Dice701Action, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: Dice701State): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-701-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-701-next"]', pulses: 3 };
    return null;
  },
