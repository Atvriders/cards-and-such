...
export interface NinepinsClassicSettings { dummy: boolean; }

export interface NinepinsClassicState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type NinepinsClassicAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: NinepinsClassicSettings): NinepinsClassicState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: NinepinsClassicState, action: NinepinsClassicAction): NinepinsClassicState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<NinepinsClassicState> = {};
...
export function isTerminal(state: NinepinsClassicState): { score: number } | null {
