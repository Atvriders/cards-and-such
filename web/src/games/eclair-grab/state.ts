...
export interface EclairGrabSettings { dummy: boolean; }
export interface EclairGrabState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type EclairGrabAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: EclairGrabSettings): EclairGrabState {
...
export function reducer(state: EclairGrabState, action: EclairGrabAction): EclairGrabState {
...
export function isTerminal(state: EclairGrabState): { score: number } | null {
