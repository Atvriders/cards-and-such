...
export interface ShuffleQuarterSettings { dummy: boolean; }
export interface ShuffleQuarterState {
  rngSeed: number;
  turn: number;
  score: number;
  lastPts: number;
...
export type ShuffleQuarterAction = { type: "throw" } | { type: "next" };

export function initialState(seed: number, _s: ShuffleQuarterSettings): ShuffleQuarterState {
  return { rngSeed: seed, turn: 1, score: 0, lastPts: 0, lastRoll: 0, phase: "ready" };
}

export function reducer(state: ShuffleQuarterState, action: ShuffleQuarterAction): ShuffleQuarterState {
...
export function isTerminal(s: ShuffleQuarterState): { score: number } | null { return s.phase === "done" ? { score: s.score } : null; }
