// Auto-generated race-game state for ludo-quick-play.
...
export const SPECIALS: Record<number, Special> = {8: {type:"safe"},16: {type:"safe"},24: {type:"safe"},32: {type:"safe"}};

export type Side = "P" | "C";
export interface LudoQuickSettings { dummy: boolean }

export interface LudoQuickState {
  rngSeed: number;
...
export function initialState(seed: number, _s: LudoQuickSettings): LudoQuickState {
  return {
    rngSeed: seed,
...
function checkWin(s: LudoQuickState): LudoQuickState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: LudoQuickState): LudoQuickState {
  let s = state;
...
export function legalMovesForP(state: LudoQuickState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: LudoQuickState, action: LudoQuickAction): LudoQuickState {
...
export function isTerminal(state: LudoQuickState): { score: number } | null {
