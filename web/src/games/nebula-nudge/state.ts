...
export interface NebulaNudgeSettings { dummy: boolean; }
export interface NebulaNudgeTarget { id: number; lane: number; ticksLeft: number; }
export interface NebulaNudgeState {
  rngSeed: number;
  targets: NebulaNudgeTarget[];
  nextId: number;
...
export type NebulaNudgeAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: NebulaNudgeSettings): NebulaNudgeState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: NebulaNudgeState, action: NebulaNudgeAction): NebulaNudgeState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: NebulaNudgeTarget[] = [];
...
export function isTerminal(state: NebulaNudgeState): { score: number } | null {
