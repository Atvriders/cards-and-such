...
export function DiceDragonFightGame({ state, dispatch, onGameOver }: GameProps<DiceDragonFightState, DiceDragonFightSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="dm-btn" onClick={() => dispatch({ type:"roll" } as DiceDragonFightAction)}>Roll Dice</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="dm-result">+{state.lastPts} pts</div>
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceDragonFightAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
