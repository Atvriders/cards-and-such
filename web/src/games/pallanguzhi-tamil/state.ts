...
export interface PallanguzhiTamilSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface PallanguzhiTamilState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type PallanguzhiTamilAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: PallanguzhiTamilSettings): PallanguzhiTamilState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: PallanguzhiTamilState, action: PallanguzhiTamilAction): PallanguzhiTamilState {
...
export function isTerminal(state: PallanguzhiTamilState): { score: number } | null {
