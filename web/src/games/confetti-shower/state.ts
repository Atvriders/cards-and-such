...
export interface ConfettiShowerSettings { dummy: boolean; }
export interface ConfettiShowerItem {
  id: number;
  lane: number;
  ticksLeft: number;
}
export interface ConfettiShowerState {
  rngSeed: number;
  items: ConfettiShowerItem[];
  nextId: number;
...
export type ConfettiShowerAction =
  | { type: "tick" }
  | { type: "pop"; id: number };
export function initialState(seed: number, _settings: ConfettiShowerSettings): ConfettiShowerState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: ConfettiShowerState, action: ConfettiShowerAction): ConfettiShowerState {
  ...
  if (action.type === "tick") {
    ...
    const newItems: ConfettiShowerItem[] = [];
...
export function isTerminal(state: ConfettiShowerState): { score: number } | null {
