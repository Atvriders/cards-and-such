...
const ConfettiShowerGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.ConfettiShowerGame as unknown as React.ComponentType<unknown> })));
...
export const confettiShowerPlugin: GamePlugin<ConfettiShowerState, ConfettiShowerAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: ConfettiShowerState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.items || state.items.length === 0) return null;
    return { selector: '[data-testid="hint-target-confetti-shower-target"]', pulses: 3 };
  },
  component:ConfettiShowerGame,
};
