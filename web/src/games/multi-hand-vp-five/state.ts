...
export interface MultiHandVpFiveSettings { dummy: boolean; }
export interface MultiHandVpFiveState { rngSeed: number; round: number; hand: number[]; phase: "deal" | "scored" | "done"; score: number; pts: number; result: string; rank: string; }
export type MultiHandVpFiveAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: MultiHandVpFiveSettings): MultiHandVpFiveState {
  return { rngSeed: seed, round: 1, hand: [], phase: "deal", score: 0, pts: 0, result: "", rank: "" };
}
export function reducer(state: MultiHandVpFiveState, action: MultiHandVpFiveAction): MultiHandVpFiveState {
...
export function isTerminal(state: MultiHandVpFiveState): { score: number } | null {
