...
export interface QwixxGemixxtSettings { dummy: boolean; }
export interface QwixxGemixxtState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type QwixxGemixxtAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: QwixxGemixxtSettings): QwixxGemixxtState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: QwixxGemixxtState, action: QwixxGemixxtAction): QwixxGemixxtState {
...
export function isTerminal(state: QwixxGemixxtState): { score: number } | null {
