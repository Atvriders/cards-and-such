...
export function QwixxGemixxtGame({ state, dispatch, onGameOver }: GameProps<QwixxGemixxtState, QwixxGemixxtSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as QwixxGemixxtAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as QwixxGemixxtAction)}>Skip</button>
      )}
    </div>
  );
}
