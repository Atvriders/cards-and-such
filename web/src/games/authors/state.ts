...
export interface AuthorsSettings { dummy: boolean; }
export interface AuthorsState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type AuthorsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: AuthorsSettings): AuthorsState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: AuthorsState, action: AuthorsAction): AuthorsState {
...
export function isTerminal(state: AuthorsState): { score: number } | null {
