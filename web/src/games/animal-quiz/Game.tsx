...
export function AnimalQuiz({ state, dispatch, onGameOver }: GameProps<QuizState, QuizSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return (
      <div className="trivia-wrap aniqui-wrap">
        <div className="trivia-done bounce-in">
          <h2>Game Over!</h2>
  ...
  return (
    <div className="trivia-wrap aniqui-wrap fade-in">
        ...
        {q.choices.map((choice, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={isResult}
              data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type: "select", choice: i } as QuizAction)}>
      ...
      <div className="trivia-actions">
        {!isResult && (
          <button className="trivia-btn submit" disabled={state.selected === null}
            onClick={() => dispatch({ type: "submit" } as QuizAction)}>Submit</button>
        )}
        {isResult && (
          <button className="trivia-btn next"
            onClick={() => dispatch({ type: "next" } as QuizAction)}>
