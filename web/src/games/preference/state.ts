// Preference — Central European 3-hand bidding game — set-trump duel here.
...
export type PreferencePhase = "playing" | "done";
...
export type PreferenceAction = { type: "play"; cardId: string };

function buildDeck(): Card[] {
  const cards: Card[] = [];
  const RANKS: Rank[] = [1, 7, 8, 9, 10, 11, 12, 13];
  const SUITS: Suit[] = ["♠", "♥", "♦", "♣"];
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `preference-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: PreferenceState, trick: readonly { seat: number; card: Card }[]): PreferenceState {
...
export function reducer(state: PreferenceState, action: PreferenceAction): PreferenceState {
  ...
  let s: PreferenceState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: PreferenceState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: PreferenceState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): PreferenceState {
...
export function isTerminal(state: PreferenceState): { score: number } | null {
