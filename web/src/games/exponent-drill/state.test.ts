...
describe("ExponentDrill initialState", () => {
  ...
  it("answers equal base^exp", () => {
    const s = initialState(3, hard20);
...
describe("ExponentDrill reducer", () => {
  it("type updates typed field", () => {
    const s = initialState(1, easy10);
    const s2 = reducer(s, { type: "type", text: "64" });
    expect(s2.typed).toBe("64");
  });
...
describe("ExponentDrill isTerminal", () => {
