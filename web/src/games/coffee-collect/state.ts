...
export interface CoffeeCollectSettings { dummy: boolean; }
export interface CoffeeCollectItem { id: number; lane: number; ticksLeft: number; }
export interface CoffeeCollectState {
  rngSeed: number;
  items: CoffeeCollectItem[];
  nextId: number;
...
export type CoffeeCollectAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CoffeeCollectSettings): CoffeeCollectState {
  return { rngSeed: seed, items: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CoffeeCollectState, action: CoffeeCollectAction): CoffeeCollectState {
  ...
  if (action.type === "tick") {
    ...
    const newOnes: CoffeeCollectItem[] = [];
...
export function isTerminal(state: CoffeeCollectState): { score: number } | null {
