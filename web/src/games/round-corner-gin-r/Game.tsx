...
export function RoundCornerGinRGame({ state, dispatch, onGameOver }: GameProps<RoundCornerGinRState, RoundCornerGinRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as RoundCornerGinRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as RoundCornerGinRAction)}>Next</button>
      </>}
    </div>
  );
}
