...
export interface EthnosAlliesState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type EthnosAlliesAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: EthnosAlliesSettings): EthnosAlliesState {
...
export function score(s: EthnosAlliesState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: EthnosAlliesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
