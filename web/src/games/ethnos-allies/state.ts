...
export const SUIT_NAMES = ["Centaur","Dwarf","Elf","Giant","Wizard","Troll"] as const;
...
export type EthnosAlliesState = DraftState;
export type EthnosAlliesAction = DraftAction;
export interface EthnosAlliesSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: EthnosAlliesSettings): EthnosAlliesState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: EthnosAlliesState, action: EthnosAlliesAction): EthnosAlliesState {
  return draftReducer(state, action);
}

export function score(state: EthnosAlliesState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: EthnosAlliesState): { score: number } | null {
