...
export function EthnosAlliesGame({ state, dispatch, onGameOver }: GameProps<EthnosAlliesState, EthnosAlliesSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Ethnos Allies</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as EthnosAlliesAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as EthnosAlliesAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
