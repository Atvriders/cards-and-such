...
export const TOTAL_TILES = 17;
export const NUM_TYPES = 6;
export interface CarcassonneTradersBuildersSettings { dummy: boolean; }
export interface CarcassonneTradersBuildersState {
  rngSeed: number;
  cells: number[];
  queue: number[];
  placed: number;
...
export type CarcassonneTradersBuildersAction = { type: "place"; index: number };
...
export function initialState(seed: number, _s: CarcassonneTradersBuildersSettings): CarcassonneTradersBuildersState {
...
export function scoreBoard(cells: number[]): number {
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    for (const ni of neighbors(i)) {
      const nv = cells[ni];
      if (nv !== undefined && nv === v) score += 1;
    }
  }
  return score;
}

export function reducer(state: CarcassonneTradersBuildersState, action: CarcassonneTradersBuildersAction): CarcassonneTradersBuildersState {
...
export function isTerminal(state: CarcassonneTradersBuildersState): { score: number } | null {
