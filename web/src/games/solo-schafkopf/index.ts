...
const SoloSchafkopfGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SoloSchafkopfGame as unknown as React.ComponentType<unknown> })));
...
export const soloSchafkopfPlugin: GamePlugin<SoloSchafkopfState, SoloSchafkopfAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: SoloSchafkopfState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-solskc-${pick.id}"]`, pulses: 3 };
    }
