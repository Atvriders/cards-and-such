...
import "./Game.css";
export function MunchkinMiniGame({ state, dispatch, onGameOver }: GameProps<MunchkinMiniState, MunchkinMiniSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as MunchkinMiniAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
