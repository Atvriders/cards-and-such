...
export interface ChickChaseSettings { dummy: boolean; }
...
export interface ChickChaseState {
  rngSeed: number;
  critters: Critter[];
  nextId: number;
...
export type ChickChaseAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: ChickChaseSettings): ChickChaseState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: ChickChaseState, action: ChickChaseAction): ChickChaseState {
...
export function isTerminal(state: ChickChaseState): { score: number } | null {
