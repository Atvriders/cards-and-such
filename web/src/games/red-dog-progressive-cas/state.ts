...
export interface RedDogProgressiveCasSettings { dummy: boolean; }
export interface RedDogProgressiveCasState { rngSeed: number; round: number; left: number | null; right: number | null; middle: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type RedDogProgressiveCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: RedDogProgressiveCasSettings): RedDogProgressiveCasState {
  return { rngSeed: seed, round: 1, left: null, right: null, middle: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: RedDogProgressiveCasState, action: RedDogProgressiveCasAction): RedDogProgressiveCasState {
  ...
  if (action.type === "play") {
    ...
    if (mid === lo || mid === hi) { pts = 0; result = "Post! (lose)"; }
      ...
      if (spread >= 9) pts = 6;
      else if (spread >= 5) pts = 14;
      else if (spread >= 2) pts = 22;
      else pts = 55;
      result = `In between! +${pts}`;
    } else { pts = 0; result = "Outside the spread"; }
    if (lo === hi) { pts = 8; result = `Pair (push) +${pts}`; }
...
export function isTerminal(state: RedDogProgressiveCasState): { score: number } | null {
