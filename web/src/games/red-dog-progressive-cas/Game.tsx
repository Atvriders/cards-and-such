...
export function RedDogProgressiveCasGame({ state, dispatch, onGameOver }: GameProps<RedDogProgressiveCasState, RedDogProgressiveCasSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as RedDogProgressiveCasAction)}>Play</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as RedDogProgressiveCasAction)}>Next</button>
      </>}
    </div>
  );
}
