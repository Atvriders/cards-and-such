...
export interface PhaseTenShedSettings { dummy: boolean; }
export interface PhaseTenShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type PhaseTenShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: PhaseTenShedSettings): PhaseTenShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: PhaseTenShedState, action: PhaseTenShedAction): PhaseTenShedState {
...
export function isTerminal(state: PhaseTenShedState): { score: number } | null {
