...
const PhaseTenShedGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PhaseTenShedGame as unknown as React.ComponentType<unknown> })));
...
export const phaseTenShedPlugin: GamePlugin<PhaseTenShedState, PhaseTenShedAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: PhaseTenShedState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-phase-ten-shed-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-phase-ten-shed-next"]', pulses: 3 };
    return null;
  },
  component: PhaseTenShedGame,
};
