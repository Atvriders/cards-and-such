...
export interface QuoitsEnglishSettings { dummy: boolean; }

export interface QuoitsEnglishState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type QuoitsEnglishAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: QuoitsEnglishSettings): QuoitsEnglishState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: QuoitsEnglishState, action: QuoitsEnglishAction): QuoitsEnglishState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<QuoitsEnglishState> = {};
...
export function isTerminal(state: QuoitsEnglishState): { score: number } | null {
