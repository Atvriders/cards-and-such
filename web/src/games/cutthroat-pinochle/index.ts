...
const CutthroatPinochleGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CutthroatPinochleGame as unknown as React.ComponentType<unknown> })));
...
export const cutthroatPinochlePlugin: GamePlugin<CutthroatPinochleState, CutthroatPinochleAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: CutthroatPinochleState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-cutpinc-${pick.id}"]`, pulses: 3 };
    }
