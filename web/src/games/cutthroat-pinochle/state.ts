// Cutthroat Pinochle — No-partner Pinochle reduced to a one-on-one trick contest.
...
export type CutthroatPinochlePhase = "playing" | "done";

export interface CutthroatPinochleState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: CutthroatPinochlePhase;
  playerLeads: boolean;
  message: string;
}

export type CutthroatPinochleAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (let copy = 0; copy < 2; copy++) {
    for (const s of SUITS) for (const r of RANKS) {
      cards.push({ suit: s, rank: r, id: `cutthroat-pinochle-${copy}-${s}${r}` });
    }
  }
  return cards;
}
...
function applyTrick(state: CutthroatPinochleState, trick: readonly { seat: number; card: Card }[]): CutthroatPinochleState {
...
export function reducer(state: CutthroatPinochleState, action: CutthroatPinochleAction): CutthroatPinochleState {
  ...
  let s: CutthroatPinochleState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: CutthroatPinochleState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: CutthroatPinochleState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): CutthroatPinochleState {
...
export function isTerminal(state: CutthroatPinochleState): { score: number } | null {
