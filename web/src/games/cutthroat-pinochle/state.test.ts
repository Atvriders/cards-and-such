...
describe("cutthroat-pinochle initialState", () => {
  it("deals correct number of cards to each player", () => {
    const s = initialState(42);
    expect(s.playerHand.length).toBe(12);
    expect(s.botHand.length).toBe(12);
  });
...
describe("cutthroat-pinochle legalPlays", () => {
  it("all cards legal when leading", () => {
    const s = initialState(42);
    expect(legalPlays(s.playerHand, s.currentTrick).length).toBeGreaterThanOrEqual(12);
  });
});

describe("cutthroat-pinochle reducer", () => {
...
describe("cutthroat-pinochle full game", () => {
