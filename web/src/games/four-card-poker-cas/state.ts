...
export interface FourCardPokerCasSettings { dummy: boolean; }
export interface FourCardPokerCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type FourCardPokerCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: FourCardPokerCasSettings): FourCardPokerCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: FourCardPokerCasState, action: FourCardPokerCasAction): FourCardPokerCasState {
...
export function isTerminal(state: FourCardPokerCasState): { score: number } | null {
