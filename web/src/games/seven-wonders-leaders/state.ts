...
export const SUIT_NAMES = ["Hero","Sage","General","Patron"] as const;
...
export type SevenWondersLeadersState = DraftState;
export type SevenWondersLeadersAction = DraftAction;
export interface SevenWondersLeadersSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SevenWondersLeadersSettings): SevenWondersLeadersState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SevenWondersLeadersState, action: SevenWondersLeadersAction): SevenWondersLeadersState {
  return draftReducer(state, action);
}

export function score(state: SevenWondersLeadersState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SevenWondersLeadersState): { score: number } | null {
