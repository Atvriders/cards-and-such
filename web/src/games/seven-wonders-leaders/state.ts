...
export interface SevenWondersLeadersState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersLeadersAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersLeadersSettings): SevenWondersLeadersState {
...
export function score(s: SevenWondersLeadersState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersLeadersState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
