...
export interface AntAttackSettings { dummy: boolean; }
...
export interface AntAttackState {
  rngSeed: number;
  critters: Critter[];
  nextId: number;
...
export type AntAttackAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: AntAttackSettings): AntAttackState {
  return { rngSeed: seed, critters: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: AntAttackState, action: AntAttackAction): AntAttackState {
...
export function isTerminal(state: AntAttackState): { score: number } | null {
