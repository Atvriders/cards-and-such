// Whiz Spades — Spades variant: bid must equal your spade count or zero (nil).
...
export type WhizSpadesPhase = "playing" | "done";

export interface WhizSpadesState {
  rngSeed: number;
  ...
  totalTricks: number;
  winThreshold: number;
  phase: WhizSpadesPhase;
  playerLeads: boolean;
  message: string;
}

export type WhizSpadesAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `whiz-spades-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: WhizSpadesState, trick: readonly { seat: number; card: Card }[]): WhizSpadesState {
...
export function reducer(state: WhizSpadesState, action: WhizSpadesAction): WhizSpadesState {
  ...
  let s: WhizSpadesState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: WhizSpadesState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: WhizSpadesState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): WhizSpadesState {
...
export function isTerminal(state: WhizSpadesState): { score: number } | null {
