...
export interface PalmFrondSettings { dummy: boolean; }
export interface PalmFrondTarget { id: number; lane: number; ticksLeft: number; }
export interface PalmFrondState {
  rngSeed: number;
  targets: PalmFrondTarget[];
  nextId: number;
...
export type PalmFrondAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _settings: PalmFrondSettings): PalmFrondState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: PalmFrondState, action: PalmFrondAction): PalmFrondState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: PalmFrondTarget[] = [];
...
export function isTerminal(state: PalmFrondState): { score: number } | null {
