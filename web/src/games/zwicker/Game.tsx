...
export function ZwickerGame({ state, dispatch, onGameOver }: GameProps<ZwickerState, ZwickerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as ZwickerAction)}>Zwick!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ZwickerAction)}>Next</button>
      </>}
    </div>
  );
}
