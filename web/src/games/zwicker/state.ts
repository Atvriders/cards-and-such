...
export interface ZwickerSettings { dummy: boolean; }
export interface ZwickerState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type ZwickerAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ZwickerSettings): ZwickerState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: ZwickerState, action: ZwickerAction): ZwickerState {
...
export function isTerminal(state: ZwickerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
