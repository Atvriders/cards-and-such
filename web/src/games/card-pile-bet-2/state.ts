...
export interface CardPileBet2Settings { rounds: "8" | "12"; }
export interface CardPileBet2State { deck: number[]; pos: number; topCard: number; coins: number; round: number; maxRounds: number; phase: "betting" | "revealed" | "gameover"; lastResult: "win" | "lose" | "tie" | null; revealedCard: number | null; bid: number; }
export type CardPileBet2Action = { type: "bet"; amount: number; dir: "higher" | "lower" } | { type: "next" };
...
export function initialState(seed: number, settings: CardPileBet2Settings): CardPileBet2State {
  const rng=mulberry32(seed); const deck=makeDeck(rng);
  return {deck,pos:1,topCard:deck[0]!,coins:60,bid:0,round:1,maxRounds:parseInt(settings.rounds,10),phase:"betting",lastResult:null,revealedCard:null};
}
export function reducer(state: CardPileBet2State, action: CardPileBet2Action): CardPileBet2State {
...
export function isTerminal(state: CardPileBet2State): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
