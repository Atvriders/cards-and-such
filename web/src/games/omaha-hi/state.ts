...
export interface OmahaHiSettings { dummy: boolean; }
export interface OmahaHiState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type OmahaHiAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: OmahaHiSettings): OmahaHiState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: OmahaHiState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
