...
import "./Game.css";
export function EverdellWoodlandGame({ state, dispatch, onGameOver }: GameProps<EverdellWoodlandState, EverdellWoodlandSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap fade-in">
      ...
      {state.phase === "drawing" && (
        <button data-testid="hint-target-everdell-woodland-primary" className="cm-btn" onClick={() => dispatch({ type:"draw" } as EverdellWoodlandAction)}>Draw</button>
      )}
      {state.phase === "scored" && (
        <>
          <div className="cm-result">+{state.lastPts} pts</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as EverdellWoodlandAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
