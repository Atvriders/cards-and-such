...
export function KalukiRGame({ state, dispatch, onGameOver }: GameProps<KalukiRState, KalukiRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as KalukiRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as KalukiRAction)}>Next</button>
      </>}
    </div>
  );
}
