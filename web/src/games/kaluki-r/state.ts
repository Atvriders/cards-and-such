...
export interface KalukiRSettings { dummy: boolean; }
export interface KalukiRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type KalukiRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: KalukiRSettings): KalukiRState {
...
export function reducer(state: KalukiRState, action: KalukiRAction): KalukiRState {
...
export function isTerminal(state: KalukiRState): { score: number } | null {
