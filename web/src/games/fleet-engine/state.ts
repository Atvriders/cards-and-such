...
export interface FleetEngineSettings { dummy: boolean; }
export interface FleetEngineState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type FleetEngineAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: FleetEngineSettings): FleetEngineState {
  return {
    rngSeed: seed,
...
export function reducer(state: FleetEngineState, action: FleetEngineAction): FleetEngineState {
...
export function isTerminal(state: FleetEngineState): { score: number } | null {
