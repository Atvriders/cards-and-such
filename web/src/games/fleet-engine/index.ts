...
const FleetEngineGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.FleetEngineGame as unknown as React.ComponentType<unknown> })));
...
export const fleetEnginePlugin: GamePlugin<FleetEngineState, FleetEngineAction, typeof settings> = {
  ...
  hint: (state: any) => {
    ...
    if ((state as any).phase === "rolling") return { selector: '[data-testid="hint-target-fleet-engine-roll"]', pulses: 3 };
    return { selector: '[data-testid="hint-target-fleet-engine-skip"]', pulses: 3 };
  },
