...
export interface WineCellarSettings { dummy: boolean; }
...
export type WineCellarAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: WineCellarSettings): WineCellarState {
...
export function reducer(state: WineCellarState, action: WineCellarAction): WineCellarState {
...
export function score(s: WineCellarState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: WineCellarState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
