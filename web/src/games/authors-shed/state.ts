...
export interface AuthorsShedSettings { dummy: boolean; }
export interface AuthorsShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type AuthorsShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: AuthorsShedSettings): AuthorsShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: AuthorsShedState, action: AuthorsShedAction): AuthorsShedState {
...
export function isTerminal(state: AuthorsShedState): { score: number } | null {
