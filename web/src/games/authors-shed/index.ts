...
const AuthorsShedGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.AuthorsShedGame as unknown as React.ComponentType<unknown> })));
...
export const authorsShedPlugin: GamePlugin<AuthorsShedState, AuthorsShedAction, typeof settings> = {
  ...
  reducer, isTerminal, 
  hint: (state: AuthorsShedState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-authors-shed-play"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-authors-shed-next"]', pulses: 3 };
    return null;
  },
  component: AuthorsShedGame,
};
