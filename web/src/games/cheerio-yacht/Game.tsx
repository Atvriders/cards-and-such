...
import "./Game.css";
export function CheerioYachtGame({ state, dispatch, onGameOver }: GameProps<CheerioYachtState, CheerioYachtSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-cheerio-yacht-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as CheerioYachtAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-cheerio-yacht-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as CheerioYachtAction)}>Next</button>
        </>
      )}
    </div>
  );
