...
export interface PineapplePokerSettings {
  startingBankroll: "500" | "1000" | "5000";
  smallBlind: "5" | "10" | "25";
}

export type PineapplePokerState = PGState<PineapplePokerSettings>;
export type PineapplePokerAction = PGAction;

export const TOTAL_HANDS = 8;

const cfg: PGConfig<PineapplePokerSettings> = {
  ...
  // Standard Pineapple: discard one card before the flop.
  onAfterDeal: (state) => ({ ...state, pendingDiscard: true, message: "Discard 1 of your 3 hole cards." }),
};
...
export function initialState(seed: number, settings: PineapplePokerSettings): PineapplePokerState {
...
export function isTerminal(state: PineapplePokerState): { score: number } | null {
