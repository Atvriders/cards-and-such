...
export interface PineapplePokerSettings { dummy: boolean; }
export interface PineapplePokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type PineapplePokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: PineapplePokerSettings): PineapplePokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: PineapplePokerState, action: PineapplePokerAction): PineapplePokerState {
...
export function isTerminal(state: PineapplePokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
