...
import "./Game.css";
export function DoubleWarShedGame({ state, dispatch, onGameOver }: GameProps<DoubleWarShedState, DoubleWarShedSettings>): JSX.Element {
  const t = isTerminal(state); useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") return <div className="dm-wrap dw-shed"><h3>Double War</h3><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap dw-shed">
      <h3>Double War</h3>
