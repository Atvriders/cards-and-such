...
export interface DoubleWarShedSettings { dummy: boolean; }
export interface DoubleWarShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type DoubleWarShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: DoubleWarShedSettings): DoubleWarShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: DoubleWarShedState, action: DoubleWarShedAction): DoubleWarShedState {
...
export function isTerminal(state: DoubleWarShedState): { score: number } | null {
