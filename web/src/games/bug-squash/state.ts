...
// Bug Squash: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface BugSquashSettings { dummy: boolean; }

export interface BugSquashTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}
...
export type BugSquashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BugSquashSettings): BugSquashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BugSquashState, action: BugSquashAction): BugSquashState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: BugSquashTarget[] = [];
...
export function isTerminal(state: BugSquashState): { score: number } | null {
