...
const PoolCheckersAmGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.PoolCheckersAmGame as unknown as React.ComponentType<unknown> })));
...
export const poolCheckersAmPlugin: GamePlugin<PoolCheckersAmState, PoolCheckersAmAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: (state: PoolCheckersAmState): HintTarget | null => ((state.phase === "playing" && state.turn === "P") ? { selector: ".ab-cell:not(.p):not(.c)", pulses: 3 } : null), component: PoolCheckersAmGame,
};
