...
export interface PoolCheckersAmSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;
export interface PoolCheckersAmState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type PoolCheckersAmAction = { type: "place"; idx: number };
export function initialState(seed: number, _settings: PoolCheckersAmSettings): PoolCheckersAmState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}
export function reducer(state: PoolCheckersAmState, action: PoolCheckersAmAction): PoolCheckersAmState {
...
export function isTerminal(state: PoolCheckersAmState): { score: number } | null {
