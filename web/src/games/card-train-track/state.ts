...
export interface CardTrainTrackSettings { dummy: boolean; }

export interface CardTrainTrackState {
  rngSeed: number;
  round: number;
  card: number | null;
  score: number;
...
export type CardTrainTrackAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardTrainTrackSettings): CardTrainTrackState {
  return { rngSeed: seed, round: 1, card: null, score: 0, lastPts: 0, phase: "draw" };
}

export function reducer(state: CardTrainTrackState, action: CardTrainTrackAction): CardTrainTrackState {
...
export function isTerminal(state: CardTrainTrackState): { score: number } | null {
