...
export interface BeggarMyNeighbourSettings { dummy: boolean; }
export interface BeggarMyNeighbourState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type BeggarMyNeighbourAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: BeggarMyNeighbourSettings): BeggarMyNeighbourState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: BeggarMyNeighbourState, action: BeggarMyNeighbourAction): BeggarMyNeighbourState {
...
export function isTerminal(state: BeggarMyNeighbourState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
