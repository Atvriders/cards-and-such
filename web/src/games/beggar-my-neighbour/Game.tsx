...
import "./Game.css";
export function BeggarMyNeighbourGame({ state, dispatch, onGameOver }: GameProps<BeggarMyNeighbourState, BeggarMyNeighbourSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") {
    return <div className="dm-wrap bmn-shed"><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses} T: {state.ties}</div><div className="dm-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="dm-wrap bmn-shed">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-beggar-my-neighbour-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as BeggarMyNeighbourAction)}>Flip!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as BeggarMyNeighbourAction)}>Next</button>
      </>}
    </div>
  );
}
