...
import "./Game.css";
export function CribbageMiniGame({ state, dispatch, onGameOver }: GameProps<CribbageMiniState, CribbageMiniSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as CribbageMiniAction)}>Peg!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as CribbageMiniAction)}>Next</button>
      </>}
    </div>
  );
}
