...
export interface IndianRSettings { dummy: boolean; }
export interface IndianRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type IndianRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: IndianRSettings): IndianRState {
...
export function reducer(state: IndianRState, action: IndianRAction): IndianRState {
...
export function isTerminal(state: IndianRState): { score: number } | null {
