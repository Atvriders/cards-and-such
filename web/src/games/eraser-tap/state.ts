...
export interface EraserTapSettings { dummy: boolean; }
...
export interface EraserTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type EraserTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: EraserTapSettings): EraserTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: EraserTapState, action: EraserTapAction): EraserTapState {
...
export function isTerminal(state: EraserTapState): { score: number } | null {
