...
export interface ZooVadisRwSettings { dummy: boolean; }
export interface ZooVadisRwState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type ZooVadisRwAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: ZooVadisRwSettings): ZooVadisRwState {
  return {
    rngSeed: seed,
...
export function reducer(state: ZooVadisRwState, action: ZooVadisRwAction): ZooVadisRwState {
...
export function isTerminal(state: ZooVadisRwState): { score: number } | null {
