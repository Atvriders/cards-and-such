...
export interface BaseballPokerSettings { dummy: boolean; }
export interface BaseballPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type BaseballPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: BaseballPokerSettings): BaseballPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: BaseballPokerState, action: BaseballPokerAction): BaseballPokerState {
...
export function isTerminal(state: BaseballPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
