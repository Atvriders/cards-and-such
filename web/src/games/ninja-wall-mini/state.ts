...
// Ninja Wall Mini: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface NinjaWallMiniSettings { dummy: boolean; }

export interface NinjaWallMiniTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface NinjaWallMiniState {
  rngSeed: number;
  targets: NinjaWallMiniTarget[];
  nextId: number;
...
export type NinjaWallMiniAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: NinjaWallMiniSettings): NinjaWallMiniState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: NinjaWallMiniState, action: NinjaWallMiniAction): NinjaWallMiniState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: NinjaWallMiniTarget[] = [];
...
export function isTerminal(state: NinjaWallMiniState): { score: number } | null {
