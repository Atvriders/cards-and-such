...
export function WonderfulWorldDraftGame({ state, dispatch, onGameOver }: GameProps<WonderfulWorldDraftState, WonderfulWorldDraftSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">It's a Wonderful World</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as WonderfulWorldDraftAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as WonderfulWorldDraftAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
