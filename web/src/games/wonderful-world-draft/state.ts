...
export interface WonderfulWorldDraftState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WonderfulWorldDraftAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WonderfulWorldDraftSettings): WonderfulWorldDraftState {
...
export function score(s: WonderfulWorldDraftState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WonderfulWorldDraftState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
