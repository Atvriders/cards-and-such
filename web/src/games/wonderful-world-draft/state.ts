...
export const SUIT_NAMES = ["Material","Energy","Science","Gold","Exploration"] as const;
...
export type WonderfulWorldDraftState = DraftState;
export type WonderfulWorldDraftAction = DraftAction;
export interface WonderfulWorldDraftSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: WonderfulWorldDraftSettings): WonderfulWorldDraftState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: WonderfulWorldDraftState, action: WonderfulWorldDraftAction): WonderfulWorldDraftState {
  return draftReducer(state, action);
}

export function score(state: WonderfulWorldDraftState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: WonderfulWorldDraftState): { score: number } | null {
