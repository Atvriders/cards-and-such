...
export interface GlenwoodPatienceSettings { dummy: boolean; }

export interface GlenwoodPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type GlenwoodPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const mids = h.filter(c=>cardRank(c)>=5 && cardRank(c)<=9).length; s += mids*5; const suits = new Set(h.map(cardSuit)); if (suits.size===1) s += 10; if (s===0) s=1; return s;
}
...
export function reducer(state: GlenwoodPatienceState, action: GlenwoodPatienceAction): GlenwoodPatienceState {
...
export function isTerminal(state: GlenwoodPatienceState): { score: number } | null {
