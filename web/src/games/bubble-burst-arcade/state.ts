...
export interface BubbleBurstArcadeSettings { dummy: boolean; }
...
export interface BubbleBurstArcadeState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type BubbleBurstArcadeAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BubbleBurstArcadeSettings): BubbleBurstArcadeState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BubbleBurstArcadeState, action: BubbleBurstArcadeAction): BubbleBurstArcadeState {
...
export function isTerminal(state: BubbleBurstArcadeState): { score: number } | null {
