...
export const MAX_CLICKS = 22;

export interface MiniBakerDozenSettings { dummy: boolean; }

export interface MiniBakerDozenState {
  rngSeed: number;
...
export type MiniBakerDozenAction = { type: "remove"; index: number } | { type: "noop" };
...
export function initialState(seed: number, _settings: MiniBakerDozenSettings): MiniBakerDozenState {
...
export function reducer(state: MiniBakerDozenState, action: MiniBakerDozenAction): MiniBakerDozenState {
...
export function isTerminal(state: MiniBakerDozenState): { score: number } | null {
