...
export function MiniBakerDozenGame({ state, dispatch, onGameOver }: GameProps<MiniBakerDozenState, MiniBakerDozenSettings>): JSX.Element {
    ...
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.layout.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "remove", index: i } as MiniBakerDozenAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
