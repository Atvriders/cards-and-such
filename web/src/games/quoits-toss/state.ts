...
export interface QuoitsTossSettings { dummy: boolean; }

export interface QuoitsTossState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type QuoitsTossAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: QuoitsTossSettings): QuoitsTossState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: QuoitsTossState, action: QuoitsTossAction): QuoitsTossState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<QuoitsTossState> = {};
...
export function isTerminal(state: QuoitsTossState): { score: number } | null {
