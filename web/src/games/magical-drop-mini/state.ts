...
// Magical Drop Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface MagicalDropMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface MagicalDropMiniState {
  rngSeed: number;
  grid: Grid;
...
export type MagicalDropMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: MagicalDropMiniSettings): MagicalDropMiniState {
...
export function reducer(state: MagicalDropMiniState, action: MagicalDropMiniAction): MagicalDropMiniState {
...
export function isTerminal(state: MagicalDropMiniState): { score: number } | null {
