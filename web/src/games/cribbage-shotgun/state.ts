...
export interface CribbageShotgunSettings { dummy: boolean; }

export interface CribbageShotgunState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type CribbageShotgunAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: CribbageShotgunSettings): CribbageShotgunState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: CribbageShotgunState, action: CribbageShotgunAction): CribbageShotgunState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<CribbageShotgunState> = {};
    ...
    const earlyWin = ((extra.myPeg !== undefined && extra.myPeg >= 61) || (extra.cpuPeg !== undefined && extra.cpuPeg >= 61));
...
export function isTerminal(state: CribbageShotgunState): { score: number } | null {
