    ...
    <div className="embch-wrap">
      ...
      <div className="embch-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button data-testid={`hint-target-embassy-chess-answer-${i}`} key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as EmbassyChessAction)}><span className="embch-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
