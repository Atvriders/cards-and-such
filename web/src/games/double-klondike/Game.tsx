...
import { doubleKlondikeRuleset } from "./state.js";
...
export function DoubleKlondikeGame({
  state,
  dispatch,
  onGameOver,
}: GameProps<DoubleKlondikeState, DoubleKlondikeSettings>): JSX.Element {
  const { onDragStart, onDragOver, onDrop } = useDragDrop();

  const handleMove = useCallback(
    (from: string, to: string, count: number) => {
      dispatch({ type: "move", fromPile: from, toPile: to, count } as DoubleKlondikeAction);
    },
    [dispatch],
  );
  ...
  const handleCardClick = useCallback(
    (pileId: string, indexFromTop: number) => {
      ...
      const target = findAutoMove(state.piles, pileId, count, doubleKlondikeRuleset);
      if (target) dispatch({ type: "move", fromPile: pileId, toPile: target, count } as DoubleKlondikeAction);
    },
  ...
  const handleStockClick = useCallback(() => {
    ...
    if (stock && stock.cards.length === 0 && waste && waste.cards.length > 0) {
      dispatch({ type: "recycle" } as DoubleKlondikeAction);
    } else {
      dispatch({ type: "draw" } as DoubleKlondikeAction);
    }
  }, [state.piles, dispatch]);
  ...
  return (
    <div className="dk">
      <div className="dk-info">
        ...
        <button className="dk-auto-btn" type="button" onClick={() => dispatch({ type: "auto-move-to-foundation" } as DoubleKlondikeAction)}>Auto-move</button>
      </div>
      <div className="dk-top-row">
        <div className="pile-wrapper dk-stock-wrapper" onClick={handleStockClick}>
