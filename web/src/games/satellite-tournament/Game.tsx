...
import "./Game.css";
export function SatelliteTournamentGame({ state, dispatch, onGameOver }: GameProps<SatelliteTournamentState, SatelliteTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-satellite-tournament-deal" onClick={() => dispatch({ type: "deal" } as SatelliteTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-satellite-tournament-next" onClick={() => dispatch({ type: "next" } as SatelliteTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
