...
export interface ConquianRSettings { dummy: boolean; }
export interface ConquianRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type ConquianRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: ConquianRSettings): ConquianRState {
...
export function reducer(state: ConquianRState, action: ConquianRAction): ConquianRState {
...
export function isTerminal(state: ConquianRState): { score: number } | null {
