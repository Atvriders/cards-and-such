...
Playing against bots: Bot A sits to your left, Bot B to your right. Click Roll Dice on your turn, then Next Turn to advance. Bots resolve automatically. Good luck — everything is in the dice!`,
  ...
  hint: (state) => {
    ...
    if (phase === "rolling") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "rolling-dice") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "preRoll") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "ready") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "playerRoll") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "roll") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "play") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "playing") return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
    if (phase === "rolled") return { selector: '[data-testid="hint-target-lcr-confirm"]', pulses: 3 };
    if (phase === "picking") return { selector: '[data-testid="hint-target-lcr-confirm"]', pulses: 3 };
    if (phase === "selecting") return { selector: '[data-testid="hint-target-lcr-confirm"]', pulses: 3 };
    if (phase === "choosing") return { selector: '[data-testid="hint-target-lcr-confirm"]', pulses: 3 };
    return { selector: '[data-testid="hint-target-lcr-roll"]', pulses: 3 };
  },
  component: LCRGame,
};
