...
export interface WillOTheWispSettings { dummy: boolean; }

export interface WillOTheWispState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type WillOTheWispAction =
  | { type: "keep" }
...
export function reducer(state: WillOTheWispState, action: WillOTheWispAction): WillOTheWispState {
...
export function isTerminal(state: WillOTheWispState): { score: number } | null {
