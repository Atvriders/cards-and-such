...
export interface TrefoilSettings { dummy: boolean; }

export interface TrefoilState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
function scoreHand(h: number[]): number {
  let s = 0; const aces = h.filter(c=>cardRank(c)===1).length; s += aces*12; const lowRanks = h.filter(c=>cardRank(c)>=2 && cardRank(c)<=4).length; s += lowRanks*3; if (s===0) s=1; return s;
}
...
export function reducer(state: TrefoilState, action: TrefoilAction): TrefoilState {
...
export function isTerminal(state: TrefoilState): { score: number } | null {
