...
export interface PuppyTapSettings { dummy: boolean; }
export interface PuppyTapTarget { id: number; lane: number; ticksLeft: number; }
export interface PuppyTapState {
  rngSeed: number;
  targets: PuppyTapTarget[];
  nextId: number;
...
export type PuppyTapAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: PuppyTapSettings): PuppyTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: PuppyTapState, action: PuppyTapAction): PuppyTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: PuppyTapTarget[] = [];
...
export function isTerminal(state: PuppyTapState): { score: number } | null {
