...
export function BetweenTwoCastlesLudwigGame({ state, dispatch, onGameOver }: GameProps<BetweenTwoCastlesLudwigState, BetweenTwoCastlesLudwigSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Between Two Castles: Ludwig</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as BetweenTwoCastlesLudwigAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as BetweenTwoCastlesLudwigAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
