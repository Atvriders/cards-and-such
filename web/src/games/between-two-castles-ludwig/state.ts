...
export const SUIT_NAMES: readonly string[] = ["Bedchamber","Hall","Garden","Tower"];
...
export interface BetweenTwoCastlesLudwigState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type BetweenTwoCastlesLudwigAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: BetweenTwoCastlesLudwigSettings): BetweenTwoCastlesLudwigState {
...
export function score(s: BetweenTwoCastlesLudwigState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: BetweenTwoCastlesLudwigState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
