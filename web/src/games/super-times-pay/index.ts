...
const SuperTimesPay = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SuperTimesPay as unknown as React.ComponentType<unknown> })));
const settings = {
...
export const superTimesPayPlugin: GamePlugin<SuperTimesPayState, SuperTimesPayAction, typeof settings> = {
  ...
  reducer, isTerminal,
  hint: (state: any) => {
    if (state.phase === "ready") return { selector: '[data-testid="hint-target-super-times-pay-deal"]', pulses: 3 };
    if (state.phase === "dealt") return { selector: '[data-testid="hint-target-super-times-pay-next"]', pulses: 3 };
    return null;
  }, component: SuperTimesPay,
};
