...
export interface SuperTimesPaySettings { rounds: "10"; }

export interface SuperTimesPayState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type SuperTimesPayAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SuperTimesPaySettings): SuperTimesPayState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: SuperTimesPayState, action: SuperTimesPayAction): SuperTimesPayState {
...
export function isTerminal(state: SuperTimesPayState): { score: number } | null {
