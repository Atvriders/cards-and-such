...
export interface FoxInTheForestSettings { dummy: boolean; }
export interface FoxInTheForestState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type FoxInTheForestAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: FoxInTheForestSettings): FoxInTheForestState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: FoxInTheForestState, action: FoxInTheForestAction): FoxInTheForestState {
...
export function isTerminal(state: FoxInTheForestState): { score: number } | null {
