...
export const MATCH_POINTS = 30;
export interface CardPairQuestSettings { dummy: boolean; }
export interface CardPairQuestState { rngSeed: number; drawIdx: number; rankCounts: number[]; matches: number; score: number; phase: "playing" | "done"; lastCard: number | null; lastMatched: boolean; }
export type CardPairQuestAction = { type: "draw" };
...
export function initialState(seed: number, _s: CardPairQuestSettings): CardPairQuestState {
...
export function reducer(state: CardPairQuestState, action: CardPairQuestAction): CardPairQuestState {
...
export function isTerminal(s: CardPairQuestState): { score: number } | null { return s.phase==="done" ? { score: s.score } : null; }
