...
export interface DiceBadmintonSettings { dummy: boolean; }

export interface DiceBadmintonState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  myPoints: number;
...
export type DiceBadmintonAction = { type: "roll" } | { type: "next" };

function evalRoll(dice: number[]): number {
  if (dice.includes(6) && dice.includes(5)) return 1; if (dice.every(d=>d<=3)) return -1; const sum = dice.reduce((a,b)=>a+b,0); if (sum>=14) return 1; if (sum<=7) return -1; return 0;
}

export function initialState(seed: number, _settings: DiceBadmintonSettings): DiceBadmintonState {
  return { rngSeed: seed, round: 1, dice: null, myPoints: 0, oppPoints: 0, lastDelta: 0, phase: "rolling" };
}

export function reducer(state: DiceBadmintonState, action: DiceBadmintonAction): DiceBadmintonState {
...
export function isTerminal(state: DiceBadmintonState): { score: number } | null {
