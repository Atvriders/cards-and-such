...
const GermanWhistGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.GermanWhistGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;

export const germanWhistPlugin: GamePlugin<GermanWhistState, GermanWhistAction, typeof settings> = {
  id: "german-whist",
  title: "German Whist",
  category: "cards",
  players: { min: 1, max: 1, multiplayer: false },
  description: "German Whist — heads-up, draw from stock.",
  howToPlay: "German Whist — heads-up, draw from stock. Play heads-up against the CPU. Click cards in your hand to play. Follow the led suit if possible. Highest of led suit wins, unless beaten by trump. Score points for tricks won (or for card values, in some variants).",
  settings,
  initialState: (seed: number, _s: S) => initialState(seed, _s as GermanWhistSettings),
  reducer,
  isTerminal,
  hint: (state: GermanWhistState): HintTarget | null => {
    ...
    if (trick.length === 0) {
      ...
      return { selector: `[data-testid="hint-target-gerwh-${pick.id}"]`, pulses: 3 };
    }
