...
export function SparkTapGame({ state, dispatch, onGameOver }: GameProps<SparkTapState, SparkTapSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => {
    if (state.phase !== "playing") { if (tickRef.current) clearInterval(tickRef.current); return; }
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as SparkTapAction), 750);
        ...
        {state.targets.map(p => {
          ...
          return (
            <button key={p.id}
              ...
              onClick={() => dispatch({ type:"pop", id:p.id } as SparkTapAction)}
