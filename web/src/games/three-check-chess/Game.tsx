...
export function ThreeCheckChessGame({ state, dispatch, onGameOver }: GameProps<ThreeCheckChessState, ThreeCheckChessSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="tcch-wrap"><div className="tcch-done bounce-in"><h2>Done!</h2><p>Correct: {state.correctCount} / {state.questions.length}</p><p style={{ fontSize:"1.8rem",fontWeight:900,color:"#27ae60" }}>{state.score} pts</p></div></div>;
  const q = state.questions[state.currentIndex]!;
  const isResult = state.phase === "result";
  const urgent = state.timeLeft <= 5 && !state.submitted;
  return (
    <div className="tcch-wrap fade-in">
      <div className="tcch-header">
        ...
        <span className="tcch-score pulse">{state.score} pts</span>
      </div>
      ...
      <div className="tcch-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button data-testid={`hint-target-three-check-chess-answer-${i}`} key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type:"select", choice:i } as ThreeCheckChessAction)}><span className="tcch-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
