...
export interface ThreeCardGutsSettings { rounds: "10"; }

export interface ThreeCardGutsState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type ThreeCardGutsAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ThreeCardGutsSettings): ThreeCardGutsState {
  return { rngSeed: seed >>> 0, score: 0, round: 1, maxRounds: 10, hand: [], lastGain: 0, phase: "ready" };
}

export function reducer(state: ThreeCardGutsState, action: ThreeCardGutsAction): ThreeCardGutsState {
...
export function isTerminal(state: ThreeCardGutsState): { score: number } | null {
