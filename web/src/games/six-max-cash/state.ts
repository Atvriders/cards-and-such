...
export interface SixMaxCashSettings { dummy: boolean; }
export interface SixMaxCashState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SixMaxCashAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SixMaxCashSettings): SixMaxCashState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SixMaxCashState, action: SixMaxCashAction): SixMaxCashState {
...
export function isTerminal(state: SixMaxCashState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
