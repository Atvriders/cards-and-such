...
import "./Game.css";
export function SixMaxCashGame({ state, dispatch, onGameOver }: GameProps<SixMaxCashState, SixMaxCashSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-six-max-cash-deal" onClick={() => dispatch({ type: "deal" } as SixMaxCashAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-six-max-cash-next" onClick={() => dispatch({ type: "next" } as SixMaxCashAction)}>Next</button>
      </>}
    </div>
  );
}
