...
export function BlinkMatchGame({ state, dispatch, onGameOver }: GameProps<BlinkMatchState, BlinkMatchSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="blnkmtc-wrap"><div className="blnkmtc-done bounce-in"><h2>Done!</h2><div>Correct: {state.correctCount} / {state.rounds.length}</div><div className="blnkmtc-final">{state.score} pts</div></div></div>;
  }
  const r = state.rounds[state.currentIndex]!;
  return (
    <div className="blnkmtc-wrap fade-in">
      <div className="blnkmtc-info">Round {state.currentIndex + 1} / {state.rounds.length}</div>
      <div className="blnkmtc-score pulse">{state.score} pts</div>
