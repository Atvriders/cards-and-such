...
export interface LimeTapSettings { dummy: boolean; }
...
export type LimeTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: LimeTapSettings): LimeTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LimeTapState, action: LimeTapAction): LimeTapState {
...
export function isTerminal(state: LimeTapState): { score: number } | null {
