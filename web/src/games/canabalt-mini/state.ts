...
// Canabalt Mini: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface CanabaltMiniSettings { dummy: boolean; }

export interface CanabaltMiniTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface CanabaltMiniState {
  rngSeed: number;
  targets: CanabaltMiniTarget[];
  nextId: number;
...
export type CanabaltMiniAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: CanabaltMiniSettings): CanabaltMiniState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: CanabaltMiniState, action: CanabaltMiniAction): CanabaltMiniState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CanabaltMiniTarget[] = [];
...
export function isTerminal(state: CanabaltMiniState): { score: number } | null {
