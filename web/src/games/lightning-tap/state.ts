...
export interface LightningTapSettings { dummy: boolean; }
...
export interface LightningTapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type LightningTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: LightningTapSettings): LightningTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LightningTapState, action: LightningTapAction): LightningTapState {
...
export function isTerminal(state: LightningTapState): { score: number } | null {
