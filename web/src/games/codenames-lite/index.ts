...
const CodenamesLiteGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CodenamesLiteGame as unknown as React.ComponentType<unknown> })));
...
Tips: the spymaster's clue word connects multiple tiles thematically. Think about what the hidden agents have in common. When in doubt, avoid tiles that seem risky and end your guessing early to preserve your chance in future turns.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-codenames-lite-action"]', pulses: 3 }; },
