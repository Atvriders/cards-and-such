...
export interface ViticultureWineSettings { dummy: boolean; }
export interface ViticultureWineState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type ViticultureWineAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: ViticultureWineSettings): ViticultureWineState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Visitor comes","Vines flourish","Harvest good","Wine ages","Tour bus arrives","Frost chills","Mama and Papa","Field replanted"];

export function reducer(state: ViticultureWineState, action: ViticultureWineAction): ViticultureWineState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Vine (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Visitor (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Sold 1 Vine (+$" + market + ")"; delta = market;
      } else { evt = "No Vine to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
    ...
    if (pay > 0) evt += " | Visitor earned +$" + pay;
...
export function score(s: ViticultureWineState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: ViticultureWineState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
