...
export interface GrizzledCoopSettings { dummy: boolean; }
export interface GrizzledCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type GrizzledCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: GrizzledCoopSettings): GrizzledCoopState {
