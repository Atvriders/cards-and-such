...
export interface RistiklappiShedSettings { dummy: boolean; }
export interface RistiklappiShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type RistiklappiShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: RistiklappiShedSettings): RistiklappiShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: RistiklappiShedState, action: RistiklappiShedAction): RistiklappiShedState {
...
export function isTerminal(state: RistiklappiShedState): { score: number } | null {
