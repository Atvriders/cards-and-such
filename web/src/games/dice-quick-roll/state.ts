...
export interface DiceQuickRollSettings { rounds: "8" | "12"; }
export interface DiceQuickRollState {
...
export type DiceQuickRollAction = { type: "roll"; amount: number } | { type: "next" };
...
function checkWin(dice: number[]): { win: boolean; label: string } {
  ...
  const win=sum>=12;
  return {win,label:win?`Sum ${sum} — Win!`:`Sum ${sum} — Need 12+`};
}

export function initialState(seed: number, settings: DiceQuickRollSettings): DiceQuickRollState {
...
export function reducer(state: DiceQuickRollState, action: DiceQuickRollAction): DiceQuickRollState {
...
export function isTerminal(state: DiceQuickRollState): { score: number } | null { return state.phase==="gameover"?{score:state.coins}:null; }
