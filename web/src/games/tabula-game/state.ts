// Auto-generated backgammon-family state for tabula-game.
...
export interface TabulaSettings { dummy: boolean }

export interface TabulaState {
  rngSeed: number;
...
export function initialState(seed: number, _s: TabulaSettings): TabulaState {
...
export function legalMoves(state: TabulaState, side: Side): { from: number; pips: number }[] {
...
function applyMove(state: TabulaState, side: Side, from: number, pips: number): TabulaState | null {
  const next: TabulaState = {
    ...state,
...
function checkWin(s: TabulaState): TabulaState {
...
function cpuTurn(state: TabulaState): TabulaState {
  let s = state;
...
export function reducer(state: TabulaState, action: TabulaAction): TabulaState {
  if (state.phase === "done") return state;

  if (action.type === "roll" && state.turn === "P" && state.phase === "rolling") {
    const diceCount = THREE_DICE ? 3 : 2;
    const r = rollDiceMulti(state.rngSeed, diceCount);
    const next: TabulaState = { ...state, rngSeed: r.nextSeed, dice: r.d, diceLeft: expandDice(r.d), phase: "moving" };
...
export function isTerminal(state: TabulaState): { score: number } | null {
