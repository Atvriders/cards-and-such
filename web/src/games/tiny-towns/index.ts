...
export const tinyTownsPlugin: GamePlugin<TinyTownsState, TinyTownsAction, typeof settings> = {
  ...
  description: "Personal grid; place resources per shared column call, build structures.",
  howToPlay: `Tiny Towns is a single-player tile-placement puzzle. Personal grid; place resources per shared column call, build structures. Tiles draw from a randomized queue, and your job is to place each one onto a small grid in a way that maximises end-of-game scoring.

How it works: 16 resources on a 4x4 grid. Each row containing all 5 different types pays +10; each unique-type column pays +5; each adjacency pays +1.
