...
export interface TriplePlayDrawSettings { dummy: boolean; }
export interface TriplePlayDrawState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type TriplePlayDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: TriplePlayDrawSettings): TriplePlayDrawState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: TriplePlayDrawState, action: TriplePlayDrawAction): TriplePlayDrawState {
...
export function isTerminal(state: TriplePlayDrawState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
