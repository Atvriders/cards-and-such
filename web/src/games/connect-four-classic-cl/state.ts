...
export function reducer(state: ConnectState, action: ConnectAction): ConnectState {
  if (action.type === "reset") {
    ...
    return initialState(seed, { dummy: false });
  }
  if (state.phase === "done" || state.result) return state;
  if (state.turn !== "P") return state;

  let col: number;
  if (action.type === "drop") col = action.col;
  else if (action.type === "place") col = action.col;
  else return state;

  if (col < 0 || col >= COLS) return state;
  const r = topRow(state.board, col);
  if (r < 0) return state;
  ...
  if (term.who === "P") {
    return { ...state, board: nb, result: "P", score: state.score + 100 + pieces, phase: "done", pieces, lastDrop: { row: r, col, who: "P" }, winLine: term.line, moveCount };
  }
  if (term.who === "draw") {
    return { ...state, board: nb, result: "draw", score: state.score + 25 + pieces, phase: "done", pieces, lastDrop: { row: r, col, who: "P" }, winLine: null, moveCount };
  }
  ...
  if (term.who === "C") {
    return { ...state, rngSeed: seed2, board: nb, result: "C", score: state.score + pieces, phase: "done", pieces, lastDrop: cpuDrop ?? state.lastDrop, winLine: term.line, moveCount };
  }
  if (term.who === "draw") {
    return { ...state, rngSeed: seed2, board: nb, result: "draw", score: state.score + 25 + pieces, phase: "done", pieces, lastDrop: cpuDrop ?? state.lastDrop, winLine: null, moveCount };
  }

  return { ...state, rngSeed: seed2, board: nb, turn: "P", pieces, lastDrop: cpuDrop ?? state.lastDrop, moveCount };
}
