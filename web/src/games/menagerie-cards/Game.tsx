...
import "./Game.css";
export function MenagerieCardsGame({ state, dispatch, onGameOver }: GameProps<MenagerieCardsState, MenagerieCardsSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-menagerie-cards-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as MenagerieCardsAction)}>Shout!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MenagerieCardsAction)}>Next</button>
      </>}
    </div>
  );
}
