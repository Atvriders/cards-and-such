...
export interface MenagerieCardsSettings { dummy: boolean; }
export interface MenagerieCardsState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type MenagerieCardsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MenagerieCardsSettings): MenagerieCardsState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: MenagerieCardsState, action: MenagerieCardsAction): MenagerieCardsState {
...
export function isTerminal(state: MenagerieCardsState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
