...
export const DECK: { name: string; value: number }[] = [
  ...
  { name: "Silver", value: 3 },
  { name: "Gold", value: 4 },
  { name: "Platinum", value: 6 },
  { name: "Province", value: 5 },
];

export interface DominionProsperitySettings { dummy: boolean; }
export interface DominionProsperityState {
  rngSeed: number;
  round: number;
  hand: number[];
  lastPts: number;
...
export type DominionProsperityAction = { type: "draw" } | { type: "next" };
export function initialState(seed: number, _s: DominionProsperitySettings): DominionProsperityState {
