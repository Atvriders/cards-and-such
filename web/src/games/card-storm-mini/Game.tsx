...
import "./Game.css";
export function CardStormMiniGame({ state, dispatch, onGameOver }: GameProps<CardStormMiniState, CardStormMiniSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <div className="cm-help">More unique ranks = more points</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardStormMiniAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
