...
describe("Mahjong Ishido (Path of Stones)", () => {
  ...
  it("clicking a pair of matching free tiles removes both", () => {
    ...
    if (!a || !b) {
      const s2 = reducer(s, { type: "select", id: free[0]!.id });
      expect(s2.selectedId).toBe(free[0]!.id);
      return;
    }
  ...
  it("clicking a non-matching pair does not remove either", () => {
