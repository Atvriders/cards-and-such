...
export function SushiGoConveyorGame({ state, dispatch, onGameOver }: GameProps<SushiGoConveyorState, SushiGoConveyorSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Sushi Go Conveyor</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as SushiGoConveyorAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as SushiGoConveyorAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
