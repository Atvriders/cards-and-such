...
export const SUIT_NAMES = ["Maki","Nigiri","Pudding","Wasabi"] as const;
...
export type SushiGoConveyorState = DraftState;
export type SushiGoConveyorAction = DraftAction;
export interface SushiGoConveyorSettings { dummy: boolean }
export type { DraftCard };

export function suitName(s: number): string { return SUIT_NAMES[s] ?? "?"; }
export function rankName(r: number): string { return String(r); }

export function initialState(seed: number, _settings: SushiGoConveyorSettings): SushiGoConveyorState {
  // ensure deterministic re-init across re-mounts using mulberry32 seed
  void mulberry32;
  return initialDraftState(seed, CONFIG);
}

export function reducer(state: SushiGoConveyorState, action: SushiGoConveyorAction): SushiGoConveyorState {
  return draftReducer(state, action);
}

export function score(state: SushiGoConveyorState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: SushiGoConveyorState): { score: number } | null {
