...
export interface SushiGoConveyorState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SushiGoConveyorAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SushiGoConveyorSettings): SushiGoConveyorState {
...
export function score(s: SushiGoConveyorState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SushiGoConveyorState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
