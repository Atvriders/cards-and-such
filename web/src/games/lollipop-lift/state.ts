...
export interface LollipopLiftSettings { dummy: boolean; }
...
export interface LollipopLiftState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type LollipopLiftAction =
  | { type: "tick" }
  | { type: "catch"; id: number };

export function initialState(seed: number, _settings: LollipopLiftSettings): LollipopLiftState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, caught: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LollipopLiftState, action: LollipopLiftAction): LollipopLiftState {
...
export function isTerminal(state: LollipopLiftState): { score: number } | null {
