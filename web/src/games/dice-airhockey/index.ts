...
const DiceAirhockeyGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceAirhockeyGame as unknown as React.ComponentType<unknown> })));
...
export const diceAirhockeyPlugin: GamePlugin<DiceAirhockeyState, DiceAirhockeyAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceAirhockeyState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-airhockey-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-airhockey-next"]', pulses: 3 };
    return null;
  },
