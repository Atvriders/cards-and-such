    ...
    <div className="quiz-wrap">
      ...
      <div className="quiz-choices">
        {q.choices.map((choice, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={isResult} onClick={() => dispatch({ type: "select", choice: i } as CryptidMiniAction)}>
      ...
      <div className="quiz-actions">
        {!isResult && (
          <button className="quiz-btn submit" disabled={state.selected === null} onClick={() => dispatch({ type: "submit" } as CryptidMiniAction)}>Submit</button>
        )}
        {isResult && (
          <button className="quiz-btn next" onClick={() => dispatch({ type: "next" } as CryptidMiniAction)}>
