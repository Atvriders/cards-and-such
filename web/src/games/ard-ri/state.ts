...
export interface ArdRiSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface ArdRiState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type ArdRiAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: ArdRiSettings): ArdRiState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: ArdRiState, action: ArdRiAction): ArdRiState {
...
export function isTerminal(state: ArdRiState): { score: number } | null {
