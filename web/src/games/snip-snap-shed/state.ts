...
export interface SnipSnapShedSettings { dummy: boolean; }
export interface SnipSnapShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SnipSnapShedAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: SnipSnapShedSettings): SnipSnapShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SnipSnapShedState, action: SnipSnapShedAction): SnipSnapShedState {
...
export function isTerminal(state: SnipSnapShedState): { score: number } | null {
