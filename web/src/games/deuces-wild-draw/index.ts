...
const DeucesWildDrawGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DeucesWildDrawGame as unknown as React.ComponentType<unknown> })));
...
export const deucesWildDrawPlugin: GamePlugin<DeucesWildDrawState, DeucesWildDrawAction, typeof settings> = {
  ...
  reducer, isTerminal,   hint: (state: DeucesWildDrawState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-deuces-wild-draw-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-deuces-wild-draw-next"]', pulses: 3 };
    return null;
  },
  component: DeucesWildDrawGame,
};
