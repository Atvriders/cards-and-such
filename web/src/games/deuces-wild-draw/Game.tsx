...
export function DeucesWildDrawGame({ state, dispatch, onGameOver }: GameProps<DeucesWildDrawState, DeucesWildDrawSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as DeucesWildDrawAction)}>Deal</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DeucesWildDrawAction)}>Next</button>
      </>}
    </div>
  );
}
