...
export interface SkittlesLongAlleySettings { dummy: boolean; }

export interface SkittlesLongAlleyState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type SkittlesLongAlleyAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: SkittlesLongAlleySettings): SkittlesLongAlleyState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: SkittlesLongAlleyState, action: SkittlesLongAlleyAction): SkittlesLongAlleyState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<SkittlesLongAlleyState> = {};
...
export function isTerminal(state: SkittlesLongAlleyState): { score: number } | null {
