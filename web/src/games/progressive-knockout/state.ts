...
export interface ProgressiveKnockoutSettings { dummy: boolean; }
export interface ProgressiveKnockoutState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type ProgressiveKnockoutAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: ProgressiveKnockoutSettings): ProgressiveKnockoutState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: ProgressiveKnockoutState, action: ProgressiveKnockoutAction): ProgressiveKnockoutState {
...
export function isTerminal(state: ProgressiveKnockoutState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
