...
export function ProgressiveKnockoutGame({ state, dispatch, onGameOver }: GameProps<ProgressiveKnockoutState, ProgressiveKnockoutSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as ProgressiveKnockoutAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as ProgressiveKnockoutAction)}>Next</button>
      </>}
    </div>
  );
}
