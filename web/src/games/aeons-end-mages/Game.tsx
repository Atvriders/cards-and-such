...
export function AeonsEndMagesGame({ state, dispatch, onGameOver }: GameProps<AeonsEndMagesState, AeonsEndMagesSettings>): JSX.Element {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as AeonsEndMagesAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as AeonsEndMagesAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
