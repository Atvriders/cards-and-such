...
export function EggDrop({ state, dispatch, onGameOver }: GameProps<EggDropState, EggDropSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as EggDropAction), 600);
    spawnRef.current = setInterval(() => dispatch({ type: "spawn" } as EggDropAction), 2000);
