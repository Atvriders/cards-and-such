...
const hint = (state: CasState): HintTarget | null => {
  ...
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-crazy-pineapple-cas-primary"]', pulses: 3 };
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-crazy-pineapple-cas-secondary"]', pulses: 3 };
  return null;
};
