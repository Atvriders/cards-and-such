...
export function PointsSaladRollGame({ state, dispatch, onGameOver }: GameProps<PointsSaladRollState, PointsSaladRollSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as PointsSaladRollAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as PointsSaladRollAction)}>Skip</button>
      )}
    </div>
  );
}
