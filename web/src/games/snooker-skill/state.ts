...
export interface SnookerSkillSettings { dummy: boolean; }

export interface SnookerSkillState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type SnookerSkillAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: SnookerSkillSettings): SnookerSkillState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: SnookerSkillState, action: SnookerSkillAction): SnookerSkillState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<SnookerSkillState> = {};
    const pot = dice[0]! >= 4;
    if (!pot) {
      pts = 0;
      extra.breakRun = 0;
...
export function isTerminal(state: SnookerSkillState): { score: number } | null {
