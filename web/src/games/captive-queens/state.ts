...
export interface CaptiveQueensSettings { dummy: boolean; }

export interface CaptiveQueensState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type CaptiveQueensAction =
  | { type: "keep" }
...
export function initialState(seed: number, _s: CaptiveQueensSettings): CaptiveQueensState {
...
export function reducer(state: CaptiveQueensState, action: CaptiveQueensAction): CaptiveQueensState {
...
export function isTerminal(state: CaptiveQueensState): { score: number } | null {
