// Seep — South-Asian capture trick game — 1v1 duel here.
...
export type SeepPhase = "playing" | "done";
...
export type SeepAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `seep-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: SeepState, trick: readonly { seat: number; card: Card }[]): SeepState {
...
export function reducer(state: SeepState, action: SeepAction): SeepState {
  ...
  let s: SeepState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: SeepState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: SeepState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): SeepState {
...
export function isTerminal(state: SeepState): { score: number } | null {
