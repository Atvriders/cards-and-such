...
export interface PharaohsPyramidSettings { dummy: boolean; }

export interface PharaohsPyramidState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type PharaohsPyramidAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  ...
  let s = 0; const total = h.reduce((a, b) => a + cardRank(b), 0); if (isPrime(total)) s += 18; s += h.length;
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: PharaohsPyramidState, action: PharaohsPyramidAction): PharaohsPyramidState {
...
export function isTerminal(state: PharaohsPyramidState): { score: number } | null {
