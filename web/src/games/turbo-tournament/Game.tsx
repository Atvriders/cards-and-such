...
export function TurboTournamentGame({ state, dispatch, onGameOver }: GameProps<TurboTournamentState, TurboTournamentSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" onClick={() => dispatch({ type: "deal" } as TurboTournamentAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as TurboTournamentAction)}>Next</button>
      </>}
    </div>
  );
}
