...
export interface MauMauSettings { dummy: boolean; }
export interface MauMauState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type MauMauAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MauMauSettings): MauMauState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: MauMauState, action: MauMauAction): MauMauState {
...
export function isTerminal(state: MauMauState): { score: number } | null {
