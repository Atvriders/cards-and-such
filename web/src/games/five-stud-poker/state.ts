...
export interface FiveStudPokerSettings { dummy: boolean; }
export interface FiveStudPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FiveStudPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FiveStudPokerSettings): FiveStudPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: FiveStudPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
