...
export interface CardMountainClimbSettings { dummy: boolean; }
export interface CardMountainClimbState { rngSeed: number; deck: number[]; drawn: number[]; score: number; phase: "drawing" | "done"; }
export type CardMountainClimbAction = { type: "draw" } | { type: "reset" };
...
export function scoreCard(c: number): number {
  return c%13+1;  // 2->1...A->13
}
export function initialState(seed: number, _settings: CardMountainClimbSettings): CardMountainClimbState {
  const rng=mulberry32(seed); const deck=shuffle(rng);
  return { rngSeed: seed, deck, drawn: [], score: 0, phase: "drawing" };
}
export function reducer(state: CardMountainClimbState, action: CardMountainClimbAction): CardMountainClimbState {
...
export function isTerminal(state: CardMountainClimbState): { score: number } | null { return state.phase==="done"?{score:state.score}:null; }
