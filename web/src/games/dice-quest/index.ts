...
const DiceQuestGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceQuestGame as unknown as React.ComponentType<unknown> })));
...
export const diceQuestPlugin: GamePlugin<DiceQuestState, DiceQuestAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceQuestState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "predict") return { selector: '[data-testid="hint-target-dice-quest-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-dice-quest-next"]', pulses: 3 };
    return null;
  },
  component:DiceQuestGame,
};
