...
export interface PretzelPinchSettings { dummy: boolean; }
...
export interface PretzelPinchState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type PretzelPinchAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: PretzelPinchSettings): PretzelPinchState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: PretzelPinchState, action: PretzelPinchAction): PretzelPinchState {
...
export function isTerminal(state: PretzelPinchState): { score: number } | null {
