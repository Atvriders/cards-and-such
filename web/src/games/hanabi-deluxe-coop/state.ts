...
export interface hanabiDeluxeCoopSettings { dummy: boolean; }
export interface hanabiDeluxeCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type hanabiDeluxeCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: hanabiDeluxeCoopSettings): hanabiDeluxeCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: hanabiDeluxeCoopState, action: hanabiDeluxeCoopAction): hanabiDeluxeCoopState {
...
export function isTerminal(state: hanabiDeluxeCoopState): { score: number } | null {
