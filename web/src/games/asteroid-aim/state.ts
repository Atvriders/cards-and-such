...
export interface AsteroidAimSettings { dummy: boolean; }
...
export interface AsteroidAimState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type AsteroidAimAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: AsteroidAimSettings): AsteroidAimState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}

export function reducer(state: AsteroidAimState, action: AsteroidAimAction): AsteroidAimState {
...
export function isTerminal(state: AsteroidAimState): { score: number } | null {
