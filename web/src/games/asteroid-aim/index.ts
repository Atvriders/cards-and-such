...
const AsteroidAimGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.AsteroidAimGame as unknown as React.ComponentType<unknown> })));
...
export const asteroidAimPlugin: GamePlugin<AsteroidAimState, AsteroidAimAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: AsteroidAimState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-asteroid-aim-target"]', pulses: 3 };
  },
  component:AsteroidAimGame,
};
