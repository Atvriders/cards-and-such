...
export interface BaoBashSettings { dummy: boolean; }
...
export type BaoBashAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: BaoBashSettings): BaoBashState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: BaoBashState, action: BaoBashAction): BaoBashState {
...
export function isTerminal(state: BaoBashState): { score: number } | null {
