...
export function CorinthMarketGame({ state, dispatch, onGameOver }: GameProps<CorinthMarketState, CorinthMarketSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as CorinthMarketAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as CorinthMarketAction)}>Skip</button>
      )}
    </div>
  );
}
