// Mendikot — Indian partnership trump game on 52 cards — 1v1 duel.
...
export type MendikotPhase = "playing" | "done";
...
export type MendikotAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `mendikot-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: MendikotState, trick: readonly { seat: number; card: Card }[]): MendikotState {
...
export function reducer(state: MendikotState, action: MendikotAction): MendikotState {
  ...
  let s: MendikotState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: MendikotState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: MendikotState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): MendikotState {
...
export function isTerminal(state: MendikotState): { score: number } | null {
