...
const RobotRescueGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RobotRescueGame as unknown as React.ComponentType<unknown> })));
...
export const robotRescuePlugin: GamePlugin<RobotRescueState, RobotRescueAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: RobotRescueState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-robot-rescue-target"]', pulses: 3 };
  },
  component:RobotRescueGame,
};
