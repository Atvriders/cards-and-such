...
export interface RobotRescueSettings { dummy: boolean; }
...
export interface RobotRescueState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type RobotRescueAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: RobotRescueSettings): RobotRescueState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}

export function reducer(state: RobotRescueState, action: RobotRescueAction): RobotRescueState {
...
export function isTerminal(state: RobotRescueState): { score: number } | null {
