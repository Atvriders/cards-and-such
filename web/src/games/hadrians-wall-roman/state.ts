...
export interface HadriansWallRomanSettings { dummy: boolean; }
export interface HadriansWallRomanState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type HadriansWallRomanAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: HadriansWallRomanSettings): HadriansWallRomanState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: HadriansWallRomanState, action: HadriansWallRomanAction): HadriansWallRomanState {
...
export function isTerminal(state: HadriansWallRomanState): { score: number } | null {
