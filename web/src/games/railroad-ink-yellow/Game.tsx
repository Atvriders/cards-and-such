...
export function RailroadInkYellowGame({ state, dispatch, onGameOver }: GameProps<RailroadInkYellowState, RailroadInkYellowSettings>): JSX.Element {
  ...
  return (
    <div className="rw-wrap">
      ...
      {state.phase === "rolling" && (
        <button className="rw-btn" onClick={() => dispatch({ type: "roll" } as RailroadInkYellowAction)}>Roll</button>
      )}
      {state.phase === "marking" && (
        <button className="rw-btn alt" onClick={() => dispatch({ type: "skip" } as RailroadInkYellowAction)}>Skip</button>
      )}
    </div>
  );
}
