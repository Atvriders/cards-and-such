...
export interface PanicWallStreetSettings { rounds: "10"; }

export interface PanicWallStreetState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type PanicWallStreetAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: PanicWallStreetSettings): PanicWallStreetState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: PanicWallStreetState, action: PanicWallStreetAction): PanicWallStreetState {
...
export function isTerminal(state: PanicWallStreetState): { score: number } | null {
