...
export interface captainIsDeadCoopSettings { dummy: boolean; }
export interface captainIsDeadCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type captainIsDeadCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: captainIsDeadCoopSettings): captainIsDeadCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: captainIsDeadCoopState, action: captainIsDeadCoopAction): captainIsDeadCoopState {
...
export function isTerminal(state: captainIsDeadCoopState): { score: number } | null {
