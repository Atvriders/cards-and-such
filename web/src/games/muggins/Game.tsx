...
import "./Game.css";
export function MugginsGame({ state, dispatch, onGameOver }: GameProps<MugginsState, MugginsSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-muggins-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as MugginsAction)}>Claim!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-muggins-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as MugginsAction)}>Next</button>
      </>}
    </div>
  );
}
