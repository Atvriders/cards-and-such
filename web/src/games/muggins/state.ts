...
export interface MugginsSettings { dummy: boolean; }
export interface MugginsState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type MugginsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: MugginsSettings): MugginsState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: MugginsState, action: MugginsAction): MugginsState {
...
export function isTerminal(state: MugginsState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
