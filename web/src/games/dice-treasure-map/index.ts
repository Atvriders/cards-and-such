...
const DiceTreasureMapGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceTreasureMapGame as unknown as React.ComponentType<unknown> })));
...
export const diceTreasureMapPlugin: GamePlugin<DiceTreasureMapState, DiceTreasureMapAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceTreasureMapState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "choose") return { selector: '[data-testid="hint-target-dice-treasure-map-roll"]', pulses: 3 };
    if (state.phase === "result") return { selector: '[data-testid="hint-target-dice-treasure-map-next"]', pulses: 3 };
    return null;
  },
  component:DiceTreasureMapGame,
};
