...
export interface KlondikeDealOneNoRedealSettings { dummy: boolean; }

export interface KlondikeDealOneNoRedealState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type KlondikeDealOneNoRedealAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank); for (let i = 0; i < r.length; i++) for (let j = i+1; j < r.length; j++) if (r[i] === r[j]) s += 9;
  if (s === 0) s = 1; return s;
}

export function initialState(seed: number, _s: KlondikeDealOneNoRedealSettings): KlondikeDealOneNoRedealState {
...
export function reducer(state: KlondikeDealOneNoRedealState, action: KlondikeDealOneNoRedealAction): KlondikeDealOneNoRedealState {
...
export function isTerminal(state: KlondikeDealOneNoRedealState): { score: number } | null {
