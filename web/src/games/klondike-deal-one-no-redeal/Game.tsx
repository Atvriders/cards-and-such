...
export function KlondikeDealOneNoRedealGame({ state, dispatch, onGameOver }: GameProps<KlondikeDealOneNoRedealState, KlondikeDealOneNoRedealSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap fade-in">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as KlondikeDealOneNoRedealAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
