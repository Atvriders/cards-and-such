...
export function HollywoodGinRGame({ state, dispatch, onGameOver }: GameProps<HollywoodGinRState, HollywoodGinRSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <button className="dm-btn" onClick={() => dispatch({ type: "score" } as HollywoodGinRAction)}>Auto-score</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as HollywoodGinRAction)}>Next</button>
      </>}
    </div>
  );
}
