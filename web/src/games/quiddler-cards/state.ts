...
export interface QuiddlerCardsSettings { dummy: boolean; }
export interface QuiddlerCardsState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type QuiddlerCardsAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: QuiddlerCardsSettings): QuiddlerCardsState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: QuiddlerCardsState, action: QuiddlerCardsAction): QuiddlerCardsState {
...
export function isTerminal(state: QuiddlerCardsState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
