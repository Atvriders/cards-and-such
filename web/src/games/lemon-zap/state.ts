...
export interface LemonZapSettings { dummy: boolean; }
...
export interface LemonZapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type LemonZapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: LemonZapSettings): LemonZapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LemonZapState, action: LemonZapAction): LemonZapState {
...
export function isTerminal(state: LemonZapState): { score: number } | null {
