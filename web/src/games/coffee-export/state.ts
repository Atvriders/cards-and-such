...
export interface CoffeeExportSettings { dummy: boolean; }
export interface CoffeeExportState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type CoffeeExportAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: CoffeeExportSettings): CoffeeExportState {
...
export function reducer(state: CoffeeExportState, action: CoffeeExportAction): CoffeeExportState {
...
export function score(s: CoffeeExportState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: CoffeeExportState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
