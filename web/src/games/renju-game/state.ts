...
export function reducer(state: GameState, action: GameAction): GameState {
  ...
  if (maxLineLen(nb, "P") > 5) {
    return { ...state, board: nb, moves, result: "C", score: 25, phase: "done" };
  }
  if (checkWin(nb, "P")) {
    return { ...state, board: nb, moves, result: "P", score: state.score + 100, phase: "done" };
  }
  ...
  if (checkWin(cnb, "C")) {
    return { ...state, rngSeed: seed2, board: cnb, moves: cmoves, result: "C", score: 0, phase: "done" };
  }
