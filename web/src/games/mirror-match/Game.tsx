...
export function MirrorMatchGame({ state, dispatch, onGameOver }: GameProps<MirrorMatchState, MirrorMatchSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") {
    return <div className="cm-wrap"><div className="cm-done bounce-in"><h2>Done!</h2><div>Matches: {state.matches}</div><div className="cm-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="cm-wrap fade-in">
      ...
      {state.phase === "scored" && (
        <>
          <div className="cm-result">{state.matched ? "Match! +10" : "No match — 0"}</div>
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as MirrorMatchAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
