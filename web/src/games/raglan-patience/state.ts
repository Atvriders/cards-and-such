...
export interface RaglanPatienceSettings { dummy: boolean; }

export interface RaglanPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type RaglanPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const evens = h.filter(c=>cardRank(c)%2===0).length; s += evens*4; const aces = h.filter(c=>cardRank(c)===1).length; s += aces*6; if (s===0) s=1; return s;
}
...
export function reducer(state: RaglanPatienceState, action: RaglanPatienceAction): RaglanPatienceState {
...
export function isTerminal(state: RaglanPatienceState): { score: number } | null {
