...
import "./Game.css";
export function CardJungleGame({ state, dispatch, onGameOver }: GameProps<CardJungleState, CardJungleSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      <div className="cm-info">Jungle — Round {state.round} / {TOTAL_ROUNDS}</div>
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardJungleAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
