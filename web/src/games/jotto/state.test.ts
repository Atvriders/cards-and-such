...
import { initialState, reducer, isTerminal, Jotto_CFG } from "./state.js";
...
describe("jotto", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(Jotto_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < Jotto_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < Jotto_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
