...
export interface HoldemNoLimitSettings { dummy: boolean; }
export interface HoldemNoLimitState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type HoldemNoLimitAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: HoldemNoLimitSettings): HoldemNoLimitState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
...
export function isTerminal(state: HoldemNoLimitState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
