...
export interface MttTournamentSettings { dummy: boolean; }
export interface MttTournamentState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type MttTournamentAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: MttTournamentSettings): MttTournamentState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: MttTournamentState, action: MttTournamentAction): MttTournamentState {
...
export function isTerminal(state: MttTournamentState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
