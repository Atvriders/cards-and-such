...
export interface JellyJarTossSettings { dummy: boolean; }
export interface JellyJarTossState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type JellyJarTossAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: JellyJarTossSettings): JellyJarTossState {
...
export function reducer(state: JellyJarTossState, action: JellyJarTossAction): JellyJarTossState {
...
export function isTerminal(state: JellyJarTossState): { score: number } | null {
