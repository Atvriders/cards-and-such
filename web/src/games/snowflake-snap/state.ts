...
export interface SnowflakeSnapSettings { dummy: boolean; }
...
export interface SnowflakeSnapState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type SnowflakeSnapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: SnowflakeSnapSettings): SnowflakeSnapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: SnowflakeSnapState, action: SnowflakeSnapAction): SnowflakeSnapState {
...
export function isTerminal(state: SnowflakeSnapState): { score: number } | null {
