...
export interface CityOfHorrorRwSettings { dummy: boolean; }
export interface CityOfHorrorRwState {
  rngSeed: number;
  cells: boolean[];
  cellValues: number[];
  rolls: number;
...
export type CityOfHorrorRwAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: CityOfHorrorRwSettings): CityOfHorrorRwState {
  return {
    rngSeed: seed,
...
export function reducer(state: CityOfHorrorRwState, action: CityOfHorrorRwAction): CityOfHorrorRwState {
...
export function isTerminal(state: CityOfHorrorRwState): { score: number } | null {
