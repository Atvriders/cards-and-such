...
export function CityOfHorrorRwGame({ state, dispatch, onGameOver }: GameProps<CityOfHorrorRwState, CityOfHorrorRwSettings>): JSX.Element {
  const t = isTerminal(state);
  useEffect(() => { if (t) onGameOver(t.score); }, [t, onGameOver]);
  if (state.phase === "done") {
    return (
      <div className="rw-wrap">
        <h3 className="rw-title">City of Horror: R&W</h3>
      ...
      <div className="rw-grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 44px)` }}>
        {state.cells.map((filled, i) => (
          <button
            ...
            onClick={() => dispatch({ type: "mark", index: i } as CityOfHorrorRwAction)}
