...
export interface ScumShedSettings { dummy: boolean; }
export interface ScumShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type ScumShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ScumShedSettings): ScumShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: ScumShedState, action: ScumShedAction): ScumShedState {
...
export function isTerminal(state: ScumShedState): { score: number } | null {
