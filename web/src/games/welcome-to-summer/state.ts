...
export interface WelcomeToSummerSettings { dummy: boolean; }
export interface WelcomeToSummerState {
  rngSeed: number;
...
export type WelcomeToSummerAction =
  | { type: "roll" }
  | { type: "place"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: WelcomeToSummerSettings): WelcomeToSummerState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: WelcomeToSummerState, action: WelcomeToSummerAction): WelcomeToSummerState {
...
export function isTerminal(state: WelcomeToSummerState): { score: number } | null {
