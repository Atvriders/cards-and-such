...
// Honey Tap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface HoneyTapSettings { dummy: boolean; }

export interface HoneyTapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface HoneyTapState {
  rngSeed: number;
  targets: HoneyTapTarget[];
  nextId: number;
...
export type HoneyTapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: HoneyTapSettings): HoneyTapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: HoneyTapState, action: HoneyTapAction): HoneyTapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: HoneyTapTarget[] = [];
...
export function isTerminal(state: HoneyTapState): { score: number } | null {
