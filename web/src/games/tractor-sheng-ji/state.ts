...
export interface TractorShengJiSettings { dummy: boolean; }
export interface TractorShengJiState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type TractorShengJiAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: TractorShengJiSettings): TractorShengJiState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: TractorShengJiState, action: TractorShengJiAction): TractorShengJiState {
...
export function isTerminal(state: TractorShengJiState): { score: number } | null {
