...
const TractorShengJiGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.TractorShengJiGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: TractorShengJiState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-tractor-sheng-ji-primary"]', pulses: 3 };
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-tractor-sheng-ji-secondary"]', pulses: 3 };
  return null;
};
...
export const tractorShengJiPlugin: GamePlugin<TractorShengJiState, TractorShengJiAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: TractorShengJiGame,
};
