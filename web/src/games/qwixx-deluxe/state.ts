...
export interface QwixxDeluxeSettings { dummy: boolean; }
export interface QwixxDeluxeState {
  rngSeed: number;
  cells: boolean[];
...
export type QwixxDeluxeAction =
  | { type: "roll" }
  | { type: "mark"; index: number }
  | { type: "skip" };

export function initialState(seed: number, _s: QwixxDeluxeSettings): QwixxDeluxeState {
  return {
    rngSeed: seed,
...
export function reducer(state: QwixxDeluxeState, action: QwixxDeluxeAction): QwixxDeluxeState {
...
export function isTerminal(state: QwixxDeluxeState): { score: number } | null {
