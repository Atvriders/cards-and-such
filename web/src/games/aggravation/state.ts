...
export interface RaceState {
  rngSeed: number;
  pPositions: number[]; // 0..TRACK; TRACK = borne off
  cPositions: number[]; // 0..TRACK; TRACK = borne off (in P-relative pips)
  dice: [number, number];
...
export type RaceAction =
  | { type: "roll" }
  | { type: "move"; checkerIdx: number; die: 0 | 1 | 2 } // 2 = both dice
  | { type: "endTurn" };
...
function cpuTurn(state: RaceState): RaceState {
  ...
  // CPU plays each die in order on a random legal checker
