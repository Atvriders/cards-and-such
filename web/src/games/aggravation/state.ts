// Auto-generated race-game state for aggravation.
...
export const SPECIALS: Record<number, Special> = {7: {type:"hub", to:25},14: {type:"safe"},21: {type:"hub", to:30},28: {type:"safe"}};

export type Side = "P" | "C";
export interface AggravationSettings { dummy: boolean }

export interface AggravationState {
  rngSeed: number;
...
export function initialState(seed: number, _s: AggravationSettings): AggravationState {
  return {
    rngSeed: seed,
...
function checkWin(s: AggravationState): AggravationState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: AggravationState): AggravationState {
  let s = state;
...
export function legalMovesForP(state: AggravationState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: AggravationState, action: AggravationAction): AggravationState {
...
export function isTerminal(state: AggravationState): { score: number } | null {
