...
const RacingStuntsGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.RacingStuntsGame as unknown as React.ComponentType<unknown> })));
...
Tips: Accelerate aggressively early to build speed. In stunt zones, Drift is the safe choice while Boost is high risk — go for Boost when you are feeling lucky or need a speed surge.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver) return null; return { selector: '[data-testid="hint-target-racing-stunts-action"]', pulses: 3 }; },
