...
export function DrawmahaPokerGame({ state, dispatch, onGameOver }: GameProps<DrawmahaPokerState, DrawmahaPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap thmDrawmaha">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-drawmaha-deal" onClick={() => dispatch({ type: "deal" } as DrawmahaPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-drawmaha-next" onClick={() => dispatch({ type: "next" } as DrawmahaPokerAction)}>Next</button>
      </>}
    </div>
  );
}
