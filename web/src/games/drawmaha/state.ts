...
export interface DrawmahaPokerSettings { dummy: boolean; }
export interface DrawmahaPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type DrawmahaPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: DrawmahaPokerSettings): DrawmahaPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: DrawmahaPokerState, action: DrawmahaPokerAction): DrawmahaPokerState {
...
export function isTerminal(state: DrawmahaPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
