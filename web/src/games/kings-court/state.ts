...
// Score by drawing Kings; +100 per match across 10 draws.

export const TOTAL_DRAWS = 10;
export const HAND_SIZE = 5;
export const TARGET_RANK = 11; // 0..12 = 2..A; J=9, Q=10, K=11, 10=8
export const POINTS_PER = 100;

export interface KingsCourtSettings { dummy: boolean; }

export interface KingsCourtState {
  rngSeed: number;
  draw: number;
  hand: number[];
  matchedIdx: number[];
...
export type KingsCourtAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _settings: KingsCourtSettings): KingsCourtState {
  return { rngSeed: seed, draw: 1, hand: [], matchedIdx: [], score: 0, phase: "dealing", lastPts: 0 };
}

export function reducer(state: KingsCourtState, action: KingsCourtAction): KingsCourtState {
...
export function isTerminal(state: KingsCourtState): { score: number } | null {
