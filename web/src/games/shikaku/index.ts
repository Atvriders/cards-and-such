...
Strategy: start with larger numbers since they constrain the shape more tightly. Numbers near the edge can only be oriented in a limited number of ways. Work from constrained cells outward, eliminating possibilities until only one valid tiling remains.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-shikaku-action"]', pulses: 3 }; },
  component: Shikaku,
};
