...
export interface DiceDeepSeaFishingSettings { dummy: boolean; }

export interface DiceDeepSeaFishingState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceDeepSeaFishingAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceDeepSeaFishingSettings): DiceDeepSeaFishingState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceDeepSeaFishingState, action: DiceDeepSeaFishingAction): DiceDeepSeaFishingState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceDeepSeaFishingState> = {};
...
export function isTerminal(state: DiceDeepSeaFishingState): { score: number } | null {
