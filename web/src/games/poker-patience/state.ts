...
export const SCORE_WIN = 11;
export const SCORE_TIE = 4;
export interface PokerPatienceSettings { dummy: boolean; }
export interface PokerPatienceState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type PokerPatienceAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: PokerPatienceSettings): PokerPatienceState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: PokerPatienceState, action: PokerPatienceAction): PokerPatienceState {
...
export function isTerminal(state: PokerPatienceState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
