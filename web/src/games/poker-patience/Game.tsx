...
import "./Game.css";
export function PokerPatienceGame({ state, dispatch, onGameOver }: GameProps<PokerPatienceState, PokerPatienceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-poker-patience-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as PokerPatienceAction)}>Place!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-poker-patience-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as PokerPatienceAction)}>Next</button>
      </>}
    </div>
  );
}
