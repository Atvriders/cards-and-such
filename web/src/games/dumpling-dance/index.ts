...
const DumplingDanceGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DumplingDanceGame as unknown as React.ComponentType<unknown> })));
...
export const dumplingDancePlugin: GamePlugin<DumplingDanceState, DumplingDanceAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: DumplingDanceState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-dumpling-dance-target"]', pulses: 3 };
  },
  component:DumplingDanceGame,
};
