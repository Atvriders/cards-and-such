...
export interface DumplingDanceSettings { dummy: boolean; }
...
export interface DumplingDanceState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type DumplingDanceAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: DumplingDanceSettings): DumplingDanceState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: DumplingDanceState, action: DumplingDanceAction): DumplingDanceState {
...
export function isTerminal(state: DumplingDanceState): { score: number } | null {
