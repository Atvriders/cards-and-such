...
export const FibbageXlQuiz_QUESTIONS: QuizQuestion[] = [
  {
...
const CFG = { totalQuestions: Math.min(10, FibbageXlQuiz_QUESTIONS.length), pool: FibbageXlQuiz_QUESTIONS };

export interface FibbageXlQuizSettings { dummy: boolean; }
export type FibbageXlQuizState = QuizState;
export type FibbageXlQuizAction = { type: "answer"; choice: number; elapsedMs: number } | { type: "next" };

export function initialState(seed: number, _s: FibbageXlQuizSettings): FibbageXlQuizState {
...
export function isTerminal(state: FibbageXlQuizState): { score: number } | null {
