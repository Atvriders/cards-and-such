...
// Bejeweled Blitz: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface BejeweledBlitzSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface BejeweledBlitzState {
  rngSeed: number;
  grid: Grid;
...
export type BejeweledBlitzAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: BejeweledBlitzSettings): BejeweledBlitzState {
...
export function reducer(state: BejeweledBlitzState, action: BejeweledBlitzAction): BejeweledBlitzState {
...
export function isTerminal(state: BejeweledBlitzState): { score: number } | null {
