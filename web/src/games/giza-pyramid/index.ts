...
const GizaPyramidGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.GizaPyramidGame as unknown as React.ComponentType<unknown> })));
...
export const gizaPyramidPlugin: GamePlugin<GizaPyramidState, GizaPyramidAction, typeof settings> = {
  ...
  hint: (state: GizaPyramidState): HintTarget | null => {
    ...
    if (king) return { selector: `[data-testid="hint-target-giza-pyramid-pyramid-${king.row}-${king.col}"]`, pulses: 3 };
    for (const a of avail) {
      for (const b of avail) {
        if (a === b) continue;
        if (a.rank + b.rank === 13) {
          return { selector: `[data-testid="hint-target-giza-pyramid-pyramid-${a.row}-${a.col}"]`, pulses: 3 };
        }
      }
    }
    ...
    if (wasteTop) {
      ...
      if (m) return { selector: `[data-testid="hint-target-giza-pyramid-pyramid-${m.row}-${m.col}"]`, pulses: 3 };
    }
    if (state.stock.length > 0) return { selector: '[data-testid="hint-target-giza-pyramid-draw"]', pulses: 3 };
    if (state.redealsRemaining > 0) return { selector: '[data-testid="hint-target-giza-pyramid-redeal"]', pulses: 3 };
    return null;
  },
  reducer,
  isTerminal,
