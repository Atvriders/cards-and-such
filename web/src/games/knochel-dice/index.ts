...
const KnochelDiceGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.KnochelDiceGame as unknown as React.ComponentType<unknown> })));
...
export const knochelDicePlugin: GamePlugin<KnochelDiceState, KnochelDiceAction, typeof settings> = {
  ...
  hint: (state) => {
    ...
    if (phase === "betting") return { selector: '[data-testid="hint-target-knochel-dice-predict"]', pulses: 3 };
    if (phase === "bet") return { selector: '[data-testid="hint-target-knochel-dice-predict"]', pulses: 3 };
    if (phase === "predict") return { selector: '[data-testid="hint-target-knochel-dice-predict"]', pulses: 3 };
    if (phase === "predicting") return { selector: '[data-testid="hint-target-knochel-dice-predict"]', pulses: 3 };
    if (phase === "roundOver") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    if (phase === "result") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    if (phase === "settled") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    if (phase === "banked") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    if (phase === "done") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    if (phase === "farkled") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    if (phase === "busted") return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
    return { selector: '[data-testid="hint-target-knochel-dice-next"]', pulses: 3 };
  },
