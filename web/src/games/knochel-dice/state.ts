...
export const PAYOUTS: number[] = [28,28,6];
export const CHOICES: string[] = ["Bone (double) (+28)","Cup (sum=7) (+28)","Toss (+6)"];
export interface KnochelDiceSettings { dummy: boolean; }
export interface KnochelDiceState {
  rngSeed: number;
  round: number;
...
export type KnochelDiceAction =
  | { type: "predict"; choice: number }
  | { type: "next" };
export function initialState(seed: number, _settings: KnochelDiceSettings): KnochelDiceState {
  return { rngSeed: seed, round: 1, prediction: null, dice: null, resultIdx: null, score: 0, phase: "predict" };
}
export function reducer(state: KnochelDiceState, action: KnochelDiceAction): KnochelDiceState {
  ...
  if (action.type === "predict") {
    ...
    const resultIdx = (()=>{ if (a===b) return 0; if (a+b===7) return 1; return 2; })();
...
export function isTerminal(state: KnochelDiceState): { score: number } | null {
