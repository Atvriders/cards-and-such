...
import "./Game.css";
export function KnochelDiceGame({ state, dispatch, onGameOver }: GameProps<KnochelDiceState, KnochelDiceSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {CHOICES.map((c, i) => (
            <button data-testid="hint-target-knochel-dice-predict" key={i} className={i % 2 === 0 ? "dm-btn" : "dm-btn alt"} onClick={() => dispatch({ type: "predict", choice: i } as KnochelDiceAction)}>{c}</button>
          ))}
        </div>
      )}
      {state.phase === "result" && state.resultIdx !== null && state.prediction !== null && (
        <>
          ...
          <button data-testid="hint-target-knochel-dice-next" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as KnochelDiceAction)}>Next</button>
        </>
      )}
    </div>
  );
