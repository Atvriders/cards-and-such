...
export const ASSET_COST = 25;
export const HIRE_COST = 45;

export interface ScovillePeppersSettings { dummy: boolean; }
export interface ScovillePeppersState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type ScovillePeppersAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: ScovillePeppersSettings): ScovillePeppersState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Habanero harvest","Ghost pepper boom","Jalapeno rush","Carolina Reaper craze","Cayenne shortage","Bell pepper glut"];

export function reducer(state: ScovillePeppersState, action: ScovillePeppersAction): ScovillePeppersState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Peppers (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Grower (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Traded 1 Peppers (+$" + market + ")"; delta = market;
      } else { evt = "No Peppers to trade"; }
    } else { return state; }
    const div = assets * 5;
    cash += div;
    const pay = workers * 9;
...
export function score(s: ScovillePeppersState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: ScovillePeppersState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
