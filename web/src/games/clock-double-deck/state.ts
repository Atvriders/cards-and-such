...
export interface ClockDoubleDeckSettings { dummy: boolean; }

export interface ClockDoubleDeckState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type ClockDoubleDeckAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; for (const c of h) { const r = cardRank(c); if (r % 3 === 0) s += 5; else s += 1; }
  if (s === 0) s = 1; return s;
}
...
export function reducer(state: ClockDoubleDeckState, action: ClockDoubleDeckAction): ClockDoubleDeckState {
...
export function isTerminal(state: ClockDoubleDeckState): { score: number } | null {
