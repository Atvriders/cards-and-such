...
describe("lights-out-5x5", () => {
  it("starts in playing with N*N cells", () => {
    const s = initialState(1, S);
    expect(s.phase).toBe("playing");
    expect(s.cells.length).toBe(25);
  });
  it("starts unsolved", () => {
    expect(isSolved(initialState(1, S).cells)).toBe(false);
  });
  it("tap toggles cell + neighbors", () => {
    const s = initialState(1, S);
    const before = [...s.cells];
    const s2 = reducer(s, { type: "tap", index: 12 });
    expect(s2.moves).toBe(1);
    let diffs = 0;
    for (let i = 0; i < 25; i++) if (before[i] !== s2.cells[i]) diffs++;
