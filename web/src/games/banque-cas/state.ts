...
export interface BanqueCasSettings { dummy: boolean; }
export interface BanqueCasState { rngSeed: number; round: number; you: number[]; cpu: number[]; phase: "see" | "scored" | "done"; score: number; pts: number; result: string; }
export type BanqueCasAction = { type: "play" } | { type: "fold" } | { type: "next" };
...
export function initialState(seed: number, _s: BanqueCasSettings): BanqueCasState {
...
export function reducer(state: BanqueCasState, action: BanqueCasAction): BanqueCasState {
...
export function isTerminal(state: BanqueCasState): { score: number } | null {
