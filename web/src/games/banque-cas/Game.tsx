  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "see" && <div className="dm-row">
        <button className="dm-btn" onClick={() => dispatch({ type: "play" } as BanqueCasAction)}>Play</button>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "fold" } as BanqueCasAction)}>Fold</button>
      </div>}
      {state.phase === "scored" && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as BanqueCasAction)}>Next</button>
      </>}
    </div>
  );
}
