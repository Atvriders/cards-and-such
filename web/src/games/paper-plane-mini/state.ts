...
// Paper Plane Mini: 30s clicker. Click moving targets to score.

export const TIMER_TICKS = 30;
export const LANES = 6;

export interface PaperPlaneMiniSettings { dummy: boolean; }

export interface PaperPlaneMiniTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface PaperPlaneMiniState {
  rngSeed: number;
  targets: PaperPlaneMiniTarget[];
  nextId: number;
...
export type PaperPlaneMiniAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: PaperPlaneMiniSettings): PaperPlaneMiniState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, misses: 0, phase: "playing" };
}

export function reducer(state: PaperPlaneMiniState, action: PaperPlaneMiniAction): PaperPlaneMiniState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: PaperPlaneMiniTarget[] = [];
...
export function isTerminal(state: PaperPlaneMiniState): { score: number } | null {
