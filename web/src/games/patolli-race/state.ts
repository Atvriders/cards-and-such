// Auto-generated race-game state for patolli-race.
import { mulberry32 } from "../../platform/game-plugin/useSeededRng.js";

export const TRACK_LEN = 20;
export const PAWNS_PER_SIDE = 6;
...
export const SPECIALS: Record<number, Special> = {5: {type:"safe"},10: {type:"safe"},15: {type:"safe"}};

export type Side = "P" | "C";
export interface PatolliSettings { dummy: boolean }

export interface PatolliState {
  rngSeed: number;
...
export function initialState(seed: number, _s: PatolliSettings): PatolliState {
  return {
    rngSeed: seed,
...
function checkWin(s: PatolliState): PatolliState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: PatolliState): PatolliState {
  let s = state;
...
export function legalMovesForP(state: PatolliState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: PatolliState, action: PatolliAction): PatolliState {
...
export function isTerminal(state: PatolliState): { score: number } | null {
