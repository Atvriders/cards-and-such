...
export interface Bowling9pinSettings { dummy: boolean; }

export interface Bowling9pinState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type Bowling9pinAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: Bowling9pinSettings): Bowling9pinState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: Bowling9pinState, action: Bowling9pinAction): Bowling9pinState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<Bowling9pinState> = {};
...
export function isTerminal(state: Bowling9pinState): { score: number } | null {
