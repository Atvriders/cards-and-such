...
import { initialState, reducer, isTerminal, ClueMini_CFG } from "./state.js";
...
describe("clue-mini", () => {
  it("starts in guess phase with empty history", () => {
    ...
    expect(s.answer.length).toBe(ClueMini_CFG.answerLength);
  });
  ...
  it("submitting the answer wins", () => {
    ...
    for (let i = 0; i < ClueMini_CFG.answerLength; i++) {
  ...
  it("running out of guesses ends the puzzle", () => {
    let s = initialState(1, S);
    for (let i = 0; i < ClueMini_CFG.maxGuesses; i++) {
      s = reducer(s, { type: "submit" });
    }
