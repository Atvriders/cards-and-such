...
export interface WelcomeDinolandSettings { dummy: boolean; }
export interface WelcomeDinolandState {
  rngSeed: number;
...
export type WelcomeDinolandAction =
  | { type: "roll" }
  | { type: "place"; index: number }
  | { type: "skip" }
  | { type: "reset" };

export function initialState(seed: number, _s: WelcomeDinolandSettings): WelcomeDinolandState {
  return {
    rngSeed: seed >>> 0,
...
export function reducer(state: WelcomeDinolandState, action: WelcomeDinolandAction): WelcomeDinolandState {
...
export function isTerminal(state: WelcomeDinolandState): { score: number } | null {
