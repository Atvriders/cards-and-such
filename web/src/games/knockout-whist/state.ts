// Knockout Whist — Whist variant: lose a card from your hand each round you don't win.
...
export type KnockoutWhistPhase = "playing" | "done";
...
export type KnockoutWhistAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `knockout-whist-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: KnockoutWhistState, trick: readonly { seat: number; card: Card }[]): KnockoutWhistState {
...
export function reducer(state: KnockoutWhistState, action: KnockoutWhistAction): KnockoutWhistState {
  ...
  if (s.playerLeads) {
    ...
    let next: KnockoutWhistState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: KnockoutWhistState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): KnockoutWhistState {
