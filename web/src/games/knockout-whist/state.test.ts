...
describe("knockout-whist initialState", () => {
  it("deals correct number of cards to each player", () => {
    const s = initialState(42);
    expect(s.playerHand.length).toBe(7);
    expect(s.botHand.length).toBe(7);
  });
...
describe("knockout-whist legalPlays", () => {
  it("all cards legal when leading", () => {
    const s = initialState(42);
    expect(legalPlays(s.playerHand, s.currentTrick).length).toBeGreaterThanOrEqual(7);
  });
});

describe("knockout-whist reducer", () => {
...
describe("knockout-whist full game", () => {
  it("completes a full game", () => {
    ...
    expect(s.playerTricks + s.botTricks).toBeGreaterThanOrEqual(3);
  });
});
