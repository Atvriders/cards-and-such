...
export interface BowlingCandlepinSettings { dummy: boolean; }

export interface BowlingCandlepinState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type BowlingCandlepinAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: BowlingCandlepinSettings): BowlingCandlepinState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: BowlingCandlepinState, action: BowlingCandlepinAction): BowlingCandlepinState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<BowlingCandlepinState> = {};
...
export function isTerminal(state: BowlingCandlepinState): { score: number } | null {
