...
const CasinoHoldemCasGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.CasinoHoldemCasGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: CasinoHoldemCasState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-casino-holdem-cas-primary"]', pulses: 3 };
  if (state.phase === "scored") return { selector: '[data-testid="hint-target-casino-holdem-cas-secondary"]', pulses: 3 };
  return null;
};
export const casinoHoldemCasPlugin: GamePlugin<CasinoHoldemCasState, CasinoHoldemCasAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: CasinoHoldemCasGame,
};
