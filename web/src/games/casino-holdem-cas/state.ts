...
export interface CasinoHoldemCasSettings { dummy: boolean; }
export interface CasinoHoldemCasState { rngSeed: number; round: number; cardA: number | null; cardB: number | null; cardC: number | null; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; }
export type CasinoHoldemCasAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: CasinoHoldemCasSettings): CasinoHoldemCasState {
  return { rngSeed: seed, round: 1, cardA: null, cardB: null, cardC: null, phase: "ready", score: 0, pts: 0, result: "" };
}
export function reducer(state: CasinoHoldemCasState, action: CasinoHoldemCasAction): CasinoHoldemCasState {
...
export function isTerminal(state: CasinoHoldemCasState): { score: number } | null {
