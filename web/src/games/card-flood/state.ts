...
export interface CardFloodSettings { dummy: boolean; }
export interface CardFloodState {
  rngSeed: number;
  round: number;
  cards: number[];
  picked: number | null;
...
export type CardFloodAction = { type: "pick"; index: number } | { type: "next" } | { type: "deal" };
...
export function initialState(seed: number, _settings: CardFloodSettings): CardFloodState {
...
export function scoreFor(c: number, _cards: number[], target: number): number {
  const suit = Math.floor(c / 13);
    return suit === target ? 20 : 5;
}
export function reducer(state: CardFloodState, action: CardFloodAction): CardFloodState {
...
export function isTerminal(state: CardFloodState): { score: number } | null {
