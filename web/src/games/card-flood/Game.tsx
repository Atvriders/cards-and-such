...
export function CardFloodGame({ state, dispatch, onGameOver }: GameProps<CardFloodState, CardFloodSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      <div className="cm-hint">Card Flood</div>
      ...
      <div className="cm-row">
        {state.cards.map((c, i) => (
          <button key={i}
            ...
            onClick={() => dispatch({ type:"pick", index:i } as CardFloodAction)}>{cardName(c)}</button>
        ))}
      </div>
      {state.phase === "scored" && (
        <>
