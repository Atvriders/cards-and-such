...
export interface FantasylandOfcSettings { dummy: boolean; }
export interface FantasylandOfcState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type FantasylandOfcAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: FantasylandOfcSettings): FantasylandOfcState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: FantasylandOfcState, action: FantasylandOfcAction): FantasylandOfcState {
...
export function isTerminal(state: FantasylandOfcState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
