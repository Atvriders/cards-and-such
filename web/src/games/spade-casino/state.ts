...
export interface SpadeCasinoSettings { dummy: boolean; }
export interface SpadeCasinoState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type SpadeCasinoAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SpadeCasinoSettings): SpadeCasinoState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: SpadeCasinoState, action: SpadeCasinoAction): SpadeCasinoState {
  ...
  // Spade bonus: +2 if your card is a spade
  if (Math.floor(you / 13) === 0) pts += 2;
...
export function isTerminal(state: SpadeCasinoState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
