...
const SpadeCasinoGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SpadeCasinoGame as unknown as React.ComponentType<unknown> })));
const settings = { dummy: { kind: "boolean" as const, label: "dummy", default: false } } as const;
type S = SettingsOf<typeof settings>;
const hint = (state: SpadeCasinoState): HintTarget | null => {
  if (isTerminal(state)) return null;
  if (state.phase === "ready") return { selector: '[data-testid="hint-target-spade-casino-primary"]', pulses: 3 };
  if (state.phase === "result") return { selector: '[data-testid="hint-target-spade-casino-secondary"]', pulses: 3 };
  return null;
};
...
export const spadeCasinoPlugin: GamePlugin<SpadeCasinoState, SpadeCasinoAction, typeof settings> = {
  ...
  reducer, isTerminal, hint: hint, component: SpadeCasinoGame,
};
