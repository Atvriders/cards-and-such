...
import "./Game.css";
export function SpadeCasinoGame({ state, dispatch, onGameOver }: GameProps<SpadeCasinoState, SpadeCasinoSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap fade-in">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-spade-casino-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as SpadeCasinoAction)}>Capture!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button data-testid="hint-target-spade-casino-secondary" className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SpadeCasinoAction)}>Next</button>
      </>}
    </div>
  );
}
