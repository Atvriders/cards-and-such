...
export function SpadeCasinoGame({ state, dispatch, onGameOver }: GameProps<SpadeCasinoState, SpadeCasinoSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button className="dm-btn" onClick={() => dispatch({ type: "play" } as SpadeCasinoAction)}>Capture!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as SpadeCasinoAction)}>Next</button>
      </>}
    </div>
  );
}
