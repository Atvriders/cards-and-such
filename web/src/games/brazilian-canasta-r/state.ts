...
export interface BrazilianCanastaRSettings { dummy: boolean; }
export interface BrazilianCanastaRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type BrazilianCanastaRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: BrazilianCanastaRSettings): BrazilianCanastaRState {
  const rng = mulberry32(seed); const hand = deal(rng, HAND_SIZE);
  const next = Math.floor(rng() * 2 ** 31);
  return { rngSeed: next, round: 1, hand, phase: "play", melds: [], score: 0, pts: 0, deadwood: 0, result: "" };
}
export function reducer(state: BrazilianCanastaRState, action: BrazilianCanastaRAction): BrazilianCanastaRState {
  ...
  if (action.type === "score") {
    ...
    for (const m of melds) pts += 18 + (m.length - 3) * 6;
    if (melds.length === 0) { pts = Math.max(0, 4 - Math.floor(deadwood / 6)); result = `No melds (deadwood ${deadwood})`; }
    else { result = `${melds.length} meld${melds.length === 1 ? "" : "s"} — deadwood ${deadwood}`; }
    if (remaining.length === 0) { pts += 30; result += " · Out!"; }
    const isLast = state.round >= TOTAL_ROUNDS;
    return { ...state, melds, pts, deadwood, result, score: state.score + pts, phase: isLast ? "done" : "scored" };
  }
  if (action.type === "next") {
    if (state.phase !== "scored") return state;
    const rng = mulberry32(state.rngSeed); const hand = deal(rng, HAND_SIZE);
    const next = Math.floor(rng() * 2 ** 31);
    return { ...state, rngSeed: next, round: state.round + 1, hand, melds: [], pts: 0, deadwood: 0, result: "", phase: "play" };
  }
  return state;
}
export function isTerminal(state: BrazilianCanastaRState): { score: number } | null {
