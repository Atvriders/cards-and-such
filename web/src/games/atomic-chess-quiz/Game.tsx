...
export function AtomicChessQuizGame({ state, dispatch, onGameOver }: GameProps<AtomicChessQuizState, AtomicChessQuizSettings>): JSX.Element {
    ...
    <div className="atomchq-wrap">
      ...
      <div className="atomchq-choices">
        {q.choices.map((choice, i) => {
          ...
          return <button key={i} className={cls} disabled={isResult} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", choice:i } as AtomicChessQuizAction)}><span className="atomchq-choice-letter">{LABELS[i]}</span>{choice}</button>;
        })}
      </div>
