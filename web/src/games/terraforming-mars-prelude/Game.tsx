...
export function TerraformingMarsPreludeGame({ state, dispatch, onGameOver }: GameProps<TerraformingMarsPreludeState, TerraformingMarsPreludeSettings>): JSX.Element {
  ...
  return (
    <div className="dr-wrap">
      <h3 className="dr-title">Terraforming Mars: Prelude</h3>
      ...
      {state.phase === "drafting" && (
        <>
          ...
          <div className="dr-offer">
            {state.offer.map((c, i) => (
              <button key={i} className="dr-card" onClick={() => dispatch({ type: "pick", idx: i } as TerraformingMarsPreludeAction)}>
      ...
      {state.phase === "round-done" && (
        <div className="dr-event">
          ...
          <button onClick={() => dispatch({ type: "next" } as TerraformingMarsPreludeAction)}>Next Round</button>
        </div>
      )}
      {state.phase === "done" && (
