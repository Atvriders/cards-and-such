...
export const SUIT_NAMES: readonly string[] = ["Steel","Plant","Energy","Heat"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface TerraformingMarsPreludeSettings { dummy: boolean; }
export interface TerraformingMarsPreludeState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type TerraformingMarsPreludeAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: TerraformingMarsPreludeSettings): TerraformingMarsPreludeState {
...
export function reducer(state: TerraformingMarsPreludeState, action: TerraformingMarsPreludeAction): TerraformingMarsPreludeState {
...
export function score(s: TerraformingMarsPreludeState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: TerraformingMarsPreludeState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
