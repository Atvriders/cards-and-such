...
export const SUIT_NAMES = ["Building","Space","Microbe","Plant","Energy","Earth"] as const;
...
export type TerraformingMarsPreludeState = DraftState;
export type TerraformingMarsPreludeAction = DraftAction;
...
export function initialState(seed: number, _settings: TerraformingMarsPreludeSettings): TerraformingMarsPreludeState {
...
export function score(state: TerraformingMarsPreludeState): number {
  return draftFinalScore(state);
}

export function isTerminal(state: TerraformingMarsPreludeState): { score: number } | null {
