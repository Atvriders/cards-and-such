...
export interface SuperHoldemSettings { dummy: boolean; }
export interface SuperHoldemState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type SuperHoldemAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: SuperHoldemSettings): SuperHoldemState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: SuperHoldemState, action: SuperHoldemAction): SuperHoldemState {
...
export function isTerminal(state: SuperHoldemState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
