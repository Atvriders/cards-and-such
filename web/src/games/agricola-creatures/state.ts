...
export interface AgricolaCreaturesSettings { dummy: boolean; }
export interface AgricolaCreaturesState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type AgricolaCreaturesAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: AgricolaCreaturesSettings): AgricolaCreaturesState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Sheep multiply","Cattle market dips","Goats stolen","Wool prices rise","Pig fair good","Pasture dries","Predator scare","Bumper crop"];

export function reducer(state: AgricolaCreaturesState, action: AgricolaCreaturesAction): AgricolaCreaturesState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Livestock (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (cash >= HIRE_COST) {
        cash -= HIRE_COST; workers += 1; evt = "Hired Shepherd (-$" + HIRE_COST + ")"; delta = -HIRE_COST;
      } else { evt = "Not enough cash to hire"; }
    } else if (action.type === "trade") {
      if (assets >= 1) {
        const market = 30 + Math.floor(r * 20);
        cash += market; assets -= 1; evt = "Sold 1 Livestock (+$" + market + ")"; delta = market;
      } else { evt = "No Livestock to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
    ...
    if (pay > 0) evt += " | Shepherd earned +$" + pay;
...
export function score(s: AgricolaCreaturesState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: AgricolaCreaturesState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
