...
export function AsianCuisineQuiz({ state, dispatch, onGameOver }: GameProps<AsianCuisineQuizState, AsianCuisineQuizSettings>): JSX.Element {
      ...
      {entry && <>
        ...
        <div className="cq-choices">
          {entry.choices.map((c, i) => {
            ...
            return <button key={i} className={cls} disabled={state.selected !== null} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", index:i } as AsianCuisineQuizAction)}>{c}</button>;
          })}
        </div>
        {state.selected !== null && <button className="cq-next" onClick={() => dispatch({ type:"next" } as AsianCuisineQuizAction)}>{state.current + 1 < state.entries.length ? "Next" : "Finish"}</button>}
      </>}
    </div>
  );
}
