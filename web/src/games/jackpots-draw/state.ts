...
export interface JackpotsDrawSettings { dummy: boolean; }
export interface JackpotsDrawState { rngSeed: number; round: number; hand: number[]; phase: "deal" | "scored" | "done"; score: number; pts: number; result: string; rank: string; }
export type JackpotsDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: JackpotsDrawSettings): JackpotsDrawState {
  return { rngSeed: seed, round: 1, hand: [], phase: "deal", score: 0, pts: 0, result: "", rank: "" };
}
export function reducer(state: JackpotsDrawState, action: JackpotsDrawAction): JackpotsDrawState {
...
export function isTerminal(state: JackpotsDrawState): { score: number } | null {
