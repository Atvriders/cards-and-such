...
export function JackpotsDrawGame({ state, dispatch, onGameOver }: GameProps<JackpotsDrawState, JackpotsDrawSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-jackpots-draw-deal" onClick={() => dispatch({ type: "deal" } as JackpotsDrawAction)}>Deal</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" data-testid="hint-target-jackpots-draw-next" onClick={() => dispatch({ type: "next" } as JackpotsDrawAction)}>Next</button>
      </>}
    </div>
  );
}
