...
const MountainClimb = /* @__PURE__ */ lazy(() => import("./MountainClimb.js").then((mod) => ({ default: mod.MountainClimb as unknown as React.ComponentType<unknown> })));
...
Strategy tip: always look one row ahead before jumping up. Shift sideways first to align with a platform above rather than jumping blindly. Conserve lives by scouting the path before committing to each upward jump.`,
  ...
  reducer,
  isTerminal,
  hint: (s: any) => { const p = (s as any).phase; if (p === "gameover" || p === "done" || p === "ended" || (s as any).gameOver || (s as any).won || (s as any).isWon || (s as any).isComplete || (s as any).complete) return null; return { selector: '[data-testid="hint-target-mountain-climb-action"]', pulses: 3 }; },
