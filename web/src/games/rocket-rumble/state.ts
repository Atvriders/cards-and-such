...
export interface RocketRumbleSettings { dummy: boolean; }
...
export interface RocketRumbleState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type RocketRumbleAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: RocketRumbleSettings): RocketRumbleState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}

export function reducer(state: RocketRumbleState, action: RocketRumbleAction): RocketRumbleState {
...
export function isTerminal(state: RocketRumbleState): { score: number } | null {
