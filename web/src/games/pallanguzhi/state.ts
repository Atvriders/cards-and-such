...
export interface PallanguzhiSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface PallanguzhiState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type PallanguzhiAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: PallanguzhiSettings): PallanguzhiState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: PallanguzhiState, action: PallanguzhiAction): PallanguzhiState {
  ...
  if (moves >= MAX_MOVES) { /* placeholder */ }
  // CPU random move on remaining empty cells
...
export function isTerminal(state: PallanguzhiState): { score: number } | null {
