...
export interface CardEqualitySettings { dummy: boolean; }

export interface CardEqualityState {
  rngSeed: number;
  round: number;
  hand: number[];
  score: number;
...
export type CardEqualityAction = { type: "deal" } | { type: "next" };
...
function scoreHand(cards: number[]): number {
  ...
  return ranks.every(r => r === ranks[0]) ? 200 : 0;
}
...
export function initialState(seed: number, _settings: CardEqualitySettings): CardEqualityState {
  return { rngSeed: seed, round: 1, hand: [], score: 0, lastPts: 0, phase: "dealing" };
}

export function reducer(state: CardEqualityState, action: CardEqualityAction): CardEqualityState {
...
export function isTerminal(state: CardEqualityState): { score: number } | null {
