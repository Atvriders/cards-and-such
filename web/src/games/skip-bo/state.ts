...
export interface SkipBoSettings { dummy: boolean; }
export interface SkipBoState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type SkipBoAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: SkipBoSettings): SkipBoState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: SkipBoState, action: SkipBoAction): SkipBoState {
...
export function isTerminal(state: SkipBoState): { score: number } | null {
