...
export function SkipBoGame({ state, dispatch, onGameOver }: GameProps<SkipBoState, SkipBoSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="skip-bo-wrap"><div className="skip-bo-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="skip-bo-final">{state.score} pts</div></div></div>;
  return (
    <div className="skip-bo-wrap">
