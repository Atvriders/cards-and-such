...
export interface LazyPineappleSettings { dummy: boolean; }
export interface LazyPineappleState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type LazyPineappleAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: LazyPineappleSettings): LazyPineappleState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: LazyPineappleState, action: LazyPineappleAction): LazyPineappleState {
...
export function isTerminal(state: LazyPineappleState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
