...
export function SwingBatGame({ state, dispatch, onGameOver }: GameProps<SwingBatState, SwingBatSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="arc-wrap fade-in"><div className="arc-done bounce-in"><h2>Done!</h2><p>Score: {state.score}</p></div></div>;
  return (
    <div className="arc-wrap fade-in">
      ...
      {state.phase==="aiming" && <>
        ...
        <button data-testid="hint-target-swing-bat-action" className="arc-btn" onClick={()=>dispatch({type:"throw"} as SwingBatAction)}>Go!</button>
      </>}
