...
const KaleKombatGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.KaleKombatGame as unknown as React.ComponentType<unknown> })));
...
export const kaleKombatPlugin: GamePlugin<KaleKombatState, KaleKombatAction, typeof settings> = {
  ...
  reducer,isTerminal,
  hint: (state: KaleKombatState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.items || state.items.length === 0) return null;
    return { selector: '[data-testid="hint-target-kale-kombat-target"]', pulses: 3 };
  },
  component:KaleKombatGame,
};
