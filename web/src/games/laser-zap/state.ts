...
// Laser Zap: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface LaserZapSettings { dummy: boolean; }

export interface LaserZapTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface LaserZapState {
  rngSeed: number;
  targets: LaserZapTarget[];
  nextId: number;
...
export type LaserZapAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: LaserZapSettings): LaserZapState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LaserZapState, action: LaserZapAction): LaserZapState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: LaserZapTarget[] = [];
...
export function isTerminal(state: LaserZapState): { score: number } | null {
