...
export function SentinelsEnvironmentGame({ state, dispatch, onGameOver }: GameProps<SentinelsEnvironmentState, SentinelsEnvironmentSettings>): JSX.Element {
  ...
  const theme = "Environment Battle";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as SentinelsEnvironmentAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as SentinelsEnvironmentAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
