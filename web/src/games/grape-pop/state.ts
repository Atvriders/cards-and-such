...
export interface GrapePopSettings { dummy: boolean; }
...
export interface GrapePopState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type GrapePopAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: GrapePopSettings): GrapePopState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: GrapePopState, action: GrapePopAction): GrapePopState {
...
export function isTerminal(state: GrapePopState): { score: number } | null {
