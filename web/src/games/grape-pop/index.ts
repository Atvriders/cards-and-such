...
const GrapePopGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.GrapePopGame as unknown as React.ComponentType<unknown> })));
...
Pop those grapes and rack up the points!`,
  ...
  reducer, isTerminal,
  hint: (state: GrapePopState): HintTarget | null => {
    if (state.phase === "done") return null;
    if (!state.targets || state.targets.length === 0) return null;
    return { selector: '[data-testid="hint-target-grape-pop-target"]', pulses: 3 };
  },
  component: GrapePopGame,
};
