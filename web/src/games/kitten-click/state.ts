...
export interface KittenClickSettings { dummy: boolean; }
export interface KittenClickTarget { id: number; lane: number; ticksLeft: number; }
export interface KittenClickState {
  rngSeed: number;
  targets: KittenClickTarget[];
  nextId: number;
...
export type KittenClickAction = { type: "tick" } | { type: "pop"; id: number };
export function initialState(seed: number, _s: KittenClickSettings): KittenClickState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}
export function reducer(state: KittenClickState, action: KittenClickAction): KittenClickState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: KittenClickTarget[] = [];
...
export function isTerminal(state: KittenClickState): { score: number } | null {
