...
export function KittenClickGame({ state, dispatch, onGameOver }: GameProps<KittenClickState, KittenClickSettings>): JSX.Element {
  ...
  if (state.phase === "done") {
    return <div className="fc-wrap"><div className="fc-done"><h2>Time's Up!</h2><div>Clicked: {state.popped} / Missed: {state.missed}</div><div className="fc-final">{state.score} pts</div></div></div>;
  }
  return (
    <div className="fc-wrap">
      <div className="fc-header">
        <span className="fc-info">Clicked: {state.popped}</span>
