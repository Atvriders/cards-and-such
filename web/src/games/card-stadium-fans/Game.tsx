...
import "./Game.css";
export function CardStadiumFansGame({ state, dispatch, onGameOver }: GameProps<CardStadiumFansState, CardStadiumFansSettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      <div className="cm-info">🏟️ Draw {state.drawCount} / {TOTAL_DRAWS}</div>
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as CardStadiumFansAction)}>{state.drawCount >= TOTAL_DRAWS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
