...
export interface CardStadiumFansSettings { dummy: boolean; }
export interface CardStadiumFansState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardStadiumFansAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardStadiumFansSettings): CardStadiumFansState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardStadiumFansState, action: CardStadiumFansAction): CardStadiumFansState {
...
export function isTerminal(state: CardStadiumFansState): { score: number } | null {
