...
export interface CardPharmacyMiniSettings { dummy: boolean; }
export interface CardPharmacyMiniState {
  rngSeed: number;
  drawCount: number;
  card: number | null;
  lastPts: number;
...
export type CardPharmacyMiniAction = { type: "draw" } | { type: "next" };
...
export function initialState(seed: number, _settings: CardPharmacyMiniSettings): CardPharmacyMiniState {
  return { rngSeed: seed, drawCount: 0, card: null, lastPts: 0, score: 0, history: [], phase: "drawing" };
}
export function reducer(state: CardPharmacyMiniState, action: CardPharmacyMiniAction): CardPharmacyMiniState {
  ...
  if (action.type === "draw") {
    ...
    if (state.history.length > 0 && suitOf(state.history[state.history.length - 1]!) === suitOf(card)) pts += 5;
...
export function isTerminal(state: CardPharmacyMiniState): { score: number } | null {
