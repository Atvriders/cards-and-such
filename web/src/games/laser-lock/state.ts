...
export interface LaserLockSettings { dummy: boolean; }
...
export interface LaserLockState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type LaserLockAction =
  | { type: "tick" }
  | { type: "hit"; id: number };

export function initialState(seed: number, _settings: LaserLockSettings): LaserLockState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, hits: 0, missed: 0, phase: "playing" };
}

export function reducer(state: LaserLockState, action: LaserLockAction): LaserLockState {
...
export function isTerminal(state: LaserLockState): { score: number } | null {
