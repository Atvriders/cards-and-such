...
export interface CribbageSkunkedRubberSettings { dummy: boolean; }

export interface CribbageSkunkedRubberState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type CribbageSkunkedRubberAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: CribbageSkunkedRubberSettings): CribbageSkunkedRubberState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: CribbageSkunkedRubberState, action: CribbageSkunkedRubberAction): CribbageSkunkedRubberState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<CribbageSkunkedRubberState> = {};
...
export function isTerminal(state: CribbageSkunkedRubberState): { score: number } | null {
