...
export interface KuhnPokerSettings { dummy: boolean; }
export interface KuhnPokerState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type KuhnPokerAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: KuhnPokerSettings): KuhnPokerState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: KuhnPokerState, action: KuhnPokerAction): KuhnPokerState {
...
export function isTerminal(state: KuhnPokerState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
