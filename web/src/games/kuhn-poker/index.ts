...
const KuhnPokerGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.KuhnPokerGame as unknown as React.ComponentType<unknown> })));
...
export const kuhnPokerPlugin: GamePlugin<KuhnPokerState, KuhnPokerAction, typeof settings> = {
  ...
  reducer, isTerminal,   hint: (state: KuhnPokerState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-kuhn-poker-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-kuhn-poker-next"]', pulses: 3 };
    return null;
  },
  component:KuhnPokerGame,
};
