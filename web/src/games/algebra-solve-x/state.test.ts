...
describe("AlgebraSolveX initialState", () => {
  ...
  it("same seed is deterministic", () => {
    const s1 = initialState(99, easy10);
    const s2 = initialState(99, easy10);
    expect(s1.questions[0]!.x).toBe(s2.questions[0]!.x);
  });

  it("solutions satisfy ax + b = c", () => {
    const s = initialState(5, hard20);
    for (const q of s.questions) {
      expect(q.a * q.x + q.b).toBe(q.c);
    }
  });
});

describe("AlgebraSolveX reducer", () => {
  ...
  it("wrong answer scores 0", () => {
    const s = initialState(1, easy10);
    const wrong = s.questions[0]!.x + 999;
...
describe("AlgebraSolveX isTerminal", () => {
  ...
  it("returns score when done", () => {
    ...
    for (let i = 0; i < 10; i++) {
      s = reducer(s, { type: "type", text: String(s.questions[i]!.x) });
