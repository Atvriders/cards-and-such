...
export interface DonutGrabSettings { duration: "20" | "30" | "45"; }
export interface FallingItem { id: number; x: number; y: number; speed: number; points: number; }
export interface DonutGrabState {
...
export type DonutGrabAction = { type: "catch"; id: number } | { type: "tick" } | { type: "spawn" };

export function initialState(seed: number, settings: DonutGrabSettings): DonutGrabState {
...
export function reducer(state: DonutGrabState, action: DonutGrabAction): DonutGrabState {
...
export function isTerminal(state: DonutGrabState): { score: number } | null {
