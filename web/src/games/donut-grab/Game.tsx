...
export function DonutGrab({ state, dispatch, onGameOver }: GameProps<DonutGrabState, DonutGrabSettings>): JSX.Element {
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => dispatch({ type: "tick" } as DonutGrabAction), 600);
    spawnRef.current = setInterval(() => dispatch({ type: "spawn" } as DonutGrabAction), 2000);
