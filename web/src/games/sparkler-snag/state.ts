...
export interface SparklerSnagSettings { dummy: boolean; }
...
export interface SparklerSnagState {
  rngSeed: number;
  targets: Target[];
  nextId: number;
...
export type SparklerSnagAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _s: SparklerSnagSettings): SparklerSnagState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: SparklerSnagState, action: SparklerSnagAction): SparklerSnagState {
...
export function isTerminal(state: SparklerSnagState): { score: number } | null {
