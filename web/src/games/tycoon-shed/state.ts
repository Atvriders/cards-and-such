...
export interface TycoonShedSettings { dummy: boolean; }
export interface TycoonShedState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type TycoonShedAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: TycoonShedSettings): TycoonShedState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: TycoonShedState, action: TycoonShedAction): TycoonShedState {
...
export function isTerminal(state: TycoonShedState): { score: number } | null {
