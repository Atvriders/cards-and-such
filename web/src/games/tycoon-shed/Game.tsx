...
export function TycoonShedGame({ state, dispatch, onGameOver }: GameProps<TycoonShedState, TycoonShedSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="dm-wrap tysh-shed"><div className="dm-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="dm-final">{state.score} pts</div></div></div>;
  return (
    <div className="dm-wrap tysh-shed">
