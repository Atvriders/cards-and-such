...
import "./Game.css";
export function GutsPokerGame({ state, dispatch, onGameOver }: GameProps<GutsPokerState, GutsPokerSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "deal" && <button className="dm-btn" data-testid="hint-target-guts-poker-deal" onClick={() => dispatch({ type: "deal" } as GutsPokerAction)}>Deal {CARDS_PER_HAND} cards</button>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.rank} — +{state.rankPts}</div>
        <button className="dm-btn alt" data-testid="hint-target-guts-poker-next" onClick={() => dispatch({ type: "next" } as GutsPokerAction)}>Next</button>
      </>}
    </div>
  );
}
