...
export const SUIT_NAMES: readonly string[] = ["Stonehenge","Manneken Pis","Abu Simbel","Great Wall"];

export interface DraftCard { id: number; rank: number; suit: number; }
export interface WonderPackExtraSettings { dummy: boolean; }
export interface WonderPackExtraState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type WonderPackExtraAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: WonderPackExtraSettings): WonderPackExtraState {
...
export function score(s: WonderPackExtraState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: WonderPackExtraState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
