...
export interface MenschSettings { dummy: boolean }
...
export function initialState(seed: number, _s: MenschSettings): MenschState {
  return {
    rngSeed: seed,
...
function checkWin(s: MenschState): MenschState {
  if (allHome(s.pPos)) {
...
function cpuTurn(state: MenschState): MenschState {
  let s = state;
...
export function legalMovesForP(state: MenschState): { pawnIdx: number; pips: number }[] {
...
export function reducer(state: MenschState, action: MenschAction): MenschState {
...
export function isTerminal(state: MenschState): { score: number } | null {
