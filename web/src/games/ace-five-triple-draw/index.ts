...
const AceFiveTripleDrawGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.AceFiveTripleDrawGame as unknown as React.ComponentType<unknown> })));
...
export const aceFiveTripleDrawPlugin: GamePlugin<AceFiveTripleDrawState, AceFiveTripleDrawAction, typeof settings> = {
  ...
  hint: (state: AceFiveTripleDrawState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "deal") return { selector: '[data-testid="hint-target-ace-five-triple-draw-deal"]', pulses: 3 };
    if (state.phase === "scored") return { selector: '[data-testid="hint-target-ace-five-triple-draw-next"]', pulses: 3 };
    return null;
  },
};
