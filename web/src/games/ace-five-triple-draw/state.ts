...
export interface AceFiveTripleDrawSettings { dummy: boolean; }
export interface AceFiveTripleDrawState { rngSeed: number; round: number; hand: number[]; rank: string; rankPts: number; score: number; phase: "deal" | "scored" | "done"; }
export type AceFiveTripleDrawAction = { type: "deal" } | { type: "next" };
...
export function initialState(seed: number, _s: AceFiveTripleDrawSettings): AceFiveTripleDrawState { return { rngSeed: seed, round: 1, hand: [], rank: "", rankPts: 0, score: 0, phase: "deal" }; }
export function reducer(state: AceFiveTripleDrawState, action: AceFiveTripleDrawAction): AceFiveTripleDrawState {
...
export function isTerminal(state: AceFiveTripleDrawState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
