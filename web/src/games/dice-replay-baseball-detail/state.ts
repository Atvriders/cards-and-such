...
export interface DiceReplayBaseballDetailSettings { dummy: boolean; }

export interface DiceReplayBaseballDetailState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceReplayBaseballDetailAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceReplayBaseballDetailSettings): DiceReplayBaseballDetailState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceReplayBaseballDetailState, action: DiceReplayBaseballDetailAction): DiceReplayBaseballDetailState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceReplayBaseballDetailState> = {};
...
export function isTerminal(state: DiceReplayBaseballDetailState): { score: number } | null {
