...
export interface FireflyFlashSettings { dummy: boolean; }
...
export interface FireflyFlashState {
  rngSeed: number;
  bugs: Bug[];
  nextId: number;
...
export type FireflyFlashAction = { type: "tick" } | { type: "pop"; id: number };

export function initialState(seed: number, _settings: FireflyFlashSettings): FireflyFlashState {
  return { rngSeed: seed, bugs: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: FireflyFlashState, action: FireflyFlashAction): FireflyFlashState {
...
export function isTerminal(state: FireflyFlashState): { score: number } | null {
