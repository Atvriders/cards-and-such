import {
  makeInitialState,
  reducer as engineReducer,
  isTerminal as engineTerminal,
  type MahjongState,
  type MahjongAction,
  type TilePos,
} from "../_shared/mahjongEngine.js";

export type MahjongPlumBlossomState = MahjongState;
export type MahjongPlumBlossomAction = MahjongAction;

// Plum blossom — 5 petals (each a 3-tile cluster) around a stacked center stamen
export const LAYOUT: TilePos[] = [
  // Top petal
  { row: 0, col: 4, layer: 0 },
  { row: 0, col: 5, layer: 0 },
  { row: 1, col: 4, layer: 0 },
  { row: 1, col: 5, layer: 0 },
  { row: 1, col: 3, layer: 0 },
  { row: 1, col: 6, layer: 0 },
  // Upper-right petal
  { row: 1, col: 7, layer: 0 },
  { row: 2, col: 7, layer: 0 },
  { row: 2, col: 8, layer: 0 },
  { row: 3, col: 7, layer: 0 },
  { row: 3, col: 8, layer: 0 },
  // Lower-right petal
  { row: 4, col: 7, layer: 0 },
  { row: 4, col: 8, layer: 0 },
  { row: 5, col: 6, layer: 0 },
  { row: 5, col: 7, layer: 0 },
  { row: 6, col: 6, layer: 0 },
  // Bottom petal
  { row: 6, col: 4, layer: 0 },
  { row: 6, col: 5, layer: 0 },
  { row: 7, col: 4, layer: 0 },
  { row: 7, col: 5, layer: 0 },
  // Lower-left petal
  { row: 6, col: 3, layer: 0 },
  { row: 5, col: 2, layer: 0 },
  { row: 5, col: 3, layer: 0 },
  { row: 6, col: 2, layer: 0 },
  // Upper-left petal
  { row: 4, col: 2, layer: 0 },
  { row: 3, col: 1, layer: 0 },
  { row: 3, col: 2, layer: 0 },
  { row: 2, col: 1, layer: 0 },
  { row: 2, col: 2, layer: 0 },
  { row: 1, col: 2, layer: 0 },
  // Center stamen — three-tier stack
  { row: 3, col: 4, layer: 0 },
  { row: 3, col: 5, layer: 0 },
  { row: 4, col: 4, layer: 0 },
  { row: 4, col: 5, layer: 0 },
  { row: 3, col: 4, layer: 1 },
  { row: 4, col: 5, layer: 1 },
  { row: 3, col: 4, layer: 2 },
  { row: 4, col: 5, layer: 2 },
];

export function initialState(seed: number, _settings: Record<string, never> = {}): MahjongState {
  void _settings;
  return makeInitialState(LAYOUT, seed);
}

export const reducer = engineReducer;
export const isTerminal = engineTerminal;
