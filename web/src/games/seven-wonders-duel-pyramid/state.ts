...
export interface SevenWondersDuelPyramidState {
  rngSeed: number;
  round: number;
  offer: DraftCard[];
  myTableau: DraftCard[];
...
export type SevenWondersDuelPyramidAction = { type: "pick"; idx: number } | { type: "next" };
...
export function initialState(seed: number, _s: SevenWondersDuelPyramidSettings): SevenWondersDuelPyramidState {
...
export function score(s: SevenWondersDuelPyramidState): number {
  const margin = s.myScore - s.cpuScore;
  return Math.max(0, s.myScore + (margin > 0 ? 25 : 0));
}
export function isTerminal(s: SevenWondersDuelPyramidState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
