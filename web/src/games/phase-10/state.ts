...
export interface Phase10Settings { dummy: boolean; }
export interface Phase10State { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type Phase10Action = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: Phase10Settings): Phase10State {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: Phase10State, action: Phase10Action): Phase10State {
...
export function isTerminal(state: Phase10State): { score: number } | null {
