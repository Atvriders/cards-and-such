...
export function SultanSoliGame({ state, dispatch, onGameOver }: GameProps<SultanSoliState, SultanSoliSettings>): JSX.Element {
  ...
  return (
    <div className="sol-wrap">
      ...
      <div className="sol-board">
        {state.hand.map((c, i) => (
          <button key={i} className="sol-card" onClick={() => dispatch({ type: "swap", index: i } as SultanSoliAction)}>
            {cardName(c)}
          </button>
        ))}
      </div>
