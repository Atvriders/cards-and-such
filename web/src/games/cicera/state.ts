...
export interface CiceraSettings { dummy: boolean; }
export interface CiceraState { rngSeed: number; round: number; you: number | null; cpu: number | null; phase: "ready" | "result" | "done"; wins: number; losses: number; ties: number; score: number; }
export type CiceraAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: CiceraSettings): CiceraState {
  return { rngSeed: seed, round: 1, you: null, cpu: null, phase: "ready", wins: 0, losses: 0, ties: 0, score: 0 };
}
export function reducer(state: CiceraState, action: CiceraAction): CiceraState {
...
export function isTerminal(state: CiceraState): { score: number } | null { return state.phase === "done" ? { score: state.score } : null; }
