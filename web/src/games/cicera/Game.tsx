...
import "./Game.css";
export function CiceraGame({ state, dispatch, onGameOver }: GameProps<CiceraState, CiceraSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "ready" && <button data-testid="hint-target-cicera-primary" className="dm-btn" onClick={() => dispatch({ type: "play" } as CiceraAction)}>Capture!</button>}
      {state.phase === "result" && state.you !== null && state.cpu !== null && <>
        ...
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as CiceraAction)}>Next</button>
      </>}
    </div>
  );
}
