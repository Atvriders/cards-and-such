...
export function CardGroceryGame({ state, dispatch, onGameOver }: GameProps<CardGroceryState, CardGrocerySettings>): JSX.Element {
  ...
  return (
    <div className="cm-wrap">
      ...
      {state.phase === "deal" && (
        <button data-testid="hint-target-card-grocery-primary" className="cm-btn" onClick={() => dispatch({ type:"deal" } as CardGroceryAction)}>Deal pair</button>
      )}
      {state.phase === "decide" && state.pair && (
        <>
          ...
          <div className="cm-row">
            <button className="cm-btn" onClick={() => dispatch({ type:"take" } as CardGroceryAction)}>Take</button>
            <button className="cm-btn alt" onClick={() => dispatch({ type:"skip" } as CardGroceryAction)}>Skip</button>
          </div>
        </>
      )}
