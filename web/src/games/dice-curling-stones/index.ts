...
const DiceCurlingStonesGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.DiceCurlingStonesGame as unknown as React.ComponentType<unknown> })));
...
export const diceCurlingStonesPlugin: GamePlugin<DiceCurlingStonesState, DiceCurlingStonesAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: DiceCurlingStonesState): HintTarget | null => {
    if (isTerminal(state)) return null;
    if (state.phase === "rolling") return { selector: '[data-testid="hint-target-dice-curling-stones-roll"]', pulses: 3 };
    if (state.phase === "rolled") return { selector: '[data-testid="hint-target-dice-curling-stones-next"]', pulses: 3 };
    return null;
  },
