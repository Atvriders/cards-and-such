...
export interface DiceCurlingStonesSettings { dummy: boolean; }

export interface DiceCurlingStonesState {
  rngSeed: number;
  round: number;
  dice: number[] | null;
  lastPts: number;
...
export type DiceCurlingStonesAction = { type: "roll" } | { type: "next" };

export function initialState(seed: number, _settings: DiceCurlingStonesSettings): DiceCurlingStonesState {
  return {
    rngSeed: seed,
    round: 1,
    dice: null,
...
export function reducer(state: DiceCurlingStonesState, action: DiceCurlingStonesAction): DiceCurlingStonesState {
  ...
  if (action.type === "roll") {
    ...
    let pts = 0;
    let logEntry = "";
    let extra: Partial<DiceCurlingStonesState> = {};
...
export function isTerminal(state: DiceCurlingStonesState): { score: number } | null {
