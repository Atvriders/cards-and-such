...
export function JapaneseCuisineQuiz({ state, dispatch, onGameOver }: GameProps<JapaneseCuisineQuizState, JapaneseCuisineQuizSettings>): JSX.Element {
      ...
      {entry && <>
        ...
        <div className="cq-choices">
          {entry.choices.map((c, i) => {
            ...
            return <button key={i} className={cls} disabled={state.selected !== null} data-testid={`hint-target-quiz-answer-${i}`} onClick={() => dispatch({ type:"select", index:i } as JapaneseCuisineQuizAction)}>{c}</button>;
          })}
        </div>
        {state.selected !== null && <button className="cq-next" onClick={() => dispatch({ type:"next" } as JapaneseCuisineQuizAction)}>{state.current + 1 < state.entries.length ? "Next" : "Finish"}</button>}
      </>}
    </div>
  );
}
