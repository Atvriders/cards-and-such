  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "play" && <div className="dm-row">
        <button className="dm-btn" onClick={() => dispatch({ type: "hit" } as DoubleAttackBjAction)}>Hit</button>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "stand" } as DoubleAttackBjAction)}>Stand</button>
      </div>}
      {state.phase === "scored" && <>
        <div className="dm-result">{state.result} — +{state.pts}</div>
        <button className="dm-btn alt" onClick={() => dispatch({ type: "next" } as DoubleAttackBjAction)}>Next</button>
      </>}
    </div>
  );
}
