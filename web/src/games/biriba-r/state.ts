...
export interface BiribaRSettings { dummy: boolean; }
export interface BiribaRState { rngSeed: number; round: number; hand: number[]; phase: "play" | "scored" | "done"; melds: number[][]; score: number; pts: number; deadwood: number; result: string; }
export type BiribaRAction = { type: "score" } | { type: "next" };
...
export function initialState(seed: number, _s: BiribaRSettings): BiribaRState {
...
export function reducer(state: BiribaRState, action: BiribaRAction): BiribaRState {
...
export function isTerminal(state: BiribaRState): { score: number } | null {
