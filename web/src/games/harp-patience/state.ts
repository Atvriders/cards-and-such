...
export interface HarpPatienceSettings { dummy: boolean; }

export interface HarpPatienceState {
  rngSeed: number;
  deck: number[];
  pos: number;
  hand: number[];
...
export type HarpPatienceAction =
  | { type: "keep" }
...
function scoreHand(h: number[]): number {
  let s = 0; const r = h.map(cardRank).sort((a,b)=>a-b); let asc=1, best=1; for (let i=1;i<r.length;i++){ const a=r[i] as number; const b=r[i-1] as number; if (a===b+1){asc++; if(asc>best)best=asc;} else if (a!==b) asc=1; } s += best*5; if (best>=4) s += 10; if (s===0) s=1; return s;
}
...
export function reducer(state: HarpPatienceState, action: HarpPatienceAction): HarpPatienceState {
...
export function isTerminal(state: HarpPatienceState): { score: number } | null {
