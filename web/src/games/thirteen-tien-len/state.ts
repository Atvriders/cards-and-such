...
export interface ThirteenTienLenSettings { dummy: boolean; }
export interface ThirteenTienLenState { rngSeed: number; round: number; you: number; cpu: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; }
export type ThirteenTienLenAction = { type: "play" } | { type: "next" };
...
export function initialState(seed: number, _s: ThirteenTienLenSettings): ThirteenTienLenState {
  return { rngSeed: seed, round: 1, you: 7, cpu: 7, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0 };
}
export function reducer(state: ThirteenTienLenState, action: ThirteenTienLenAction): ThirteenTienLenState {
...
export function isTerminal(state: ThirteenTienLenState): { score: number } | null {
