...
export function ThirteenTienLenGame({ state, dispatch, onGameOver }: GameProps<ThirteenTienLenState, ThirteenTienLenSettings>): JSX.Element {
  ...
  if (state.phase === "done") return <div className="thirteen-tien-len-wrap"><div className="thirteen-tien-len-done"><h2>Done!</h2><div>W: {state.wins} L: {state.losses}</div><div className="thirteen-tien-len-final">{state.score} pts</div></div></div>;
  return (
    <div className="thirteen-tien-len-wrap">
