...
export interface CardLighthouseSettings { dummy: boolean; }
export interface CardLighthouseState {
  rngSeed: number;
  round: number;
  cards: number[];
  picked: number | null;
...
export type CardLighthouseAction = { type: "pick"; index: number } | { type: "next" } | { type: "deal" };
...
export function initialState(seed: number, _settings: CardLighthouseSettings): CardLighthouseState {
...
export function scoreFor(c: number, _cards: number[], target: number): number {
  const suit = Math.floor(c / 13);
    const r = c % 13;
    if (suit !== target) return 3;
    return r + 5;
}
export function reducer(state: CardLighthouseState, action: CardLighthouseAction): CardLighthouseState {
...
export function isTerminal(state: CardLighthouseState): { score: number } | null {
