...
export function DiceBallroomGame({ state, dispatch, onGameOver }: GameProps<DiceBallroomState, DiceBallroomSettings>): JSX.Element {
  ...
  return (
    <div className="dm-wrap">
      ...
      {state.phase === "predict" && (
        <div className="dm-row">
          {OPTIONS.map(o => (
            <button data-testid="hint-target-dice-ballroom-roll" key={o.label} className="dm-btn" onClick={() => dispatch({ type:"predict", choice:o.label } as DiceBallroomAction)}>{o.label} (+{o.points})</button>
          ))}
        </div>
      )}
      {state.phase === "result" && (
        <>
          ...
          <button className="dm-btn alt" onClick={() => dispatch({ type:"next" } as DiceBallroomAction)}>Next</button>
        </>
      )}
    </div>
  );
