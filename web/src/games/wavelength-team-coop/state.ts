...
export interface wavelengthTeamCoopSettings { dummy: boolean; }
export interface wavelengthTeamCoopState {
  rngSeed: number;
  round: number;
  playerRoll: number;
  cpuRoll: number;
...
export type wavelengthTeamCoopAction = { type: "play" } | { type: "next" };
export function initialState(seed: number, _s: wavelengthTeamCoopSettings): wavelengthTeamCoopState {
  return { rngSeed: seed, round: 1, playerRoll: 0, cpuRoll: 0, lastPts: 0, teamScore: 0, phase: "ready" };
}
export function reducer(state: wavelengthTeamCoopState, action: wavelengthTeamCoopAction): wavelengthTeamCoopState {
...
export function isTerminal(state: wavelengthTeamCoopState): { score: number } | null {
