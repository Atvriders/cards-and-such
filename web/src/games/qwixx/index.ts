...
const Qwixx = /* @__PURE__ */ lazy(() => import("./Qwixx.js").then((mod) => ({ default: mod.Qwixx as unknown as React.ComponentType<unknown> })));
const qwixxSettings = {
  mode: {
    kind: "enum" as const,
    label: "Mode",
...
The game ends when two rows are locked or you collect 4 penalties (shown as X marks). Score each row using the triangular scoring table: 1 mark = 1 pt, 2 = 3, 3 = 6, 4 = 10, ... up to 66 pts for 11 marks. Each penalty subtracts 5 points. Maximize your score by planning ahead.`,
  ...
  hint: (state) => {
    ...
    if (phase === "rolling") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "rolling-dice") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "preRoll") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "ready") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "playerRoll") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "roll") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "play") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    if (phase === "playing") return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
    return { selector: '[data-testid="hint-target-qwixx-roll"]', pulses: 3 };
  },
  component: Qwixx,
};
