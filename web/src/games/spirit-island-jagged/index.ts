...
import { SpiritIslandJagged_CFG, initialState, reducer, isTerminal } from "./state.js";
import { coopHintSelector } from "../_shared/coop-engine.js";
const SpiritIslandJaggedGame = /* @__PURE__ */ lazy(() => import("./Game.js").then((mod) => ({ default: mod.SpiritIslandJaggedGame as unknown as React.ComponentType<unknown> })));
const settings = {
...
export const spiritIslandJaggedPlugin: GamePlugin<SpiritIslandJaggedState, SpiritIslandJaggedAction, typeof settings> = {
  ...
  reducer,
  isTerminal,
  hint: (state: SpiritIslandJaggedState): HintTarget | null => {
    const sel = coopHintSelector(state, SpiritIslandJagged_CFG);
    return sel ? { selector: sel, pulses: 3 } : null;
  },
  component: SpiritIslandJaggedGame,
};

export default spiritIslandJaggedPlugin;
