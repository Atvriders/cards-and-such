...
export function SpiritIslandJaggedGame({ state, dispatch, onGameOver }: GameProps<SpiritIslandJaggedState, SpiritIslandJaggedSettings>): JSX.Element {
  ...
  const theme = "Jagged Earth Spirits";
  if (state.phase === "done") {
  ...
  return (
    <div className="coop-wrap">
      ...
      {state.phase === "ready" && (
        <button className="coop-btn" onClick={() => dispatch({ type: "play" } as SpiritIslandJaggedAction)}>Play Round</button>
      )}
      {state.phase === "rolled" && (
        <>
          <div className="coop-result">+{state.lastPts} together</div>
          <button className="coop-btn alt" onClick={() => dispatch({ type: "next" } as SpiritIslandJaggedAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next Round"}</button>
        </>
      )}
    </div>
  );
