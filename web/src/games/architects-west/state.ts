...
export interface ArchitectsWestSettings { dummy: boolean; }
export interface ArchitectsWestState {
  rngSeed: number;
  turn: number;
  cash: number;
  assets: number;
...
export type ArchitectsWestAction = { type: "invest" } | { type: "save" } | { type: "hire" } | { type: "trade" } | { type: "next" };

export function initialState(seed: number, _s: ArchitectsWestSettings): ArchitectsWestState {
  return { rngSeed: seed, turn: 1, cash: 200, assets: 0, workers: 0, lastEvent: null, lastDelta: 0, phase: "choosing" };
}

const EVENT_LINES = ["Stone delivered","Apprentice hired","Gold sealed","Black market deal","Cathedral progress","Guard arrest","Patron tip","Virtue earned"];

export function reducer(state: ArchitectsWestState, action: ArchitectsWestAction): ArchitectsWestState {
  ...
  if (state.phase === "choosing") {
    ...
    if (action.type === "invest") {
      if (cash >= ASSET_COST) {
        cash -= ASSET_COST; assets += 1; evt = "Bought 1 Stone (-$" + ASSET_COST + ")"; delta = -ASSET_COST;
      ...
      if (assets >= 1) {
        ...
        cash += market; assets -= 1; evt = "Sold 1 Stone (+$" + market + ")"; delta = market;
      } else { evt = "No Stone to sell"; }
    } else { return state; }
    const div = assets * 8;
    cash += div;
...
export function score(s: ArchitectsWestState): number {
  const net = s.cash + s.assets * ASSET_COST + s.workers * HIRE_COST;
  return Math.max(0, net);
}
export function isTerminal(s: ArchitectsWestState): { score: number } | null { return s.phase === "done" ? { score: score(s) } : null; }
