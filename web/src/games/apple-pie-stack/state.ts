...
export interface ApplePieStackSettings { dummy: boolean; }
export interface ApplePieStackState { rngSeed: number; targets: number[]; roundIndex: number; score: number; phase: "aiming" | "result" | "done"; power: number; lastDiff: number; lastPts: number; }
export type ApplePieStackAction = { type: "setPower"; value: number } | { type: "throw" } | { type: "next" };

export function initialState(seed: number, _settings: ApplePieStackSettings): ApplePieStackState {
...
export function reducer(state: ApplePieStackState, action: ApplePieStackAction): ApplePieStackState {
...
export function isTerminal(state: ApplePieStackState): { score: number } | null {
