...
// Pill Drop Mini: 6x6 match-3, swap adjacent gems, 3+ in row clears, gravity, score, 60s timer.
...
export interface PillDropMiniSettings { dummy: boolean; }

export type Cell = number;
export type Grid = Cell[][];

export interface PillDropMiniState {
  rngSeed: number;
  grid: Grid;
...
export type PillDropMiniAction =
  | { type: "tick" }
...
export function initialState(seed: number, _settings: PillDropMiniSettings): PillDropMiniState {
...
export function reducer(state: PillDropMiniState, action: PillDropMiniAction): PillDropMiniState {
...
export function isTerminal(state: PillDropMiniState): { score: number } | null {
