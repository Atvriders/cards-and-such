...
export interface TrucSettings { dummy: boolean; }
export interface TrucState { rngSeed: number; round: number; phase: "ready" | "scored" | "done"; score: number; pts: number; result: string; wins: number; losses: number; tricksWon: number; tricksLost: number; }
export type TrucAction = { type: "play" } | { type: "next" };

export function initialState(seed: number, _s: TrucSettings): TrucState {
  return { rngSeed: seed, round: 1, phase: "ready", score: 0, pts: 0, result: "", wins: 0, losses: 0, tricksWon: 0, tricksLost: 0 };
}
export function reducer(state: TrucState, action: TrucAction): TrucState {
...
export function isTerminal(state: TrucState): { score: number } | null {
