...
export interface ZammaSettings { dummy: boolean; }
export type Cell = "P" | "C" | null;

export interface ZammaState {
  rngSeed: number;
  board: Cell[];
  turn: "P" | "C";
  moves: number;
...
export type ZammaAction = { type: "place"; idx: number };

export function initialState(seed: number, _settings: ZammaSettings): ZammaState {
  return { rngSeed: seed, board: Array(SIZE * SIZE).fill(null), turn: "P", moves: 0, result: null, score: 0, phase: "playing" };
}

export function reducer(state: ZammaState, action: ZammaAction): ZammaState {
...
export function isTerminal(state: ZammaState): { score: number } | null {
