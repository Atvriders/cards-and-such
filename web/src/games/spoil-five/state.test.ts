...
describe("spoil-five initialState", () => {
...
describe("spoil-five legalPlays", () => {
  it("all cards legal when leading", () => {
    const s = initialState(42);
    expect(legalPlays(s.playerHand, s.currentTrick).length).toBeGreaterThanOrEqual(5);
  });
});

describe("spoil-five reducer", () => {
...
describe("spoil-five full game", () => {
  it("completes a full game", () => {
    ...
    expect(s.playerTricks + s.botTricks).toBeGreaterThanOrEqual(2);
  });
});
