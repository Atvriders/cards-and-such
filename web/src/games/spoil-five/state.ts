// Spoil Five — Irish trick game where five tricks decides the round.
...
export type SpoilFivePhase = "playing" | "done";

export interface SpoilFiveState {
  rngSeed: number;
...
export type SpoilFiveAction = { type: "play"; cardId: string };
...
function buildDeck(): Card[] {
  ...
  for (const s of SUITS) for (const r of RANKS) {
    cards.push({ suit: s, rank: r, id: `spoil-five-${s}${r}` });
  }
  return cards;
}
...
function applyTrick(state: SpoilFiveState, trick: readonly { seat: number; card: Card }[]): SpoilFiveState {
...
export function reducer(state: SpoilFiveState, action: SpoilFiveAction): SpoilFiveState {
  ...
  let s: SpoilFiveState = { ...state, rngSeed: nextSeed };
  ...
  if (s.playerLeads) {
    ...
    let next: SpoilFiveState = { ...s, playerHand: newPlayerHand, botHand: newBotHand, currentTrick: trick };
    ...
    let next: SpoilFiveState = { ...s, playerHand: newPlayerHand, currentTrick: trick };
    next = applyTrick(next, trick);
    if (next.phase === "playing" && !next.playerLeads) {
      const lead = botChoose(next.botHand, [], rng);
      const reducedBot = next.botHand.filter(c => c.id !== lead.id);
...
export function initialState(seed: number): SpoilFiveState {
...
export function isTerminal(state: SpoilFiveState): { score: number } | null {
