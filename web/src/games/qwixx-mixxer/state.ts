...
export interface QwixxMixxerSettings { rounds: "10"; }

export interface QwixxMixxerState {
  rngSeed: number;
  score: number;
  round: number;
  maxRounds: number;
...
export type QwixxMixxerAction = { type: "roll" } | { type: "next" };
...
export function initialState(seed: number, _settings: QwixxMixxerSettings): QwixxMixxerState {
  return {
    rngSeed: seed >>> 0,
    score: 0,
    round: 1,
...
export function reducer(state: QwixxMixxerState, action: QwixxMixxerAction): QwixxMixxerState {
...
export function isTerminal(state: QwixxMixxerState): { score: number } | null {
