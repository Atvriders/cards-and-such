...
// Coin Grab: simple 30-second clicker.
export const TIMER_TICKS = 30;
export const LANES = 6;

export interface CoinGrabSettings { dummy: boolean; }

export interface CoinGrabTarget {
  id: number;
  lane: number;
  ticksLeft: number;
}

export interface CoinGrabState {
  rngSeed: number;
  targets: CoinGrabTarget[];
  nextId: number;
...
export type CoinGrabAction =
  | { type: "tick" }
  | { type: "pop"; id: number };

export function initialState(seed: number, _settings: CoinGrabSettings): CoinGrabState {
  return { rngSeed: seed, targets: [], nextId: 1, ticksRemaining: TIMER_TICKS, score: 0, popped: 0, missed: 0, phase: "playing" };
}

export function reducer(state: CoinGrabState, action: CoinGrabAction): CoinGrabState {
  ...
  if (action.type === "tick") {
    ...
    const newTargets: CoinGrabTarget[] = [];
...
export function isTerminal(state: CoinGrabState): { score: number } | null {
