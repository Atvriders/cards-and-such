...
import "./Game.css";
export function HighFiveCardsGame({ state, dispatch, onGameOver }: GameProps<HighFiveCardsState, HighFiveCardsSettings>): JSX.Element {
    ...
    <div className="cm-wrap">
      ...
      {state.phase === "scored" && (
        <>
          ...
          <button className="cm-btn alt" onClick={() => dispatch({ type:"next" } as HighFiveCardsAction)}>{state.round >= TOTAL_ROUNDS ? "Finish" : "Next"}</button>
        </>
      )}
    </div>
  );
