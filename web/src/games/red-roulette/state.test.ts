...
const S = { dummy: false };
describe("RedRoulette", () => {
  it("starts in predict phase", () => { const s = initialState(1, S); expect(s.phase).toBe("predict"); });
  it("predict reveals card and produces result/done", () => { const s = reducer(initialState(1, S), { type:"predict", choice:"red" }); expect(s.card).not.toBeNull(); expect(["result","done"]).toContain(s.phase); });
