...
/**
 * W2473 — the `chip-board` button (the per-category board chip in the
 ...
 *    a category chip (e.g. the board chip becomes a popover trigger
 *    with its own JSX) could silently drop the attribute for just that
 *    chip without breaking W1258.
 *  - W2467 (LobbyChipArcadeType.test.tsx) pins the same attribute on
 *    `chip-arcade` only — the analogous coverage exists for the arcade
 *    category chip but not for the board category chip.
 *  - W1962 (LobbyChipTag.test.tsx) pins `tagName === "BUTTON"` for every
 *    chip including `chip-board`, but a `<button>` without an explicit
 *    `type` attribute still satisfies that assertion while defaulting
 *    to "submit" inside a form.
 *  - W1433 (LobbyChipBoardBadge.test.tsx) and W1499
 *    (LobbyChipBoardGlyphAria.test.tsx) pin the badge count and the
 *    glyph span respectively for `chip-board`, but neither reads the
 ...
 * or extending W1258 / W2467) mirrors the W2467 / W2313 / W1408 pattern
 * (per-chip / per-arrow `type` pins) so this shares the
 * `src/pages/Lobby` vitest path filter without colliding with
 * concurrent edits to either mega-file.
 */
describe("LobbyPage — chip-board type attribute (W2473)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders chip-board with an explicit type=\"button\" attribute", () => {
    render(
