...
/**
 * W2620: StatsPage's "Personal records" h2 — the section heading inside
 ...
 * StatsPage.tsx) — is intentionally rendered bare with NO explicit `role`
 * attribute. Assistive tech relies solely on the implicit `heading` role
 * with `aria-level=2` derived from the literal `<h2>` tag.
 *
 ...
 * `role` attribute on the Personal records h2 itself. A refactor that
 * added e.g. `role="heading"` (a redundant duplicate of the implicit
 * role the `<h2>` tag already provides) or — far worse — `role="none"`
 * / `role="presentation"` (which would silently strip the heading from
 * the accessibility tree without altering the visible "Personal records"
 ...
 * established W2491 (StatsActivityH2NoRole) NoRole pattern so the test
 * shares the `src/pages/StatsPersonalRecords` vitest path filter without
 * colliding with concurrent edits to the mega-file.
 */
describe("StatsPage stats-personal-records — h2 role attribute absence", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("W2620: stats-personal-records 'Personal records' h2 has no role attribute", () => {
    render(
      <MemoryRouter>
        <ConfirmProvider>
          <StatsPage />
