...
/**
 * W2683 — the LobbyPage `chip-recently-played` Chip <button> MUST NOT
 * carry a `form` attribute. The Chip helper (LobbyPage.tsx) renders its
 ...
 * OTHER attribute absences but NOT `form`:
 ...
 * (or any other value) to the chip-recently-played <button>. This file
 * closes that gap, mirroring W2677's `chip-arcade` form-absence pin
 * and W2675's `chip-board` form-absence pin.
 *
 ...
 * changes.
 */
describe("LobbyPage — chip-recently-played button has no form attribute (W2683)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-recently-played <button> does NOT carry a form attribute", () => {
    render(
    ...
    // textarea, etc.); if a future refactor moved the testid down onto
    // an inner span the rest of this assertion would be meaningless, so
    // guard the tagName explicitly.
    expect(chip.tagName).toBe("BUTTON");

    // The actual contract: no `form` attribute on chip-recently-played.
    // Use `hasAttribute` rather than `getAttribute(...)` truthiness — a
