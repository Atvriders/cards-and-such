...
/**
 * W2619 — the `chip-favorites` button (the per-filter favorites chip in
 ...
 * (pinned by W2... in LobbyChipFavoritesRole.test.tsx) inside a
 * `role="tablist"` chip-strip, and toggling it filters the lobby grid
 * to the user's favorited tiles. There is no LobbyPage state in which
 * the favorites chip becomes disabled — even an empty favorites set
 * leaves the chip clickable (it just renders an empty grid). Selection
 * is communicated via `aria-pressed` / `aria-selected`, NOT by
 * `aria-disabled`.
 *
 ...
 *    on the `chip-favorites` button specifically. Sibling files such as
 *    LobbyChipFavoritesRole.test.tsx,
 *    LobbyChipFavoritesType.test.tsx,
 *    LobbyChipFavoritesNoAriaControls.test.tsx,
 *    LobbyChipFavoritesNoAriaLabel.test.tsx,
 *    LobbyChipFavoritesNoId.test.tsx,
 *    LobbyChipFavoritesNoStyle.test.tsx and
 *    LobbyChipFavoritesGlyphAria.test.tsx each pin a different ARIA /
 *    DOM attribute on the chip-favorites button — none read the
 *    `aria-disabled` slot.
 ...
 *    chip-favorites, so a branch that special-cases the favorites chip
 *    with an `aria-disabled` (e.g. when the favorites set is empty)
 *    would slip past every existing pin.
 *
 * We resolve the chip via its stable `data-testid="chip-favorites"`
 * (the testId wired at LobbyPage.tsx via `testId={`chip-${filter}`}`)
 ...
 * or extending an existing chip-favorites test) mirrors the per-surface-
 * attribute pattern used by LobbyChipArcadeNoAriaDisabled.test.tsx
 * (W2617), LobbyChipBoardNoAriaDisabled.test.tsx (W2616) and
 * LobbyChipCardsNoAriaDisabled.test.tsx (W2615) so this shares the
 * `src/pages/Lobby` vitest path filter without colliding with concurrent
 * edits to either mega-file.
 */
describe("LobbyPage — chip-favorites has no aria-disabled attribute (W2619)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders chip-favorites without an aria-disabled attribute (chip is always interactive)", () => {
    render(
