...
/**
 * W2679 — the LobbyPage `chip-favorites` Chip <button> MUST NOT carry a
 ...
 * closes that gap, mirroring the W2675 pin already in place for
 * `chip-board` and the analogous pins for `chip-cards`.
 *
 ...
 * changes.
 */
describe("LobbyPage — chip-favorites button has no form attribute (W2679)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-favorites <button> does NOT carry a form attribute", () => {
    render(
    ...
    // an inner span the rest of this assertion would be meaningless,
    // so guard the tagName explicitly.
    expect(chip.tagName).toBe("BUTTON");

    // The actual contract: no `form` attribute on chip-favorites. Use
