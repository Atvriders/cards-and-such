...
/**
 * W2690 — the LobbyPage `chip-dice` Chip <button> MUST NOT carry a
 ...
 *   - LobbyChipDiceNoStyle pins inline-style absence.
 ...
 *   - LobbyChipDiceNoAutofocus pins `autofocus` absence.
 *
 * None of those would catch a regression that added `form="search"`
 * (or any other value) to the chip-dice <button>. This file closes
 * that gap, mirroring W2677's `chip-arcade` form-absence pin.
 *
 * Resolves the chip via its stable `data-testid="chip-dice"` so the
 * assertion is locale-independent and immune to translation-key
 * changes.
 */
describe("LobbyPage — chip-dice button has no form attribute (W2690)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-dice <button> does NOT carry a form attribute", () => {
    render(
    ...
    // The actual contract: no `form` attribute on chip-dice. Use
