...
/**
 * W3261 — the inner chip-strip track `.lobby-chips` (the
 * `<div role="tablist">` filter rail rendered inside LobbyPage.tsx)
 * carries NO `ontransitionstart` inline event handler attribute.
 *
 ...
 *     >
 *
 * `ontransitionstart` is a legacy inline DOM event handler attribute
 * that fires when a CSS transition begins on the element. Authoring
 * it as an HTML attribute is wrong on the chip strip because:
 ...
 *     them, so a stray `ontransitionstart="..."` becomes a runtime
 ...
 *    cover `ontransitionstart`. A regression that added
 *    `ontransitionstart="..."` (e.g. by mistakenly inlining a CSS
 *    transition listener as an HTML attribute) would slip past
 *    every existing pin.
 *
 * The pin: `track.hasAttribute("ontransitionstart") === false` and
 * `track.getAttribute("ontransitionstart") === null`. Both forms are
 ...
 */
describe("LobbyPage — .lobby-chips tablist has no ontransitionstart attribute (W3261)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders the inner .lobby-chips <div role=\"tablist\"> WITHOUT an ontransitionstart attribute", () => {
    render(
    ...
    // The pin: NO ontransitionstart attribute is authored on the
    // chip strip. A regression that adds `ontransitionstart=""`,
    // `ontransitionstart="handler()"`, or any other inline event
    // handler string would fail here.
    expect(track!.hasAttribute("ontransitionstart")).toBe(false);
    expect(track!.getAttribute("ontransitionstart")).toBe(null);
  });
});
