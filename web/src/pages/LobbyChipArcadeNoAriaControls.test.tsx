...
/**
 * W2581 — the LobbyPage `chip-arcade` Chip <button> MUST NOT carry an
 ...
 * codebase:
 *
 *   1. There is no single tabpanel id that the arcade chip "owns" — the
 ...
 *   - W2467 / LobbyChipArcadeType pins `type="button"`.
 *   - W2482 / LobbyChipArcadeNoId pins `id`-absence.
 *   - W1962 family / LobbyChipArcadeRole pins `role="tab"`.
 *   - LobbyChipArcadeAriaSelectedDefault / AriaPressedDefault pin the
 *     boolean ARIA state attrs in their default (inactive) form.
 *
 * None of those would catch a regression that added
 * `aria-controls={someId}` to the per-category chip wrapper — e.g. via
 * the inline `CATEGORY_ORDER.map` at LobbyPage.tsx ~L1964 forwarding an
 ...
 * changes.
 */
describe("LobbyPage — chip-arcade button has no aria-controls attribute (W2581)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-arcade <button> does NOT carry an aria-controls attribute", () => {
    render(
    ...
    // and not a child span. Going through `data-testid="chip-arcade"`
    ...
    // The actual contract: no `aria-controls` attribute on chip-arcade.
