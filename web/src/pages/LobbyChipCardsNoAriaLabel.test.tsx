...
/**
 * W2595 — the `chip-cards` button (the per-category cards chip in the
 ...
 *    on the `chip-cards` button specifically. Sibling files such as
 ...
 *    LobbyChipCardsNoId.test.tsx and LobbyChipCardsNoStyle.test.tsx
 *    all fetch the chip via `data-testid="chip-cards"` but each pins
 *    a different attribute — none read the `aria-label` slot.
 *  - LobbyChipBoardNoAriaLabel.test.tsx (W2594) pins the same
 *    attribute absence on the sibling chip-board chip but does not
 *    cover chip-cards, so a branch that special-cases the cards chip
 *    with an `aria-label` would slip past every existing pin.
 *  - LobbyH1NoAriaLabel.test.tsx pins the same attribute absence on
 *    a different surface (the H1), again leaving chip-cards exposed.
 *
 ...
 * or extending an existing chip-cards test) mirrors the W2594
 ...
 */
describe("LobbyPage — chip-cards has no aria-label attribute (W2595)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders chip-cards without an aria-label attribute (visible text supplies the accessible name)", () => {
    render(
