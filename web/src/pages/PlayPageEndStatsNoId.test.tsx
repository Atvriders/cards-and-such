/**
 * Pinpoint test for the PlayPage end-banner end-stats `<dl>` `id`
 * attribute absence (W2031).
 *
 * Observable behavior:
 ...
 *   located in tests, CSS, and DOM queries via the `.end-stats`
 *   className and the `data-testid="end-stats"` attribute — never via
 *   a DOM `id`. Adding an `id` would change CSS targeting surface,
 *   `document.getElementById` discoverability, and hash-link / aria
 *   anchor semantics; the end-stats `<dl>` is currently not intended
 *   to act as such an anchor.
 *
 ...
 *   `id="end-stats"` (or a stylesheet-driven id graft) could land
 *   silently. This test fills the gap with the minimum surface:
 *   render → drive a win → grab `[data-testid="end-stats"]` and
 *   assert `dl.hasAttribute("id") === false`. `hasAttribute` returns
 *   true for any string value (including ""), so this fails loudly
 *   the moment the wrapper grows any kind of DOM id.
 *
 * Strategy mirrors W1907 (PlayPageEndStatsContainerClass.test.tsx):
 ...
 *     `data-testid="end-stats"`, and assert `hasAttribute("id")` is
 *     false on that node.
 */
...
const hoisted = vi.hoisted(() => {
  const TEST_GAME_ID = "end-stats-no-id-fixture";
  type S = { moves: number };
  type Action = { type: string };
  const fixturePlugin = {
    id: TEST_GAME_ID,
    title: "End Stats No Id Fixture",
    category: "cards" as const,
    players: { min: 1, max: 1, multiplayer: false },
    description:
      "Test-only plugin for the end-stats <dl> id-attribute absence test.",
...
describe("PlayPage end-stats <dl> id-attribute absence (W2031)", () => {
  it("renders the end-stats definition list with no id attribute", async () => {
    ...
    expect(endStats.hasAttribute("id")).toBe(false);
  });
});
