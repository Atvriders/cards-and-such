/**
 * Pinpoint test for the PlayPage loss-banner headline `style` attribute
 * absence (W2121).
 *
 * Observable behavior:
 ...
 *     </div>
 *   inside the `[data-testid="end-panel"]` section. All visual styling
 *   for the wrapper flows through the `.loss-banner-headline` class hook
 *   in PlayPage.css — never via an inline `style={{ … }}` prop. Adding
 *   an inline style would silently override the stylesheet's cascade,
 *   defeat user-facing theme overrides, and split the styling contract
 *   across two surfaces (CSS file + JSX literal).
 *
 ...
 *     • no id attribute on wrapper (W2059 — PlayPageLossBannerNoId)
 *     • no id attribute on inner h2 (W2060 — PlayPageLossBannerH2NoId)
 *
 *   None pin the `style` attribute on the loss-banner wrapper itself.
 *   That gap means an accidental `style={{ … }}` prop (or a re-render
 *   that grafts inline CSS in) could land silently. This test fills the
 *   gap with the minimum surface: render → drive a loss → grab
 *   `[data-testid="end-banner-loss"]` and assert
 *   `banner.hasAttribute("style") === false`. `hasAttribute` returns
 *   true for any string value (including ""), so this fails loudly the
 *   moment the wrapper grows any kind of inline style.
 *
 ...
 *     and assert `hasAttribute("style")` is false on that node.
 */
...
const hoisted = vi.hoisted(() => {
  const TEST_GAME_ID = "loss-banner-no-style-fixture";
  type State = { lost: boolean };
  type Action = { type: string };
  const fixturePlugin = {
    id: TEST_GAME_ID,
    title: "Loss Banner No Style Fixture",
    category: "cards" as const,
    players: { min: 1, max: 1, multiplayer: false },
    description:
      "Test-only plugin for the loss-banner <div> style-attribute absence test.",
...
describe("PlayPage loss-banner <div> style-attribute absence (W2121)", () => {
  it("renders the loss-banner headline with no inline style attribute", async () => {
    ...
    // Pin "no style". `hasAttribute("style")` returns true for any
    // string value (including ""), so this fails loudly the moment the
    // wrapper grows any kind of inline style prop.
