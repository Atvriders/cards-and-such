 ...
 * an inline `style` attribute (W2180).
 *
 * Observable behavior:
 ...
 *   inline `style` prop is set in the JSX. Sibling tests cover the
 *   element's tag, parent chain, sibling presence, text content and
 *   absence of `id` (W2058) — but none assert the absence of an inline
 *   `style` attribute. A regression that introduced a `style={...}` here
 *   (e.g. for an ad-hoc color highlight on new records) would bypass the
 *   stylesheet's cascade and slip past every existing testid-based
 *   structural check.
 *
 * Strategy:
 *   Reuse the hoisted-fixture WIN-pattern from W2058 (endStatsBestNoId):
 ...
 *   `dd.hasAttribute("style") === false`. This is a single, focused
...
// style-attribute presence on that node.
// ---------------------------------------------------------------------------
const hoisted = vi.hoisted(() => {
  const TEST_GAME_ID = "end-stats-best-no-style-fixture";
  type S = { moves: number };
  type Action = { type: string };
  const fixturePlugin = {
    id: TEST_GAME_ID,
    title: "End Stats Best No-Style Fixture",
    category: "cards" as const,
    players: { min: 1, max: 1, multiplayer: false },
    description: "Test-only plugin for the win-banner end-stats-best style-absence test.",
...
describe("PlayPage win-banner end-stats-best style absence (W2180)", () => {
  it("does not set an inline style attribute on the end-stats-best <dd>", async () => {
    ...
    // Sole assertion: the end-stats-best <dd> must NOT carry an inline
    // `style` attribute. The JSX identifies it solely by `data-testid`
    // and styling flows from the cascading stylesheet, not inline props.
