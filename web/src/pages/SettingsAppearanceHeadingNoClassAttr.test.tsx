...
// W2424 — focused coverage of the Appearance section heading <h2> NOT
// having a `class` attribute at all. SettingsPage.tsx renders
// `<h2 id="settings-appearance-heading">Appearance</h2>` with no className
// prop, so React never emits a `class` attribute on the rendered element.
// W1844 pins the heading's tagName ("H2"), W891 pins role/level/name, and
// W918 pins the parent <section>'s aria-labelledby wiring — but nothing
...
// `class=""` tokens on this aria-labelledby target.
describe("SettingsPage Appearance heading has no class attribute (W2424)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders the Appearance #settings-appearance-heading <h2> without a class attribute", () => {
