...
/**
 * W1499 — the `chip-board` chip wires `glyph={CATEGORY_GLYPHS[cat]}` per
 * LobbyPage.tsx ~L1971, and CATEGORY_GLYPHS.board (LobbyPage.tsx ~L620)
 * is the literal "▦" lattice character. The `Chip` component
 ...
 *     contract on the `chip-cards` glyph (♣) but never touches a
 *     non-cards category chip glyph.
 *   - W1458 / W1462 / W1470 / W1481 pin the user-data chip glyphs
 *     (hidden / recently-played / favorites / top-rated) but none of
 *     them assert anything about a registry-driven category chip glyph
 *     other than `chip-cards`.
 *   - W1374 (LobbyChipBoardBadge.test.tsx) pins the chip-board count
 *     badge but never reads the glyph span at all — a regression that
 *     dropped `aria-hidden` from the board glyph (or replaced the span
 *     with bare text) would slip past the badge audit.
 *
 * This is the first analogous coverage on the `chip-board` glyph. A
 ...
 * would either expose the bare lattice character to assistive tech or
 ...
 */
describe("LobbyPage — chip-board glyph carries aria-hidden=\"true\" on fresh mount (W1499)", () => {
  it("renders the chip-board glyph span with aria-hidden=\"true\" and the literal '▦' text", () => {
    render(
    ...
    // Pin the literal glyph character. CATEGORY_GLYPHS.board
    // (LobbyPage.tsx ~L620) is "▦", and the chip-board wiring at
    ...
    // bare "▦" lattice before the chip label — the wrapping span
