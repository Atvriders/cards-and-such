...
/**
 * W1434 — the `chip-dice` badge count surfaces the number of GAMES
 * whose category is `"dice"` per the Chip wiring at LobbyPage.tsx
 * ~L1969 (`count={categoryCounts[cat]}`). The `categoryCounts` map is
 * built at LobbyPage.tsx ~L1003 by iterating over `GAMES` and bumping
 * `counts[g.category]` per entry, so for the `dice` category the
 ...
 * `/^\d[\d,]*$/`, so a regression that mis-wired the `dice` category
 ...
 *
 * This pins the `chip-dice` badge against the live registry on a
 ...
 * rationale as W1424 / W1229 / W1208: shares the `src/pages/Lobby`
 * vitest path filter without colliding with concurrent edits to the
 * mega-file.
 */
describe("LobbyPage — chip-dice badge counts dice games (W1434)", () => {
  it("renders the chip-dice count badge as the registry's dice count", () => {
    render(
    ...
    const expected = GAMES.filter((g) => g != null && g.category === "dice").length;

    // The chip itself MUST exist — guards against a regression that
    // accidentally hid or renamed the dice category chip.
    ...
    // the count source is `categoryCounts.dice`, so the contract is
    // the localized count of dice entries in `GAMES`.
