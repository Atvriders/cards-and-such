...
/**
 * W2467 — the `chip-arcade` button (the per-category arcade chip in
 ...
 *    a category chip (e.g. the arcade chip becomes a popover trigger
 ...
 *    chip including `chip-arcade`, but a `<button>` without an explicit
 *    `type` attribute still satisfies that assertion while defaulting
 *    to "submit" inside a form.
 *  - W1435 (LobbyChipArcadeBadge.test.tsx) and W1507
 *    (LobbyChipArcadeGlyphAria.test.tsx) pin the badge count and the
 *    glyph span respectively for `chip-arcade`, but neither reads the
 ...
 * or extending W1258) mirrors the W2313 / W1408 pattern (per-arrow
 * `type` pins) so this shares the `src/pages/Lobby` vitest path filter
 * without colliding with concurrent edits to either mega-file.
 */
describe("LobbyPage — chip-arcade type attribute (W2467)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders chip-arcade with an explicit type=\"button\" attribute", () => {
    render(
