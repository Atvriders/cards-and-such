/**
 * Pinpoint test for the PlayPage loss-banner headline `aria-label` attribute
 * absence (W2400).
 *
 * Observable behavior:
 ...
 *     </div>
 *   inside the `[data-testid="end-panel"]` section. The wrapper carries
 *   ONLY `role="status"` + `aria-live="polite"` for assistive-tech
 *   announcement; the human-readable announcement comes from its own
 *   children (the inner `<h2>` "Game over" heading and the encouragement
 *   `<p>`). Adding an `aria-label` to the wrapper would override that
 *   natural reading — screen readers announce the aria-label string in
 *   place of the descendant text content for `role="status"` regions —
 *   silently swapping the encouragement message for whatever string an
 *   accidental `aria-label="…"` prop carried. The end-panel's outer
 *   `<section data-testid="end-panel">` is the dialog-labeled surface
 *   (it carries the `aria-label="Game over"` when the loss banner is up,
 *   pinned elsewhere); the inner headline `<div>` deliberately does NOT.
 *
 ...
 *     • inline style absence (W2121 — PlayPageLossBannerNoStyle)
 *     • tabindex absence (W2297 — PlayPageLossBannerNoTabindex)
 *
 *   None pin the `aria-label` attribute on the loss-banner wrapper itself.
 *   That gap means an accidental `aria-label="…"` prop could land silently
 *   and quietly shadow the encouragement reading for assistive-tech users.
 *   This test fills the gap with the minimum surface: render → drive a
 *   loss → grab `[data-testid="end-banner-loss"]` and assert
 *   `banner.hasAttribute("aria-label") === false`. `hasAttribute` returns
 *   true for any string value (including ""), so this fails loudly the
 *   moment the wrapper grows any kind of aria-label.
 *
 * Strategy mirrors W2297 (PlayPageLossBannerNoTabindex.test.tsx):
 ...
 *     and assert `hasAttribute("aria-label")` is false on that node.
 */
...
const hoisted = vi.hoisted(() => {
  const TEST_GAME_ID = "loss-banner-no-aria-label-fixture";
  type State = { lost: boolean };
  type Action = { type: string };
  const fixturePlugin = {
    id: TEST_GAME_ID,
    title: "Loss Banner No Aria-Label Fixture",
    category: "cards" as const,
    players: { min: 1, max: 1, multiplayer: false },
    description:
      "Test-only plugin for the loss-banner <div> aria-label-attribute absence test.",
...
describe("PlayPage loss-banner <div> aria-label-attribute absence (W2400)", () => {
  it("renders the loss-banner headline with no aria-label attribute", async () => {
    ...
    // Without this guard, the aria-label-absence check could pass for the
    ...
    // Pin "no aria-label". `hasAttribute("aria-label")` returns true for
    // any string value (including ""), so this fails loudly the moment
    // the wrapper grows any kind of aria-label that would shadow the
    // descendant text reading for `role="status"` announcements.
