...
/**
 * W2599 — the `chip-arcade` button (the per-category arcade chip in
 ...
 * LobbyChipArcadeRole.test.tsx) inside a `role="tablist"` chip-strip,
 ...
 *    LobbyChipCardsNoAriaLabel.test.tsx pin the same attribute absence
 *    on different category chips but do not cover chip-arcade, so a
 *    branch that special-cases the arcade chip with an `aria-label`
 ...
 * attribute pattern used by LobbyChipBoardNoAriaLabel.test.tsx (W2594)
 * and LobbyChipCardsNoAriaLabel.test.tsx so this shares the
 * `src/pages/Lobby` vitest path filter without colliding with
 * concurrent edits to either mega-file.
 */
describe("LobbyPage — chip-arcade has no aria-label attribute (W2599)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders chip-arcade without an aria-label attribute (visible text supplies the accessible name)", () => {
    render(
