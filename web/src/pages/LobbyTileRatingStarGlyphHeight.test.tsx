...
/**
 * W1744 — every star button inside the lobby tile-rating widget contains
 * exactly one inner <svg> glyph that declares the literal `height="100%"`
 ...
 *   - W1731 (LobbyTileRatingStarGlyphWidth.test.tsx): SVG width="100%".
 *
 * None of those pin the inner SVG's `height` attribute. StarRating.tsx
 * ~L179-185 renders an <svg> with `height="100%"` (~L182). A refactor
 * that hard-codes a pixel value (e.g. `height="14"`) — easy to do by
 * accidentally copy-pasting from the sibling "Saved" toast SVG which
 * uses `height="14"` (~L125) — would break the responsive star sizing
 ...
 * rationale as W1556/W1569/W1695/W1731: shares the `src/pages/Lobby`
 * vitest path filter without colliding with concurrent edits to the
 * mega-file.
 */
describe('LobbyPage — tile-rating star inner svg glyph height="100%" (W1744)', () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it('declares height="100%" on the inner svg glyph of all 5 star buttons', async () => {
    ...
    // `height="100%"` (~L182) so the star scales with its parent button.
    ...
    for (const star of stars) {
      ...
      expect(glyphs[0]).toHaveAttribute("height", "100%");
    }
  });
});
