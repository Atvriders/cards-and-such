...
/**
 * W2525 — the `chip-dice` button (the per-category dice chip in the
 ...
 *    pass the aria-pressed pin but break tablist semantics — this
 *    pin catches that.
 *  - LobbyChipCardsAriaSelectedDefault (W2509),
 *    LobbyChipBoardAriaSelectedDefault, and
 *    LobbyChipArcadeAriaSelectedDefault pin `aria-selected` on
 *    sibling chips, not on chip-dice.
 *  - LobbyChipDiceBadge, LobbyChipDiceGlyphAria, LobbyChipDiceNoId,
 *    and LobbyChipDiceType all fetch the chip-dice button but each
 *    pins a different attribute (count text, glyph aria-hidden, id
 *    absence, type="button") and none of them reads `aria-selected`.
 ...
 * or extending LobbyChipDiceAriaPressedDefault) mirrors the
 * per-chip-attribute pattern so this shares the
 * `src/pages/Lobby` vitest path filter without colliding with
 * concurrent edits to either mega-file.
 */
describe("LobbyPage — chip-dice default aria-selected (W2525)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders chip-dice with aria-selected=\"false\" on initial render (chip-all is active by default)", () => {
    render(
