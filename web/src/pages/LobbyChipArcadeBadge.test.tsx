...
/**
 * W1435 — the `chip-arcade` badge count surfaces the number of GAMES
 * whose category is `"arcade"` per the Chip wiring at LobbyPage.tsx
 * ~L1969 (`count={categoryCounts[cat]}`). The `categoryCounts` map is
 * built at LobbyPage.tsx ~L1003 by iterating over `GAMES` and bumping
 * `counts[g.category]` per entry, so for the `arcade` category the
 ...
 * `chip-dice`, `chip-arcade`), `chip-arcade` was previously the last
 * category chip without a per-chip pin. The chip-strip count audit at
 * LobbyPage.test.tsx ~L2333 only checks each badge matches
 * `/^\d[\d,]*$/`, so a regression that mis-wired the `arcade` category
 ...
 *
 * This pins the `chip-arcade` badge against the live registry on a
 ...
 * rationale as W1229 / W1424 / W1208: shares the `src/pages/Lobby`
 * vitest path filter without colliding with concurrent edits to the
 * mega-file.
 */
describe("LobbyPage — chip-arcade badge counts arcade games (W1435)", () => {
  it("renders the chip-arcade count badge as the registry's arcade count", () => {
    render(
    ...
    const expected = GAMES.filter((g) => g != null && g.category === "arcade").length;

    // The chip itself MUST exist — guards against a regression that
    // accidentally hid or renamed the arcade category chip.
    ...
    // the count source is `categoryCounts.arcade`, so the contract is
    // the localized count of arcade entries in `GAMES`.
