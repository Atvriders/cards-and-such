...
/**
 * W2701 — the LobbyPage `chip-solitaire` Chip <button> MUST NOT carry a
 ...
 *   3. The peer per-category chips (chip-all, chip-arcade, chip-board,
 *      chip-cards, chip-dice, etc.) are all expected to share the
 ...
 *   - LobbyChipSolitaireNoId pins `id`-absence on chip-solitaire.
 *   - LobbyChipSolitaireType pins the explicit `type="button"`.
 *   - LobbyChipSolitaireNoName pins `name`-absence on chip-solitaire.
 *   - LobbyChipSolitaireNoForm pins `form`-absence on chip-solitaire.
 *   - LobbyChipSolitaireNoAriaLabel / NoAriaControls / NoAriaDisabled
 ...
 * changes.
 */
describe("LobbyPage — chip-solitaire button has no value attribute (W2701)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-solitaire <button> does NOT carry a value attribute", () => {
    render(
    ...
    // wrapper and not a child span. If a future refactor moved the
    // testid down onto an inner span the wrong element could pass this
    // assertion trivially (spans never get a `value` attribute).
    expect(chip.tagName).toBe("BUTTON");

    // The actual contract: no `value` attribute on chip-solitaire. Use
