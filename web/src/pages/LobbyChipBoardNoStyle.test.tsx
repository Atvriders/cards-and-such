...
/**
 * W2519 — the LobbyPage `chip-board` Chip <button> MUST NOT carry an
 ...
 *   - W1499 / LobbyChipBoardGlyphAria pins the glyph + `aria-hidden`.
 *   - W2473 / LobbyChipBoardType pins the explicit `type="button"`.
 *   - W2495 / LobbyChipBoardNoId pins the `id`-attribute absence.
 *
 * The closest analogues are W2146 / LobbyChipAllNoStyle (pins
 * style-absence on `chip-all`) and the family of *NoStyle pins on
 * sibling chip surfaces. Because the `<Chip>` helper is shared, a
 * regression that introduced `style={{...}}` on `chip-all` would fail
 * W2146 — but a regression that added an inline `style` to JUST the
 * per-category chips (e.g. via the inline `CATEGORY_ORDER.map` at
 * LobbyPage.tsx ~L1964 passing an extra prop only on the dynamic
 ...
 * changes.
 */
describe("LobbyPage — chip-board button has no inline style attribute (W2519)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-board <button> does NOT carry a style attribute", () => {
    render(
    ...
    // The actual contract: no `style` attribute on chip-board. Use
