...
/**
 * W2582 — the `chip-solitaire` button (the per-category chip emitted by
 * the `Chip` helper at LobbyPage.tsx ~L2651-2666) MUST NOT carry an
 ...
 * few negative attributes for the `solitaire` chip:
 *   - LobbyChipSolitaireType pins `type="button"`.
 *   - LobbyChipSolitaireRole pins `role="tab"`.
 *   - LobbyChipSolitaireGlyphAria pins glyph aria-hidden.
 *   - LobbyChipSolitaireAriaSelectedDefault pins the default false-state
 *     of `aria-selected`.
 *   - LobbyChipSolitaireNoId pins id-absence.
 *   - LobbyChipSolitaireNoStyle pins inline-style absence.
 *   - LobbySolitaireChipBadge pins the badge count text.
 *
 ...
 *
 * Per the W2582 dispatch directive: pin one untested attribute on the
 * `data-testid="chip-solitaire"` element. `aria-controls`-absence is
 * the canonical "negative" pin that complements the existing
 * `role="tab"` positive pin and locks down the contract that these
 * chips behave as filter buttons, not as tabpanel controllers. Mirrors
 * the W2576 sibling pin established in
 * `LobbyChipCardsNoAriaControls.test.tsx`.
 *
 * Sibling-file placement (rather than appending to LobbyPage.test.tsx)
 * follows the W2576 / W1424 / W1495 / W2469 pattern so the test shares
 * the `src/pages/Lobby` vitest path filter without colliding with
 * concurrent edits to the mega-file.
 */
describe("LobbyPage — chip-solitaire has no aria-controls attribute (W2582)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders the chip-solitaire button without an `aria-controls` attribute", () => {
    render(
    ...
    // Belt-and-braces: reading the attribute by name reflects the DOM
    // and normalises to null when absent. Catches a regression that
    // sets the attribute via a ref or via setAttribute on mount (which
    // would NOT show up in the initial JSX but WOULD reflect on the
    // DOM).
