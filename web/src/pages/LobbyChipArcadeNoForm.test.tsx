...
/**
 * W2677 — the LobbyPage `chip-arcade` Chip <button> MUST NOT carry a
 ...
 *   - LobbyChipArcadeNoId pins `id`-absence.
 *   - LobbyChipArcadeNoName pins `name`-absence.
 *   - LobbyChipArcadeNoStyle pins inline-style absence.
 *   - LobbyChipArcadeNoAriaLabel pins `aria-label` absence.
 *   - LobbyChipArcadeNoAriaControls pins `aria-controls` absence.
 *   - LobbyChipArcadeNoAriaDisabled pins `aria-disabled` absence.
 *   - LobbyChipArcadeNoAutofocus pins `autofocus` absence.
 *
 * None of those would catch a regression that added `form="search"`
 * (or any other value) to the chip-arcade <button>. This file closes
 * that gap, mirroring W2675's `chip-board` form-absence pin.
 *
 ...
 * changes.
 */
describe("LobbyPage — chip-arcade button has no form attribute (W2677)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("the chip-arcade <button> does NOT carry a form attribute", () => {
    render(
    ...
    // The actual contract: no `form` attribute on chip-arcade. Use
