 ...
 *
 * We use the `klondike` tile because it always exists in GAMES and is
 * the same fixture used by lobby-flows.spec.ts.
 */
...
test.describe("lobby tile context menu", () => {
  test("right-click on klondike tile opens tile-menu", async ({ page }) => {
  ...
  test("clicking tile-menu-fav bumps the favorites chip count", async ({ page }) => {
    ...
    // renders the count via .toLocaleString(), so we read its trailing
    // numeric segment rather than a dedicated testid.
    const favChip = page.getByTestId("chip-favorites");
    await expect(favChip).toContainText(/\b0\b/);

    const tile = page.getByTestId("tile-klondike");
    await tile.click({ button: "right" });
    await expect(page.getByTestId("tile-menu")).toBeVisible();

    await page.getByTestId("tile-menu-fav").click();
    ...
    await expect(page.getByTestId("tile-fav-toggle-klondike")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    // And the chip count bumps to 1.
    await expect(favChip).toContainText(/\b1\b/);
  ...
  test("ArrowDown then Enter activates the second item (Copy link)", async ({ page }) => {
    ...
    await expect(page).toHaveURL("/");
    // And we should still be on the lobby grid (klondike tile still mounted).
